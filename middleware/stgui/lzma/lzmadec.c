...
void DoLZMADecompression(U32 addr)
{
    ...
    if ((CompressedFileLength != OriginalFileLength) && ((OriginalFileLength-CompressedFileLength-0x100)>32)) // It implys that U-Boot is compressed.
    {
        ...
        U32 in = DECOMPRESS_BUF_CACHEABLE + 0x100;
        const U8 *header = (U8 *)in;
        SizeT in_size_pure = CompressedFileLength - (LZMA_PROPS_SIZE + 8);
        ISzAlloc g_Alloc = { SzAlloc, SzFree };
        g_alloc_buf = (unsigned char *)(U8 *)MEMORY_POOL_CACHEABLE;

        unpack_size = 0;
        for (i = 0; i < 8; i++)
            unpack_size += (U32)header[LZMA_PROPS_SIZE + i] << (i * 8);
        out_size = unpack_size;
        // Start to decompress
        res = LzmaDecode((Byte *)u32RAM_START, &out_size, ((unsigned char *)in) + (LZMA_PROPS_SIZE + 8), &in_size_pure, header, LZMA_PROPS_SIZE, LZMA_FINISH_ANY, &status, &g_Alloc);
        if (res != 0)
        {
        }
        else
