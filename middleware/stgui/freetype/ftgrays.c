...
#define FT_MAX_GRAY_SPANS  32
                              ...
                              const FT_Vector*  to )
  {
    ...
    if ( level <= 1 )
    {
      ...
      to_x  = UPSCALE( to->x );
      to_y  = UPSCALE( to->y );
      mid_x = (( ras.x + to_x + ((UPSCALE( control->x ))<<1) )>>2);
      mid_y = (( ras.y + to_y + ((UPSCALE( control->y ))<<1) )>>2);
    ...
    Draw:
      {
        ...
        to_x  = arc[0].x;
        to_y  = arc[0].y;
        mid_x = (( ras.x + to_x + (arc[1].x<<1) )>>2);
        mid_y = (( ras.y + to_y + (arc[1].y<<1) )>>2);
                              ...
                              const FT_Vector*  to )
  {
    ...
    if ( level <= 1 )
    {
      ...
      to_x  = UPSCALE( to->x );
      to_y  = UPSCALE( to->y );
      mid_x = (( ras.x + to_x + 3 * UPSCALE( control1->x + control2->x ) )>>3);
      mid_y = (( ras.y + to_y + 3 * UPSCALE( control1->y + control2->y ) )>>3);
    ...
    Draw:
      {
        ...
        to_x  = arc[0].x;
        to_y  = arc[0].y;
        mid_x = (( ras.x + to_x + 3 * ( arc[1].x + arc[2].x ) )>>3);
        mid_y = (( ras.y + to_y + 3 * ( arc[1].y + arc[2].y ) )>>3);
                       ...
                       int     acount )
  {
    ...
    /*                                                           */
    coverage = (int)( area >> ( ((PIXEL_BITS)<<1) + 1 - 8 ) );
                                                    /* use range 0..256 */
    if ( coverage < 0 )
      coverage = -coverage;
  ...
  gray_sweep( RAS_ARG_ const FT_Bitmap*  target )
  {
    ...
    for ( yindex = 0; yindex < ras.ycount; yindex++ )
    {
      ...
      for ( ; cell != NULL; cell = cell->next )
      {
        ...
        if ( cell->x > x && cover != 0 )
          gray_hline( RAS_VAR_ x, yindex, cover * ((ONE_PIXEL)<<1),
                      cell->x - x );

        cover += cell->cover;
        area   = cover * ( (ONE_PIXEL)<<1 ) - cell->area;
