...
#define TT_NEXT_ULONG   FT_NEXT_ULONG
  ...
  tt_cmap4_next( TT_CMap4  cmap )
  {
    ...
    for ( ;; )
    {
      ...
      if ( charcode <= end )
      {
        if ( values )
        {
          FT_Byte*  p = values + (( charcode - cmap->cur_start )<<1);
                     ...
                     FT_Validator  valid )
  {
    ...
    ends      = table   + 14;
    starts    = table   + 16 + (num_segs<<1);
    deltas    = starts  + (num_segs<<1);
    offsets   = deltas  + (num_segs<<1);
    glyph_ids = offsets + (num_segs<<1);
...

    {
      ...
      for ( n = 0; n < num_segs; n++ )
      {
        ...
        if ( offset && offset != 0xFFFFU )
        {
          ...
          {
            if ( p < glyph_ids                                ||
                 p + (( end - start + 1 )<<1) > table + length )
              FT_INVALID_DATA;
          }
          else
          {
            if ( p < glyph_ids                              ||
                 p + (( end - start + 1 )<<1) > valid->limit )
              FT_INVALID_DATA;
          }
                     ...
                     FT_Validator  valid )
  {
    ...
    if ( table + length > valid->limit || length < 10 + (count<<1) )
      FT_INVALID_TOO_SHORT;
                      ...
                      FT_Validator  valid )
  {
    ...
    if ( table + length > valid->limit || length < 20 + (count<<1) )
      FT_INVALID_TOO_SHORT;
