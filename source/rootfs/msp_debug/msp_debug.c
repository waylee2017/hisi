...
* Description:set debug level;//CNcomment:设置模块是否输出到pc终端，只支持32个模块，所以目前模块定义还不能超过32个。
* Parameters:no
* Return Value:no
***************************************************************************/
HI_S32 SetDebugLevel(HI_U32 u32Pos, HI_U32 u32Level)
{
    ...
    HI_SYS_SetLogLevelByModId((HI_MOD_ID_E)u32ModID, (HI_LOG_LEVEL_E)u32LogLevel);
...
HI_S32 SetDebugLevelForce(HI_U32 u32Pos, HI_U32 u32Cmd, HI_U32 u32ForceLevel)
{
	...
	HI_MPI_LogPrintPosSet((HI_MOD_ID_E)u32ModID, (LOG_OUTPUT_POS_E)u32Pos);
	HI_SYS_SetLogLevelByModId((HI_MOD_ID_E)u32ModID, (HI_LOG_LEVEL_E)u32ForceLevel);
...
#define STRIP_INVALID_HEAD(nr) do{\
	...
	for(;index < nr; index++) \
	{\
		fgets(line_buf, sizeof(line_buf), stream);\
	}\
}while(0)
	...
	while(1)
	{
		...
		else
		{
			if(FD_ISSET(g_iDebugSocket, &t_Readfd))
			{
				...
				iLen = My_recvfrom(g_iDebugSocket, ucBuf, iCmdLen, 0, (struct sockaddr*)&tAddr, &iAddrLen);
			...
			case DEBUG_CONN:
			{
				...
				if(g_bVodSleep)
				{
					sprintf(ucBuf, "Debuger Connect Succeed ON SLEEP, Port=%u, Connected!\n", g_usUdpPort);
				}
				else
				*/
				{
