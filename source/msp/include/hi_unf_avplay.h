...
typedef enum hiUNF_AVPLAY_STATUS_E
{
    ...
    HI_UNF_AVPLAY_STATUS_PREPLAY,  /**<PEPlay*//**<CNcomment: 预播放 */
...
typedef struct hiUNF_AVPLAY_FRAMERATE_PARAM_S
{
    HI_UNF_AVPLAY_FRMRATE_TYPE_E    enFrmRateType;  /**<The source of frame rate*//**<CNcomment: 帧率来源类型 */
    HI_UNF_VCODEC_FRMRATE_S         stSetFrameRate;   /**<Setting frame rate*//**<CNcomment: 设置的帧率 */
}HI_UNF_AVPLAY_FRAMERATE_PARAM_S;
...
typedef struct hiUNF_AVPLAY_CONTROL_INFO_S
{
    ...
    HI_U32 u32BackwardOptimizeFlag;  /*Backward optimize flag, 1 means optimize the backward fast play performance*/
    HI_U32 u32DispOptimizeFlag;      /*Display optimize flag, 1 means optimize the VO display performance*/
...
/**
\brief Searches for registered dynamic audio decoding libraries. CNcomment:查找注册音频动态解码库 CNend
\attention \n
\param[in] enDstCodecID Codec ID of an audio decoding library  CNcomment:音频解码库CodecID CNend
\retval ::HI_SUCCESS Success CNcomment:成功 CNend
\retval ::HI_FAILURE Failure CNcomment:失败 CNend
\see \n
N/A
...
/**
\brief Sets the speed in slow playing mode. CNcomment:设置慢放的速度 CNend
\attention \n
...
CNcomment:设置慢放的速度。1代表正常，2代表2分之1速度。如果要从慢放恢复到正常播放,仍调用该接口，u32Repeat设置为1 CNend
\param[in] hAvplay             AVPLAY handle CNcomment:AV播放句柄 CNend
\param[in] u32Repeat           Slow playing speed CNcomment:慢放速度 CNend
\retval ::HI_SUCCESS Success  CNcomment:成功 CNend
\retval ::HI_ERR_AVPLAY_DEV_NO_INIT      The AVPLAY is not initialized. CNcomment:AVPLAY未初始化 CNend
\retval ::HI_ERR_AVPLAY_NULL_PTR         The input pointer is null. CNcomment:输入指针为空 CNend
\retval ::HI_ERR_AVPLAY_INVALID_PARA     The input parameter is invalid. CNcomment:输入参数非法 CNend
...
/**
\brief Plays videos or audios in trick play mode. Then the AVPLAY is in fast forward mode. CNcomment:设置AVPLAY为快进模式 CNend
\attention \n
N/A
\param[in] hAvplay  AVPLAY handle CNcomment: AV播放句柄 CNend
\param[in] bEnable  FFMode Enable Flag CNcomment: 快进使能标志 CNend
\param[in] u32Speed AVPLAY play speed CNcomment: AV播放句柄 CNend
\retval ::HI_SUCCESS Success  CNcomment:成功 CNend
\retval ::HI_ERR_AVPLAY_DEV_NO_INIT      The AVPLAY is not initialized. CNcomment:AVPLAY未初始化 CNend
\retval ::HI_ERR_AVPLAY_NULL_PTR         The input pointer is null. CNcomment:输入指针为空 CNend
\retval ::HI_ERR_AVPLAY_INVALID_PARA     The input parameter is invalid. CNcomment:输入参数非法 CNend
