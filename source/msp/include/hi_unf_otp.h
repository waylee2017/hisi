...
/** 
\brief  Get hdcp root key from otp, 16bytes length. The HDCP rootkey is not supported to read any more.
\brief CNcomment:读取芯片内部设置的hdcp root key，必须为16字节长。芯片不再支持获取HDCP rootkey。 CNend
\attention \n
N/A
...
\brief CNcomment:锁定芯片内部设置的hdcp root key。锁定后，hdcp root key将不可被修改。 CNend
\attention \n
N/A
\retval ::HI_SUCCESS  Call this API successful.          			CNcomment:API系统调用成功。 CNend
\retval ::HI_FAILURE  Call this API fails. 							CNcomment:API系统调用失败。 CNend
...
/** 
\brief Get stb root key from otp. The STB rootkey is not supported to read any more.
\brief CNcomment:读取芯片内部设置的stb root key。芯片不再支持读取STB rootkey。 CNend
\attention \n
N/A
...
\brief CNcomment:锁住芯片内部设置的stb root key。锁定后，stb root key将不可被修改。 CNend
\attention \n
N/A
\retval ::HI_SUCCESS  Call this API successful.          			CNcomment:API系统调用成功。 CNend
\retval ::HI_FAILURE  Call this API fails. 							CNcomment:API系统调用失败。 CNend
