...
static HI_S32 DetFrm_MPEG2(DF_CTX_S *pDFCtx)
{
    ...
    HI_U8    *pu8Data = (HI_U8*)pstCtx->s32NalAddr;
    HI_U8     picture_coding_type;

    //record the first nal addr
    if(pstCtx->u32CurFrmStart == 0)
    {
        pstCtx->u32CurFrmStart = pstCtx->s32NalAddr;
    }
    pstCtx->u32CurFrmEnd = pstCtx->s32NalAddr;
    ...
    )
{
    ...
    DF_ASSERT_RET( (HI_S32)BITS_2_BYTES((HI_U32)BsPos(&Bs))<=s32DataLen, "data NOT enough for 'first_mb_in_slice'" );
    *ps32FirstMBInSlice = (HI_S32)u32Val;
    ...
    DF_ASSERT_RET( (HI_S32)BITS_2_BYTES((HI_U32)BsPos(&Bs))<=s32DataLen, "data NOT enough for 'slice_type'" );
    *ps32SliceType = (HI_S32)SlcType2FrmType[u32Val];
    ...
    DF_ASSERT_RET( (HI_S32)BITS_2_BYTES((HI_U32)BsPos(&Bs))<=s32DataLen, "data NOT enough for 'pic_parameter_set_id'" );
    *ps32PPSId = (HI_S32)u32Val;
...
HI_S32 DetFrm_H264(DF_CTX_S *pDFCtx)
{
    ...
    DF_CTX_S *pstCtx = pDFCtx;
    HI_U8    *pu8Data = (HI_U8*)pstCtx->s32NalAddr;
    HI_S32    s32DataLen = pstCtx->s32NalLen;
    ...
    {
        if( RET_OK == DetFrm_H264Slice(pu8Data, s32DataLen, &s32FirstMb, &s32SliceType, &s32PpsID) )
        {
            if(s32FirstMb == 0)
            {
