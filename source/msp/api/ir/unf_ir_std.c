...
HI_VOID IrValToProtocol(HI_U32 u32Val, HI_CHAR *pIrProtocol)
{

	switch(u32Val)
	{
		case 0:
			memcpy(pIrProtocol,"NEC with simple repeat code",30);
			break;
		case 1:
			memcpy(pIrProtocol,"TC9012",30);
			break;
		case 2:
			memcpy(pIrProtocol,"NEC with full repeat code",30);
			break;
		case 3:
			memcpy(pIrProtocol,"SONY",30);
			break;
		case 5:
			memcpy(pIrProtocol,"NEC 2Head with simple repeat code",30);
			break;
		case 6:
			memcpy(pIrProtocol,"Philips RC-5/RC-5X",30);
			break;
		case 7:
			memcpy(pIrProtocol,"NEC 2Head with full repeat code",30);
			break;
		case 8:
			memcpy(pIrProtocol,"Philips RC-6",30);
			break;
		case 9:
			memcpy(pIrProtocol,"Rstep",30);
			break;		
		default:
			memcpy(pIrProtocol,"error protocol",30);
			break;
...
HI_S32 HI_UNF_IR_Enable (HI_BOOL bEnable)
{
    ...
    if (Ret != HI_SUCCESS)
    {
        return HI_FAILURE;//HI_ERR_IR_ENABLE_FAILED;
    }
...
{    
    ...
    if (Ret != HI_SUCCESS)
    {
        return HI_FAILURE; //HI_ERR_IR_SET_BLOCKTIME_FAILED;
    }
    ...
    if (Ret == sizeof(IR_KEY_S))
    {
	    ...
	    IrValToProtocol(Irkey.IrProtocol,pszProtocolName);
...
HI_S32 HI_UNF_IR_EnableKeyUp(HI_BOOL bEnable)
{
    ...
    if (Ret != HI_SUCCESS)
    {
        return HI_FAILURE;//HI_ERR_IR_SET_KEYUP_FAILED;
    }
...
HI_S32 HI_UNF_IR_EnableRepKey(HI_BOOL bEnable)
{
    ...
    if (Ret != HI_SUCCESS)
    {
        return HI_FAILURE;//HI_ERR_IR_SET_REPEAT_FAILED;
    }
...
HI_S32 HI_UNF_IR_SetRepKeyTimeoutAttr(HI_U32 u32TimeoutMs)
{
    ...
    if (Ret != HI_SUCCESS)
    {
        return HI_FAILURE;//HI_ERR_IR_SET_REPKEYTIMEOUT_FAILED;
    }
...
HI_S32 HI_UNF_IR_SetCodeType(HI_UNF_IR_CODE_E enIRCode)
{
    ...
    {       
        ...
        return HI_FAILURE;//HI_ERR_IR_SETFORMAT_FAILED;
    }
...
HI_S32 HI_UNF_IR_Reset(HI_VOID)
{
    ...
    if (Ret != HI_SUCCESS)
    {
        return HI_FAILURE;//HI_ERR_IR_RESET_FAILED;
    }
