...
typedef struct
{
	...
	HI_BOOL IsPubPalette;  /*Decoder uses common Palette or not*//** CNcomment:解码图片是否使用公共调色板,只对Clut格式有效.若不使用公共的调色板,则使用自带的调色板*/
}HIGO_DEC_SURINFO_S;
...
/**
\brief Set the leave os memory size that dec can used.  CNcomment:解码可以使用的系统内存剩余大小 CNend
\attention \n
N/A.CNcomment: 无 CNend
\param[in] Decoder handle. CNcomment:解码实例句柄 CNend
\param[in] u32MaxMemSize.  CNcomment:the leave os memory size CNend
...
N/A.CNcomment:无 CNend
*/
#ifdef HI_CONFIG_HIGO_ADD_SETMEMSIZE_FUNCTION
HI_S32 HI_GO_SetDecMemSizeInfo(HI_HANDLE Decoder, HI_U32 u32MaxMemSize);
#endif
