...
#define SAT_C_MAX (4800)    //expand from 4200 to 4800,in case local freq(5750),search all if freq 950--2150
#define SAT_KU_MIN (10600)
#define SAT_KU_MAX (13450) //expand from 12750 to 13450,in case local freq(11300),search all if freq 950--2150
...
static HI_VOID TUNER_GetBsCtrlPara(HI_U32 u32TunerId, HI_UNF_TUNER_BLINDSCAN_PARA_S *pstPara, BLINDSCAN_CTRL_T *pstBlindScanCtrl)
{
    HI_S32 s32IFStart = 0;
    HI_S32 s32IFStop = 0;
    ...
    else
    {
        if(HI_UNF_TUNER_FE_LNB_UNICABLE != s_stSatPara[u32TunerId].stLNBConfig.enLNBType)
        {
            ...
            {
                /* C band */
                if (HI_UNF_TUNER_FE_LNB_BAND_C == s_stSatPara[u32TunerId].stLNBConfig.enLNBBand)    //local freq 5150-5750,search if freq 950-2150 here
                {
                ...
                /* Ku band */
                else     //local freq 9750- ,search if freq 950-2150 here
                {
...
static HI_VOID* TUNER_BlindScanThread(HI_VOID* pstBlindScan)
{
    ...
    BLINDSCAN_CTRL_T stBlindScanCtrl ;
    HI_UNF_TUNER_SAT_TPINFO_S stResult;
    HI_UNF_TUNER_SAT_TPINFO_S *pstTP;
    ...
    HI_U32 fec_ok_cnt = 0, last_fec_ok_cnt = 0, fec_no_ok_cnt = 0, fs_cur;
    HI_U8 stop_quit=0;
	...
	pstTP = malloc(BUF_TP_NUM * sizeof(HI_UNF_TUNER_SAT_TPINFO_S));
	if (HI_NULL == pstTP)
	{
        HI_ERR_TUNER("Alloc memory fail!\n");
        return HI_NULL;
	}
	memset(pstTP, 0, BUF_TP_NUM * sizeof(HI_UNF_TUNER_SAT_TPINFO_S));
	
	memset(&stBlindScanCtrl,0,sizeof(BLINDSCAN_CTRL_T));
    ...
    TUNER_GetBsCtrlPara(u32TunerId, &stPara, &stBlindScanCtrl);
    ...
    for (i = 0; i < (HI_S32)(stBlindScanCtrl.u32ScanTimes); i++)
    {
        ...
        {
            sort_tp_and_deredun(&s32TPNum, pstTP);

            for (k = 0; k < s32TPNum; k++)
            {
                if((*(pstTP+k)).cbs_reliablity < 60)
                {
                    break;
                }
                else
            ...
            for (k = 0; k < s32TPNum; k++)
            {
                fs_cur = (*(pstTP+k)).u32SymbolRate;

                if((*(pstTP+k)).cbs_reliablity > 10)
                {
                    //unicable need to convert downklink freq to if freq.
                    stTPParm.pu32Frequency = &((*(pstTP+k)).u32Freq);
                    stTPParm.pu32SymbolRate = &fs_cur;
                    stTPParm.enPolar = (*(pstTP+k)).enPolar;
                    stTPParm.cbs_reliablity = (*(pstTP+k)).cbs_reliablity;
                ...
                (*(pstTP+k)).u32SymbolRate = fs_cur; //Hz
                ...
                if (fec_ok_cnt > last_fec_ok_cnt)
                {
                    ...
                    if(fec_ok_cnt > 1)
                    {
                        ...
                        {
                            fs_dlt = ((*(pstTP+kk)).u32SymbolRate > fs_cur) ? ((*(pstTP+kk)).u32SymbolRate - fs_cur)/(fs_cur/1000) : \
                                                                         (fs_cur - (*(pstTP+kk)).u32SymbolRate)/(fs_cur/1000);
                            if_dlt = ((*(pstTP+kk)).u32Freq > (*(pstTP+k)).u32Freq) ? ((*(pstTP+kk)).u32Freq - (*(pstTP+k)).u32Freq) : \
                                                                            ((*(pstTP+k)).u32Freq - (*(pstTP+kk)).u32Freq);
                    ...
                    if(!same_tp)
                    {
                        ...
                        *(pstTP+fec_ok_cnt - 1) = *(pstTP+k);
                        unNotify.pstResult = &(*(pstTP+fec_ok_cnt - 1));   //notice ?  origin is "&stTP[k]", changed by zouzhiyong
                        /* Notify new TP */
                        stPara.unScanPara.stSat.pfnEVTNotify(u32TunerId, HI_UNF_TUNER_BLINDSCAN_EVT_NEWRESULT, &unNotify);
                    }
                }
...
HI_VOID *TUNER_DVBT2ScanThread(HI_VOID* pstTerScan)
{
	...
	else   //dvb-t2
	{
		...
		for(i=0,k=0; i<u32Num; i++)
		{
			if(u32GrpNum >= T2_GROUP_SUM)
			{
				HI_ERR_TUNER("plp group number is more than %d.\n",T2_GROUP_SUM);
				s_stTerPara[u32TunerId].bScanBusy = HI_FALSE;
				return HI_NULL;
			}
...
HI_S32 HI_UNF_TUNER_Init(HI_VOID)
{
    ...
    s_strDeftTunerAttr[0].enI2cChannel = HI_UNF_I2C_CHANNEL_QAM;
    ...
    s_strDeftTunerAttr[0].enTunerDevType = HI_UNF_TUNER_DEV_TYPE_CD1616;
    ...
    s_strDeftTunerAttr[1].enI2cChannel = HI_UNF_I2C_CHANNEL_1;
    ...
    s_strDeftTunerAttr[2].enI2cChannel = HI_UNF_I2C_CHANNEL_0;
    ...
    for(u8Port = 0; u8Port < UNF_TUNER_NUM; u8Port++)
    {
        ...
        s_strCurTunerAttr[u8Port].enI2cChannel = HI_UNF_I2C_CHANNEL_MAX;
        ...
        s_stSatAttr[u8Port].u32ResetGpioNo = 0;
...
HI_S32 HI_UNF_TUNER_GetSatAttr(HI_U32   u32tunerId , HI_UNF_TUNER_SAT_ATTR_S *pstSatTunerAttr)
{
    if(UNF_TUNER_NUM <= u32tunerId)
    {
        HI_ERR_TUNER("Input parameter(u32tunerId)invalid,invalid tunerId is: %d\n",u32tunerId);
    ...
    if (HI_NULL == pstSatTunerAttr)
    {
        HI_ERR_TUNER("Input parameter(pstSatTunerAttr)invalid\n");
        return HI_ERR_TUNER_INVALID_POINT;
    }

    memcpy(pstSatTunerAttr, &s_stSatAttr[u32tunerId], sizeof(HI_UNF_TUNER_SAT_ATTR_S));
    return HI_SUCCESS;
}
...
HI_S32 HI_UNF_TUNER_SetTerAttr(HI_U32   u32tunerId , const HI_UNF_TUNER_TER_ATTR_S *pstTerTunerAttr)
{
    ...
    s32Ret = ioctl(s_s32TunerFd, TUNER_SETTERATTR_CMD, (HI_U32)&stTunerData);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_S32 HI_UNF_TUNER_GetTerAttr(HI_U32   u32tunerId , HI_UNF_TUNER_TER_ATTR_S *pstTerTunerAttr)
{
    if(UNF_TUNER_NUM <= u32tunerId)
    {
        HI_ERR_TUNER("Input parameter(u32tunerId)invalid,invalid tunerId is: %d\n",u32tunerId);
    ...
    if (HI_NULL == pstTerTunerAttr)
    {
        HI_ERR_TUNER("Input parameter(pstTerTunerAttr)invalid\n");
        return HI_ERR_TUNER_INVALID_POINT;
    }

    memcpy(pstTerTunerAttr, &s_stTerAttr[u32tunerId], sizeof(HI_UNF_TUNER_TER_ATTR_S));
    return HI_SUCCESS;
}
...
HI_S32 HI_UNF_TUNER_SetCabAttr(HI_U32   u32tunerId , const HI_UNF_TUNER_CAB_ATTR_S *pstCabTunerAttr)
{
    ...
    stTunerData.u32Port = u32tunerId;
    stTunerData.u32Data = (HI_U32)pstCabTunerAttr;
    s32Ret = ioctl(s_s32TunerFd, TUNER_SETCABATTR_CMD, (HI_U32)&stTunerData);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_S32 HI_UNF_TUNER_GetCabAttr(HI_U32   u32tunerId , HI_UNF_TUNER_CAB_ATTR_S *pstCabTunerAttr)
{
    if(UNF_TUNER_NUM <= u32tunerId)
    {
        HI_ERR_TUNER("Input parameter(u32tunerId)invalid,invalid tunerId is: %d\n",u32tunerId);
    ...
    if (HI_NULL == pstCabTunerAttr)
    {
        HI_ERR_TUNER("Input parameter(pstCabTunerAttr)invalid\n");
        return HI_ERR_TUNER_INVALID_POINT;
    }

    memcpy(pstCabTunerAttr, &s_stCabAttr[u32tunerId], sizeof(HI_UNF_TUNER_CAB_ATTR_S));
    return HI_SUCCESS;
}
...
HI_S32 HI_UNF_TUNER_Open (HI_U32  u32tunerId)
{
    ...
    s32Ret = ioctl(s_s32TunerFd, TUNER_CLOSE_CMD, (HI_U32)&stTunerData);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_S32 HI_UNF_TUNER_Close(HI_U32    u32tunerId)
{
    ...
    memset(&stTunerData,0,sizeof(TUNER_DATA_S));
    s32Result = ioctl(s_s32TunerFd, TUNER_CLOSE_CMD, (HI_U32)&stTunerData);
    if (HI_SUCCESS != s32Result)
    {
        HI_ERR_TUNER("Tuner TUNER_CLOSE_CMD error\n");
        return HI_ERR_TUNER_FAILED_SELECTI2CCHANNEL;
...
static HI_VOID* TUNER_TerSignalDetect(HI_VOID *pstSignal)
{
    ...
    while(s32TimeOut > 0)
    {
        ...
        s_stCurrentSignal[stTunerSignal.u32Port] = stTunerSignal.stSignal;
...
static HI_S32 MultiMode_Demod_CheckSignalChange(HI_U32 u32tunerId,HI_UNF_TUNER_SIG_TYPE_E enSignalType)
{
    ...
    if((enSignalType >= HI_UNF_TUNER_SIG_TYPE_DVB_T)
        && (enSignalType <= HI_UNF_TUNER_SIG_TYPE_ISDB_T)
    ...
    switch(enSignalType)
    {
        ...
        case HI_UNF_TUNER_SIG_TYPE_J83B:
            ...
            stTunerData.u32Port = u32tunerId;
            stTunerData.u32Data = (HI_U32)&(s_stCabAttr[u32tunerId]); 
            s32Ret = ioctl(s_s32TunerFd, TUNER_SETCABATTR_CMD, (HI_U32)&stTunerData);
            break;
        case HI_UNF_TUNER_SIG_TYPE_SAT:
            ...
            stTunerData.u32Port = u32tunerId;
            stTunerData.u32Data = (HI_U32)&(s_stSatAttr[u32tunerId]);
            s32Ret = ioctl(s_s32TunerFd, TUNER_SETSATATTR_CMD, (HI_U32)&stTunerData);
            break;
        ...
        case HI_UNF_TUNER_SIG_TYPE_DVB_T2:
            ...
            stTunerData.u32Port = u32tunerId;
            stTunerData.u32Data = (HI_U32)&(s_stTerAttr[u32tunerId]);
            s32Ret = ioctl(s_s32TunerFd, TUNER_SETTERATTR_CMD, (HI_U32)&stTunerData);
            break;
        default:
            break;
    ...
    for(i = 0; i < UNF_TUNER_NUM; i++)
    {
        ...
        if(HI_UNF_DEMOD_DEV_TYPE_3138 == s_strCurTunerAttr[i].enDemodDevType)
        {
            switch(s_strCurTunerAttr[i].enSigType)
            {
                case HI_UNF_TUNER_SIG_TYPE_CAB:
                case HI_UNF_TUNER_SIG_TYPE_J83B:
                    stTunerData.u32Port = i;
                    stTunerData.u32Data = (HI_U32)&(s_stCabAttr[i]); 
                    s32Ret = ioctl(s_s32TunerFd, TUNER_SETCABATTR_CMD, (HI_U32)&stTunerData);
                    break;
                case HI_UNF_TUNER_SIG_TYPE_SAT:
                    stTunerData.u32Port = i;
                    stTunerData.u32Data = (HI_U32)&(s_stSatAttr[i]);
                    s32Ret = ioctl(s_s32TunerFd, TUNER_SETSATATTR_CMD, (HI_U32)&stTunerData);
                ...
                case HI_UNF_TUNER_SIG_TYPE_DVB_T2:
                    stTunerData.u32Port = i;
                    stTunerData.u32Data = (HI_U32)&(s_stTerAttr[i]);
                    s32Ret = ioctl(s_s32TunerFd, TUNER_SETTERATTR_CMD, (HI_U32)&stTunerData);
                    break;
                default:
                    break;
            ...
            if (HI_SUCCESS != s32Ret)
            {
                HI_ERR_TUNER("Tuner TUNER_SETTERATTR_CMD error,signal type:%d\n",s_strCurTunerAttr[i].enSigType);
                return HI_ERR_TUNER_FAILED_SETTERATTR;
            }
        }
    }
...
static HI_S32 TUNER_Connect_C(HI_U32 u32tunerId, const HI_UNF_TUNER_CONNECT_PARA_S *pstConnectPara, HI_U32 u32TimeOut)
{
	...
	curfreq = pstConnectPara->unConnectPara.stCab.u32Freq;
    ...
    if ((stTunerPara.unSRBW.u32SymbolRate < 870000) || (stTunerPara.unSRBW.u32SymbolRate > 7200000))
    {
...
static HI_S32 TUNER_Connect_T_T2(HI_U32 u32tunerId, const HI_UNF_TUNER_CONNECT_PARA_S *pstConnectPara, HI_U32 u32TimeOut)
{
    ...
    /*sample_tuner启动时，自动调用connect()函数时u32TimeOut=500ms*/
    if(u32TimeOut == 0)
    {
        stTunerSignal.u32Port = u32tunerId;
        stTunerSignal.stSignal = stTunerPara;
    ...
    else
    {
        ...
        {
            stTunerSignal.u32Port = u32tunerId;
            stTunerPara.u8DVBTMode = 1;     /*搜索DVB-T信号*/
            stTunerSignal.stSignal = stTunerPara;
            stTunerSignal.enSigType = HI_UNF_TUNER_SIG_TYPE_DVB_T;
            s_stCurrentSignal[u32tunerId] = stTunerPara;
            ...
            else
            {
                ...
                if(s32TimeLeft >= 500)
                {
                    s32TimeLeft = s32TimeLeft - 500;    /*每500ms循环一次搜索连接的动作*/
                    u32TimeSpan = 0;
                    while (u32TimeSpan < 500)
                    {
                        stTunerData.u32Port = u32tunerId;
            ...
            stTunerSignal.u32Port = u32tunerId;
            stTunerPara.u8DVBTMode = 0; /*搜索DVB-T2信号*/
            stTunerSignal.stSignal = stTunerPara;
            stTunerSignal.enSigType = HI_UNF_TUNER_SIG_TYPE_DVB_T2;
            s_stCurrentSignal[u32tunerId] = stTunerPara;
            ...
            else
            {
                ...
                if(s32TimeLeft >= 1000)
                {
                	s32TimeLeft = s32TimeLeft - 1000;   /*每1000ms循环一次搜索连接的动作*/
                	u32TimeSpan = 0;
                    while (u32TimeSpan < 1000)
                    {
                        stTunerData.u32Port = u32tunerId;
...
static HI_S32 TUNER_GetBER_C(HI_U32 u32tunerId, const TUNER_DATABUF_S *pstTunerDataBuf, HI_U32 *pu32BER)
{
    ...
    HI_INFO_TUNER("Tuner HI_TUNER_GetBER,pu32BER[0]:%d,pu32BER[1]:%d,pu32BER[2]:%d\n", 
                            pu32BER[0], pu32BER[1], pu32BER[2]);
...
static HI_S32 TUNER_GetSNR_S(HI_U32 u32tunerId, const TUNER_DATA_S *pu32StTunerData, HI_DOUBLE *pdoubleDSnr)
{
    ...
    switch (s_strCurTunerAttr[u32tunerId].enDemodDevType)
    {
        ...
        case HI_UNF_DEMOD_DEV_TYPE_3138:
	        ...
	        {
	            dSnr = 10.0 * log10(8192.0 / (HI_DOUBLE)(pu32StTunerData->u32Data));
	        }
	        else
	        {
	            dSnr = 50.0;
	        ...
	        break;
        case HI_UNF_DEMOD_DEV_TYPE_AVL6211:
            dSnr = (HI_U32)(pu32StTunerData->u32Data / 100.0);
            break;
        default:
            dSnr = 0;
			HI_ERR_TUNER("demod type err:%d\n",s_strCurTunerAttr[u32tunerId].enDemodDevType);
...
static HI_S32 TUNER_GetSNR_C(HI_U32 u32tunerId, const TUNER_DATA_S *pu32StTunerData, HI_DOUBLE *pdoubleDSnr)
{
    ...
    switch (s_strCurTunerAttr[u32tunerId].enDemodDevType)
    {
        ...
        case HI_UNF_DEMOD_DEV_TYPE_3138:
        {
	        ...
	        {
                case QAM_TYPE_16:           
                    dSnr = 10.0 * log10(161 / dTmp_HiDVBC);
                    break;

                case QAM_TYPE_32:
                    dSnr = 10.0 * log10(180 / dTmp_HiDVBC);
                    break;

                case QAM_TYPE_64:
                    dSnr = 10.0 * log10(168 / dTmp_HiDVBC);
                    break;

                case QAM_TYPE_128:
                    dSnr = 10.0 * log10(184 / dTmp_HiDVBC);
                    break;

                case QAM_TYPE_256:
                    dSnr = 10.0 * log10(170 / dTmp_HiDVBC);
                    break;
        ...
        case HI_UNF_DEMOD_DEV_TYPE_J83B:
        {
            ...
            {
                case QAM_TYPE_16:
                    dSnr = 10.0 * log10((5.0 / 18.0) / dTmp);
                    break;

                case QAM_TYPE_32:
                    dSnr = 10.0 * log10(0.2 / dTmp);
                    break;

                case QAM_TYPE_64:
                    dSnr = 10.0 * log10(((42.0 / 14.0) / 14.0) / dTmp);
                    break;

                case QAM_TYPE_128:
                    dSnr = 10.0 * log10(((82.0 / 22.0) / 22.0) / dTmp);
                    break;

                case QAM_TYPE_256:
                    dSnr = 10.0 * log10(((170.0 / 32.0) / 32.0) / dTmp);
                    break;
        ...
        case HI_UNF_DEMOD_DEV_TYPE_MXL214:
            *pdoubleDSnr = (HI_DOUBLE) (pu32StTunerData->u32Data /10);
            break;
        case HI_UNF_DEMOD_DEV_TYPE_TDA18280:
            dSnr = (HI_DOUBLE) (pu32StTunerData->u32Data);
            break; 
...
HI_S32 HI_UNF_TUNER_GetSNR(HI_U32   u32tunerId , HI_U32 *pu32SNR)               /* range : 0-255  */
{
    ...
    if (HI_UNF_TUNER_SIG_TYPE_DVB_T == s_strCurTunerConnectPara[u32tunerId].enSigType
        ...
        || (HI_UNF_TUNER_SIG_TYPE_DVB_T|HI_UNF_TUNER_SIG_TYPE_DVB_T2) == s_strCurTunerConnectPara[u32tunerId].enSigType)
    {
    ...
    switch (s_strCurTunerAttr[u32tunerId].enTunerDevType)
    {
        ...
        { 										
			...
			else if((stTunerDataBuf.u32DataBuf[2] >= 1000) && (stTunerDataBuf.u32DataBuf[2] < 2000))
			{
				...
				else if( (curfreq >= 163000 ) && (curfreq < 172000 ) )
				{
						*pu32SignalStrength = stTunerDataBuf.u32DataBuf[2] + 5;
				}
				else
			...
			{				
				...
				{
				   if(curfreq < (s_SignalLevelR836[i].s32SignalFreq *1000))
				   {
					   if(0 == i)
					   {
				   ...
				   else if(( s_SignalLevelR836[i].s32SignalFreq *1000) == curfreq)
				   {
					   s32levelcorrect = s_SignalLevelR836[i].s32SignalLevel;
					   break;
				   }
        ...
        {
			s32Agc = (HI_S32)(110.0 - (((s32Agc - 691.0) * 80.0) / (2503.0 - 691.0)));
			
			if((curfreq >= 610000) || (curfreq <= 131000))
			{
				s32Agc = s32Agc - 11;
			}
			else if((curfreq >= 490000) && (curfreq < 610000))
			{
				s32Agc = s32Agc - 10;
			}
			else 
    	...
    	{  
    		...
    		if( (curfreq >= 100000 ) && (curfreq < 171000 ) )
    		{
    			*pu32SignalStrength = (HI_U32)(s32Agc - 1);
    		}
    		else if( (curfreq >= 187000 ) && (curfreq < 203000 ) )
    		{
    			*pu32SignalStrength = (HI_U32)(s32Agc - 3);
    		}
    		else if( curfreq == 203000 )
    		{
    			*pu32SignalStrength = (HI_U32)(s32Agc - 1);
    		}
    		else if( (curfreq >= 211000 ) && (curfreq < 283000 ) )
    		{
    			*pu32SignalStrength = (HI_U32)(s32Agc - 5);
    		}
    		else if( curfreq == 283000 )
    		{
    			*pu32SignalStrength = (HI_U32)(s32Agc - 2);
    		}
    		else if( (curfreq >= 339000 ) && (curfreq < 714000 ) )
    		{
    			*pu32SignalStrength = (HI_U32)(s32Agc + 2);
    		}
    		else if( (curfreq >= 786000 ) && (curfreq <= 858000 ) )
    		{
    			*pu32SignalStrength = (HI_U32)(s32Agc - 2);
    		}
    		else
...
static HI_S32 TUNER_GetSignalStrength_T(HI_U32 u32tunerId, HI_U32 *pu32SignalStrength)
{
    ...
    if(HI_SUCCESS != s32Result )
    {
        HI_ERR_TUNER("call TUNER_GET_SIGNALSTRENGTH_CMD fail.\n");
        return s32Result;
    }
...
HI_S32 HI_UNF_TUNER_GetSignalStrength(HI_U32   u32tunerId , HI_U32 *pu32SignalStrength)
{
    ...
    else if (HI_UNF_TUNER_SIG_TYPE_DTMB == s_strCurTunerConnectPara[u32tunerId].enSigType)
    {
        stTunerDataBuf.u32Port = u32tunerId;
        memset(stTunerDataBuf.u32DataBuf, 0, sizeof(stTunerDataBuf.u32DataBuf));
    ...
    else
    {
        stTunerDataBuf.u32Port = u32tunerId;
        memset(stTunerDataBuf.u32DataBuf, 0, sizeof(stTunerDataBuf.u32DataBuf));
        s32Ret = ioctl(s_s32TunerFd, TUNER_GET_SIGNALSTRENGTH_CMD, (HI_U32)&stTunerDataBuf);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_TUNER("Tuner TUNER_GET_SIGNALSTRENGTH_CMD error\n");
            *pu32SignalStrength = 0;
...
static HI_S32 TUNER_GetSignalQuality_S(HI_U32 u32TunerId, HI_U32 *pu32SignalQuality)
{
    ...
    TUNER_REFER_SNR_S astDVBS216apskRefSNR[6];
    TUNER_REFER_SNR_S astDVBS232apskRefSNR[5];
    ...
    switch (s_strCurTunerAttr[u32TunerId].enDemodDevType)
    {
        ...
        default:
            ...
            memset(astDVBS216apskRefSNR, 0, sizeof(astDVBS216apskRefSNR));
            memset(astDVBS232apskRefSNR, 0, sizeof(astDVBS232apskRefSNR));
    }
...
{  
    ...
    {
        static const HI_U32 nordigNonHDVBTdB1000[4][5] = {
            /*   1/2,   2/3,   3/4,   5/6,   7/8                  */    
        ...
        switch(stTunerSignalInfo.stInfo.unSignalInfo.stTer.enFECRate)
        {
    ...
    else if(HI_UNF_TUNER_SIG_TYPE_DVB_T2 == stTunerSignalInfo.stInfo.enSigType)
    {
        ...
        switch(stTunerSignalInfo.stInfo.unSignalInfo.stTer.enFECRate)
        {
            ...
            case HI_UNF_TUNER_FE_FEC_2_5:
                ...
                break;
            default:
                HI_ERR_TUNER("Tuner enFECRate[0x%x] is error,use defaule \n",stTunerSignalInfo.stInfo.unSignalInfo.stTer.enFECRate);
                enFECRate = 0;
                break;
        }
...
HI_S32 HI_UNF_TUNER_GetSignalQuality(HI_U32 u32TunerId, HI_U32 *pu32SignalQuality)
{
    ...
    if(HI_UNF_DEMOD_DEV_TYPE_ATBM8869 == s_strCurTunerAttr[u32TunerId].enDemodDevType)
    {
...
HI_S32 HI_UNF_TUNER_GetRealFreqSymb( HI_U32 u32TunerId, HI_U32 *pu32Freq, HI_U32 *pu32Symb )
{
                 ...
                 && HI_UNF_TUNER_SIG_TYPE_DTMB >= s_strCurTunerConnectPara[u32TunerId].enSigType)
    {
        if ((HI_UNF_DEMOD_DEV_TYPE_3137 == s_strCurTunerAttr[u32TunerId].enDemodDevType)
            || (HI_UNF_DEMOD_DEV_TYPE_3138 == s_strCurTunerAttr[u32TunerId].enDemodDevType))
        {
            *pu32Freq = (HI_U32)((HI_DOUBLE)s_stCurrentSignal[u32TunerId].u32Frequency + stTunerDataBuf.u32DataBuf[0]);
            *pu32Symb = (HI_U32)((HI_DOUBLE)s_stCurrentSignal[u32TunerId].unSRBW.u32BandWidth + stTunerDataBuf.u32DataBuf[1]);
        }
        else if (HI_UNF_DEMOD_DEV_TYPE_ATBM8869 == s_strCurTunerAttr[u32TunerId].enDemodDevType)
        {
            *pu32Freq = (HI_U32)((HI_S32)s_stCurrentSignal[u32TunerId].u32Frequency + (HI_S32)stTunerDataBuf.u32DataBuf[0]);
            *pu32Symb = s_stCurrentSignal[u32TunerId].unSRBW.u32BandWidth;
        }
        else
        {
            *pu32Freq = (HI_U32)((HI_S32)s_stCurrentSignal[u32TunerId].u32Frequency - (HI_S32)stTunerDataBuf.u32DataBuf[0]);
...
HI_S32 HI_UNF_TUNER_MonitorISDBTLayer(HI_U32 u32TunerId, HI_UNF_TUNER_MONITOR_LAYERS_CONFIG_S *pstMonLayersConfig)
{
    ...
    stISDBTLayer.stISDBTLayerCfg = *pstMonLayersConfig;
