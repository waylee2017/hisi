...
HI_S32 Scr_BlindScanTone(HI_VOID *pstScanPara)
{
	...
	HI_UNF_TUNER_UNICABLE_SCAN_PARA_S *pLocalScanPara;
	HI_UNF_TUNER_UNICABLE_SCAN_USER_BAND_NOTIFY_S stNotify;
	HI_DOUBLE		dThresholdStart = 0,dThresholdStop = 0,dToneFreq = 0,dPercent = 0.0;
	HI_U16 u16Progress = 0,u16PreProgress = 0;
    FILE *fp_unic = HI_NULL;
	//timeval stStart, stStop;

	//gettimeofday(&stStart,NULL);
	pLocalScanPara = (HI_UNF_TUNER_UNICABLE_SCAN_PARA_S *)pstScanPara;
	...
	for(s32Index = SCR_D_SCAN_FREQ_START; s32Index < SCR_D_SCAN_FREQ_END ; s32Index++)
	{
		...
		(HI_VOID)TUNER_Get_Agc(u32TunerPort,s32Index, &(sSCRArray[s32Count].s32Agc));
		s32Count++;
		...
		if(g_u32UnicableStopping)
		{
			...
			pLocalScanPara->pfnEVTNotify(u32TunerPort,HI_UNF_TUNER_UNICABLE_SCAN_EVT_STATUS, &stNotify);
			return HI_SUCCESS;
		}
        ...
        if(s32Count%5 == 0)
        {
            ...
            if(u16Progress > u16PreProgress)
            {
                ...
                pLocalScanPara->pfnEVTNotify(u32TunerPort,HI_UNF_TUNER_UNICABLE_SCAN_EVT_PROGRESS, &stNotify);
            }
        }
    }
    ...
    {
        for(s32Index2 = 0;s32Index2 < u32UBCount;s32Index2++)
        {
    ...
    fp_unic = fopen("/mnt/unicable_log.txt", "w");
    fprintf(fp_unic,"++++sMinAgc.u32Freq:%d+++sMinAgc.u32Agc:%d+++sMaxAgc.u32Freq:%d+++sMaxAgc.u32Agc:%d\r\n",sMinAgc[3].s32Freq,
        sMinAgc[3].s32Agc,sBoundaryAgc.s32Freq,sBoundaryAgc.s32Agc);
    fprintf(fp_unic,"++++++++u32Index:%d++++++u32Threshold:%d\r\n",s32Index,s32Threshold);

    for(s32Index = 0;s32Index < SCR_USER_BAND_NUMBERS; s32Index++)
    {
        fprintf(fp_unic,"++++TONE %d freq is:%d\r\n",s32Index,sMinAgc[s32Index].s32Freq);
    }
    ...
    for(u32UBCount = 0; u32UBCount < SCR_USER_BAND_NUMBERS;u32UBCount++)
    {
        ...
        fprintf(fp_unic,"++++sMinAgc.u32Freq:%d+++sMinAgc.u32Agc:%d\r\n",sMinAgc[u32UBCount].s32Freq,sMinAgc[u32UBCount].s32Agc);
        fprintf(fp_unic,"freq start:%dMHz ",sSCRArray[s32MinAgcIndex - 50].s32Freq);
        for(s32Index = s32MinAgcIndex -50; s32Index < s32MinAgcIndex + 50; s32Index++)
        {
            fprintf(fp_unic,"%d ",sSCRArray[s32Index].s32Agc);
        }
	...
	for(u32UBCount = 0; u32UBCount < u32RealUBNumber;u32UBCount++)
	{
		...
		for(i = 0;i < 10;i++)
		{
			(HI_VOID)TUNER_Get_Agc(u32TunerPort, sMinAgc[u32UBCount].s32Freq, &s32AgcValue);
...
{
    static HI_UNF_TUNER_UNICABLE_SCAN_PARA_S stLocalScanPara;
    HI_S32 s32Result;
    pthread_t*      pScanUB = HI_NULL;
    ...
    if (HI_NULL != pScanUB)
    {
        (HI_VOID)pthread_join(*pScanUB, HI_NULL);
        HI_FREE(HI_ID_TUNER, pScanUB);
        pScanUB = HI_NULL;
    }

    pScanUB = (pthread_t*)HI_MALLOC(HI_ID_TUNER, sizeof(pthread_t));
    if (HI_NULL == pScanUB)
    {
    ...
    stLocalScanPara = stScanPara;
    /*全频段扫描疑似tone信号，并保存*/
    s32Result = pthread_create(pScanUB, 0, (HI_VOID *)Scr_BlindScanTone, &stLocalScanPara);
