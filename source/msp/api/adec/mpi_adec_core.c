...
HI_S32 ADEC_FoundRegisterDeoder(HI_U32 enDstCodecID)
{
    HI_HA_DECODE_S *p, *pNext;
    p = g_hFirstDecoder;
    while (p)
    {
        pNext = p->pstNext;
        if( HA_GET_ID(enDstCodecID)== HA_GET_ID(p->enCodecID))
        {
            return HI_SUCCESS;
        }
        p = pNext;
...
static HI_VOID ADECDeInitOutBuf(ADEC_CHAN_S* pstAdecChan, ADEC_STREAM_OUTBUF_S*  pstOutBuf )
{
    ...
    if (HI_NULL_PTR != pstOutBuf->pu32BaseAddr)
    {
        ...
        {
#ifdef ADEC_MMZ_OUTBUF_SUPPORT
            (HI_VOID)HI_MMZ_Flush(pstOutBuf->sAdecOutMMzBuf.phyaddr);
...
static HI_VOID ADECInitProcInfo(ADEC_PROC_ITEM_S* pstAdecInfo)
{
    ...
    pstAdecInfo->u32SavePcmCnt = 0;
    pstAdecInfo->u32SaveEsCnt = 0;
...
static HI_S32 ADECInitChannel( ADEC_CHAN_S* pstAdecChan )
{
    ...
    HI_WARN_ADEC("ADEC Core Init channel success \n");
    return HI_SUCCESS;
}
...
static HI_S32 ADECDeInit_Channel( ADEC_CHAN_S* pstAdecChan )
{
    ...
    ADECDeInitInBuf(pstAdecChan, &pstAdecChan->InStreamBuf);
    ADECDeInitDecoder(&pstAdecChan->decAttr);
    HI_WARN_ADEC("ADEC Core DeInit channel success \n");
...
#endif
    HI_WARN_ADEC("ADEC Core Open channel success \n");
...
static HI_VOID  ADECCloseChannel(ADEC_CHAN_S* pstAdecChan)
{
    ...
    if (pstAdecChan->AdecDevFd >= 0)
    {
        Ret = ioctl(pstAdecChan->AdecDevFd, DRV_ADEC_PROC_EXIT, &pstAdecChan->pstAdecInfo);
        if (Ret < 0)
        {
           HI_WARN_ADEC("ADEC ioctl Failed \n");
           return ;
        }
    ...
    HI_WARN_ADEC("ADEC Core Close channel success \n");
...
HI_S32 ADEC_Open(HI_HANDLE* phAdec)
{
    ...
    *phAdec = chan_id;
    ...
    HI_WARN_ADEC("ADEC Core Open success \n");
...
HI_S32 ADEC_Close(HI_HANDLE hAdec)
{
    ...
    HI_WARN_ADEC("ADEC Core Close success \n");
                       ...
                       HI_U32 u32PtsMs)
{
    ...
    if ( pstAdecInfo->enEsCtrlState == ADEC_CMD_CTRL_START )
    {
        ...
        if (fEstream)
        {
            HI_S32 s32Len = fwrite(pstStream1->pu8Data, 1, pstStream1->u32Size, fEstream);   // discard PTS
			...
			{
	            s32Len = fwrite(pstStream2->pu8Data, 1, pstStream2->u32Size, fEstream);   // discard PTS
...
HI_S32 ADEC_GetBufferStatus (HI_HANDLE hAdec, ADEC_BUFSTATUS_S* pstBufStatus)
{
    ...
    CHECK_ADEC_STATE(pstAdecChan->beAssigned, &pstAdecChan->ApiMutex);
    CHECK_ADEC_STATE_WARNING(pstAdecChan->beWork, &pstAdecChan->ApiMutex);
    pstInBuf = &(pstAdecChan->InStreamBuf);
#ifdef HA_HW_CODEC_SUPPORT
    if (HI_TRUE == pstAdecChan->bHwDecoder)
    {
                          ...
                          ADEC_EXTFRAMEINFO_S* pstExtInfo)
{
    ...
    if ( pstAdecChan->pstAdecInfo->enPcmCtrlState == ADEC_CMD_CTRL_START )
    {
        ...
        if (fPcm)
        {
            HI_S32 s32Len = fwrite((HI_VOID*)pstAOFrame->ps32PcmBuffer, sizeof(short), pstAOFrame->u32PcmSamplesPerFrame * pstAOFrame->u32Channels, fPcm);	 // discard PTS
...
HI_S32 ADEC_DropStream(HI_HANDLE hAdec, HI_U32 u32SeekPts)
{
    ...
    CHECK_ADEC_STATEARG3(pstAdecChan->beAssigned, &pstAdecChan->IOMutex, &pstAdecChan->DataMutex, &pstAdecChan->ApiMutex);
    CHECK_ADEC_STATEARG3(pstAdecChan->beWork, &pstAdecChan->IOMutex, &pstAdecChan->DataMutex, &pstAdecChan->ApiMutex);
