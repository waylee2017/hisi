...
HI_S32 HI_MPI_ADEC_FoundRegisterDeoder(HI_U32 enDstCodecID)
{
    HI_S32 retval;
    retval = ADEC_FoundRegisterDeoder(enDstCodecID);
    return retval;
}
...
HI_S32 HI_MPI_ADEC_Open(HI_HANDLE *phAdec)
{
    ...
    if ((HI_S32)(*phAdec) < ADEC_INSTANCE_MAXNUM)
    {
        ...
        if (!ptrmem)
        {
			...
			if(retval != HI_SUCCESS)
			{
				HI_ERR_ADEC("ADEC_Close failed %d\n",retval);
			}
            return HI_FAILURE;
        }
    }
    ...
    *phAdec = *phAdec | (HI_ID_ADEC << 16);
     ...
     HI_WARN_ADEC("HDEC OPEN Success\n");
    return HI_SUCCESS;
}
...
HI_S32 HI_MPI_ADEC_Close (HI_HANDLE hAdec)
{
    ...
    HI_WARN_ADEC("ADEC Close return value %d\n",retval);
    return retval;
}
...
HI_S32 HI_MPI_ADEC_Start(HI_HANDLE hAdec)
{
    ...
    HI_WARN_ADEC("ADEC Start return value %d\n",retval);
    return retval;
}
...
HI_S32 HI_MPI_ADEC_Stop(HI_HANDLE hAdec)
{
    ...
    HI_WARN_ADEC("ADEC Stop return value %d\n",retval);
    return retval;
}
...
HI_S32 HI_MPI_ADEC_DropStream(HI_HANDLE hAdec, HI_U32 u32SeekPts)
{
    HI_S32 retval;
    ADECGetRealChn(hAdec);
    retval = ADEC_DropStream(hAdec, u32SeekPts);
...
HI_S32 HI_MPI_ADEC_CheckNewEvent(HI_HANDLE hAdec, ADEC_EVENT_S *pstNewEvent)
{
    HI_S32 retval;
    ADECGetRealChn(hAdec);
    retval = ADEC_CheckNewEvent(hAdec, pstNewEvent);
    return retval;
}
#ifdef __cplusplus
 #if __cplusplus
