...
#define PVRFileGetRealFNameHisi(fileName, realName, nodeIdx) do {\
        ...
        {\
            snprintf(realName, PVR_MAX_FILENAME_LEN,"%s.%04d.ts", fileName, nodeIdx); \
        }\
    }while(0)
...
ssize_t PVR_READALL(void *buf, size_t size, PVR_FILE fd,  off64_t offset)
{
    ...
    do {
        if ((n = PVR_READ((void*)((HI_U8 *)buf + nread), (size - (size_t)nread), fd, offset + (off64_t)nread)) == (-1))
        {
            //lint -e774
            if (NULL != &errno)
            {
...
HI_BOOL PVR_ACCESS(const HI_CHAR *filename)
{
    HI_S32 s32Fd;
    s32Fd = PVR_OPEN(filename, PVR_FOPEN_MODE_DATA_READ);
    if (s32Fd == -1)
    {
        return HI_FALSE;
    }
...
ssize_t PVR_PREAD64(HI_U8 *pMem, HI_U32 size, PVR_FILE64 file, HI_U64 offset)
{
    ...
    if (readLen1 != size) /* need another file */
    {
        ...
        readed = (HI_U32)PVR_READ((void*)(pMem+readLen1), (size_t)readLen2, sysFd,(off64_t)(offset - pPvrFile->u64StartOfCurFd));
        if (-1 == (HI_S32)readed)
        {
            //perror("PVR_READ...333...");
            return -1;
                 ...
                 const void *data)
{
    ...
    else
    {
        ...
        ret = sys_tem((const char*)system_mount_command);/*unsafe function correct, this API is not used any more*/
