...
STATIC HI_S32 PVRIndexGetNextEntry(PVR_INDEX_HANDLE handle, PVR_INDEX_ENTRY_S *pEntry);
#ifndef HI_PVR_SOFT_INDEX_SUPPORT
...
static HI_BOOL PVRIndexCheckReadFrmIsInFile(HI_U32 u32ReadFrm,PVR_CYC_HEADER_INFO_S *pstCycHeaderInfo)
{
    ...
    if (0 != u32LastFrame) /*if u32LastFrame == 0, just beginning of recording, it should return HI_FALSE*/
    {
        if (u32StartFrame < u32EndFrame)
        {
            /*no rewind: object frame should located [u32StartFrame, u32EndFrame]*/
        ...
        {
            /*rewind:  object frame should located [u32StartFrame, u32LastFrame]  or [0, u32EndFrame]*/
...
static HI_U32 PVRIndexGetRecTimeStamp(PVR_INDEX_HANDLE handle, const HI_UNF_DMX_REC_INDEX_S *pstRecIdx)
{
    ...
    HI_WARN_PVR("\t(0x%08x: %ums, %ums, %ums)\n", (HI_U32)handle, pstTimeStamp->u32CurFrameTimeMs, pstRecIdx->u32DataTimeMs, pstTimeStamp->u32OverFlowTimeMs);

    /*the first time, just initialize the pstTimeStamp->u32CurFrameTimeMs and return pstTimeStamp->u32CurFrameTimeMs*/
    if ((0 ==  pstTimeStamp->u32CurFrameTimeMs) && (0 == pstTimeStamp->u32OverFlowTimeMs))
    {
        pstTimeStamp->u32CurFrameTimeMs = pstRecIdx->u32DataTimeMs;
        return pstTimeStamp->u32CurFrameTimeMs;
    }

    /*overflow: add the new time to pstTimeStamp->u32OverFlowTimeMs*/
    if (pstTimeStamp->u32CurFrameTimeMs >= pstRecIdx->u32DataTimeMs)
    {
        pstTimeStamp->u32OverFlowTimeMs += pstTimeStamp->u32CurFrameTimeMs;
    }

    /*we should correct the timestamp with the pstTimeStamp->u32OverFlowTimeMs*/
    pstTimeStamp->u32CurFrameTimeMs = pstRecIdx->u32DataTimeMs;
    return pstTimeStamp->u32CurFrameTimeMs + pstTimeStamp->u32OverFlowTimeMs;
#else
...
STATIC INLINE HI_VOID PVRIndexSetDftAttr(PVR_INDEX_HANDLE handle)
{
    ...
    memset(&handle->stStartFrame, 0x00, sizeof(PVR_INDEX_ENTRY_S));
    memset(&handle->stEndFrame, 0x00, sizeof(PVR_INDEX_ENTRY_S));
...
//直接读取end的下一个index，该index属于无效区间，必须直接从文件中读取
                                      ...
                                      HI_U64 u64NextEndOffset)
{
...
#ifdef HI_PVR_SOFT_INDEX_SUPPORT     
    HI_U32 u32StartFrameOld = pstCycMgr->u32StartFrame;

    u32StartFrameOld = (u32StartFrameOld + 10)%(pstCycMgr->u32LastFrame + 1);
    s32Ret = PVRIndexGetFBwardIPBFrameNum(hIndex, pstCycMgr ,1, PVR_INDEX_FRAME_I, u32StartFrameOld, &u32NextIFrm);
#else
    ...
    if (HI_SUCCESS == s32Ret)
    {
        ...
        if (s32Ret == HI_SUCCESS)
        {
            ...
            memcpy(&hIndex->stStartFrame, &stEntryTmp, sizeof(PVR_INDEX_ENTRY_S));
        }
        else
        {
                                                        ...
                                                        PVR_INDEX_ENTRY_S *pstEndEntry)
{
    ...
    memcpy(&hIndex->stEndFrame, pstEndEntry, sizeof(PVR_INDEX_ENTRY_S));
    memcpy(&hIndex->stLastFrame, pstEndEntry, sizeof(PVR_INDEX_ENTRY_S));
                                                           ...
                                                           PVR_INDEX_ENTRY_S *pstEndEntry)
{
    ...
    if (pstCycMgr->u32CycTimes == 0)
    {
        if (pstCycMgr->u32EndFrame == -1)
        {
            ...
            memcpy(&hIndex->stStartFrame, pstEndEntry, sizeof(PVR_INDEX_ENTRY_S));
        }
        else
        {
            pstCycMgr->u32EndFrame++;
                                     ...
                                     PVR_INDEX_ENTRY_S *pstIdxEntry)
{
    ...
    {
        /*bTimeRewinding表明当前是发生时间回绕时index落后，此时的maxsize和TimeRewindMaxSize
                已经更新为ts文件的大小，回绕点后的index的globaloffset必须实际达到TimeRewindMaxSize
                时才进行回绕处理
             */
        if (hIndex->bTimeRewinding == HI_TRUE)
        {
            //index的offset为当前globaloffset减去上一圈的TimeRewindMaxSize
            pstIdxEntry->u64Offset = pstIdxEntry->u64GlobalOffset - (hIndex->u64TimeRewindMaxSize - pstCycMgr->u64MaxCycSize);
        }
        else
        {
            if (hIndex->u64TimeRewindMaxSize > pstIdxEntry->u64GlobalOffset)
            {
                pstIdxEntry->u64Offset = pstIdxEntry->u64GlobalOffset - (hIndex->u64TimeRewindMaxSize - pstCycMgr->u64MaxCycSize);
            }
            else
    ...
    {
        pstIdxEntry->u64Offset = pstIdxEntry->u64GlobalOffset - pstCycMgr->u64MaxCycSize * pstCycMgr->u32CycTimes;
    }
}
                                       ...
                                       PVR_INDEX_ENTRY_S *pstIdxEntry)
{
    if (pstCycMgr->enRewindType == PVR_INDEX_REWIND_BY_TIME)
    {
        /*两种回绕处理机制
                   ...
                   */
        if (pstIdxEntry->u64GlobalOffset < hIndex->u64FileSizeGlobal)   //index落后
        {
...
static HI_S32 PVRIndexProcDisableRewind(PVR_INDEX_HANDLE hIndex,
                                       ...
                                       PVR_INDEX_ENTRY_S *pstIdxEntry)
{
    if (hIndex->bRecReachFix)   //如果已经到达指定大小或时间，不进行处理
    {
        return HI_FAILURE;
    }
...
HI_S32 PVR_Index_SaveToCache(PVR_INDEX_HANDLE hIndex, const HI_UNF_DMX_REC_INDEX_S *pstDmxIndexInfo)
{
    ...
    pstIdxBuf = &(hIndex->stIdxWriteCache);
#ifdef HI_PVR_SOFT_INDEX_SUPPORT
    if (HI_UNF_FRAME_TYPE_UNKNOWN != pstDmxIndexInfo->enFrameType)
#else
    if ((HI_UNF_FRAME_TYPE_I != pstDmxIndexInfo->enFrameType) &&
...
static HI_S32 PVRIndexCheckFileCorrect(PVR_INDEX_HANDLE handle, HI_U64 u64FileSize)
{
    ...
    u32IndexEntryNum = (HI_U32)((u64IndexFileSize - (HI_U64)handle->u32IdxStartOffsetLen)/(HI_U64)sizeof(stEndEntry));
        ...
        (0 == handle->stCycMgr.u64MaxCycSize))      //not rewind
    {
        ...
        {
            HI_ERR_PVR("HeadInfo's CycInfo.EndFrame(%u) or LastFrame(%u) > indexEntryNum(%u).\n", 
        ...
        s32Ret = PVRIndexGetEntryByNum(handle, &stEndEntry,handle->stCycMgr.u32EndFrame);
        if (s32Ret == HI_SUCCESS)
        {
            u64GlobalOffset = stEndEntry.u64GlobalOffset;
            u32FrameSize = stEndEntry.u32FrameSize;
            if (u64GlobalOffset + u32FrameSize > u64FileSize)
            {
                ...
                while (u64GlobalOffset + u32FrameSize > u64FileSize)
                {
                    ...
                    u32CurEndNum--;
                    s32Ret = PVRIndexGetEntryByNum(handle, &stEndEntry,u32CurEndNum);
                    if (HI_SUCCESS != s32Ret)
                    {
                                     ...
                                     HI_BOOL *pIsNoIdx)
{
    ...
    if (PVR_CHECK_FILE_EXIST(szIndexName))
    {
        *pIsNoIdx = HI_FALSE;
        ...
        HI_INFO_PVR("(S/E/L:%u/%u/%u), filesize:%llu.\n", 
        ...
        HI_INFO_PVR("(S/E/L:%u/%u/%u), filesize:%llu.\n", 
            ...
            handle->stCycMgr.u32LastFrame,
            u64TsFileSize);
        ret = PVR_Index_PlayGetFileAttrByFileName(pstPlayAttr->szFileName, handle, &handle->stIndexFileAttr);
        if (HI_SUCCESS != ret)
        {
            HI_ERR_PVR("PVR read Index File Header failed, ret = 0x%08x !\n", ret);
            PVR_CLOSE(handle->s32ReadFd);
            PVR_CLOSE(handle->s32SeekFd);
            PVR_CLOSE(handle->s32HeaderFd);
            goto ErrorExit;
...
PVR_INDEX_HANDLE PVR_Index_CreatRec(HI_U32 chnID, HI_UNF_PVR_REC_ATTR_S *pstRecAttr)
{
...
#ifdef HI_PVR_EXTRA_BUF_SUPPORT
    u32ReadIdxCacheLen = PVR_INDEX_READ_CACHE_NUM * sizeof(PVR_INDEX_ENTRY_S);
    u32WriteIdxCacheLen = PVR_INDEX_WRITE_CACHE_NUM * sizeof(PVR_INDEX_ENTRY_S);
#else
    ...
    if (ret == HI_SUCCESS)
    {
        if (enPort >= HI_UNF_DMX_PORT_3_RAM)
        {
...
HI_VOID PVR_Index_ResetRecAttr(PVR_INDEX_HANDLE handle)
{
    ...
    memset(&(handle->stTimeStamp), 0x00, sizeof(handle->stTimeStamp));
    memset(&handle->stCurRecFrame, 0, sizeof(PVR_INDEX_ENTRY_S));
#ifdef HI_PVR_EXTRA_BUF_SUPPORT
    memset(&handle->stFileCycInfo, 0x00, sizeof(handle->stFileCycInfo));
    handle->stFileCycInfo.u32EndFrame = -1;
...
HI_S32 PVR_Index_SetCADataInfo(HI_S32 s32Fd, HI_U8* pBuff, HI_U32 u32CADataLen)
{
    ...
    s32WriteRet = (HI_S32)PVR_WRITE(pBuff, u32CADataLen, s32Fd, PVR_GET_CA_DATA_OFFSET());
    if (s32WriteRet != (HI_S32)u32CADataLen)
    {
...
*****************************************************************************/
STATIC HI_S32 PVRIndexGetPlayNextEntry(PVR_INDEX_HANDLE handle, PVR_INDEX_ENTRY_S *pEntry)
{
    ssize_t readNum;
...
{
    HI_U32 u32StartFrm, u32EndFrm, u32LastFrm;
    ...
    pstRecStatus->u32CurTimeInMs = handle->stEndFrame.u32DisplayTimeMs - handle->stStartFrame.u32DisplayTimeMs;
    pstRecStatus->u32EndTimeInMs = handle->stEndFrame.u32DisplayTimeMs;
    pstRecStatus->u32StartTimeInMs = handle->stStartFrame.u32DisplayTimeMs;
...
HI_S32 PVR_Index_PlayGetFileAttrByFileName(const HI_CHAR *pFileName, PVR_INDEX_HANDLE pIdxHandle, HI_UNF_PVR_FILE_ATTR_S *pAttr)
{
    ...
    if((HI_NULL == pIdxHandle) || bRetry)
    {
        ...
        if (readNum != (HI_S32)sizeof(PVR_INDEX_ENTRY_S))
        {
            HI_WARN_PVR("read end_idx failed, endframe:%d, offset:%d.\n",stIdxHeaderInfo.stCycInfo.u32EndFrame, s32Offset);
            PVR_CLOSE(fdIdx);
            return HI_ERR_PVR_FILE_CANT_READ;
        }
