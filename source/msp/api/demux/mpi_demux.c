...
static HI_S32 DMX_MPI_PortGetType(const HI_UNF_DMX_PORT_E Port, DMX_PORT_MODE_E *PortMode, HI_U32 *PortId)
{
    ...
    switch (Port)
    {
        case HI_UNF_DMX_PORT_0_TUNER :
        case HI_UNF_DMX_PORT_1_TUNER :
        case HI_UNF_DMX_PORT_2_TUNER :
        case HI_UNF_DMX_PORT_3_TUNER :
        case HI_UNF_DMX_PORT_4_TUNER :
        {
        ...
        case HI_UNF_DMX_PORT_3_RAM :
        case HI_UNF_DMX_PORT_4_RAM :
        case HI_UNF_DMX_PORT_5_RAM :
        case HI_UNF_DMX_PORT_6_RAM :
        {
            HI_U32 Id = (HI_U32)Port - (HI_U32)HI_UNF_DMX_PORT_BASE_RAM;
...
HI_S32 HI_MPI_DMX_GetCapability(HI_UNF_DMX_CAPABILITY_S *pstCap)
{
    ...
    if (HI_SUCCESS == ret)
    {
        pstCap->u32PortNum = Param.u32TunerPortNum + Param.u32RamPortNum;
        pstCap->u32DmxNum = Param.u32DmxNum;
        pstCap->u32CommonChannelNum = Param.u32CommonChannelNum;
        pstCap->u32AudioChannelNum = Param.u32AudioChannelNum ;
        pstCap->u32VideoChannelNum = Param.u32VideoChannelNum;
...
HI_S32 HI_MPI_DMX_GetTSPortId(HI_U32 u32DmxId, HI_UNF_DMX_PORT_E *penPortId)
{
    ...
    if (HI_SUCCESS == ret)
    {
        *penPortId = (HI_UNF_DMX_PORT_E)Param.PortId;
        ...
        {
            *penPortId = (HI_UNF_DMX_PORT_E)(HI_UNF_DMX_PORT_BASE_RAM + Param.PortId);
        }
    }
...
HI_S32 HI_MPI_DMX_GetTSPortPacketNum(HI_U32 u32PortId, HI_UNF_DMX_PORT_PACKETNUM_S *sPortStat)
{
    HI_S32              ret;
    ...
    ret = DMX_MPI_PortGetType((HI_UNF_DMX_PORT_E)u32PortId, &Param.PortMode, &Param.PortId);
    if (HI_SUCCESS != ret)
    {
        return ret;
    }
...
HI_S32 HI_MPI_DMX_CreateTSBuffer(HI_UNF_DMX_PORT_E enPortId, HI_U32 u32TsBufSize, HI_HANDLE *phTsBuffer)
{
    ...
    if (HI_SUCCESS == ret)
    {
        ...
        if (0 == Param.TsBuf.u32BufUsrVirAddr)
        {
            ...
            if (-1 == ioctl(g_s32DmxFd, CMD_DEMUX_TS_BUFFER_DEINIT, (HI_S32)&PortId))
            {
...
HI_S32 HI_MPI_DMX_GetTSBufferPortId(HI_HANDLE hTsBuffer, HI_UNF_DMX_PORT_E *penPortId)
{
    ...
    *penPortId = (HI_UNF_DMX_PORT_E)(HI_UNF_DMX_PORT_BASE_RAM + PortId);
...
HI_S32 HI_MPI_DMX_GetBufferStatus(HI_HANDLE hChannel, HI_UNF_DMX_BUF_STATUS_S *pBufStat)
{
    HI_MPI_DMX_BUF_STATUS_S stBufStat;
    HI_S32 s32Ret;
    if (HI_NULL == pBufStat)
    {
        HI_ERR_DEMUX("parameter pstChAttr is NULL.\n");
        return HI_ERR_DMX_NULL_PTR;
    }
    s32Ret = HI_MPI_DMX_GetPESBufferStatus(hChannel, &stBufStat);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_DEMUX(" HI_MPI_DMX_GetPESBufferStatus failed:%x.\n",s32Ret);
        return s32Ret;
    }
    pBufStat->u32BufSize  = stBufStat.u32BufSize;
    pBufStat->u32UsedSize = stBufStat.u32UsedSize;
    return HI_SUCCESS;
}
...
HI_S32 HI_MPI_DMX_GetPesExtData(HI_HANDLE hChannel, HI_MPI_PES_EXT_DATA_S *pPesExtData)
{
    HI_S32             ret;
    DMX_PesExtData_S Param;

    MPIDmxCheckDeviceFd();
    MPIDmxCheckPointer(pPesExtData);

    memset(&Param, 0x0, sizeof(DMX_PesExtData_S));
    Param.hChannel= hChannel;

    ret = ioctl(g_s32DmxFd, CMD_DEMUX_PES_EXT_DATA_GET, (HI_S32)&Param);
    if (HI_SUCCESS == ret)
    {
        memcpy(pPesExtData, &(Param.stPesExtData), sizeof(HI_MPI_PES_EXT_DATA_S));
    }
