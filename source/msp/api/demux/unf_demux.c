...
{
    return HI_MPI_DMX_GetTSPortPacketNum(u32PortId, sPortStat);
}
...
{
    return HI_MPI_DMX_AcquireRecDataAndIndex(hRecChn, pstRecDataIdx);
}

HI_S32 HI_UNF_DMX_ReleaseRecDataAndIndex(HI_HANDLE hRecChn, HI_UNF_DMX_REC_DATA_INDEX_S* pstRecDataIdx)
{
    return HI_MPI_DMX_ReleaseRecDataAndIndex(hRecChn, pstRecDataIdx);
}
