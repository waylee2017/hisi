...
HI_VOID SYNC_ResetStatInfo(SYNC_S *pSync, SYNC_CHAN_E enChn)
{
    if (enChn == SYNC_CHAN_VID)
    {
        ...
        pSync->CrtBufStatus.VidBlockFlag = HI_FALSE;
        pSync->CrtBufStatus.VidBufPercent = 0;
        pSync->CrtBufStatus.VfmwFrmNum = 0;
        pSync->CrtBufStatus.VfmwStrmSize = 0;
        pSync->CrtBufStatus.VidStrmInBps = 0;
        pSync->CrtBufStatus.VidEmptyFlag = HI_FALSE;
        pSync->CrtBufStatus.VidLowFlag = HI_FALSE;
        
        pSync->VidDiscard = HI_FALSE;
        ...
        pSync->VidInfo.BufNum = 0;
        pSync->VidInfo.IntTime = 20;
        pSync->VidInfo.DelayTime = 20;
        pSync->VidInfo.DispTime = 1;
        pSync->VidInfo.u32LastVidDispTime = -1;
        pSync->TplaySpeed = 0;
    ...
    if (enChn == SYNC_CHAN_AUD)
    {
        ...
        pSync->CrtBufStatus.AudBlockFlag = HI_FALSE;
       ...
       )
    {
		...
		pSync->PcrSyncInfo.PCRPreSyncAdjust = HI_FALSE;
...
HI_S32 HI_MPI_SYNC_Tplay(HI_HANDLE hSync, HI_U32 u32TplaySpeed)
{
    ...
    pSync->TplaySpeed = u32TplaySpeed;
...
HI_S32 HI_MPI_SYNC_SetBufState(HI_HANDLE hSync, HI_BOOL VidDiscard, SYNC_BUF_STATUS_S stBufStatus)
{
    ...
    if ((stBufStatus.VidBlockFlag != HI_TRUE)
      &&(stBufStatus.VidBlockFlag != HI_FALSE)
       )
    {
        HI_ERR_SYNC("para VidBlockFlag is invalid.\n");
        return HI_ERR_SYNC_INVALID_PARA;
    }

    if ((stBufStatus.AudBlockFlag != HI_TRUE)
      &&(stBufStatus.AudBlockFlag != HI_FALSE)
       )
    {
        HI_ERR_SYNC("para AudBlockFlag is invalid.\n");
        return HI_ERR_SYNC_INVALID_PARA;
    }
...
HI_S32 HI_MPI_SYNC_GetStatusEx(HI_HANDLE hSync, SYNC_STATUS_S *pSyncStatus)
{
    ...
    pSyncStatus->u32LastVidPts  = pSync->VidLastPts;
    pSyncStatus->u32LastDispTime = pSync->VidInfo.u32LastVidDispTime;
