...
#define PVR_CIPHER_MAX_LENGTH_ONCE  (1024*1024-16)
...
HI_S32 HI_UNF_ADVCA_Init(HI_VOID)
{
	...
	if(g_AdvcaInitCounter > 0)
    {
        ...
        if (HI_SUCCESS != ret)
        {
            HI_ERR_CA("get DVBKeyLadderStage err. \n");
        ...
        if (HI_SUCCESS != ret)
        {
            HI_ERR_CA("get R2RKeyLadderStage err. \n");
			(HI_VOID)HI_UNF_ADVCA_DeInit();
            return HI_FAILURE;
        }
...
HI_S32 HI_UNF_ADVCA_EnableSecBoot(HI_UNF_ADVCA_FLASH_TYPE_E enFlashType)
{
    ...
    if( (enFlashType == HI_UNF_ADVCA_FLASH_TYPE_NOR) || (enFlashType == HI_UNF_ADVCA_FLASH_TYPE_EMMC) )
    {
        HI_ERR_CA("Failed, not support flash type:0x%x.\n", (HI_U32)enFlashType);
        return HI_ERR_CA_INVALID_PARA;
    }
...
HI_S32 HI_UNF_ADVCA_SetFlashTypeEx(HI_UNF_ADVCA_FLASH_TYPE_E enFlashType)
{
    ...
    if( (enFlashType == HI_UNF_ADVCA_FLASH_TYPE_NOR) || (enFlashType == HI_UNF_ADVCA_FLASH_TYPE_EMMC) )
    {
        HI_ERR_CA("Failed, not support flash type:%d.\n", (HI_U32)enFlashType);
        return HI_ERR_CA_INVALID_PARA;
    }    
...
HI_VOID ADVCA_PVR_RemoveCaFile(HI_CHAR *pszFileName, HI_U32 u32FileNameLen)
{
    HI_CHAR szIdxFileName[128];
    HI_CHAR szKeyFileName[128];
    HI_S32 s32Fd = -1;
    
    memset(szIdxFileName, 0x00, 128*sizeof(HI_CHAR));
    memset(szKeyFileName, 0x00, 128*sizeof(HI_CHAR));

    if ((HI_NULL == pszFileName) || (128 <= u32FileNameLen) || (strlen(pszFileName) != u32FileNameLen))
    {
        HI_ERR_CA("Invalid filename or filename length(%u)\n", u32FileNameLen);
        return;
    }

    snprintf(szIdxFileName, 128, "%s.idx", pszFileName);
    szIdxFileName[127] = '\0';
    if (HI_SUCCESS != ADVCA_PVR_GetCADataFileName(szIdxFileName, szKeyFileName))
    {
        HI_ERR_CA("Invalid recording filename(%s)!\n", pszFileName);
        return;
    }
    (HI_VOID)remove(szKeyFileName);
    return;
}
...
HI_S32 HI_UNF_ADVCA_PVR_RecOpen(HI_U32 u32RecChnID)
{
    ...
    ADVCA_PVR_RemoveCaFile(recAttr.szFileName, recAttr.u32FileNameLen);
...
HI_S32 HI_UNF_ADVCA_PVR_RecClose(HI_U32 u32RecChnID)
{
    ...
    {
        if (HI_SUCCESS != ADVCA_PVR_SetCAData(pstPvrRecCaData->FileName, pstPvrRecCaData, HI_NULL))
        {
            HI_ERR_CA("something error while save the ca data!the recording is wrong\n");
        }
    }
...
HI_S32 HI_UNF_ADVCA_PVR_PlayOpen(HI_U32 u32PlayChnID, HI_UNF_ADVCA_PVR_PlAY_INFOR_S stPlayInfo)
{
    ...
    PvrPlayChannelInfo[PlyChnIndex].bIsTimeshiftPlay = HI_FALSE;//stPlayInfo.bIsTimeshiftPlay;
    PvrPlayChannelInfo[PlyChnIndex].u32RecChnID = 0xFF;//stPlayInfo.u32RecChnID;

    /*Every time start pvr play, we need reset the value of track key*/
    memset(PvrPlayChannelInfo[PlyChnIndex].PvrPlayTrackKey, 0x00, 16);
...
static HI_S32 ADVCA_PVR_WriteCallBack(HI_UNF_PVR_CA_CALLBACK_ARGS_S* pstCAPVRArgs)    
{     
    HI_S32 ret = 0;
...
HI_S32 HI_UNF_ADVCA_PVR_WriteCallBack(HI_UNF_PVR_CA_CALLBACK_ARGS_S* pstCAPVRArgs)
{
    ...
    memset(&stCAPVRArgs, 0x00, sizeof(HI_UNF_PVR_CA_CALLBACK_ARGS_S));

    CA_CheckPointer(pstCAPVRArgs);
    CA_CheckPointer(pstCAPVRArgs->pFileName);
    
    u64GlobalOffset = pstCAPVRArgs->u64GlobalOffset;
    u32DataSize = pstCAPVRArgs->u32DataSize;
    u32PhyAddr = pstCAPVRArgs->u32PhyAddr;
    memcpy(&stCAPVRArgs, pstCAPVRArgs, sizeof(stCAPVRArgs));
    while (0 != u32DataSize)
    {
        if (PVR_CIPHER_MAX_LENGTH_ONCE < u32DataSize)
        {
            stCAPVRArgs.u64GlobalOffset = u64GlobalOffset;
            stCAPVRArgs.u32PhyAddr = u32PhyAddr;
            stCAPVRArgs.u32DataSize = PVR_CIPHER_MAX_LENGTH_ONCE;
            s32Ret = ADVCA_PVR_WriteCallBack(&stCAPVRArgs);
            if (HI_SUCCESS != s32Ret)
            {
                HI_ERR_CA("call ADVCA_PVR_WriteCallBack failed, ret = 0x%08x\n", s32Ret);
                return s32Ret;
            }
            u64GlobalOffset += PVR_CIPHER_MAX_LENGTH_ONCE;
            u32PhyAddr += PVR_CIPHER_MAX_LENGTH_ONCE;
            u32DataSize -= PVR_CIPHER_MAX_LENGTH_ONCE;
        }
        else
        {
            stCAPVRArgs.u64GlobalOffset = u64GlobalOffset;
            ...
            u64GlobalOffset += u32DataSize;
            u32PhyAddr += u32DataSize;
            u32DataSize = 0;
        }
    }
    return HI_SUCCESS;
...
static HI_BOOL ADVCA_PVR_CheckIsTimeshift(HI_U32 u32PlayIndex, HI_CHAR *pFileName)
{
    HI_CA_PVR_CADATA_S *pstRecCadata = HI_NULL;
    HI_U32 i = 0;
    ...
    if (HI_TRUE == PvrPlayChannelInfo[u32PlayIndex].bIsTimeshiftPlay)
    {
        return HI_TRUE;
    }
    if (INVALID_VALUE == PvrPlayChannelInfo[u32PlayIndex].u32RecChnID)
    {
        return HI_FALSE;
    }
    for (i = 0; i < PVR_CA_MAX_CHANNEL; i++)
    {
        ...
        pstRecCadata = (HI_CA_PVR_CADATA_S *)&(PvrRecChannelInfo[i].stPvrRecCaData);
        if (0 == strncmp(pstRecCadata->FileName, pFileName, strlen(pFileName)))
        {
            printf("check timeshift\n");
            PvrPlayChannelInfo[u32PlayIndex].u32RecChnID = PvrRecChannelInfo[i].u32ChnID;
            PvrPlayChannelInfo[u32PlayIndex].bIsTimeshiftPlay = HI_TRUE;
            return HI_TRUE;
        }
    }        
    PvrPlayChannelInfo[u32PlayIndex].u32RecChnID = INVALID_VALUE;
    return HI_FALSE;
}

static HI_S32 ADVCA_PVR_ReadCallBack(HI_UNF_PVR_CA_CALLBACK_ARGS_S* pstCAPVRArgs)
{   
    ...
    ADVCA_PVR_LOCK();  
    PvrPlayChannelInfo[PlyChnIndex].bIsTimeshiftPlay = ADVCA_PVR_CheckIsTimeshift(PlyChnIndex, pFileName);
    ...
    {        
        ...
        if (u32DataSize < u32DataSize1)
        {
            HI_ERR_CA("(u32DataSize1,  u32DataSize) = (%u, %u, %u)\n", u32DataSize1,  u32DataSize);
            return HI_FAILURE;
        }
        else
        {
...
HI_S32 HI_UNF_ADVCA_PVR_ReadCallBack(HI_UNF_PVR_CA_CALLBACK_ARGS_S* pstCAPVRArgs)
{
    ...
    while (0 != u32DataSize)
    {
        if (PVR_CIPHER_MAX_LENGTH_ONCE < u32DataSize)
        {
            ...
            stCAPVRArgs.u32PhyAddr = u32PhyAddr;
            stCAPVRArgs.u32DataSize = PVR_CIPHER_MAX_LENGTH_ONCE;
            s32Ret = ADVCA_PVR_ReadCallBack(&stCAPVRArgs);
            if (HI_SUCCESS != s32Ret)
            {
                HI_ERR_CA("call ADVCA_PVR_WriteCallBack failed, ret = 0x%08x\n", s32Ret);
                return s32Ret;
...
HI_S32 HI_UNF_ADVCA_PVR_CreateCAPrivateFile(HI_U32 u32RecChnID,HI_CHAR * pCAPrivateFileName, HI_UNF_ADVCA_Time_S *pCurTime)
{
    ...
    //if (HI_NULL == pstPvrRecCaData)
    //{
    //    HI_ERR_CA("pstPvrRecCaData is null\n");
    //    free(pstMACPrivateFileHead);
    //    return HI_FAILURE;
    //}
...
HI_S32 HI_UNF_ADVCA_PVR_CheckCAPrivateFileMAC( HI_CHAR * pCAPrivateFileName, HI_UNF_ADVCA_Time_S *pOutRecordStartTime)
{
    ...
    {
        HI_ERR_CA("error :pstPrivateFileHead->FPNum (%d) is too large!\n", pstPrivateFileHead->FPNum);
        goto CheckMacError;
    }
    HI_INFO_CA("HI_UNF_ADVCA_PVR_CheckCAPrivateFileMAC:pstPrivateFileHead->FPNum is %d------>\n", pstPrivateFileHead->FPNum);

    HI_U32 u32tmp = (pstPrivateFileHead->FPNum)*((HI_U32)sizeof(ADVCA_PVR_MAC_FP_INFO_S));

    pu8Buf = (HI_U8 *)malloc(sizeof(ADVCA_PVR_CA_PrivateFileHead_S) + u32tmp);
    if (pu8Buf == HI_NULL)
    {
        close(fd);
        HI_ERR_CA("error, fail to malloc memory!\n");
...
HI_S32 HI_UNF_ADVCA_PVR_SaveURI( HI_U32 u32RecChnID,HI_CHAR * pCAPrivateFileName ,HI_UNF_PVR_URI_S* pstPVRURI)
{
    ...
    pu8Buf = malloc(sizeof(ADVCA_PVR_CA_PrivateFileHead_S)
...
//++++
    stVersion.enChipTypeSoft = HI_CHIP_TYPE_HI3716M;
	stVersion.enChipVersion = HI_CHIP_VERSION_V310;
//++++
    ...
    else if(((stVersion.enChipTypeSoft == HI_CHIP_TYPE_HI3716M) && (stVersion.enChipVersion == HI_CHIP_VERSION_V310)) ||

            ((stVersion.enChipTypeSoft == HI_CHIP_TYPE_HI3110E) && (stVersion.enChipVersion == HI_CHIP_VERSION_V500)))
    {
        //Hi3716MV310
        u32DCASID_MSBAddr = 0x10180110;
        u32DCASID_LSBAddr = 0x10180114;
    }
    else
    {
        //Hi3716CV200
...
HI_S32 HI_UNF_ADVCA_SetMiscKlLevel(HI_UNF_ADVCA_KEYLADDER_LEV_E enLevel)
{
	...
	stCmdParam.pu8ParamBuf[0] = enLevel;
    s32Ret = ioctl(g_s32CaFd, CMD_CA_SUPPER_ID, &stCmdParam);
    if (HI_SUCCESS != s32Ret)
    {
