...
extern HI_S32 HI_UNF_CIPHER_GetHandleConfig(HI_HANDLE hCipherHandle, HI_UNF_CIPHER_CTRL_S* pstCtrl);
extern HI_S32 HI_UNF_CIPHER_ConfigHandle(HI_HANDLE hCipher, HI_UNF_CIPHER_CTRL_S* pstCtrl);
...
#ifdef CFG_HI_CIPHER_HASH_IRDETO_CBCMAC
...
#if defined(CFG_HI_CIPHER_AES_CBCMAC) || defined(CFG_HI_CIPHER_HASH_IRDETO_CBCMAC)
...
#ifdef CFG_HI_CIPHER_AES_CBCMAC
...
static HI_S32 Cipher_HashMsgPadding(CIPHER_HASH_DATA_S *pCipherHashData)
{
    ...
    memset(pCipherHashData->au8Padding, 0, sizeof(pCipherHashData->au8Padding));
    ...
    memset(pCipherHashData->au8Padding, 0x80, 1);
    memset(pCipherHashData->au8Padding + 1, 0, pCipherHashData->u32PaddingLen - 1 - 8);
    /* write 8 bytes fix data length */
    memset(au8PadLen, 0, sizeof(au8PadLen));
     ...
     || (HI_UNF_CIPHER_HASH_TYPE_HMAC_SHA256 == pCipherHashData->enShaType) )
    {
        for (i = 0; i < 8; i++)
        {
            au8PadLen[i] = ((u32Tmp * 8 + 512) >> (7 - i) * 8) & 0xff;
        }
    }
    else
#endif
    {
        for (i = 0; i < 8; i++)
        {
            au8PadLen[i] = ((u32Tmp * 8) >> ((7 - i) * 8)) & 0xff;
        }
    }

    memcpy(pCipherHashData->au8Padding + pCipherHashData->u32PaddingLen - 8, au8PadLen, 8);
...
static HI_S32 CIPHER_HashFinal(HI_HANDLE hHashHandle, HI_U8 *pu8OutputHash)
{
    ...
    memcpy(pu8OutputHash, pstCipherHashData->u8Output, pstCipherHashData->u32OutputLen);
...
HI_S32 HI_MPI_CIPHER_HashUpdate(HI_HANDLE hHashHandle, HI_U8 *pu8InputData, HI_U32 u32InputDataLen)
{
    ...
    if((NULL == pu8InputData) || (pstCipherHashData == NULL))
    {
        HI_ERR_CIPHER("Invalid parameter!\n");
        return HI_ERR_CIPHER_INVALID_PARA;
    }
...
HI_S32 HI_MPI_CIPHER_HashFinal(HI_HANDLE hHashHandle, HI_U8 *pu8OutputHash)
{
    ...
    if((NULL == pu8OutputHash) || (pstCipherHashData == NULL))
    {
        HI_ERR_CIPHER("Invalid parameter!\n");
        return HI_ERR_CIPHER_INVALID_PARA;
    }
...
HI_S32 HI_MPI_CIPHER_LoadHDCPKey(HI_U8 *pu8EncryptedHDCPKey, HI_U32 u32Len)
{
    ...
    if ( (NULL == pu8EncryptedHDCPKey) || (332 != u32Len) )
    {
...
HI_S32 HI_MPI_CIPHER_GetRandomNumber(HI_U32 *pu32RandomNumber, HI_U32 u32TimeOutUs)
{
    ...
    if (Ret != HI_SUCCESS && HI_ERR_CIPHER_NO_AVAILABLE_RNG != Ret)
    {
        HI_WARN_CIPHER("Get random number failed, ret = %x\n", Ret);
		return Ret;
    }
