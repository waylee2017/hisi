...
HI_VOID JPEG_HDEC_GetImagInfo(j_decompress_ptr cinfo)
{
		...
		/**
		 **计算宏块的宽和高，配置size寄存器，逻辑写的时候是按照MCU写的，所以内存大小要足够
		 **/
        ...
        switch(cinfo->out_color_space){
			...
			case JCS_CrCbY:
			case JCS_YCbCr:
				 HI_GFX_GetStride(u32StrideNeedWidth * 3,&(pJpegHandle->stJpegSofInfo.u32DisplayStride),pJpegHandle->u32StrideAlign);
			...
			case JCS_RGBA_8888:
			#endif
				 HI_GFX_GetStride(u32StrideNeedWidth * 4,&(pJpegHandle->stJpegSofInfo.u32DisplayStride),pJpegHandle->u32StrideAlign);
                 pJpegHandle->stJpegSofInfo.u32DecStride  =  YWidthTmp * 4;
				 cinfo->out_color_components = 4;
				 break;
			case JCS_ARGB_1555:
			case JCS_ABGR_1555:
				 HI_GFX_GetStride(u32StrideNeedWidth * 2,&(pJpegHandle->stJpegSofInfo.u32DisplayStride),pJpegHandle->u32StrideAlign);
