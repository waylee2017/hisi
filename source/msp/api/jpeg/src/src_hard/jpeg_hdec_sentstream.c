...
HI_S32 JPEG_HDEC_SendStreamFromPhyMem(j_decompress_ptr cinfo)
{
      ...
      **/
      ...
      do
      {
          ...
          s32Ret = JPEG_HDEC_GetIntStatus(pJpegHandle, &eIntStatus, JPEG_INTTYPE_DELAY_TIME);
...
HI_S32 JPEG_HDEC_SendStreamFromReturnPhyMem(j_decompress_ptr cinfo)
{
      ...
      **/
      ...
      do
      {
          ...
          eIntStatus = JPG_INTTYPE_ERROR;
          (HI_VOID)JPEG_HDEC_GetIntStatus(pJpegHandle, &eIntStatus, JPEG_INTTYPE_DELAY_TIME);
          if (JPG_INTTYPE_CONTINUE == eIntStatus)
          {
              continue;
          }
...
static inline HI_BOOL JPEG_HDEC_CheckCallBackFuncStreamIsNull(j_decompress_ptr cinfo, HI_U32 *pReadStreamPos)
{
    ...
    *pReadStreamPos = 0;
    ...
    bFillStream = cinfo->src->fill_input_buffer(cinfo);
    if (cinfo->src->bytes_in_buffer)
    {/** android: cinfo->src->bytes_in_buffer equal to zero **/
        bReachEOF = HI_FALSE;
    }

    if ((2 == cinfo->src->bytes_in_buffer) && (0xFF == (*(cinfo->src->next_input_byte))) && (0xD9 == (*(cinfo->src->next_input_byte + 1))))
    {/** all can use this check:cinfo->src->bytes_in_buffer equal to two, and dfb will dispose for this **/
        bReachEOF = HI_TRUE;
    }

    if (HI_FALSE == bFillStream)
    {/** all can use this check, skia and web use this **/
        bReachEOF = HI_TRUE;
    }
