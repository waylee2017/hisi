...
#define CODEC_FIND_CODEC_BY_STRUCT(pCodec, pCodecParam) \
    { \
        ...
        s32LockRet = CODEC_LOCK(s_stParam.stMutex); \
        if(HI_SUCCESS != s32LockRet)\
        {\
            HI_ERR_CODEC("[%s][%d] CODEC_LOCK failed!!\n", __func__,__LINE__); \
        }\
        ...
        s32LockRet = CODEC_UNLOCK(s_stParam.stMutex); \
        if(HI_SUCCESS != s32LockRet)\
        {\
            HI_ERR_CODEC("[%s][%d] CODEC_UNLOCK failed!!\n", __func__,__LINE__); \
        }\
    }
...
static HI_VOID CODEC_UnRegister(CODEC_PARAM_S *pstCodecParam)
{
    ...
    s32LockRet = CODEC_LOCK(pstCodecParam->stMutex);
    ...
    s32LockRet = CODEC_UNLOCK(pstCodecParam->stMutex);
...
HI_CODEC_S *HI_CODEC_Create(HI_HANDLE *phInst, const HI_CODEC_OPENPARAM_S *pstParam)
{
    ...
    if (HI_NULL == pstCodecParam)
    {
        HI_ERR_CODEC("No usable codec:%d.\n", pstParam->enID);
        return HI_NULL;
    }
    ...
    if (HI_NULL == pstInst)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_CODEC("[%s][%d] Destroy failed!!\n", __func__, __LINE__);
        }
