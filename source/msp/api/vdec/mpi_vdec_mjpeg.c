...
#define HI_MJPEG_MAX_CHANNEL (HI_VDEC_MAX_INSTANCE)
...
HI_U32 g_lowDelayFrameIndex[HI_VDEC_MAX_INSTANCE];
extern HI_U32 g_lowDelayVdecHandle[HI_VDEC_MAX_INSTANCE];
...
HI_S32 MJPEG_Create(HI_HANDLE *phInst, const HI_CODEC_OPENPARAM_S *pstParam)
{
    ...
    s32LockRet = VDEC_LOCK(s_stMjpegMutex);
    ...
    for (i = 0; i < HI_MJPEG_MAX_CHANNEL; i++)
    {
        if (!s_stMjpegParam[i].bUsed)
        {
            ...
            s32LockRet = VDEC_UNLOCK(s_stMjpegMutex);
...
HI_S32 MJPEG_Destroy(HI_HANDLE hInst)
{
    ...
    if (((hInst & 0xff) < HI_VDEC_MAX_INSTANCE) && ((hInst & 0xff) > 0))
    {
...
HI_S32 MJPEG_GetStreamInfo(HI_HANDLE hInst, HI_CODEC_STREAMINFO_S *pstStreamInfo)
{
    ...
    pstStreamInfo->stVideo.enAspectRatio = HI_CODEC_ASPECT_RATIO_BUTT;
...
static HI_VOID MJPEG_WaitDecodeOver(JPG_HANDLE Handle, JPG_STATE_E *pState, HI_U32 *pStateIndex)
{
    HI_S32 cnt;
    HI_S32 s32Ret;
    HI_U8 *pBufAddr = HI_NULL;
    HI_U32 BufSize;
    
    for (cnt = 0; cnt < 500; cnt++)
    {
        s32Ret = JPG_IsNeedStream(Handle, (HI_VOID **)&pBufAddr, &BufSize);
        s32Ret = JPG_GetStatus(Handle, pState, pStateIndex);

        if (*pState == JPG_STATE_DECODEERR || *pState == JPG_STATE_DECODED)
        {
            if (*pState == JPG_STATE_DECODEERR)
            {
...
static HI_VOID MJPEG_GetOutFrame(HI_CODEC_STREAM_S *pstIn, JPG_SURFACE_S *pSurface, HI_CODEC_FRAME_S *pstOut)
{
    switch (pSurface->SurfaceInfo.MbSurface.MbFmt)
    {
    ...
    pstOut->unInfo.stVideo.u32Width = pSurface->SurfaceInfo.MbSurface.YWidth;
    pstOut->unInfo.stVideo.u32Height = pSurface->SurfaceInfo.MbSurface.YHeight;

    pstOut->unInfo.stVideo.u32YAddr = pSurface->SurfaceInfo.MbSurface.YPhyAddr;
    pstOut->unInfo.stVideo.u32YStride = pSurface->SurfaceInfo.MbSurface.YStride;

    pstOut->unInfo.stVideo.u32UAddr = pSurface->SurfaceInfo.MbSurface.CbCrPhyAddr;
    pstOut->unInfo.stVideo.u32UStride = pSurface->SurfaceInfo.MbSurface.CbCrStride;
...
HI_S32  MJPEG_DecodeFrame(HI_HANDLE hInst, HI_CODEC_STREAM_S *pstIn, HI_CODEC_FRAME_S *pstOut)
{
    ...
    MJPEG_WaitDecodeOver(Handle, &State, &StateIndex);

    if (State == JPG_STATE_DECODED)
    {
        MJPEG_GetOutFrame(pstIn, &Surface, pstOut);
...
HI_S32 MJPEG_DecodeFrame(HI_HANDLE hInst, HI_CODEC_STREAM_S *pstIn, HI_CODEC_FRAME_S *pstOut)
{
    ...
    /** 硬件解码失败转软解使用的**/
    stSurfaceDesc.stOutSurface.pOutVir[0]  = (HI_CHAR *)pstOut->stOutputAddr.u32Vir;
    stSurfaceDesc.stOutSurface.pOutVir[1]  = (HI_CHAR *)(pstOut->stOutputAddr.u32Vir + stJpegInfo.u32OutSize[0]);
