...
HI_S32 HAL_AOE_ENGINE_SetAvcAttr(AOE_ENGINE_ID_E enENGINE, HI_UNF_SND_AVC_ATTR_S* pstAvcAttr) //avc
{
    HI_S32 Ret = HI_FAILURE;
    CHECK_ENGINE_OPEN(enENGINE);
...
HI_S32 HAL_AOE_ENGINE_SetAvcEnable(AOE_ENGINE_ID_E enENGINE, HI_BOOL bEnable)
{
    ...
    s32Ret = iHAL_AOE_ENGINE_SetAvcEnable(enENGINE, bEnable);
    if (HI_SUCCESS != s32Ret)
    {
        return s32Ret;
    }

    s32Ret = iHAL_AOE_ENGINE_SetAvcChangeAttr(enENGINE, HI_TRUE);
    if (HI_SUCCESS != s32Ret)
    {
        return s32Ret;
    }
...
HI_S32 HAL_AOE_ENGINE_SetPreGainEnable(AOE_ENGINE_ID_E enENGINE, SND_ENGINE_PREGAIN_S* pstPreGain)
{
    ...
    return iHAL_AOE_ENGINE_SetPreGainEnable(enENGINE, pstPreGain);
}
#endif
