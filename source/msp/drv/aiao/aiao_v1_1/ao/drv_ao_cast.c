...
static HI_VOID  CastSetMute(HI_HANDLE hSndOp,HI_BOOL bMute)
{
    AOE_AOP_ID_E Aop;

    if(hSndOp == HI_NULL)
    {
        HI_ERR_AIAO("CastSetVolume  hSndOp=%p\n", hSndOp);
        return ;
    }
    Aop = SND_OpGetAopId(hSndOp);
...
static HI_VOID  CastSetVolume(HI_HANDLE hSndOp,HI_U32 u32VolumeLdB, HI_U32 u32VolumeRdB)
{
    AOE_AOP_ID_E Aop;
...
HI_S32 CAST_CreateNew(SND_CARD_STATE_S *pCard, HI_UNF_SND_CAST_ATTR_S *pstCastAttr, MMZ_BUFFER_S *pstMMz, HI_U32 hCast)
{
	...
	state->u32KernelVirtAddr = pstMMz->u32StartVirAddr;
    state->hCast  = hCast;
    state->CastId = CastId;
