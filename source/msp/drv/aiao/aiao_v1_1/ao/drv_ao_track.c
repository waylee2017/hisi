...
HI_S32 TrackCreateEngine(HI_HANDLE* phSndEngine, AOE_ENGINE_CHN_ATTR_S* pstAttr, SND_ENGINE_TYPE_E enType)
{
    ...
    *phSndEngine = HI_NULL;
    ...
    if (pstAttr->u32DataFormat == 0)
    {
        SND_ENGINE_PREGAIN_S stPreGain;

        stPreGain.bPreGainEn = HI_TRUE;
        stPreGain.u32PreGainInt = VOLUME_0dB + 2;//2dB
        stPreGain.s32PreGainDec = 3;            //0.375dB
        HAL_AOE_ENGINE_SetPreGainEnable(enEngine, &stPreGain);
    }
#endif
...
HI_S32 GetHdmiChangeMode(SND_CARD_STATE_S* pCard, SND_TRACK_STREAM_ATTR_S* pstAttr, TRACK_STREAMMODE_CHANGE_E* penChange)
{
    ...
    if (SND_HDMI_MODE_PCM == mode)
    {
        ...
        {
            *penChange = TRACK_STREAMMODE_CHANGE_LBR2PCM;
        }
        else if (SND_HDMI_MODE_HBR == pCard->enHdmiPassthrough)
        {
            *penChange = TRACK_STREAMMODE_CHANGE_HBR2PCM;
        }
    }
    else if (SND_HDMI_MODE_LBR == mode)
    {
        ...
        {
            *penChange = TRACK_STREAMMODE_CHANGE_PCM2LBR;
        }
        else if (SND_HDMI_MODE_LBR == pCard->enHdmiPassthrough)
        {
            if (pstAttr->u32LbrSampleRate != stSndPortAttr.u32SampleRate)
            {
                *penChange = TRACK_STREAMMODE_CHANGE_LBR2LBR;
            }
        }
        else if (SND_HDMI_MODE_HBR == pCard->enHdmiPassthrough)
        {
            *penChange = TRACK_STREAMMODE_CHANGE_HBR2LBR;
        }
    }
    else if (SND_HDMI_MODE_HBR == mode)
    {
        if (SND_HDMI_MODE_PCM == pCard->enHdmiPassthrough)
        {
            *penChange = TRACK_STREAMMODE_CHANGE_PCM2HBR;
        }
        else if (SND_HDMI_MODE_LBR == pCard->enHdmiPassthrough)
        {
            *penChange = TRACK_STREAMMODE_CHANGE_LBR2HBR;
        }
        else if (SND_HDMI_MODE_HBR == pCard->enHdmiPassthrough)
        {
            if (pstAttr->u32HbrSampleRate != stSndPortAttr.u32SampleRate)
            {
                *penChange = TRACK_STREAMMODE_CHANGE_HBR2HBR;
            }
        }
    }
...
HI_S32 GetSpdifChangeMode(SND_CARD_STATE_S* pCard, SND_TRACK_STREAM_ATTR_S* pstAttr, TRACK_STREAMMODE_CHANGE_E* penSpdifChange)
{
    HI_S32 s32Ret = HI_SUCCESS;
    SND_OP_ATTR_S stSndPortAttr;
    ...
    if (bdisPassThrough)
    {
        ...
        {
            *penSpdifChange = TRACK_STREAMMODE_CHANGE_LBR2PCM;
        }
    }
    else
    {
        ...
        if (SND_SPDIF_MODE_LBR == pCard->enSpdifPassthrough)
        {
            if (pstAttr->u32LbrSampleRate != stSndPortAttr.u32SampleRate)
            {
                *penSpdifChange = TRACK_STREAMMODE_CHANGE_LBR2LBR;  /* shtream change samplerate */
            }
        }
        else
        {
            *penSpdifChange = TRACK_STREAMMODE_CHANGE_PCM2LBR;     /* atcive pass-through */
        }
    }
                               ...
                               STREAMMODE_CHANGE_ATTR_S* pstChange)
{
    ...
    if (HI_UNF_SND_TRACK_TYPE_MASTER == pTrack->stUserTrackAttr.enTrackType)
    {
        ...
        {
            Ret = GetSpdifChangeMode(pCard, pstAttr, &enChange);
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AO("GetSpdifChangeMode Failed\n");
                return;
            }
            pstChange->enSpdifChange = enChange;
        }

        if (SND_HDMI_MODE_NONE != pCard->enHdmiPassthrough)
        {
            enChange = TRACK_STREAMMODE_CHANGE_NONE;
            Ret = GetHdmiChangeMode(pCard, pstAttr, &enChange);
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AO("GetHdmiChangeMode Failed\n");
                return;
            }
            pstChange->enHdmiChnage  = enChange;
        }
    }
}
                                ...
                                SND_TRACK_STREAM_ATTR_S* pstAttr)
{
    ...
    HI_UNF_HDMI_CODING_TYPE_E enAudioFormat = HDMI_AUDIO_CODING_REFER_STREAM_HEAD;
        ...
        || (TRACK_STREAMMODE_CHANGE_HBR2LBR == enMode))
    {
        ...
        {
            enAudioFormat = HDMI_AUDIO_CODING_AC3;
        }
        ...
        {
            enAudioFormat = HDMI_AUDIO_CODING_DTS;
        }
             ...
             || (TRACK_STREAMMODE_CHANGE_HBR2HBR == enMode))
    {
        ...
        if (IEC61937_DATATYPE_71_LPCM == pstAttr->u32HbrFormat)
        {
            ...
            enAudioFormat = HDMI_AUDIO_CODING_PCM;
        ...
        else if (IEC61937_DATATYPE_DOLBY_DIGITAL_PLUS == pstAttr->u32HbrFormat)
        {
            ...
            enAudioFormat = HDMI_AUDIO_CODING_DDPLUS;  //set hdmi_audio_interface_spdif audio codec type

        }
#if 0//!defined(CHIP_TYPE_hi3716mv310)
...
HI_VOID TRACKStartAip(SND_CARD_STATE_S* pCard, SND_TRACK_STATE_S* pTrack)
{
    HI_U32 u32DelayMs = 0;
...
HI_S32 TRACK_GetStatus(SND_CARD_STATE_S* pCard, HI_U32 u32TrackID, HI_U32* pu32TrackStatus)
{
    SND_TRACK_STATE_S* pTrack;

    if (HI_NULL == pu32TrackStatus)
    {
        return HI_FAILURE;
    }
    pTrack = (SND_TRACK_STATE_S*)pCard->hSndTrack[u32TrackID];

    *pu32TrackStatus = (HI_U32)pTrack->enCurnStatus;
...
#ifdef  HI_VIRTUAL_TRACK_SUPPORT
...
HI_S32 TRACK_SetUsedByKernel(SND_CARD_STATE_S* pCard, HI_U32 u32TrackID)
{
    ...
    HAL_AOE_AIP_SetDataSource(pTrack->enAIP[SND_ENGINE_TYPE_PCM], HI_TRUE);

    if (HI_UNF_SND_TRACK_TYPE_MASTER == pTrack->stUserTrackAttr.enTrackType)
    {
        if (SND_SPDIF_MODE_NONE != pCard->enSpdifPassthrough)
        {
            HAL_AOE_AIP_SetDataSource(pTrack->enAIP[SND_ENGINE_TYPE_SPDIF_RAW], HI_TRUE);
        }

        if (SND_HDMI_MODE_NONE != pCard->enHdmiPassthrough)
        {
            HAL_AOE_AIP_SetDataSource(pTrack->enAIP[SND_ENGINE_TYPE_HDMI_RAW], HI_TRUE);
        }
    }
...
HI_S32 Track_ReadProc( struct seq_file* p, SND_CARD_STATE_S* pCard )
{
    ...
    for (i = 0; i < AO_MAX_TOTAL_TRACK_NUM; i++)
    {
        if (pCard->uSndTrackInitFlag & ((HI_U32)1L << i))
        {
            ...
            PROC_PRINT(p,
                       "          SpeedRate(%.2d), AddMuteFrames(%.4d), SendCnt(Try/OK)(%.6u/%.6u)\n",
