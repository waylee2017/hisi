                                    ...
                                    struct snd_info_buffer *buffer)
{
    ...
    struct hiaudio_data *had = entry->private_data;
     ...
     snd_iprintf(buffer,
                        "irq_num=0x%x, SndOpen=0x%x  TrackId=0x%x\n",
                         had->u32IrqNum,had->u32SndOpen, had->u32TrackId
                         );
     snd_iprintf(buffer,
                        "TrackState=0x%x ,SendTrack(Try/Ok)=0x%x/0x%x\n",
                         atomic_read(&had->atmTrackState), had->u32SendTryCnt, had->u32HadSent
                         );
     snd_iprintf(buffer,
