...
static HI_S32 PortBufInit(AIAO_PORT_S hPort)
{
    ...
    HI_U32 u32WptrAddr = 0, u32RptrAddr = 0;
...
HI_S32 iHAL_AIAO_P_SetSpdifCategoryCode(HI_HANDLE handle, AIAO_SPDIF_CATEGORYCODE_E eCategoryCode)
{
    ...
    AIAO_ASSERT_NULL(hPort);
    pstConfig = &hPort->stUserCongfig;
    AIAO_HW_SetSPDIF_Category_SCMS(hPort->enPortID, pstConfig->enSPDIFSCMSMode, eCategoryCode);
...
HI_S32 iHAL_AIAO_P_SetSpdifSCMSMode(HI_HANDLE handle, AIAO_SPDIF_SCMS_MODE_E eSCMSMode)
{
    ...
    AIAO_HW_SetSPDIF_Category_SCMS(hPort->enPortID, eSCMSMode, pstConfig->enSPDIFCategoryCode);
...
#ifdef HI_SND_HDMI_I2S_SPDIF_MUX
        AIAO_HW_SetOpType(hPort->enPortID, pstConfig->enOpType);
#endif
...
HI_S32 iHAL_AIAO_T_Create(AIAO_TIMER_ID_E enTimerID, const AIAO_Timer_Create_S *pstParam, HI_HANDLE *phandle,AIAO_TimerIsrFunc** pIsr)
{
    ...
    memset(hPort, 0, sizeof(AIAO_PORT_CTX_S));
    memcpy(&hPort->stTimerParam, pstParam, sizeof(AIAO_Timer_Create_S));
    hPort->enTimerID = enTimerID;
    *phandle = (HI_HANDLE)hPort;
...
{
    AIAO_TIMER_S hPort = (AIAO_TIMER_S)handle;
    AIAO_ASSERT_NULL(hPort);
    AUTIL_AIAO_FREE(HI_ID_AIAO, hPort);
    return;
}
HI_S32 iHAL_AIAO_T_SetTimerAttr(HI_HANDLE handle,const AIAO_TIMER_Attr_S *pstAttrParam)
{
    AIAO_TIMER_S hPort = (AIAO_TIMER_S)handle;
    AIAO_ASSERT_NULL(hPort);
    AIAO_TIMER_SetAttr(hPort->enTimerID,&(pstAttrParam->stIfAttr), pstAttrParam->u32Config);
    hPort->stTimerAttr.u32Config = pstAttrParam->u32Config;
    memcpy(&(hPort->stTimerAttr.stIfAttr),&(pstAttrParam->stIfAttr),sizeof(AIAO_IfTimerAttr_S));
    hPort->stTimerStatus.enRate = pstAttrParam->stIfAttr.enRate;
    hPort->stTimerStatus.u32Config = pstAttrParam->u32Config;
    return HI_SUCCESS;
}
HI_S32 iHAL_AIAO_T_SetTimerEnalbe(HI_HANDLE handle,HI_BOOL bEnalbe)
{
    AIAO_TIMER_S hPort = (AIAO_TIMER_S)handle;
    AIAO_ASSERT_NULL(hPort);
    AIAO_TIMER_SetEnable(hPort->enTimerID,bEnalbe);
    hPort->stTimerStatus.bEnable = bEnalbe;
    return HI_SUCCESS;
}
HI_S32 iHAL_AIAO_T_GetStatus(HI_HANDLE handle, AIAO_TIMER_Status_S *pstStatus)
{
    AIAO_TIMER_S hPort = (AIAO_TIMER_S)handle;
    AIAO_ASSERT_NULL(hPort);
    memcpy(pstStatus, &hPort->stTimerStatus, sizeof(AIAO_TIMER_Status_S));
    return HI_SUCCESS;
}
HI_VOID iHAL_AIAO_T_TIMERProcess(HI_HANDLE handle)
{
    ...
    pstTimerStatus = &hPort->stTimerStatus;
...
HI_U32 iHAL_AIAO_P_WriteData(HI_HANDLE handle, HI_U8 * pu32Src, HI_U32 u3SrcLen)
{
    ...
    Bytes = CIRC_BUF_Write(pstCB, pu32Src, u3SrcLen, HI_FALSE); // copy_from_user or pu32Src = NULL
...
HI_U32 iHAL_AIAO_P_QueryBufData_ProvideRptr(HI_HANDLE handle, HI_U32 *pu32Rptr)
{
    ...
    AIAO_ASSERT_NULL(hPort);
    return CIRC_BUF_QueryBusy_ProvideRptr(&hPort->stCB, pu32Rptr);
}
