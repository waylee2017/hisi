...
{
    HI_PRINT("echo %s %s > /proc/msp/demux_main  -- begin save es\n",SAVE_ES_STR,START_STR);
    HI_PRINT("echo %s %s > /proc/msp/demux_main  -- stop save es\n",SAVE_ES_STR,STOP_STR);
    HI_PRINT("echo %s %s x[portid] > /proc/msp/demux_main  -- begin save allts\n",SAVE_ALLTS_STR,START_STR);
    HI_PRINT("echo %s %s > /proc/msp/demux_main  -- stop save allts\n",SAVE_ALLTS_STR,STOP_STR);
    HI_PRINT("echo %s %s x[ram portid]> /proc/msp/demux_main  -- begin save ram port ts\n",SAVE_IPTS_STR,START_STR);
    HI_PRINT("echo %s %s > /proc/msp/demux_main  -- stop save ram port ts\n",SAVE_IPTS_STR,STOP_STR);
    HI_PRINT("echo %s %s x[dmxid] > /proc/msp/demux_main  -- begin save all channels ts\n",SAVE_DMXTS_STR,START_STR);
    HI_PRINT("echo %s %s > /proc/msp/demux_main  -- stop save dmx ts\n",SAVE_DMXTS_STR,STOP_STR);
    HI_PRINT("echo %s > /proc/msp/demux_main  -- show help info\n",HELP_STR);
}
...
static HI_S32 DMXProcWrite(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    ...
    p1 = (char *)get_zeroed_page(GFP_KERNEL);
    if (!p1)
    {
        HI_ERR_DEMUX("get page failed.\n");
        return -ENOMEM;
    }

    if (copy_from_user(p1, buf, count < PAGE_SIZE ? count : PAGE_SIZE))
    {
...
static HI_S32 DMXProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    for (DmxId = 0; DmxId < DMX_CNT; DmxId++)
    {
        ...
        if (HI_SUCCESS == ret)
        {
            snprintf(PortStr, 8,"%u", (DMX_PORT_MODE_TUNER == PortMode) ? PortId : HI_UNF_DMX_PORT_BASE_RAM + PortId);
        }
...
static HI_S32 DMXPortProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    for (i = 0; i < DMX_RAMPORT_CNT; i++)
    {
		...
		if ( PortAttr.enPortType !=  HI_UNF_DMX_PORT_TYPE_USER_DEFINED)
		{
            ...
            HI_UNF_DMX_PORT_BASE_RAM + i, TsPacks, PortAttr.u32SyncLockTh, PortAttr.u32SyncLostTh, str);
		}
		else
		{
		 	PROC_PRINT(p, "%3u    0x%-8x 0x0         %u/%u     %s         %d          %dn",
            HI_UNF_DMX_PORT_BASE_RAM + i, TsPacks, PortAttr.u32SyncLockTh, PortAttr.u32SyncLostTh, str,PortAttr.u32UserDefLen1,PortAttr.u32UserDefLen2);
		}
...
static HI_S32 DMXTsBufProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    for (PortId = 0; PortId < DMX_RAMPORT_CNT; PortId++)
    {
        ...
        PROC_PRINT(p, "%u 0x%-8x 0x%-8x 0x%-8x 0x%-8x 0x%-8x %10u/%-10u %u\n",
                HI_UNF_DMX_PORT_BASE_RAM + PortId,
                BufInfo.PhyAddr,
                BufInfo.BufSize,
                BufInfo.UsedSize,
                BufInfo.Read,
...
static HI_S32 DMXRecvIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_DEMUX_SELECT_DATA_FLAG:
        {
            ...
            if (pPara->channelnum <= DMX_CHANNEL_CNT)
            {
                HI_U32 ChannelNum = pPara->channelnum;
                HI_HANDLE Channel[ChannelNum];

                if (copy_from_user(Channel, pPara->channel, sizeof(HI_HANDLE) * ChannelNum))
                {
                ...
                {
                    ret = HI_DRV_DMX_SelectDataHandle(Channel, ChannelNum, pPara->u32Flag, pPara->u32TimeOutMs);
                }
            }
            else
            {
                HI_ERR_DEMUX("pPara->channelnum[0x%x] is too big.\n", pPara->channelnum);
            }
...
static HI_S32 DMXAvIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_DEMUX_PES_EXT_DATA_GET :
        {
            DMX_PesExtData_S *Param = (DMX_PesExtData_S *)arg;

            ret = HI_DRV_DMX_GetPesExtData(Param->hChannel, &Param->stPesExtData);
...
HI_S32 DMX_OsrIoctl(struct inode *inode, struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (CmdType)
    {
        ...
        case DEMUX_KSET_CMD:
        {
			if ( g_pDscFunc == HI_NULL )
			{
			    HI_ERR_DEMUX("not support descrambler\n");
				return HI_FAILURE;
			}
			else
			{
				ret = g_pDscFunc->DMXKeyIoctl(file, cmd, arg);
			}
            break;
        }
    #endif
