...
HI_U32 DmxHalGetClk(HI_VOID)
{
    ...
    PeriCrg64.value = DMX_READ_REG(PERI_CRG64);
...
{
    U_DMX_TSOUT_CFG tsout_cfg;
    HI_U32 u32TSOSourcePortID = 0;

    /*
    ox0             :    No source.
    0x1 ~ 0x3    :    DVB1 ~ DVB3 source.
    0x4 ~ 0x7    :    IP1 ~ IP4
    */
    u32TSOSourcePortID = PortAttr->enTSSource + 0x1;

    tsout_cfg.u32                   = DMX_READ_REG(TSOUT_CFG(PortId));
    ...
    DMX_WRITE_REG(TSOUT_CFG(PortId), tsout_cfg.u32);

    DMX_COM_EQUAL(tsout_cfg.u32, DMX_READ_REG(TSOUT_CFG(PortId)));
}
#endif
...
HI_VOID DmxHalIPPortEnableJudgeNullDesc(HI_U32 PortId)
{
    ...
    SyncSet.value = DMX_READ_REG(IP_SYNC_LEN(PortId));
    SyncSet.bits.ip_judge_null_desc = 1;
...
HI_VOID DmxHalSetRecType(HI_U32 DmxId, DMX_REC_TYPE_E RecType)
{
    ...
    #if defined(CHIP_TYPE_hi3716cv200) || defined(CHIP_TYPE_hi3716mv310) || defined(CHIP_TYPE_hi3716mv320) || defined(CHIP_TYPE_hi3110ev500)
        case 6:
...
HI_VOID DmxHalConfigHardware(HI_VOID)
{
    ...
    /* set demux 27M clock source as epll 27M */
    U_PERI_CRG22 regCrg22;
    regCrg22.value = SYS_READ_REG(PERI_CRG22);
    regCrg22.bits.clk_27m_sel = 1;
    SYS_WRITE_REG(PERI_CRG22, regCrg22.value);
    ...
    PeriCrg64.value = SYS_READ_REG(PERI_CRG64);

    PeriCrg64.bits.pvr_ts0_clk_sel  = 1;

    SYS_WRITE_REG(PERI_CRG64, PeriCrg64.value);
}
 ...
 */
static inline HI_VOID __DmxHalClearOQEopIntStatus(HI_U32 OQId)
{
    HI_U32  offset  = OQId >> 5;
    HI_U32  bit     = OQId & 0x1f;
    HI_U32  value = 1 << bit;
 ...
 */
static inline HI_VOID __DmxHalEnableOQEopInt(HI_U32 OQId)
{
    HI_U32  offset  = OQId >> 5;
    HI_U32  bit     = OQId & 0x1f;
    HI_U32  value;
 ...
 */
static inline HI_VOID __DmxHalDisableOQEopInt(HI_U32 OQId)
{
    HI_U32  offset  = OQId >> 5;
    HI_U32  bit     = OQId & 0x1f;
    HI_U32  value;
...
static inline HI_VOID DxmHalSetOQRegMask(HI_U32 u32MaskValue)
{
    DMX_WRITE_REG(OQ_WR_MASK, u32MaskValue & 0xf);
}

static inline HI_U32 DxmHalGetOQRegMask(HI_VOID)
{
