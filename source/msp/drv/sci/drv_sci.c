...
HI_VOID SCI_Init(HI_VOID)
{
    ...
    for (i = HI_UNF_SCI_PORT0; i < HI_SCI_PORT_NUM; i++)
    {
		...
		g_SciPara[i].SciAttr.enResetMode = HI_UNF_SCI_MODE_CMOS;
		g_SciPara[i].SciAttr.enVccEnMode = HI_UNF_SCI_MODE_CMOS;
#endif
...
HI_S32 SCI_Open(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_PROTOCOL_E enSciProtocol, HI_U32 Frequency)
{
    ...
    snprintf(s8Buff[enSciPort], 12,"sci%d", enSciPort);
...
HI_S32 SCI_SendData(HI_UNF_SCI_PORT_E enSciPort, HI_U8 *pDataBuf, HI_U32 BufSize, HI_U32 *pDataLen, HI_U32 TimeoutMs)
{
    ...
    if (g_SciSendDataBuf[enSciPort].Read < g_SciSendDataBuf[enSciPort].DataLen)
    {
        ...
        if(Ret<0)
        {
        	HI_WARN_SCI("wait_event_interruptible_timeout wakeup.\n");
        }
    }
...
EXPORT_SYMBOL(SCI_Reset);
EXPORT_SYMBOL(SCI_SendData);
EXPORT_SYMBOL(SCI_ConfVcc);
EXPORT_SYMBOL(SCI_ConfDetect);
EXPORT_SYMBOL(SCI_ConfClkMode);	
EXPORT_SYMBOL(SCI_GetSciPara);
EXPORT_SYMBOL(SCI_GetSciAtrBuf);
EXPORT_SYMBOL(SCI_Deactive);
EXPORT_SYMBOL(SCI_GetStatus);
EXPORT_SYMBOL(SCI_Open);
EXPORT_SYMBOL(SCI_Close);
EXPORT_SYMBOL(SCI_Init);
EXPORT_SYMBOL(SCI_ReceiveData);
