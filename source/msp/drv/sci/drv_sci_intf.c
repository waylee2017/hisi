...
HI_S32 SCI_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    for (ii = HI_UNF_SCI_PORT0; ii < HI_SCI_PORT_NUM; ii++)
    {
        ...
        snprintf(s8Buff,sizeof(s8Buff), "sci%d", ii);

        if (0 == strncmp(pProcItem->entry_name, s8Buff,strlen(pProcItem->entry_name)))
        {
            enSciPort = ii;
            break;
        }
    ...
    if (0 != g_SciOpenNum[enSciPort])
    {
		...
		PROC_PRINT(p, "%-20s:%s\n", "ResetkMode", g_ClockModeString[SciPara.SciAttr.enResetMode]);
		PROC_PRINT(p, "%-20s:%s\n", "VccEnMode", g_ClockModeString[SciPara.SciAttr.enVccEnMode]);
#endif
...
#if defined(CHIP_TYPE_hi3716mv330)
	case CMD_SCI_CONF_RESET_MODE:
    {
        SCI_RESET_S   *pSciReset;

        pSciReset = (SCI_RESET_S *)arg;

        Ret = SCI_ConfResetMode(pSciReset->enSciPort, pSciReset->enResetMode);

        break;
    }

	case CMD_SCI_CONF_VCCEN_MODE:
    {
        SCI_VCCEN_S   *pSciVccEn;

        pSciVccEn = (SCI_VCCEN_S *)arg;

        Ret = SCI_ConfVccEnMode(pSciVccEn->enSciPort, pSciVccEn->enVccEnMode);
...
static HI_S32 SCI_DRV_Open(struct inode *inode, struct file *filp)
{
	...
	if(Ret != HI_SUCCESS)
	{
		HI_WARN_SCI("down_interruptible wakeup.\n");
	}
...
HI_S32 SCI_DRV_ModInit(HI_VOID)
{
    ...
    snprintf(g_SciRegisterData.devfs_name,HIMEDIA_DEVICE_NAME_MAX_LEN, UMAP_DEVNAME_SCI);
    ...
    for (ii = 0; ii < HI_SCI_PORT_NUM; ii++)
    {
        ...
        snprintf(s8Buff[ii],sizeof(s8Buff), "sci%d", ii);
