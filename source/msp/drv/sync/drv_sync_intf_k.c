...
static HI_S32 SYNC_InitUsedProc(HI_VOID)
{
    ...
    for (i=0; i<SYNC_MAX_NUM; i++)
    {
        ...
        {
			snprintf(ProcName, sizeof(ProcName),"%s%02d", HI_MOD_SYNC, i);
            ...
            if (!pProcItem)
            {
                ...
                for(j=0; j<i; j++)
                {
                    ...
                    {
						snprintf(ProcName, sizeof(ProcName),"%s%02d", HI_MOD_SYNC, j);
...
{    
    ...
    if (g_pSyncProcPara)
    {
        ...
        snprintf(ProcName, sizeof(ProcName),"%s%02d", HI_MOD_SYNC, SyncId);
        HI_DRV_PROC_RemoveModule(ProcName);
    }
...
/*opentv5 also use those interfaces bellow now*/
