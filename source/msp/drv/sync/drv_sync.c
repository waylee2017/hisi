...
HI_U32 SYNC_CalcPcrGradient(HI_U32 PcrDelta, HI_U32 PcrLastSysTime)
{
    HI_U32   SysDelta;
    HI_U32   PcrGradient = 1000;

    SysDelta = SYNC_GetSysTimeCost(PcrLastSysTime);

    if ( SysDelta!= 0 )   
    {
        PcrGradient = PcrDelta * 1000 / SysDelta;
    }

    return PcrGradient;
}

HI_U32 SYNC_Get_PcrTimeCost(HI_U32 CalPcrGradient, HI_U32 PcrLastSysTime)
{
    HI_U32   SysDelta;
    HI_U32   PcrDelta;
    ...
    PcrDelta = (CalPcrGradient * SysDelta + 500) / 1000;

    HI_INFO_SYNC(">>>>SysDelta %d, PcrDelta %d, PcrRatio %d\n", SysDelta, PcrDelta, CalPcrGradient);
...
HI_U32 SYNC_GetLocalTime(SYNC_S *pSync, SYNC_CHAN_E enChn)
{
    ...
    else if (SYNC_CHAN_PCR == enChn)
    {
		...
		CostPcrTime = SYNC_Get_PcrTimeCost(pSync->PcrSyncInfo.CalPcrGradient, pSync->PcrSyncInfo.PcrLastSysTime);

        CrtLocaltime = pSync->PcrSyncInfo.PcrLastLocalTime + CostPcrTime;
    }
...
HI_S32 SYNC_ResumeSync(HI_U32 SyncId)
{
    ...
    if (pSync->PcrSyncInfo.PcrPauseLocalTime != -1)
    {
	    ...
	    pSync->PcrSyncInfo.CurPcrGradient = 1000;
        pSync->PcrSyncInfo.LstPcrGradient = 1000;
        pSync->PcrSyncInfo.CalPcrGradient = 1000;
...
HI_VOID SYNC_AudPreSync(SYNC_S *pSync, SYNC_CHAN_E enChn)
{
    HI_S32      VidAudDiff;
    HI_S32      AudPcrDiff;
    HI_S32      VidPcrDiff;
    ...
    else
    {
        ...
        else
        {
            ...
            if (VidAudDiff > VID_LEAD_DISCARD_THRESHOLD)
            {
                ...
                pSync->PreSyncFinish = HI_TRUE;
                pSync->AudOpt.SyncProc = SYNC_PROC_CONTINUE;
                HI_INFO_ASYNC(SYNC_CHAN_AUD, "PreSync VidAudDiff %d > VID_LEAD_DISCARD_THRESHOLD %d\n", VidAudDiff, VID_LEAD_DISCARD_THRESHOLD);
                HI_INFO_VSYNC(SYNC_CHAN_VID, "PreSync VidAudDiff %d > VID_LEAD_DISCARD_THRESHOLD %d\n", VidAudDiff, VID_LEAD_DISCARD_THRESHOLD);
                HI_DRV_STAT_Event(STAT_EVENT_PRESYNC,0);
            }
            else
            {
                if (VidAudDiff <= pSync->SyncAttr.stSyncStartRegion.s32VidPlusTime)
                {
                    ...
                    if (HI_UNF_SYNC_REF_PCR == pSync->SyncAttr.enSyncRef)
                    {
                        if (SYNC_PCR_ADJUST_SCR == pSync->PcrSyncInfo.enPcrAdjust)
                        {
                            ...
                            AudPcrDiff = pSync->AudInfo.Pts - PcrLocalTime;
                            VidPcrDiff = pSync->VidPreSyncTargetTime - PcrLocalTime - pSync->VidInfo.DelayTime;
...
HI_VOID SYNC_AudPreSync_old(SYNC_S *pSync, SYNC_CHAN_E enChn)
{
    HI_S32      VidAudDiff; 
    HI_S32      AudPcrDiff;
    HI_S32      VidPcrDiff;
...
HI_VOID SYNC_PCRPreSync(SYNC_S *pSync, SYNC_CHAN_E enChn)
{  
    HI_S32      AudPcrDiff;
    HI_S32      VidPcrDiff;
    HI_U32      PcrLocalTime;

    if (!pSync->PcrSyncInfo.PcrFirstCome)  //pcr doesn't come, continue accumulate datas
    {
        pSync->AudOpt.SyncProc = SYNC_PROC_REPEAT;
        pSync->VidOpt.SyncProc = SYNC_PROC_REPEAT;
    }
    else      //pcr comes, start to adjust aud and vid
    {
        PcrLocalTime = SYNC_GetLocalTime(pSync, SYNC_CHAN_PCR);
        ...
        //this scenario, we choose AudPreSync, because discard streams may be too slow in dvb mode. 
        //And if stream is unload,  discard stream will be worsen.
        if (AudPcrDiff < 0 || VidPcrDiff< 0)
        {
            pSync->PcrSyncInfo.PCRPreSyncAdjust = HI_TRUE;
        }
        //accumulate aud and vid if stream is unload
        if (!pSync->PcrSyncInfo.PCRPreSyncAdjust)
        {
            if ((abs(AudPcrDiff) >= VID_LEAD_DISCARD_THRESHOLD) || (abs(VidPcrDiff) >= VID_LEAD_DISCARD_THRESHOLD)
                || ((abs(AudPcrDiff) < PCR_LAG_STOP_THRESHOLD) || (abs(VidPcrDiff) < PCR_LAG_STOP_THRESHOLD)))
            {
                pSync->PcrSyncInfo.PCRPreSyncAdjust = HI_TRUE;
                if (pSync->VidInfo.Pts  > pSync->AudInfo.Pts)
                {
                    pSync->PreSyncTarget = SYNC_CHAN_VID;
                    pSync->VidPreSyncTargetTime = pSync->VidInfo.Pts;
                }
...
HI_VOID SYNC_PreSync(SYNC_S *pSync, SYNC_CHAN_E enChn)
{
    ...
    if (CostSysTime >= pSync->SyncAttr.u32PreSyncTimeoutMs)
    {
        ...
        HI_INFO_ASYNC(SYNC_CHAN_AUD, "PreSync TimeOut %d AudBufTime %d AudFrameNum %d VidBufNum %d\n", CostSysTime, pSync->AudInfo.BufTime, pSync->AudInfo.FrameNum, pSync->VidInfo.BufNum);
        HI_INFO_VSYNC(SYNC_CHAN_VID, "PreSync TimeOut %d AudBufTime %d AudFrameNum %d VidBufNum %d\n", CostSysTime, pSync->AudInfo.BufTime, pSync->AudInfo.FrameNum, pSync->VidInfo.BufNum);
        HI_DRV_STAT_Event(STAT_EVENT_PRESYNC,0);
    ...
    if (pSync->CrtBufStatus.VidBlockFlag || pSync->CrtBufStatus.AudBlockFlag)
    {
        pSync->PreSyncEndSysTime = SYNC_GetSysTime();
        pSync->PreSyncFinish = HI_TRUE;
        pSync->VidOpt.SyncProc = SYNC_PROC_CONTINUE;
        ...
        HI_INFO_ASYNC(SYNC_CHAN_AUD, "PreSync BufBlock Aud %d Vid %d\n", pSync->CrtBufStatus.AudBlockFlag, pSync->CrtBufStatus.VidBlockFlag);
    ...
    if ((HI_UNF_SYNC_REF_PCR == pSync->SyncAttr.enSyncRef)
        && (SYNC_AUD_ADJUST_SCR != pSync->PcrSyncInfo.enPcrAdjust))
    {
        SYNC_PCRPreSync(pSync, enChn);
    }
    else
    {  
        SYNC_AudPreSync(pSync, enChn);
...
HI_VOID SYNC_BufFund(SYNC_S *pSync)
{
    ...
    if (CostSysTime > BUF_FUND_TIMEOUT)
    {
        ...
        HI_INFO_ASYNC(SYNC_CHAN_AUD, "BufFund TimeOut %d AudBufTime %d AudFrameNum %d VidBufNum %d\n", CostSysTime, pSync->AudInfo.BufTime, pSync->AudInfo.FrameNum, pSync->VidInfo.BufNum);
        HI_INFO_VSYNC(SYNC_CHAN_VID, "BufFund TimeOut %d AudBufTime %d AudFrameNum %d VidBufNum %d\n", CostSysTime, pSync->AudInfo.BufTime, pSync->AudInfo.FrameNum, pSync->VidInfo.BufNum);
    ...
    if (pSync->VidEnable && pSync->AudEnable)
    {
           ...
           )
        {
            ...
            HI_INFO_ASYNC(SYNC_CHAN_AUD, "BufFund Ok %d AudBufTime %d AudFrameNum %d VidBufNum %d\n", CostSysTime, pSync->AudInfo.BufTime, pSync->AudInfo.FrameNum, pSync->VidInfo.BufNum);
            HI_INFO_VSYNC(SYNC_CHAN_VID, "BufFund Ok %d AudBufTime %d AudFrameNum %d VidBufNum %d\n", CostSysTime, pSync->AudInfo.BufTime, pSync->AudInfo.FrameNum, pSync->VidInfo.BufNum);
        }
        else
        {
            pSync->VidOpt.SyncProc = SYNC_PROC_REPEAT;
            ...
            HI_INFO_ASYNC(SYNC_CHAN_AUD, "BufFund %d AudBufTime %d AudFrameNum %d VidBufNum %d\n", CostSysTime, pSync->AudInfo.BufTime, pSync->AudInfo.FrameNum, pSync->VidInfo.BufNum);
            HI_INFO_VSYNC(SYNC_CHAN_VID, "BufFund %d AudBufTime %d AudFrameNum %d VidBufNum %d\n", CostSysTime, pSync->AudInfo.BufTime, pSync->AudInfo.FrameNum, pSync->VidInfo.BufNum);
        }
    }
...
HI_VOID SYNC_AudReSync(SYNC_S *pSync)
{
    ...
    /* buffer blocked */
    if (pSync->CrtBufStatus.AudBlockFlag)
    {
        pSync->PreSyncEndSysTime = SYNC_GetSysTime();
        pSync->AudReSync = HI_FALSE;
        pSync->AudOpt.SyncProc = SYNC_PROC_CONTINUE;
...
HI_VOID SYNC_PcrSyncRepeatAud(SYNC_S *pSync, SYNC_CHAN_E enChn, HI_S32 VidAudDiff)
{
    ...
    if(!pSync->CrtBufStatus.AudBlockFlag)
    {
        if (pSync->CrtBufStatus.AudEmptyFlag)
        {
...
{    
    ...
    if (!pSync->CrtBufStatus.VidBlockFlag)
    {
        /* abs(VidPcrDiff) and abs(AudPcrDiff) must be less than PCR_LAG_STOP_THRESHOLD = 100
         so abs(VidAudDiff) largest 2*PCR_LAG_STOP_THRESHOLD = 200
        */
...
HI_VOID SYNC_CheckBufStatus(SYNC_S *pSync, SYNC_CHAN_E enChn)
{
    HI_U32  StrmFrmNum = 0;

    /*InBps is too small*/
    if(pSync->CrtBufStatus.VidStrmInBps < 100)
    {
        StrmFrmNum = 0;
    }
    else
    {
        /*how many frames in vfmw stream buffer*/
        if(0 == pSync->VidInfo.FrameTime)
        {
            StrmFrmNum = (pSync->CrtBufStatus.VfmwStrmSize * 1000)/(pSync->CrtBufStatus.VidStrmInBps * 40);
        }
        else
        {
            StrmFrmNum = (pSync->CrtBufStatus.VfmwStrmSize * 1000)/(pSync->CrtBufStatus.VidStrmInBps * pSync->VidInfo.FrameTime);
        }        
    }
    ...
    if(SYNC_CHAN_VID == enChn)
    {
        if(pSync->VidInfo.BufNum <= 2)
        {
            pSync->CrtBufStatus.VidEmptyFlag = HI_TRUE;
        }
        else if(pSync->VidInfo.BufNum + pSync->CrtBufStatus.VfmwFrmNum <= 5)
        {
            if((pSync->VidInfo.BufNum + pSync->CrtBufStatus.VfmwFrmNum <= 3)
                ||(pSync->VidInfo.BufNum + pSync->CrtBufStatus.VfmwFrmNum + StrmFrmNum <= 5)
                )
            {
                pSync->CrtBufStatus.VidEmptyFlag = HI_TRUE;
            }
            ...
            {
                pSync->CrtBufStatus.VidEmptyFlag = HI_FALSE;
            }
        }
        else
        {
        ...
        if (pSync->CrtBufStatus.VidEmptyFlag)
        {
            pSync->AudAdjustFlag = HI_TRUE;
            pSync->CrtBufStatus.VidLowFlag = HI_TRUE;
        }
        else
        {
	        if( (pSync->VidInfo.BufNum + pSync->CrtBufStatus.VfmwFrmNum <= 3)
	            || (pSync->VidInfo.BufNum + pSync->CrtBufStatus.VfmwFrmNum + StrmFrmNum <= 8)
	           )
	        {
	            pSync->CrtBufStatus.VidLowFlag = HI_TRUE;
	        }
	        ...
	        {
	            pSync->CrtBufStatus.VidLowFlag = HI_FALSE;
	        }

	        if(!pSync->CrtBufStatus.VidLowFlag)
	        {
	            pSync->AudAdjustFlag = HI_FALSE;
	        }
        }
    ...
    HI_INFO_VSYNC(enChn, "Buf status: VoFrmNum %u,VfmwFrmNum %u,StrmSize %u,VfmwStrmFrmNum %u,VidInBps %u\n",
        pSync->VidInfo.BufNum, pSync->CrtBufStatus.VfmwFrmNum,pSync->CrtBufStatus.VfmwStrmSize, StrmFrmNum, pSync->CrtBufStatus.VidStrmInBps);

    if(SYNC_CHAN_AUD == enChn)
    {
        if((pSync->AudInfo.BufTime <= 72) && (pSync->AudInfo.FrameNum <= 3))
        {
            pSync->CrtBufStatus.AudEmptyFlag = HI_TRUE;
        }
        else
        {
            pSync->CrtBufStatus.AudEmptyFlag = HI_FALSE;
        }

        if((pSync->AudInfo.BufTime <= 120) && (pSync->AudInfo.FrameNum <= 5))
        {
            pSync->CrtBufStatus.AudLowFlag = HI_TRUE;
        }
        else
        {
            pSync->CrtBufStatus.AudLowFlag = HI_FALSE;
        }
    }
...
HI_VOID SYNC_VidProc(HI_HANDLE hSync, SYNC_VID_INFO_S *pVidInfo, SYNC_VID_OPT_S *pVidOpt)
{
    ...
    pSync = g_SyncGlobalState.SyncInfo[hSync&0x0f].pSync;
    ...
    HI_INFO_VSYNC(SYNC_CHAN_VID, "VidInfo SrcPts %-8d,Pts %-8d,FrameTime %d,BufNum %2d,IntTime %d,DelayTime %d,DispTime %d,SysTime %d\n",
                     pVidInfo->SrcPts, pVidInfo->Pts, pVidInfo->FrameTime, pVidInfo->BufNum, pVidInfo->IntTime, pVidInfo->DelayTime,pVidInfo->DispTime, SysTime);
    ...
    if (!pSync->VidEnable)
    {
        ...
        *pVidOpt = pSync->VidOpt;
        ...
        HI_INFO_VSYNC(SYNC_CHAN_VID, "--------Vid Stop Proc %d--------\n", pVidOpt->SyncProc);
        return;
    }
    ...
    if (SYNC_STATUS_PAUSE == pSync->CrtStatus)
    {
        ...
        *pVidOpt = pSync->VidOpt;
        ...
        HI_INFO_VSYNC(SYNC_CHAN_VID, "--------Vid Pause Proc %d--------\n", pVidOpt->SyncProc);
        return;
    }

    if (SYNC_STATUS_TPLAY == pSync->CrtStatus)
    {
        HI_U32 WaitMs,LeftMs;
        if (!pSync->VidFirstPlay)
        {
            ...
            pSync->VidOpt.SyncProc = SYNC_PROC_TPLAY;
            *pVidOpt = pSync->VidOpt;
            return;
        }

        /* no speed control. fb and ff with pSync->TplaySpeed ==0  */
        if (pSync->TplaySpeed == 0)
        {
            pSync->VidLastPts = pVidInfo->Pts;
            pSync->VidOpt.SyncProc = SYNC_PROC_TPLAY;
            *pVidOpt = pSync->VidOpt;
        ...
        /* ff with TplaySpeed != 0 */
        /* repeat displaying I frames to realize Tplay */
        if (pSync->VidLastPts != 0xffffffff)
        {
            if (pVidInfo->Pts >= pSync->VidLastPts)
            {
                pVidInfo->FrameTime = pVidInfo->Pts - pSync->VidLastPts;
            }
            else
            {
                /* 1.It's responsibility of application to process  rewind of stream file;
                   2.We just process rewind of pts. Play directly*/
#if 0
                pVidInfo->FrameTime = pSync->VidLastPts - pVidInfo->Pts;

                if (pVidInfo->FrameTime > (PCR_TIME_MAX / 2))
                {
                    pVidInfo->FrameTime = PCR_TIME_MAX + pVidInfo->Pts - pSync->VidLastPts;
                }
#endif
                pSync->VidLastPts = pVidInfo->Pts;
                pSync->VidOpt.SyncProc = SYNC_PROC_TPLAY;
                *pVidOpt = pSync->VidOpt;
                HI_INFO_VSYNC(SYNC_CHAN_VID, "Pts or file rewind\n");
                return;
            }
        }

        /* Time need to wait between two I frame display */
        WaitMs = pVidInfo->FrameTime / pSync->TplaySpeed;  /*TplaySpeed == 0 has been processed*/
        if (WaitMs < pVidInfo->IntTime)
        {
            pSync->VidOpt.SyncProc = SYNC_PROC_DISCARD;
            *pVidOpt = pSync->VidOpt;
            HI_INFO_VSYNC(SYNC_CHAN_VID, "--------Vid Tplay Proc %d--------\n", pVidOpt->SyncProc);
            return;
        }
        else
        {
            pSync->VidOpt.Repeat = WaitMs / pVidInfo->IntTime;
        }

        LeftMs = WaitMs % pVidInfo->IntTime; /* WaitMs_Need - WaitMs_Actual */
        pSync->VidLastPts = pVidInfo->Pts - (LeftMs * pSync->TplaySpeed); /* Wait next time */
        pSync->VidOpt.SyncProc = SYNC_PROC_TPLAY;
        *pVidOpt = pSync->VidOpt;

        HI_INFO_VSYNC(SYNC_CHAN_VID, "--------Vid Tplay %d Proc %d--------\n", pVidOpt->Repeat, pVidOpt->SyncProc);
        return;
    }
    ...
    /*check current buf status, used for unusual situation*/
    SYNC_CheckBufStatus(pSync, SYNC_CHAN_VID);
    ...
    /* do not sync when  LoopBackFlag is false */
    ...
    if ((SYNC_PROC_DISCARD == pSync->VidOpt.SyncProc) && (pSync->AudAdjustFlag))
    {
        pSync->VidOpt.SyncProc = SYNC_PROC_PLAY;
...
HI_VOID SYNC_AudProc(HI_HANDLE hSync, SYNC_AUD_INFO_S *pAudInfo, SYNC_AUD_OPT_S *pAudOpt)
{
    ...
    SYNC_CheckBufStatus(pSync, SYNC_CHAN_AUD);
...
HI_VOID SYNC_PcrProc(HI_HANDLE hSync, HI_U32 PcrTime)
{
    ...
    if (pSync->PcrSyncInfo.PcrSeriesCnt >= PTS_SERIES_COUNT)
    {
        if (SYNC_PCR_ADJUST_SCR == pSync->PcrSyncInfo.enPcrAdjust)
        {
        	pSync->PcrSyncInfo.CurPcrGradient = SYNC_CalcPcrGradient(PcrDelta, pSync->PcrSyncInfo.PcrLastSysTime);

            HI_INFO_SYNC(">>>>lst %d, cur %d\n", pSync->PcrSyncInfo.LstPcrGradient, pSync->PcrSyncInfo.CurPcrGradient);

            if (abs(pSync->PcrSyncInfo.CurPcrGradient - pSync->PcrSyncInfo.LstPcrGradient) < 200)
            {
                HI_INFO_SYNC(">>>>update CalPcrGradient %d\n", pSync->PcrSyncInfo.CalPcrGradient);
                pSync->PcrSyncInfo.CalPcrGradient = pSync->PcrSyncInfo.CurPcrGradient;
            }
            pSync->PcrSyncInfo.LstPcrGradient = pSync->PcrSyncInfo.CurPcrGradient;
