...
static HI_S32 hi3138_mts_serial_config(HI_U32 u32TunerPort, HI_UNF_TUNER_OUPUT_MODE_E enTsType, HI_UNF_TUNER_SIG_TYPE_E enSigType)
{
    ...
    if(HI_UNF_TUNER_OUTPUT_MODE_SERIAL == enTsType)
    {
        ...
        hi3136_set_ts_type(u32TunerPort, HI_UNF_TUNER_OUTPUT_MODE_PARALLEL_MODE_A);
        ...
        hi3136_set_ts_out(u32TunerPort, &stTsOut);
...
HI_VOID hi3138_read_tuner_registers(HI_U32 u32TunerPort, HI_VOID * p)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U8 i;
    HI_U8 u8Data = 0;
    ...
    for (i = 0; i < (sizeof(stHi3138Allocation)/sizeof(stHi3138Allocation[0])); i++)
    {
        PROC_PRINT(pSeqFile, "stHi3138Allocation[%d].u32TunerPort:%d\n",i,stHi3138Allocation[i].u32TunerPort);
        PROC_PRINT(pSeqFile, "stHi3138Allocation[%d].enHi3138SigType:%d\n",i,stHi3138Allocation[i].enHi3138SigType);
        PROC_PRINT(pSeqFile, "stHi3138Allocation[%d].enADCId:%d\n",i,stHi3138Allocation[i].enADCId);
        PROC_PRINT(pSeqFile, "stHi3138Allocation[%d].enInnerDemodId:%d\n",i,stHi3138Allocation[i].enInnerDemodId);
        PROC_PRINT(pSeqFile, "stHi3138Allocation[%d].enTSOutputId:%d\n",i,stHi3138Allocation[i].enTSOutputId);
        PROC_PRINT(pSeqFile, "stHi3138Allocation[%d].enI2CGateWaySel:%d\n",i,stHi3138Allocation[i].enI2CGateWaySel);
        PROC_PRINT(pSeqFile, "stHi3138Allocation[%d].enIFOut:%d\n",i,stHi3138Allocation[i].enIFPath);
        PROC_PRINT(pSeqFile, "stHi3138Allocation[%d].enTsType:%d\n",i,stHi3138Allocation[i].enTsType);
        PROC_PRINT(pSeqFile, "stHi3138Allocation[%d].enSyncPin:%d\n",i,stHi3138Allocation[i].enSyncPin);
        PROC_PRINT(pSeqFile, "stHi3138Allocation[%d].enLNBSrc:%d\n",i,stHi3138Allocation[i].enLNBSrc);
        PROC_PRINT(pSeqFile, "******************************************************\n");
    }
...
HI_S32 hi3138_init(HI_U32 u32TunerPort, HI_UNF_TUNER_I2cChannel_E enI2cChannel, HI_UNF_TUNER_DEV_TYPE_E enTunerDevType)
{
    HI_U8 u8Temp = 0;
...
HI_S32 hi3138_set_multi_mode_chip_attr(HI_U32 u32TunerPort,HI_UNF_TUNER_MULTI_MODE_CHIP_ATTR_S *enPortAttr)
{
    ...
    for(i = 0; i < (sizeof(stHi3138Allocation)/sizeof(stHi3138Allocation[0])); i++)
    {
    	if(HI_UNF_TUNER_SIG_TYPE_BUTT == stHi3138Allocation[i].enHi3138SigType)
    	{
            ...
            if (0 == i)
            {
                stHi3138Allocation[i].enADCId = HI_UNF_TUNER_ADC0;
            }
            else
            {
                stHi3138Allocation[i].enADCId = HI_UNF_TUNER_ADC1;
            }
            #if 0
...
HI_S32 hi3138_blindscan_init(HI_U32 u32TunerPort, TUNER_BLINDSCAN_INITPARA_S *pstPara)
{
    ...
    hi3138_i2c_chn_sel(u32TunerPort);
...
HI_S32 hi3138_send_continuous_22K(HI_U32 u32TunerPort,HI_BOOL b22k_on) // 设置22k,
{
    ...
    hi3138_i2c_chn_sel(u32TunerPort);
    hi3136_send_continuous_22K(u32TunerPort,b22k_on);
...
HI_S32 hi3138_DiSEqC_send_msg(HI_U32 u32TunerPort, HI_UNF_TUNER_DISEQC_SENDMSG_S *pstSendMsg)
{
    ...
    hi3138_i2c_chn_sel(u32TunerPort);
    hi3136_DiSEqC_send_msg(u32TunerPort, pstSendMsg);
...
HI_S32 hi3138_data_sample(HI_U32 u32TunerPort, TUNER_DATA_SRC_E enDataSrc, HI_U32 u32DataLen, HI_UNF_TUNER_SAMPLE_DATA_S *pstData)
{
    HI_S32 s32Ret = HI_SUCCESS;
    
    if (g_stTunerOps[u32TunerPort].enSigType == HI_UNF_TUNER_SIG_TYPE_SAT)
    {
        s32Ret = hi3136_data_sample(u32TunerPort, u32TunerPort, u32DataLen, pstData);
    }
    else if ((g_stTunerOps[u32TunerPort].enSigType == HI_UNF_TUNER_SIG_TYPE_DVB_T) ||
                (g_stTunerOps[u32TunerPort].enSigType == HI_UNF_TUNER_SIG_TYPE_DVB_T2))
    {
        s32Ret = hi3137_data_sample(u32TunerPort, u32TunerPort, u32DataLen, pstData);
    }
    else if (g_stTunerOps[u32TunerPort].enSigType == HI_UNF_TUNER_SIG_TYPE_ISDB_T)
    {
    }
