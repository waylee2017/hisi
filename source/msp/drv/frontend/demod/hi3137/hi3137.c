...
HI_S32 hi3137_init(HI_U32 u32TunerPort, HI_UNF_TUNER_I2cChannel_E enI2cChannel, HI_UNF_TUNER_DEV_TYPE_E enTunerDevType )
{
    HI_U8 u8InitData = 0;
...
HI_S32 hi3137_set_ter_attr(HI_U32 u32TunerPort, HI_UNF_TUNER_TER_ATTR_S *pstTerTunerAttr)
{
	...
	HI_INFO_TUNER("<<<<<<<<<pstTerTunerAttr->u32DemodClk:%d\n",pstTerTunerAttr->u32DemodClk);
    u32ClkDemoKhz = 60000;
    u32ClkFecKhz = 150000;
    u32ClkAdcKhz = 30000;
    u32ClkIfKhz = 5380;
...
static HI_VOID dvbt2_connect_config_3137(HI_U32 u32TunerPort, TUNER_ACC_QAM_PARAMS_S *pstChannel, 
...
static HI_VOID dvbt2_connect_config_3138(HI_U32 u32TunerPort, TUNER_ACC_QAM_PARAMS_S *pstChannel, HI_S32 *ps32Fs, HI_U32 *pu32ChkTim)
{
    HI_U8 u8BW = 4;
    HI_U8 u8Thr = 0;
    HI_U8 u8PosMod = 0;
...
HI_S32 hi3137_connect(HI_U32 u32TunerPort, TUNER_ACC_QAM_PARAMS_S *pstChannel)
{
    ...
    if(HI_UNF_DEMOD_DEV_TYPE_3137 == g_stTunerOps[u32TunerPort].enDemodDevType)
    {
        dvbt2_connect_config_3137(u32TunerPort, pstChannel, &u32FreqOffsetKhz, &s32Fs, &u32ChkTim);
    }
    else if(HI_UNF_DEMOD_DEV_TYPE_3138 == g_stTunerOps[u32TunerPort].enDemodDevType)
    {
        dvbt2_connect_config_3138(u32TunerPort, pstChannel, &s32Fs, &u32ChkTim);
    }
    ...
    //check sync flag:1 locked;0 unlock
    ...
    //check fec flag:1 locked;0 unlock
    for(i = 0;i < u32ChkTim;i++)
    {
...
HI_VOID hi3137_read_tuner_registers(HI_U32 u32TunerPort, HI_VOID *p)
{
    HI_U8 i;
    HI_U8 u8Data = 0;
    struct seq_file *pSeqFile = (struct seq_file *)p;
        

    PROC_PRINT(pSeqFile, "  addr ");
    for(i = 0;i <= 0x0F;i++)
    {
        PROC_PRINT(pSeqFile, "%2x ",i);
    }
...
static HI_VOID hi3137_tp_scan_set_plp_t2(HI_U32 u32TunerPort, HI_U8 u8NumPlp, HI_UNF_TUNER_TER_TPINFO_S *pstScanResult)
{
    HI_U8 j;
    HI_U8 u8PlpId = 0;
    HI_U8 u8PlpGrpId = 0;
...
static HI_VOID hi3137_tp_scan_set_plp_t(HI_U32 u32TunerPort, HI_U8 u8NumPlp, HI_UNF_TUNER_TER_TPINFO_S *pstScanResult)
{
    HI_U8 i, j;
    HI_U8 u8PlpId = 0;
    HI_U8 u8PlpGrpId = 0;
...
static HI_S32 hi3137_tp_scan_set_result(HI_U32 u32TunerPort, HI_UNF_TUNER_TER_SCAN_ATTR_S *pstScanAttr, HI_UNF_TUNER_TER_TPINFO_S *pstScanResult)
{
    ...
    for(i = 0; i < u8ChkTim; i++)
    {
        ...
        if(u8InitData & 0x20)
        {
            ...
            else   //dvb-t2
            {
                ...
                hi3137_tp_scan_set_plp_t2(u32TunerPort, u8NumPlp, &stScanResult);
    ...
    if((stScanResult.u8DVBTMode == 0) && (stScanResult.enChannelMode == HI_UNF_TUNER_TER_MIXED_CHANNEL))
    {
        ...
        u8InitData = u8InitData | 0x1;
        qam_write_byte(u32TunerPort, Hi3137_PLP_CTRL, u8InitData);
        hi3137_tp_scan_set_plp_t(u32TunerPort, u8NumPlp, &stScanResult);
        stScanResult.u8ProgNum += u8NumPlp;
    }
...
HI_S32 hi3137_data_sample(HI_U32 u32TunerPort, TUNER_DATA_SRC_E enDataSrc, HI_U32 u32DataLen, HI_UNF_TUNER_SAMPLE_DATA_S *pstData)
{
    TUNER_I2C_DATA_S stI2cData;
    HI_S16 i;
    HI_U8 u8DataBuf[20] = {0},u8SampBuf[4],u8Status = 0x6A,u8DataL,u8DataM = 0,u8DataH = 0;
    ...
    for(i = 0;i < u32DataLen*4;i++)
    {
        ...
        tuner_i2c_receive_data(g_stTunerOps[u32TunerPort].enI2cChannel,g_stTunerOps[u32TunerPort].u32DemodAddress,
            &stI2cData);
        ...
        else
        {
            ...
            pstData[i/4].s32DataIP = ((u8DataH & 0x7F)<<3) + ((u8DataM & 0xE0)>>5);
        }
