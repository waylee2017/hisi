...
HI_BOOL hi3136_Set_Ts_Out_Enable(HI_U32 u32TunerPort,HI_BOOL bTS_ON)
{
    ...
    if(HI_UNF_DEMOD_DEV_TYPE_3138 == g_stTunerOps[u32TunerPort].enDemodDevType)
    {
        if(bTS_ON)
        {
            qam_write_bit(u32TunerPort, HI3138_S_TS_CTRL0, 1, 1);
        }
        else
        {
            qam_write_bit(u32TunerPort, HI3138_S_TS_CTRL0, 1, 0);
        }
    }
    else
    {
...
HI_S32 hi3136_read_tuner_reg(HI_U32 u32TunerPort, HI_U8 RegAddress, HI_U8 *RegData)
{
    ...
    HI_S32 s32Ret = HI_SUCCESS;
    HI_UNF_TUNER_I2cChannel_E enI2cChannel = 0;
    HI_U8 u8DevAddr = 0;
...
HI_VOID hi3136_read_tuner_registers(HI_U32 u32TunerPort, HI_VOID *p)
{
    HI_U8 i;
    HI_U8 u8Data = 0;
...
static HI_U32 hi3136_get_ber_dvbs(HI_U32 u32TunerPort)
{
    ...
    u32ErrNum = (temp1 << 8) + temp2;
    u32ErrorRate = u32ErrNum * 10000 / total_num * 100000;
...
HI_S32 hi3136_init(HI_U32 u32TunerPort, HI_UNF_TUNER_I2cChannel_E enI2cChannel, HI_UNF_TUNER_DEV_TYPE_E enTunerDevType)
{
...
HI_S32 hi3136_tp_verify(HI_U32 u32TunerPort, TUNER_TP_VERIFY_PARAMS_S * pstChannel)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        for(m=0; m<average_num; m++) // calculating average value of carrier error after several blindscan
        {
            //set key register
            ...
            // TP>=1 blindscan success,then starting blindscan type of known frequency and symbolrate,read num of tp,pick out the most reliable tp
            if(temp_TPnum != 0)
            {
                max_finded = 0;
                max_reliablity = 0;
