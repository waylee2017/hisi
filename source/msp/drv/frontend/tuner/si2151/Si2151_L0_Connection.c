...
/*     All functions are declared in Si2151_L0_API.h                                                         */
/*************************************************************************************************************/
/* Change log:
...
static HI_U32 s_u32Si2151TunerPort = 0 ;
...
HI_U32  si2151_tda_write(HI_U8 uAddress, HI_U32 uNbData, HI_U8 * pDataBuff)
{
	HI_U8  i;
	HI_S32 s32Ret;
	...
	qam_config_i2c_out (s_u32Si2151TunerPort, 1);
	...
	s32Ret = tuner_i2c_send_data (g_stTunerOps[s_u32Si2151TunerPort].enI2cChannel, uAddress, &stI2cDataStr);
	if (HI_SUCCESS != s32Ret)
	{
		HI_ERR_TUNER("[si2151 error]%s %d IIC write err!status:0x%x\n", __FUNCTION__, __LINE__, s32Ret);
		return HI_FAILURE;
	}
 ...
 HI_U32  si2151_tda_read(HI_U8 uAddress,HI_U32 uNbData, HI_U8 * pDataBuff)
{
	HI_S32 i;
	HI_S32 s32Ret;
	TUNER_I2C_DATA_S stI2cDataStr = { 0 };
	...
	s32Ret = tuner_i2c_receive_data_si(g_stTunerOps[s_u32Si2151TunerPort].enI2cChannel, uAddress /*|0x01 */ ,&stI2cDataStr);
	if (HI_SUCCESS != s32Ret)
	{
		HI_ERR_TUNER("[si2151 error]%s %d IIC read err! status:0x%x\n", __FUNCTION__, __LINE__, s32Ret);
		return HI_FAILURE;
	}	
	for (i = 0; i < uNbData; i++)
	{
...
int     L0_ReadBytes         (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pucDataBuffer) {
  ...
  switch (i2c->connectionType) {
   ...
   // nbReadBytes = 0;
	nbReadBytes = iNbBytes;
	si2151_tda_read ((HI_U8)g_stTunerOps[s_u32Si2151TunerPort].u32TunerAddress, iNbBytes, pucDataBuffer);
...
int     L0_WriteBytes        (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pucDataBuffer) {
 ...
 // HI_U32 u32RegAddr = 0x00;
  ...
  for (i=0; i < iNbBytes ;    i++) 
  {
	...
	u32databuffer[i] = pucDataBuffer[i];
  ...
  switch (i2c->connectionType) {
    ...
    case CUSTOMER:
		...
		nbWrittenBytes = iNbBytes;
		 ...
		 si2151_tda_write ((HI_U8)g_stTunerOps[s_u32Si2151TunerPort].u32TunerAddress,  iNbBytes, pucDataBuffer);
...
void si2151_I2C_SetPort(unsigned int  TunerPort)
{
    s_u32Si2151TunerPort = TunerPort;
}
