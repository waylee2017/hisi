...
R828_ErrCode R828_Init(void)
{
	...
	if(R828_IMR_done_flag==FALSE)
	{
	  ...
	  IMR_Data[2].Gain_X  = IMR_Data[1].Gain_X;
	  IMR_Data[2].Phase_Y = IMR_Data[1].Phase_Y;
	  IMR_Data[2].Value = IMR_Data[1].Value;
	  //if(R828_IMR(2, FALSE) != RT_Success)
		//return RT_Fail;

	  if(R828_IMR(4, FALSE) != RT_Success)   //Full K node 4
		return RT_Fail;
...
//	UINT8 IQ_Cunt  = 0;
	...
	for(VGA_Cunt = 12;VGA_Cunt < 16;VGA_Cunt ++)
	{
		...
		if(R828_IMR_done_flag==FALSE)
		{
			R828_Init_Delay_MS(VGA_DELAY);
		}
		else
		{
			R828_Delay_MS(VGA_DELAY);
		}
...
R828_ErrCode R828_Muti_Read(UINT8 IMR_Reg, UINT16* IMR_Result_Data)  //jason modified
{
	...
	if(R828_IMR_done_flag==FALSE)
	{
		R828_Init_Delay_MS(ADC_READ_DELAY);
	}
	else
	{
		R828_Delay_MS(ADC_READ_DELAY);
	}
...
UINT8  R828_Filt_Cal_ADC(UINT32 IF_Freq, UINT8 R828_BW)
{
	...
	if(I2C_Write(&R828_I2C) != RT_Success)
     ...
     R828_I2C.RegAddr = 0x0B;	
	 R828_Arry[6] = (R828_Arry[6] & 0x00) | R828_BW | 0x0F;	
     R828_I2C.Data = R828_Arry[6];		
     if(I2C_Write(&R828_I2C) != RT_Success)
		return RT_Fail;	
	 ...
	 for(VGA_Cunt = 0;VGA_Cunt < 16;VGA_Cunt ++)
	{
	 ...
	 for(i=initial_cnt; i<3; i++)
	 {
		 ...
		 R828_I2C.RegAddr = 0x0B;	
	     R828_Arry[6] = (R828_Arry[6] & 0x00) | R828_Bandwidth;	  
         R828_I2C.Data = R828_Arry[6];		
         if(I2C_Write(&R828_I2C) != RT_Success)
		      return RT_Fail;	
		 ...
		 //R828_Delay_MS(FILTER_DELAY); //delay ms
		 
		 if(R828_IMR_done_flag==FALSE)
		 {
			 R828_Init_Delay_MS(FILTER_DELAY);
		 }
		 else
		 {
			 R828_Delay_MS(FILTER_DELAY);
		 }
	 ...
	 for(u1FilterCode=0; u1FilterCode<16; u1FilterCode++)
	 {
		 ...
		 if((u1FilterCalValue+FILTER_ATT*ADC_READ_COUNT) < u1FilterCalValuePre) //att >2dB@f+1.4M
		 {
...
HI_S32 r820c_set_tuner(HI_U32 u32TunerPort, HI_UNF_TUNER_I2cChannel_E i2c_channel, HI_U32 puRF)
{
...
HI_VOID r820c_tuner_standby(HI_U32 u32TunerPort, HI_U32 u32Status)
{
    R828_ErrCode err = RT_Success;
    R828_LoopThrough_Type R828_LoopSwitch = SIGLE_IN;
		
   err = R828_Standby(R828_LoopSwitch);
    if(RT_Success != err)
    {
        HI_ERR_TUNER("R828_Standby\n");
        return;
    }
    return;
}
