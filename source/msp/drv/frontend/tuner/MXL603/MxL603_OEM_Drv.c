...
MXL_STATUS MxLWare603_OEM_ReadRegister_dvb_c(UINT8 devId, UINT8 RegAddr, UINT8 *DataPtr)
{
  ...
  // P(Stop condition)
    ...
    for(i=0; i<10; i++)
    {
        if(HI_STD_I2C_NUM > g_MxlI2cChnNum)
        {
            status = HI_DRV_I2C_Read(g_MxlI2cChnNum, (HI_U8)I2C_Addr, u32RegAddr, 2, DataPtr, 1);
        }
        else
        {
            status = HI_DRV_GPIOI2C_ReadExt(g_MxlI2cChnNum, (HI_U8)I2C_Addr, u32RegAddr, 2, DataPtr, 1);
        }
...
MXL_STATUS MxLWare603_OEM_ReadRegister(UINT8 devId, UINT8 RegAddr, UINT8 *DataPtr)
{
    ...
    if(g_stTunerOps[devId].enDemodDevType == HI_UNF_DEMOD_DEV_TYPE_MN88472)
    {
        ...
        */
        ...
        {
            status = HI_DRV_GPIOI2C_WriteExtNoSTOP(g_stTunerOps[devId].enI2cChannel, (g_stTunerOps[devId].u32DemodAddress + DMD_BANK_T2_)<<1, 0xF7, 1, buf, 1);
            status = HI_DRV_GPIOI2C_ReadExtDirectly(g_stTunerOps[devId].enI2cChannel, (g_stTunerOps[devId].u32DemodAddress + DMD_BANK_T2_)<<1, 0, 0, DataPtr, 1);
        }
