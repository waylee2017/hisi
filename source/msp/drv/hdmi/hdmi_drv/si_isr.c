...
HI_U32 SI_RSEN_Status( void )
{
    ...
    {
        return (HI_U32)((GetSysStat() & (BIT_RSEN))?HI_TRUE:HI_FALSE);
    }
}
...
#ifdef HDMI_WORKQUEUE_SUPPORT                
                hdmi_event_struct.event = HI_UNF_HDMI_EVENT_RSEN_CONNECT; 
                delay = (HZ * 100)/ 1000;   //delay 100ms              
                HPD_INFO("BIT_INT_RSEN : queue_delayed_work\n");
                DRV_HDMI_schedule_work(&(hdmi_event_struct.hdmi_event_work), delay);       //can't power down the phy, should change    
#else
