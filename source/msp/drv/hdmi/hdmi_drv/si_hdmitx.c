...
//volatile HDMI_REG_CRG_S *g_pHDMIRegCrg   = (HDMI_REG_CRG_S *)IO_ADDRESS(HI_CRG_BASE_ADDR);
volatile HDMI_REG_CRG_S *g_pHDMIRegCrg   = (HDMI_REG_CRG_S *)IO_ADDRESS(HDMI_CRG_BASE_ADDR);
...
#define CEC_OTP      0x101801ac
#define soc_fuse     0x0f000000
#define CEC_IO       0x10203090 
#define ioctrl_36_SW 0x00000f00


void SET_CEC_IO(void)
{
    HI_U32 u32Reg1, u32Reg2;

    DRV_HDMI_ReadRegister(CEC_OTP, &u32Reg1);
    DRV_HDMI_ReadRegister(CEC_IO, &u32Reg2);
    COM_INFO("add:0x%x:0x%x,add:0x%x:0x%x \n",CEC_OTP, u32Reg1,CEC_IO,u32Reg2);
    
    u32Reg1 = (u32Reg1 & soc_fuse) >> 16;
    u32Reg2 = (u32Reg2 & (~ioctrl_36_SW)) | u32Reg1;
    DRV_HDMI_WriteRegister(CEC_IO, u32Reg2);
}
...
HI_VOID SI_HW_ResetCtrl(int iEnable)
{
    ...
    g_pHDMIRegCrg->PERI_CRG67.u32 = u32Ctrller;    
}
...
HI_VOID SI_HW_ResetPhy(int iEnable)
{
    ...
    u32Phy = g_pHDMIRegCrg->PERI_CRG68.u32;
    if (iEnable == 0)
    {
        u32Phy |= 0xf;
        u32Phy &= (~0x70);
...
#endif 
    g_pHDMIRegCrg->PERI_CRG68.u32 = u32Phy;    
...
{
    COM_INFO("--> SI_SW_ResetHDMITX in\n");
    SI_AssertHDMITX_SWReset(BIT_TX_SW_RST | BIT_TX_FIFO_RST);
    DelayMS(5);

    // delayMs(5) to allow TCLK (sent to Rx across the HDMS link) to stabilize
    SI_ReleaseHDMITX_SWReset(BIT_TX_SW_RST | BIT_TX_FIFO_RST);
    DelayMS(5);
    COM_INFO("--> SI_SW_ResetHDMITX out\n");

    return;
}

/*******************************************************************************
\brief:     Wait for pixel clock stable
\param[in]: void
\retval     void
*******************************************************************************/
void SI_WaitForPixelClkStable(void)
{
    ...
    COM_INFO("--> SI_WaitForPixelClkStable in\n");
    ...
    if (0 == TimeOut)
    {
        COM_INFO("Pixel clk is not stable in 200ms!!!\n");
    }
    COM_INFO("--> SI_WaitForPixelClkStable out\n");
...
{
    return (HI_U8)(BIT_TXHDMI_MODE & ReadByteHDMITXP1(AUDP_TXCTRL_ADDR));
}
//--------------------------------------------------------------------------
