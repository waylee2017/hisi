...
{
    if(HI_UNF_HDMI_FORCE_HDMI == DRV_Get_DefaultOutputMode(HI_UNF_HDMI_ID_0))
    {
        OutputState = CABLE_PLUGIN_HDMI_OUT;
    }
    else
...
HI_U8 SI_TimerHandler(void)
{
    ...
    if(OutputState == CABLE_PLUGIN_CHECK_EDID)
    {
        ...
        if( (OutputState == CABLE_PLUGIN_HDMI_OUT) || (OutputState == CABLE_PLUGIN_DVI_OUT) )
        {
            ...
            SI_timer_count();
#ifdef  HDMI_WORKQUEUE_SUPPORT    
            hdmi_event_struct.event = HI_UNF_HDMI_EVENT_HOTPLUG; 
            delay = (HZ * 100)/ 1000;   //delay 100ms              
            HPD_INFO("SI_TimerHandler : queue_delayed_workn HDMI or DVI out\n");
            DRV_HDMI_schedule_work(&(hdmi_event_struct.hdmi_event_work), delay);
#else
            DelayMS(200);
    ...
    if((OutputState == CABLE_PLUGIN_HDMI_OUT) || (OutputState == CABLE_PLUGIN_DVI_OUT))
    {
        ...
        if( (AuthState == REPEATER_AUTH_REQ) || ( AuthState == REQ_SHA_CALC ) || ( AuthState == REQ_SHA_HW_CALC ))
        {
            ...
            if( AuthState == REQ_SHA_HW_CALC )
            {
                ...
                {                
                    HI_PRINT("SHA_HW_Handler ok, enter hdcp protocol 3\n");
