...
#define HDMI_N_VALUE_11648  11648
#define HDMI_N_VALUE_5824   5824
...
// Globals: none
//------------------------------------------------------------------------------
void SI_SetAudioPath (HI_U8 *pu8AudioPath, HI_UNF_ENC_FMT_E eFmt)
{
    /////////////////////////////////////////////////////////////////////////////////////////////////////
    //
    ...
    SI_BlockWriteEEPROM(4, EE_TX_AUDIOPATH_ADDR, pu8AudioPath);

    SI_Set_N_Fs();
    if (  (HI_UNF_ENC_FMT_720P_60 == eFmt)
        ||(HI_UNF_ENC_FMT_1080i_60 == eFmt))
     {
         u32N = HDMI_N_VALUE_11648;
         SI_WriteNValue(u32N);
     }
     else if (HI_UNF_ENC_FMT_1080P_60 == eFmt)
     {
         u32N = HDMI_N_VALUE_5824;
         SI_WriteNValue(u32N);
     }

    bAudioModes = pu8AudioPath[0] & SiI_AudioModesSelect;   // Input audio format
...
*/
    ...
    {
        bRegVal3 = pu8AudioPath[2];             // Input I2S sample length
        WriteByteHDMITXP1(I2S_IN_LEN, bRegVal3);

        bRegVal1 &= (~BIT_SPDIF_SELECT);
        bRegVal1 &= (~SD_0_3_EN);       // Clear bits 7:4

        if (pu8AudioPath[0] & SiI_MultyChannel)
        {
        ...
        else                            // Not DSD Audio. Could be I2S or HBR
        {
            ...
            {
                WriteByteHDMITXP1( I2S_CHST4_ADDR, pu8AudioPath[1]);   // Fs
                WriteByteHDMITXP1( I2S_CHST5_ADDR, (pu8AudioPath[2] | ((pu8AudioPath[1] << 4) & 0xF0)));   // "Original Fs" and Length

                WriteByteHDMITXP1( I2S_IN_CTRL_ADDR, pu8AudioPath[3]);
            }
            else                        // HBRA
            {
