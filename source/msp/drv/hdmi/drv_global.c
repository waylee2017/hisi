...
static HI_UNF_HDMI_SINK_CAPABILITY_S g_stEdidInfo[HI_UNF_HDMI_ID_BUTT];
...
extern HI_U8 g_pUnfFmtString[HI_UNF_ENC_FMT_BUTT+1][DEF_FILE_NAMELENGTH];
#endif 
...
{
    HI_UNF_ENC_FMT_E   enEncFmt = HI_UNF_ENC_FMT_1080i_50;
    ...
    //if ((enEncFmt < HI_UNF_ENC_FMT_861D_640X480_60) || DRV_Get_Is4KFmt(enEncFmt))
...
{
    return &g_stHdmiChnParam[enHdmi].stSPDInfoFrame;
}
...
HI_VOID DRV_Set_SSCEnable(HI_UNF_HDMI_ID_E enHdmi,HI_BOOL bSSCEnable)
{
    ...
    {
        g_stHdmiChnParam[enHdmi].bSSCEnable = bSSCEnable;
    }
...
HI_BOOL DRV_Get_IsSSCEnable(HI_UNF_HDMI_ID_E enHdmi)
{
    if (HI_UNF_HDMI_ID_BUTT > enHdmi)
    {
        return g_stHdmiChnParam[enHdmi].bSSCEnable;
    }
    else
    {
        return 0;
...
{
    if((enFmt == HI_UNF_ENC_FMT_VESA_800X600_60)
     || (enFmt == HI_UNF_ENC_FMT_861D_640X480_60)
     || (enFmt == HI_UNF_ENC_FMT_VESA_1024X768_60)
     || (enFmt == HI_UNF_ENC_FMT_VESA_1280X720_60)

     || (enFmt == HI_UNF_ENC_FMT_VESA_1280X800_60)
     || (enFmt == HI_UNF_ENC_FMT_VESA_1280X1024_60)
     || (enFmt == HI_UNF_ENC_FMT_VESA_1360X768_60)

     || (enFmt == HI_UNF_ENC_FMT_VESA_1366X768_60)     
     || (enFmt == HI_UNF_ENC_FMT_VESA_1400X1050_60)
     || (enFmt == HI_UNF_ENC_FMT_VESA_1440X900_60)

     || (enFmt == HI_UNF_ENC_FMT_VESA_1440X900_60_RB)     
     || (enFmt == HI_UNF_ENC_FMT_VESA_1600X900_60_RB)
     || (enFmt == HI_UNF_ENC_FMT_VESA_1600X1200_60)    
     
     || (enFmt == HI_UNF_ENC_FMT_VESA_1680X1050_60)     
     //|| (enFmt == HI_UNF_ENC_FMT_VESA_1680X1050_60_RB)
     || (enFmt == HI_UNF_ENC_FMT_VESA_1920X1080_60)

     || (enFmt == HI_UNF_ENC_FMT_VESA_1920X1200_60)     
     //|| (enFmt == HI_UNF_ENC_FMT_VESA_1680X1050_60_RB)
     || (enFmt == HI_UNF_ENC_FMT_VESA_2048X1152_60)

     //|| (enFmt == HI_UNF_ENC_FMT_VESA_2560X1440_60_RB)     
     //|| (enFmt == HI_UNF_ENC_FMT_VESA_2560X1600_60_RB)

     //|| (enFmt == HI_UNF_ENC_FMT_CUSTOM)
     )
    {
        return HI_TRUE;
    }
...
{
    if((DRV_N_Pa_Adjust(enFmt) == HI_UNF_ENC_FMT_PAL) ||
        (DRV_N_Pa_Adjust(enFmt) == HI_UNF_ENC_FMT_NTSC))
    {
        return HI_TRUE;
    }
...
HI_U32 DRV_Calculate_TMDSCLK(HI_UNF_ENC_FMT_E fmt, HI_UNF_HDMI_DEEP_COLOR_E enDeepColorMode)
{
    ...
    u8Index = Transfer_VideoTimingFromat_to_VModeTablesIndex(fmt);
...
HI_S32 DRV_N_Pa_Adjust(HI_UNF_ENC_FMT_E SrcFmt)
{
    ...
    if(SrcFmt >= HI_UNF_ENC_FMT_BUTT)
    {        
        COM_ERR("Wrong Src Format : %d\n", SrcFmt);
        return Ret;  
    }
    ...
    switch(SrcFmt)
    {
        ...
        case HI_UNF_ENC_FMT_SECAM_COS:
            Ret = HI_UNF_ENC_FMT_PAL;
            break;
            
        case HI_UNF_ENC_FMT_NTSC:
        case HI_UNF_ENC_FMT_NTSC_J:
        case HI_UNF_ENC_FMT_NTSC_PAL_M:
            Ret = HI_UNF_ENC_FMT_NTSC;            
            break;

        default :   
            
#ifndef HI_ADVCA_FUNCTION_RELEASE
//            COM_INFO("Isn't NTSC/PAL small Format : %s\n", g_pUnfFmtString[SrcFmt]);
#endif
            Ret = SrcFmt;
    }
...
{
#if 1
    HI_UNF_ENC_FMT_E dstFmt = SrcFmt;
#else
...
HI_U8 *DRV_Event2String(HI_UNF_HDMI_EVENT_TYPE_E event)
{
    ...
    switch(event)
    {
        case HI_UNF_HDMI_EVENT_HOTPLUG :
            str = HDMI_STR(HI_UNF_HDMI_EVENT_HOTPLUG);
            break;
            
        case HI_UNF_HDMI_EVENT_NO_PLUG :
            str = HDMI_STR(HI_UNF_HDMI_EVENT_NO_PLUG);
            break;
            
        case HI_UNF_HDMI_EVENT_EDID_FAIL :
            str = HDMI_STR(HI_UNF_HDMI_EVENT_EDID_FAIL);
            break;
            
        case HI_UNF_HDMI_EVENT_HDCP_FAIL :
            str = HDMI_STR(HI_UNF_HDMI_EVENT_HDCP_FAIL);
            break;
            
        case HI_UNF_HDMI_EVENT_HDCP_SUCCESS :
            str = HDMI_STR(HI_UNF_HDMI_EVENT_HDCP_SUCCESS);
            break;
            
        case HI_UNF_HDMI_EVENT_RSEN_CONNECT :
            str = HDMI_STR(HI_UNF_HDMI_EVENT_RSEN_CONNECT);
            break;
            
        case HI_UNF_HDMI_EVENT_RSEN_DISCONNECT :
            str = HDMI_STR(HI_UNF_HDMI_EVENT_RSEN_DISCONNECT);
            break;
        ...
        case HI_UNF_HDMI_EVENT_BUTT :
            str = HDMI_STR(HI_UNF_HDMI_EVENT_BUTT);
            break;
        default:
            str = HDMI_STR(HI_UNF_HDMI_EVENT_BUTT);
            break;
