...
#define MANUFACTUR_ENAME_LENGTH 3
...
HI_S32 GetFormatDelay(HI_UNF_HDMI_ID_E enHdmi,HI_U32 *DelayTime)
{
    HI_U32 i = 0;
    HI_U32 Delay = 0;
    HI_UNF_HDMI_SINK_CAPABILITY_S *pSinkCap = DRV_Get_SinkCap(enHdmi);   
    ...
    {            
        for (i = 0; WholeList[i] != NULL; i++)
        {
            ...
            if((pSinkCap->u32IDProductCode == WholeList[i]->u32IDProductCode) && 
                (!HI_OSAL_Strncmp(pSinkCap->u8IDManufactureName, WholeList[i]->u8IDManufactureName, MANUFACTUR_ENAME_LENGTH)))
            {
        ...
        if(Delay == 0)
        {
            ...
            if(!HI_OSAL_Strncmp(pSinkCap->u8IDManufactureName, "SKY",MANUFACTUR_ENAME_LENGTH) || 
                !HI_OSAL_Strncmp(pSinkCap->u8IDManufactureName, "SKW",MANUFACTUR_ENAME_LENGTH))
            {
...
HI_S32 GetmuteDelay(HI_UNF_HDMI_ID_E enHdmi,HI_U32 *DelayTime)
{
    ...
    else
    {
        ...
        if(MuteDelay == 0)
        {
            if(pstVidAttr->enVideoFmt == HI_UNF_ENC_FMT_1080P_30 ||
                pstVidAttr->enVideoFmt == HI_UNF_ENC_FMT_1080P_25 ||
                pstVidAttr->enVideoFmt == HI_UNF_ENC_FMT_1080P_24)
            {
...
void Check1stOE(HI_UNF_HDMI_ID_E enHdmi)
{
    ...
    {
        //if sincap is invalid, the data can not euqal to compatibility list
        if((pSinkCap->u32IDProductCode == WholeOEList[i]->u32IDProductCode) && 
            (!HI_OSAL_Strncmp(pSinkCap->u8IDManufactureName,WholeOEList[i]->u8IDManufactureName,MANUFACTUR_ENAME_LENGTH)))
        {
    ...
    if((!HI_OSAL_Strncmp(pSinkCap->u8IDManufactureName,"SNY",MANUFACTUR_ENAME_LENGTH)))
    {
        if(pSinkCap->u32YearOfManufacture == 2008 || pSinkCap->u32YearOfManufacture == 2009)
        {
