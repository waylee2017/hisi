...
typedef struct
{
    ...
    GPIO_INT_ATTR_S   GpioIntAttr[HI_GEN_GPIO_MAX_BIT_NUM];
...
static HI_BOOL DRV_GPIO_Convert(HI_U32 u32GpioNo, HI_U32 *pu32GroupNo, HI_U32 *pu32BitNo)
{
    if (u32GpioNo >= g_u8GpioMaxNum)
    {
	 ...
	 if (( u32GpioNo / HI_GPIO_BIT_NUM)==4)
    {
        HI_ERR_GPIO("para u32GpioNo  is invalid.\n");
        return HI_FALSE;
    }
...
HI_S32 HI_DRV_GPIO_SetBitIntEnable(HI_U32 u32GpioNo, HI_BOOL bEnable)
{
    ...
    if (DRV_GPIO_Convert(u32GpioNo, &u32GroupNo, &u32BitNo) == HI_FALSE)
    {
        return HI_ERR_GPIO_INVALID_PARA;
    }
    GpioUsrAddr = g_GpioUsrAddr[u32GroupNo];
    ...
    HI_INFO_GPIO("u32GpioGroup = %d u32BitX = %d  bEnable = %d GpioUsrAddr = 0x%x\n", u32GroupNo, u32BitNo, bEnable , GpioUsrAddr);
...
HI_S32 HI_DRV_GPIO_Init(void)
{
    ...
    HI_DRV_GPIO_GetGpioNum(&g_GpioNum);
    ...
    for (i = 0; i < g_GpioNum.u8GpioGrpNum; i++)
    {
        ret = osal_request_irq(HI_GPIO_IRQ_NO + i, GPIO_Isr, IRQF_SHARED, g_GpioIrqName[i], (HI_VOID *)&g_stGpioAttr);
        if (HI_SUCCESS != ret)
        {
...
void  HI_DRV_GPIO_DeInit(void)
{
    ...
    for (i = 0; i < g_GpioNum.u8GpioGrpNum; i++)
    {
        osal_free_irq(HI_GPIO_IRQ_NO + i, g_GpioIrqName[i], (HI_VOID *)&g_stGpioAttr);
