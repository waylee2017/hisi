...
#define  CNT_SECLEADS (IR_REG_BASE + 0x38)
#define  CNT_SECLEADE (IR_REG_BASE + 0x3c)
#define  IR_TRAIL (IR_REG_BASE + 0x40)
#define  IR_MODE_BIT (IR_REG_BASE + 0x44)
#define  IR_CFG2 (IR_REG_BASE + 0x48)
#define  IR_FILTER_EN (IR_REG_BASE + 0x4C)
#define  IR_CFG3 (IR_REG_BASE + 0x50)
...
HI_VOID Reset_KeyBuf(HI_VOID)
{
	HI_U32 i;
	for(i=0;i<IR_MAX_BUF;i++)
	{
		memset(g_IrAttr.IrKeyBuf, 0, sizeof(g_IrAttr.IrKeyBuf));
		g_IrAttr.IrKeyBuf[i].IrKeyDataH = 0;
		g_IrAttr.IrKeyBuf[i].IrKeyDataL = 0;
		g_IrAttr.IrKeyBuf[i].IrProtocol = 0xff;
		g_IrAttr.IrKeyBuf[i].IrKeyState = HI_UNF_KEY_STATUS_BUTT;		
	}
	return;
}
...
HI_S32 IR_ProcRead(struct seq_file *p, HI_VOID *v)
{
	...
	else if (HI_UNF_IR_CODE_NEC_2HEAD_SIMPLE == g_IrAttr.IrCode)
    {
        snprintf(IrCodeString,sizeof(IrCodeString), "IR_CODE_NEC_2HEADS_SIMPLE");
    }
	else if (HI_UNF_IR_CODE_RC5 == g_IrAttr.IrCode)
    {
        snprintf(IrCodeString,sizeof(IrCodeString), "IR_CODE_RC5");
    }
	else if (HI_UNF_IR_CODE_NEC_2HEAD_FULL == g_IrAttr.IrCode)
    {
        snprintf(IrCodeString,sizeof(IrCodeString), "IR_CODE_NEC_2HEADS_FULL");
    }
	else if (HI_UNF_IR_CODE_RC6 == g_IrAttr.IrCode)
    {
        snprintf(IrCodeString,sizeof(IrCodeString), "IR_CODE_RC6");
    }
	else if (HI_UNF_IR_CODE_RSTEP == g_IrAttr.IrCode)
    {
        snprintf(IrCodeString,sizeof(IrCodeString), "IR_CODE_RSTEP");
    }
    else
    {
...
HI_VOID IR_Config(HI_VOID)
{
	...
	Value  = g_IrAttr.IrDevPara.SecLeadsMin << 16;
    Value |= g_IrAttr.IrDevPara.SecLeadsMax;
    WRITE_REG(CNT_SECLEADS, Value);

	Value  = g_IrAttr.IrDevPara.SecLeadeMin << 16;
    Value |= g_IrAttr.IrDevPara.SecSeadeMax;
    WRITE_REG(CNT_SECLEADE, Value);
	
	Value  = g_IrAttr.IrDevPara.TrailerMin << 16;
    Value |= g_IrAttr.IrDevPara.TrailerMax;
    WRITE_REG(IR_TRAIL, Value);

	Value = g_IrAttr.IrDevPara.Modebit;
	WRITE_REG(IR_MODE_BIT, Value);
	
	READ_REG(IR_DATAH);
	READ_REG(IR_DATAL);
	WRITE_REG(IR_INTC, 0x0F);
...
HI_S32 Ir_Isr(int irq, void *dev_id, struct pt_regs *regs)
{
    ...
    else if (READ_REG(IR_INTS) & IR_INT_RELEASE)
    {
        ...
        if ((g_IrAttr.bKeyUp) && (HI_UNF_KEY_STATUS_UP != IR_BUF_LAST.IrKeyState)&&(HI_UNF_KEY_STATUS_BUTT != IR_BUF_LAST.IrKeyState))
        {
            ...
            IR_BUF_HEAD.IrProtocol = g_IrAttr.IrCode;
...
static long Ir_Ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case CMD_IR_SET_FORMAT:
        ...
        if (HI_UNF_IR_CODE_NEC_SIMPLE == g_IrAttr.IrCode)
        {
			...
			g_IrAttr.IrDevPara.CodeType = 0;
        }
        else if (HI_UNF_IR_CODE_TC9012 == g_IrAttr.IrCode)
        {
            g_IrAttr.IrDevPara = g_IrDevPara[4];
			g_IrAttr.IrDevPara.CodeType = 1;
        }
        else if (HI_UNF_IR_CODE_NEC_FULL == g_IrAttr.IrCode)
        {
            g_IrAttr.IrDevPara = g_IrDevPara[5];
			g_IrAttr.IrDevPara.CodeType = 2;
        }
        else if (HI_UNF_IR_CODE_SONY_12BIT == g_IrAttr.IrCode)
        {
            g_IrAttr.IrDevPara = g_IrDevPara[12];
			g_IrAttr.IrDevPara.CodeType = 3;
        }
		else if (HI_UNF_IR_CODE_NEC_2HEAD_SIMPLE == g_IrAttr.IrCode)
        {
            g_IrAttr.IrDevPara = g_IrDevPara[16];
			g_IrAttr.IrDevPara.CodeType = 4;
        }
		else if (HI_UNF_IR_CODE_RC5 == g_IrAttr.IrCode)
        {
            g_IrAttr.IrDevPara = g_IrDevPara[17];
			g_IrAttr.IrDevPara.CodeType = 5;
        }
		else if (HI_UNF_IR_CODE_NEC_2HEAD_FULL == g_IrAttr.IrCode)
        {
            g_IrAttr.IrDevPara = g_IrDevPara[18];
			g_IrAttr.IrDevPara.CodeType = 6;
        }
		else if (HI_UNF_IR_CODE_RC6 == g_IrAttr.IrCode)
        {
            g_IrAttr.IrDevPara = g_IrDevPara[19];
			g_IrAttr.IrDevPara.CodeType = 7;
        }
		else if (HI_UNF_IR_CODE_RSTEP == g_IrAttr.IrCode)
        {
            g_IrAttr.IrDevPara = g_IrDevPara[20];
			g_IrAttr.IrDevPara.CodeType = 8;
        }
...
HI_S32 IR_Open(struct inode *inode, struct file *filp)
{
    ...
    if (1 == atomic_inc_return(&g_IrCount))
    {
        ...
        g_IrAttr.IrDevPara = g_IrDevPara[0];
        g_IrAttr.IrDevPara.CodeType = 0;  //NEC uPD6121G
        g_IrAttr.Head = 0;
        g_IrAttr.Tail = 0;
        ...
        Ret = osal_request_irq(IR_IRQ_NO, (irq_handler_t)Ir_Isr, IRQF_SHARED, "hi_ir_std_irq", HI_NULL);
        if (Ret != HI_SUCCESS)
        {
...
HI_S32 IR_Close(struct inode *inode, struct file *filp)
{
    ...
    {
        osal_free_irq(IR_IRQ_NO, "hi_ir_std_irq", HI_NULL);
