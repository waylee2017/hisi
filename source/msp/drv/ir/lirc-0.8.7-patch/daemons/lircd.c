...
int clis[MAX_CLIENTS];      /* fd used to transfer message with client*/
...
int waitfordata(long maxusec)
{
	...
	while(1)
	{
		do{
			...
			timerclear(&tv);
			reconnect=0;
			for(i=0;i<peern;i++)   /* peern is zero if -c not set */
			{
				if(peers[i]->socket!=-1)
				{
			...
			LOGPRINTF(2,"%s %d tv(%u %u)",__FUNCTION__,__LINE__,tv.tv_sec,tv.tv_usec);				      					
			...
			if(timerisset(&release_time))
			{
				...
				{
					timerclear(&tv);
					LOGPRINTF(2,"%s %d",__FUNCTION__,__LINE__);						      					
				}
				else
				{
					struct timeval gap;
			...
			{
				ret=select(maxfd+1,&fds,NULL,NULL,NULL);    /* block indefinitely */
			}
#endif
			   ...
			   timercmp(&now, &release_time, >))
			{
				...
				release_message = trigger_release_event
					(&release_remote_name,
					 &release_button_name);
  			    LOGPRINTF(2,"%s %d rls_msg %s",__FUNCTION__,__LINE__,release_message);						      
				if(release_message)
				{
			...
			if(maxusec>0)
			{
				if(ret==0)
				{
					LOGPRINTF(2,"%s %d tv(%u %u) rls(%u %u) recon %d",__FUNCTION__,__LINE__,tv.tv_sec,tv.tv_usec,release_time.tv_sec,release_time.tv_usec,reconnect);
					return(0);
				}
		...
		for(i=0;i<clin;i++)
		{
			...
			{
				FD_CLR(clis[i],&fds);
				LOGPRINTF(1,"get_command from local client %d",i);
...
void loop()
{
	...
	while(1)
	{
		...
		if(message!=NULL)
		{
			...
			LOGPRINTF(2,"input_message %s",message);
