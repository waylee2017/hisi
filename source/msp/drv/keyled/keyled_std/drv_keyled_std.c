...
/* disalbe keypad  only, not affect other hardware para*/
...
HI_S32 KEYLED_SetMode(HI_KEYLED_MODE_S KeyledMode)
{
    ...
    if (HI_CHIP_TYPE_HI3716M != enChipType || HI_CHIP_VERSION_V300 != enChipVersion)
    {
        HI_ERR_KEYLED("This setting don't support in this chip type! \n");
        return HI_FAILURE;
    }
    ...
    if (HI_KEYLED_1MUL8 == KeyledMode.KeyScanMode)
    {
        ...
        if (HI_KEYLED_4LED == KeyledMode.LedNum)
        {
...
HI_S32 KEYLED_Init(HI_VOID)
{
    ...
    err = osal_request_irq(KEYLED_IRQ, keyled_interrupt_handler, IRQF_SHARED, "keyled_std", (HI_VOID*)&hkeyledIRQ);
    if (0 != err)
    {
...
//    HI_INFO_KEYLED("enChipType =%d enChipVersion = %d \n ",enChipType ,enChipVersion);
    ...
    keyled_dev.KeyLedMode.KeyScanMode = HI_KEYLED_2MUL4;
