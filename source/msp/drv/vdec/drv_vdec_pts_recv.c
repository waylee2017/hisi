...
#define PTS_REC_MAX_NUM         (HI_VDEC_MAX_INSTANCE)
...
static PTSREC_CHAN_INFO_S s_stPtsRecChan[HI_VDEC_MAX_INSTANCE];
...
HI_S32 PTSREC_SetFrmRate(HI_HANDLE hHandle, HI_UNF_AVPLAY_FRAMERATE_PARAM_S *pstFrmRate)
{
    HI_U32 u32DivNum;
    ...
    {
        // 1
        u32DivNum = pstFrmRate->stSetFrameRate.u32fpsInteger * 1000 + pstFrmRate->stSetFrameRate.u32fpsDecimal;

        if ((u32DivNum > PTS_REC_MAX_FRAMERATE * 1000) || (0 == u32DivNum))
        {
            HI_ERR_PTSREC("invalid frame rate, Integer(%d), Decimal(%d) \n", pstFrmRate->stSetFrameRate.u32fpsInteger, pstFrmRate->stSetFrameRate.u32fpsDecimal);
            return HI_FAILURE;
        }
...
HI_S32 PTSREC_GetFrmRate(HI_HANDLE hHandle, HI_UNF_AVPLAY_FRAMERATE_PARAM_S *pstFrmRate)
{
    HI_U32 u32Val;
    ...
    if (s_stPtsRecChan[hHandle].u32SetFrameTime != HI_INVALID_TIME)
    {
        ...
        pstFrmRate->stSetFrameRate.u32fpsInteger = u32Val / 1000;
        pstFrmRate->stSetFrameRate.u32fpsDecimal = u32Val % 1000;
    }
    else
    {
        HI_ERR_PTSREC("Didn't set\n");
        pstFrmRate->stSetFrameRate.u32fpsInteger = 0;
        pstFrmRate->stSetFrameRate.u32fpsDecimal = 0;
    }
...
static HI_VOID PTSREC_HandleValidSrcPTS(HI_HANDLE hHandle, IMAGE *pstImage)
{
    ...
    if (pstPtsInfo->u32LastSrcPts != HI_INVALID_PTS)
    {
...
static HI_U32 PTSREC_GetFrameTime_User(PTSREC_CHAN_INFO_S *pstPtsInfo, IMAGE *pstImage)
{
    HI_U32 u32FrameTime = HI_INVALID_TIME;

    /* Choose u32FrameTime from: 1.SetFrameTime 2.CalcFrameTime 3.GuessFrameTime*/
    if (pstPtsInfo->u32SetFrameTime != HI_INVALID_TIME)
    {
        u32FrameTime = pstPtsInfo->u32SetFrameTime;
    }
    else if (pstPtsInfo->u32CalcFrameTime != HI_INVALID_TIME)
    {
        u32FrameTime = pstPtsInfo->u32CalcFrameTime;
    }
    else
    {
        if (pstPtsInfo->u32GesFrameTime == HI_INVALID_TIME)
        {
...
static HI_U32 PTSREC_GetFrameTime_Other(PTSREC_CHAN_INFO_S* pstPtsInfo, IMAGE* pstImage)
{
    HI_U32 u32FrameTime = HI_INVALID_TIME;

    /* Choose u32FrameTime from: 1.u32CalcFrameTime 2.u32SetFrameTime 3.GuessFrameTime*/
    if (pstPtsInfo->u32CalcFrameTime != HI_INVALID_TIME)
    {
        u32FrameTime = pstPtsInfo->u32CalcFrameTime;
    }
    else if (pstPtsInfo->u32SetFrameTime != HI_INVALID_TIME)
    {
        u32FrameTime = pstPtsInfo->u32SetFrameTime;
    }
    else
...
static HI_VOID PTSREC_HandleInValidSrcPTS(HI_HANDLE hHandle, IMAGE* pstImage)
{
    HI_U32 u32NumPass = 0;
    HI_U32 u32FrameTime = HI_INVALID_TIME;
    HI_U32 u32SrcPts = 0;
    ...
    if (pstPtsInfo->u32LastSrcPts != HI_INVALID_PTS)
    {
        ...
        if (pstPtsInfo->enFrmRateType == HI_UNF_AVPLAY_FRMRATE_TYPE_USER)
        {
            u32FrameTime = PTSREC_GetFrameTime_User(pstPtsInfo, pstImage);
        }
        else /* HI_UNF_AVPLAY_FRMRATE_TYPE_USER_PTS ... */
        {
            u32FrameTime = PTSREC_GetFrameTime_Other(pstPtsInfo, pstImage);
        }

        u32Pts = pstPtsInfo->u32LastSrcPts + (u32FrameTime * u32NumPass) / 1000;
    }
    else if (pstPtsInfo->u32LastSrcPts != HI_INVALID_PTS)
    {
...
HI_U32 PTSREC_CalcStamp(HI_HANDLE hHandle, HI_UNF_VCODEC_TYPE_E enVdecType, IMAGE* pstImage)
{
    ...
    {
        PTSREC_HandleInValidSrcPTS(hHandle, pstImage);
    }
