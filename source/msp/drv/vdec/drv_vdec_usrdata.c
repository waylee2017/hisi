...
static USRDATA_PARAM_S g_stUsrData[HI_VDEC_MAX_INSTANCE];
...
HI_S32 USRDATA_Alloc(HI_HANDLE hHandle, HI_DRV_VDEC_USERDATABUF_S *pstBuf)
{
    ...
    if ((hHandle >= HI_VDEC_MAX_INSTANCE) || (HI_NULL == pstBuf) || (g_stUsrData[hHandle].bUsed))
    {
...
HI_S32 USRDATA_Acq(HI_HANDLE hHandle, HI_UNF_VIDEO_USERDATA_S *pstUsrData, HI_UNF_VIDEO_USERDATA_TYPE_E *penType)
{
    ...
    if ((hHandle >= HI_VDEC_MAX_INSTANCE) || (HI_NULL == pstUsrData) || (HI_NULL == penType))
    {
        return HI_FAILURE;
    }
...
HI_S32 USRDATA_Arrange(HI_HANDLE hHandle, USRDAT *pstUsrData)
{
    ...
    if ((hHandle >= HI_VDEC_MAX_INSTANCE) || (HI_NULL == pstUsrData) ||
