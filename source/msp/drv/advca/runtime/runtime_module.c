...
#define SC_SYS_BASE     IO_ADDRESS(0x101E0000)
...
static int module_copy_thread_proc(void* argv)
{
    ...
    g_u32ModuleVirAddr = g_stMmzBuf.u32StartVirAddr;
    ...
    memset((void*)g_u32ModuleVirAddr, 0x0, g_u32ModuleUsedSize);
...
void Config_LPC_Setting(void)
{
    if (HI_TRUE == g_bRuntimeCheckEnable)
    {
        ...
        writel(0x1, (void *)IO_ADDRESS(MCU_START_REG));//Start MCU
        writel(PMOC_RUNTIME_CHECK_OK, (void *)SC_GEN27);//runtime check vector is ready
        writel(0x3,(void *)IO_ADDRESS(KL_LPC_DISABLE));//lpc_ram_wr_disable, lpc_rst_disable
...
static int calc_kernel_hash(unsigned int u32Hash[5])
{
    ...
    unsigned int u32KernelVirAddr = 0;
    unsigned int i = 0;
...
static int store_check_vector(void)
{
    ...
    pu32Ptr = (unsigned int*)(((unsigned char*)g_u32C51CheckVectorVirAddr) + 0x64);
    u32SectionNum = 0x01;
    HI_REG_WRITE32(pu32Ptr, u32SectionNum);
    ...
    for(i = 0; i < 5; i ++)
    {
        pu32Ptr = (unsigned int*)(((unsigned char*)g_u32C51CheckVectorVirAddr) + 0x68) + i;
        HI_REG_WRITE32(pu32Ptr, u32HashValue[i]);
    }
