...
/*
 *	buile LLI for memory to memory DMA tranfer, only usecd in opentv5.
 */
int dmac_buildllim2m_opentv5(unsigned int *ppheadlli, unsigned int pdest, unsigned int psource, unsigned int totaltransfersize,
                     unsigned int uwnumtransfers)
{
    ...
    if (ppheadlli != NULL)
    {
        ...
        for (j = 0; j < lli_num; j++)
        {
            ...
            {
                dmac_writew(address, ((DMAC_CxCONTROL_LLIM2M_OPENTV5 & (~0xfff)) | (uwnumtransfers) | 0x80000000));
            }
            else if ((j == (lli_num - 1)) && (last_lli == 1))
            {
                dmac_writew(address, ((DMAC_CxCONTROL_LLIM2M_OPENTV5
                                       & (~0xfff)) | ((totaltransfersize % uwnumtransfers)) | 0x80000000));
            }
            else
            {
                dmac_writew(address, (((DMAC_CxCONTROL_LLIM2M_OPENTV5 & (~0xfff)) | (uwnumtransfers)) & 0x7fffffff));
            }
                       ...
                       unsigned long arg)
{
    ...
    ret = (long)HI_DRV_UserCopy(pFile->f_path.dentry->d_inode, pFile, cmd, arg, dma_ioctl);
...
static int __INIT__ dmac_module_init(void)
{
    ...
    if (atomic_inc_return(&g_DMACInitFlag) == 1)
    {
        ...
        HI_DRV_PROC_AddModule("dma", &stFnOpt, NULL);
    }
#ifdef MODULE    
...
EXPORT_SYMBOL(dma_driver_init);
EXPORT_SYMBOL(dma_driver_exit);
...
EXPORT_SYMBOL(dmac_buildllim2m_opentv5);
