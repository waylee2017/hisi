...
#define  VFMW_CHIP_TYPE_Hi3716MV310     15
#define  VFMW_CHIP_TYPE_Hi3716MV330     16

#define  VFMW_CHIP_TYPE_Hi3798MV100     20
...
typedef enum hiVDEC_CID_E
{
    ...
    VDEC_CID_GET_IMAGE_INTF,                        /* get the stream access interface for the decode channel */
    ...
    VDEC_CID_SET_DISCARDPICS_PARAM,                 /* set discard pictures parameters*/
    VDEC_CID_FRAME_DNR,                             /* NR Capture*/
    ...
    VDEC_CID_GET_SCDBUFFER_VIRADDR,
    VDEC_CID_GET_VFMW_INFO,
    VDEC_CID_RWZB_STATE,
...
typedef struct
{
    ...
    UINT32 u32Is1D; //l00273086
    UINT32 is_processed_by_dnr;
    UINT32 u32Circumrotate; //l00273086
    UINT32 u32IsLastFrame; //l00273086
    UINT32 resetNum; //l00273086
    UINT32 u32ChanTick; //l00284814s
} IMAGE;
...
typedef struct
{
    ...
    MEM_DESC_S  ChanMemPmv;
} VDEC_CHAN_MEM_DETAIL_S;

typedef struct
{
    SINT32 SelfAllocChanMem_vdh;  /* 标识通道 vdh 是否是自己分配的，1: 自己分配, 0: 外部分配 */
    SINT32 SelfAllocChanMem_scd;  /* 标识通道 scd 是否是自己分配的，1: 自己分配, 0: 外部分配 */
    SINT32 SelfAllocChanMem_ctx;  /* 标识通道 ctx 是否是自己分配的，1: 自己分配, 0: 外部分配 */
    SINT32 SelfAllocChanMem_pmv;  /* 标识通道 pmv 是否是自己分配的，1: 自己分配, 0: 外部分配 */
} VDEC_CHAN_MEM_FLAG_S;

typedef struct
{
    SINT32 s32DynamicSwitchDnrEnable; /*Dynamic Switch Dnr flag, 1:enable; others:disable*/
    VDEC_CHAN_CAP_LEVEL_E DnrSwitchThreshold;   /*Dnr Switch Threshold, Disable Dnr when stream param(w,h) <= Threshold*/
    SINT32 DnrNum;
    SINT32 s32DnrEnable;
} VDEC_CHAN_DSD_PARAM_S; //l00273086 (DSD means Dynamic Switch Dnr)
...
typedef struct
{
    ...
    VDEC_CHAN_DSD_PARAM_S  DynamicSwitchDnrParam; //l00273086
} VDEC_CHAN_OPTION_S;
...
typedef struct
{
    IMAGE *pstCaptureInFrm;
    IMAGE *pstCaptureOutFrm;
} VDEC_DNR_Capture;
