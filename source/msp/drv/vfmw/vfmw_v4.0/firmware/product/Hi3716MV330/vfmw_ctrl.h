...
#define MAX_DSD_FRAME         MAX_FRAME_NUM //l00273086
...
typedef struct hiVFMW_CHAN_MEM_S
{
    ...
    SINT32         s32SelfAllocChanMem_pmv;  /* 标识通道 pmv 是否是自己分配的，1: 自己分配, 0: 外部分配 */
    STREAM_INTF_S  stStreamIntf;    /* 码流接口 */
    MEM_RECORD_S   stChanMem_pmv;       /* 该通道的pmv存储资源 */
} VFMW_CHAN_MEM_S;

/*解码器DSD的帧存占用状态 l0027086*/
typedef enum
{
    NO_USE = 0,
    VO_USING,
...
/*解码器DSD中的帧存 l00273086*/
typedef struct hi_VFMW_CHAN_DSD_FRAME
{
    DSD_FRAME_STATE_E eFrameState;
    SINT32 s32FrameAddr;
    SINT32 s32FrameSize;
    SINT32 s32UsingCount;
} VFMW_CHAN_DSD_FRAME_S;
...
typedef struct hiVFMW_CHAN_S
{
    ...
    FSP_INST_S      FspInst;
    VFMW_CHAN_DSD_FRAME_S stFrameArray[MAX_DSD_FRAME];// DSD帧 l00273086
    DNR_CHAN_CFG_S      stDnrChanCfg;//l00273086
    SINT32 s32H264Score;
    ...
    SINT32 s32SelfAllocChanMem_pmv;
    MEM_RECORD_S   stChanMem_pmv;       /* 该通道的PMV存储资源 */
    UINT32 u32ChanTick; // 通道创建次数记录，l00284814
...
VOID VCTRL_VdmPostProc( SINT32 ChanId, SINT32 ErrRatio, LUMA_INFO_S *pLumaInfo);
...
SINT32 VCTRL_GetDSDFlag(SINT32 ChanID); //l00273086
SINT32 VCTRL_RlsDSDFrame( SINT32 ChanID, IMAGE *pImage);//l00273086
VOID VCTRL_GetChanPmvSize(SINT32 FrmNum, SINT32 Width, SINT32 Height, SINT32 *s32ChanPmvSize);

SINT32 VDEC_SETDNR_ENABLE(SINT32 ChanID, SINT32 enable);
SINT32 VDEC_GETDNR_ENABLE(SINT32 ChanID, SINT32 *enable);

VOID VCTRL_ClearDecparam(SINT32 ChanId);

VOID VCTRL_EnableEsBuf(SINT32 ChanID);
VOID VCTRL_DisableEsBuf(SINT32 ChanID);
SINT32 VCTRL_IsEsBufEnable(SINT32 ChanID);
