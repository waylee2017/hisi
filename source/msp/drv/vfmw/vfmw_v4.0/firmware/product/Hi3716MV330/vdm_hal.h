...
typedef struct
{
    ...
    // PMV buffer
    SINT32    PMVAddr[30];//这里实际上应该为MAX_FRAME_NUM才合理，但是如果改为MAX_FRAME_NUM，编译器会报错，所以暂时改为30,等待后续更好的方案
    SINT32    HafPmvOffset;
    SINT32    ValidPMVNum;
    SINT32    ImgSlotLen; //帧尺寸 l00273086
    UINT32    u32StrideByte;//地址对齐耗损l00273086
...
typedef struct hiVDMHAL_FUN_PTR_S
{
    ...
    SINT32 (*pfun_VDMHAL_ArrangeMem)( SINT32 MemAddr, SINT32 MemSize, SINT32 Width, SINT32 Height, SINT32 PmvNum, SINT32 FrameNum, SINT32 UserDec, SINT32 ChanID, VDMHAL_MEM_ARRANGE_S *pVdmMemArrange, UINT32 align_length);
    ...
    SINT32 (*pfun_VDMHAL_GetPmvSize)(SINT32 Width, SINT32 Height);
} VDMHAL_FUN_PTR_S;
...
#define VDMHAL_ArrangeMem(MemAddr, MemSize, Width, Height, PmvNum, FrameNum, UserDec, eFlag, pVdmMemArrange, align_length) \
    (g_vdm_hal_fun_ptr.pfun_VDMHAL_ArrangeMem? \
		g_vdm_hal_fun_ptr.pfun_VDMHAL_ArrangeMem(MemAddr, MemSize, Width, Height, PmvNum, FrameNum, UserDec, eFlag, pVdmMemArrange, align_length):\
     VDMHAL_NULL_FUN_PTR)
...
#define VDMHAL_GetPmvSize( Width, Height) \
    (g_vdm_hal_fun_ptr.pfun_VDMHAL_GetPmvSize? \
     g_vdm_hal_fun_ptr.pfun_VDMHAL_GetPmvSize( Width, Height):\
     VDMHAL_NULL_FUN_PTR)
