...
static DMX_DEV_OSI_S *g_pDmxDevOsi = HI_NULL;
...
static HI_BOOL Hi3716MV300Flag = HI_FALSE;
#endif
...
{	
	if ( pDmxDevOsi == NULL )
	{
	    HI_FATAL_DEMUX("Dsc get demux device pointer failed, NULL pointer\n");
		return HI_FAILURE;
	}
...
HI_VOID DescInitHardFlag(HI_VOID)
{
    ...
    {
        DmxDevOsi->KeyAesCbcHardFlag = DMX_KEY_HARDONLY_FLAG;
    }
...
HI_S32 DMXDescramblerCreate0(HI_U32 *KeyId, HI_UNF_DMX_DESCRAMBLER_ATTR_S *Attr)
{
 ...
 //           KeyLen = DMX_KEY_MIN_LEN / sizeof(HI_U32);
    ...
    if (0 != mutex_lock_interruptible(&DmxDevOsi->lock_Key))
    {
    ...
    mutex_unlock(&DmxDevOsi->lock_Key);
...
HI_S32 DMXDescramblerCreate1(HI_U32 *KeyId, HI_UNF_DMX_DESCRAMBLER_ATTR_S *Attr)
{
    ...
    switch (Attr->enDescramblerType)
    {
        ...
        case HI_UNF_DMX_DESCRAMBLER_TYPE_AES_CBC :
            ...
            if ((HI_UNF_DMX_CA_NORMAL == Attr->enCaType) && (DMX_KEY_HARDONLY_FLAG == DmxDevOsi->KeyAesCbcHardFlag))
            {
                return HI_ERR_DMX_NOT_SUPPORT;
            }
			break;
        ...
        default :
            HI_ERR_DEMUX("Invalid DescramblerType %d\n", Attr->enDescramblerType);
...
HI_S32 DMX_OsiDescramblerGetAttr(HI_U32 KeyId, HI_UNF_DMX_DESCRAMBLER_ATTR_S *pstDescramblerAttr)
{
...
HI_S32 DMX_OsiDescramblerSetAttr(HI_U32 KeyId, HI_UNF_DMX_DESCRAMBLER_ATTR_S *pstDescramblerAttr)
{
    HI_S32 ret = HI_ERR_DMX_INVALID_PARA;
    DMX_KeyInfo_S *KeyInfo = &g_pDmxDevOsi->DmxKeyInfo[KeyId];
