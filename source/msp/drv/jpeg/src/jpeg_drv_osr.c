...
#define JPEG_IRQ_NAME             "jpeg"
#define JPEG_DEV_NAME             "jpeg"
...
static DRV_JPEG_IOCTLFUNC_ITEM gs_DrvJpegCtlFunc[DRV_JPEG_IOCTLFUNC_ITEM_NUM_MAX] = {
     ...
     {CMD_JPG_SET_LOWDELAY_LINE, JPEG_OSR_SetLowdelayLine},  {19,NULL}
   };
...
#ifndef CONFIG_GFX_BVT_SDK
    DECLARE_GFX_NODE(JPEG_DEV_NAME,DRV_JPEG_Open, DRV_JPEG_Close,DRV_JPEG_Mmap,
...
HI_VOID JPEG_DRV_ModExit(HI_VOID)
{
    ...
    osal_free_irq(JPGD_IRQ_NUM, JPEG_IRQ_NAME,(HI_VOID*)s_pstruJpgOsrDev);
...
HI_S32 JPEG_DRV_K_ModInit(HI_VOID)
{
...
#ifndef CONFIG_GFX_BVT_SDK
     s32Ret = HI_GFX_MODULE_Register(HIGFX_JPGDEC_ID, JPEG_DEV_NAME, &s_JpegExportFuncs);
     if (HI_SUCCESS != s32Ret)
     {
          JPEG_DRV_ModExit();
          return HI_FAILURE;
     ...
     s32Ret = osal_request_irq(JPGD_IRQ_NUM, (irq_handler_t)DRV_JPEG_IsrFunction, IRQF_SHARED, JPEG_IRQ_NAME, s_pstruJpgOsrDev);
     if (HI_SUCCESS != s32Ret)
     {
         JPEG_DRV_ModExit();
         return HI_FAILURE;
