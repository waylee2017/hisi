...
#if  defined(CHIP_TYPE_hi3716mv310) || defined(CHIP_TYPE_hi3110ev500) || defined (CHIP_TYPE_hi3716mv320)|| defined (CHIP_TYPE_hi3716mv330)
#else
static HI_U32 g_u32ChipVersion;
#endif
...
#ifdef HI_I2C_FIFO_SUPPORT
HI_S32 I2C_DRV_WaitFifoWriteEnd(HI_U32 I2cNum)
{
    HI_U32 I2cSrReg;
    HI_U32 i = 0;
    ...
    do
    {
        ...
        i++;
    } while( ((I2cSrReg & I2C_OVER_INTR) != I2C_OVER_INTR)&&((I2cSrReg & I2C_TXF_OVER) != I2C_TXF_OVER));
...
HI_S32 I2C_DRV_WaitStop(HI_U32 I2cNum)
{
    HI_U32 I2cSrReg;
    HI_U32 i = 0;
    ...
    do
    {
        ...
        i++;
    } while((I2cSrReg & I2C_END_INTR) != I2C_END_INTR);
    if (I2cSrReg & I2C_ACK_INTR)
    {
        HI_ERR_I2C("wait write data timeout!\n");
        return HI_FAILURE;
...
#ifdef HI_I2C_FIFO_SUPPORT
HI_S32 I2C_DRV_WaitRxFifoFull(HI_U32 I2cNum)
{
    HI_U32 I2cSrReg;
    HI_U32 i = 0;
    ...
    do
    {
        ...
        i++;
    } while ((I2cSrReg & I2C_RXTIDE) != I2C_RXTIDE);
...
#ifdef HI_I2C_FIFO_SUPPORT
HI_S32 I2C_DRV_FifoWrite(HI_U32 I2cNum, HI_U8 I2cDevAddr, HI_U32 I2cRegAddr, HI_U32 I2cRegAddrByteNum, HI_U8 *pData,
                     HI_U32 DataLen, HI_BOOL bWithStop)
{
    HI_U32 i;
	HI_U32 TmpLen,txtide;
    //    unsigned long   IntFlag;
    HI_U32 RegAddr;
	...
	I2C_WRITE_REG((g_I2cKernelAddr[I2cNum] + I2C_TXCOUNT_REG), 0x0);
	I2C_WRITE_REG((g_I2cKernelAddr[I2cNum] + I2C_TXTIDE_REG), 0x1);
    /* send devide address */
    I2C_WRITE_REG((g_I2cKernelAddr[I2cNum] + I2C_TXR_REG), (I2cDevAddr & WRITE_OPERATION));
    I2C_WRITE_REG((g_I2cKernelAddr[I2cNum] + I2C_COM_REB), (I2C_WRITE | I2C_START));

	if (I2C_DRV_WaitFifoWriteEnd(I2cNum))
	{
        //local_irq_restore(IntFlag);
        //HI_ERR_I2C("wait write data timeout!%s, %d\n", __func__, __LINE__);
        return HI_ERR_I2C_WRITE_TIMEOUT;
	...
	{
		/*set txfifo tide*/
		txtide = (TmpLen > 32 )?32:TmpLen;
		I2C_WRITE_REG((g_I2cKernelAddr[I2cNum] + I2C_TXTIDE_REG), txtide);
		while(txtide)
		{
			I2C_WRITE_REG((g_I2cKernelAddr[I2cNum] + I2C_TXR_REG), (*(pData + i)));
			TmpLen--;				
...
int I2C_DRV_FifoRead(HI_U32 I2cNum, HI_U8 I2cDevAddr, HI_BOOL bSendSlave, HI_U32 I2cRegAddr, HI_U32 I2cRegAddrByteNum,
                 HI_U8 *pData, HI_U32 DataLen)
{
    HI_U32 dataTmp = 0xff;
	HI_U32 TmpLen,RxTide;
    HI_U32 i;
	...
	/* clear interrupt flag*/
	I2C_WRITE_REG((g_I2cKernelAddr[I2cNum] + I2C_ICR_REG), I2C_CLEAR_ALL);	 
	I2C_WRITE_REG((g_I2cKernelAddr[I2cNum] + I2C_RXCOUNT_REG), 0x0);
	/* recv data */
...
int  i2c_pm_suspend(PM_BASEDEV_S *pdev, pm_message_t state)
{
    int i;
    int ret;
	HI_FATAL_I2C("i2c_pm_suspend start \n");
...
#if  defined(CHIP_TYPE_hi3716mv310) || defined(CHIP_TYPE_hi3110ev500) || defined (CHIP_TYPE_hi3716mv320)|| defined (CHIP_TYPE_hi3716mv330)
	regI2CStore[0] = I2C_READ_REG(IO_ADDRESS(I2C_CRG_ADDR));
#else
    regI2CStore[0] = I2C_READ_REG(IO_ADDRESS(I2C0_CRG_ADDR));
    regI2CStore[1] = I2C_READ_REG(IO_ADDRESS(I2C1_CRG_ADDR));
    regI2CStore[2] = I2C_READ_REG(IO_ADDRESS(I2C2_CRG_ADDR));
    regI2CStore[3] = I2C_READ_REG(IO_ADDRESS(I2C3_CRG_ADDR));
    regI2CStore[4] = I2C_READ_REG(IO_ADDRESS(QAMI2C_CRG_ADDR));
#endif	
    up(&g_I2cMutex);
    HI_FATAL_I2C("i2c_pm_suspend ok \n");
    return 0;
}
...
#if  defined(CHIP_TYPE_hi3716mv310) || defined(CHIP_TYPE_hi3110ev500) || defined (CHIP_TYPE_hi3716mv320)|| defined (CHIP_TYPE_hi3716mv330)
	I2C_WRITE_REG(IO_ADDRESS(I2C_CRG_ADDR), regI2CStore[0]);
#else

    I2C_WRITE_REG(IO_ADDRESS(I2C0_CRG_ADDR), regI2CStore[0]);
    I2C_WRITE_REG(IO_ADDRESS(I2C1_CRG_ADDR), regI2CStore[1]);
    I2C_WRITE_REG(IO_ADDRESS(I2C2_CRG_ADDR), regI2CStore[2]);
    I2C_WRITE_REG(IO_ADDRESS(I2C3_CRG_ADDR), regI2CStore[3]);
    I2C_WRITE_REG(IO_ADDRESS(QAMI2C_CRG_ADDR), regI2CStore[4]);
#endif
    // 1
    // 2
    for (i = 0; i < HI_STD_I2C_NUM; i++)
...
static HI_VOID HI_DRV_I2C_Open(HI_VOID)
{
    ...
    {
				/*disable all i2c interrupt*/
        I2C_WRITE_REG((g_I2cKernelAddr[i] + I2C_CTRL_REG),0);
        /*  config scl clk rate*/
        I2C_DRV_SetRate(i, I2C_DFT_RATE);

        /*clear all i2c interrupt*/
        I2C_WRITE_REG((g_I2cKernelAddr[i] + I2C_ICR_REG), I2C_CLEAR_ALL);
#ifndef HI_I2C_FIFO_SUPPORT
        /*enable relative interrupt*/
        I2C_WRITE_REG((g_I2cKernelAddr[i] + I2C_CTRL_REG), (I2C_ENABLE | I2C_UNMASK_TOTAL | I2C_UNMASK_ALL));
#else
				I2C_WRITE_REG((g_I2cKernelAddr[i] + I2C_CTRL_REG), (I2C_MODE_CTRL|I2C_ENABLE | I2C_UNMASK_TOTAL | I2C_UNMASK_ALL));	
#endif
    }
...
#else
			Ret = I2C_DRV_FifoWrite(I2cData.I2cNum, I2cData.I2cDevAddr, I2cData.I2cRegAddr, I2cData.I2cRegCount, pData,
                                I2cData.DataLen, HI_TRUE);
...
#else
	    Ret = I2C_DRV_FifoRead(I2cData.I2cNum, I2cData.I2cDevAddr, HI_TRUE, I2cData.I2cRegAddr, I2cData.I2cRegCount,
                               pData, I2cData.DataLen);
#endif
            if (HI_SUCCESS == Ret)
            {
...
HI_S32 HI_DRV_I2C_Init(HI_VOID)
{
    ...
    g_I2cKernelAddr[2] = IO_ADDRESS(ADCI2C_PHY_ADDR);
    g_I2cKernelAddr[3] = IO_ADDRESS(QAMI2C_PHY_ADDR);

	u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C_CRG_ADDR));
    u32RegVal |= (0x1 << 4);
    u32RegVal &=~(0x1 << 5);
    I2C_WRITE_REG(IO_ADDRESS(I2C_CRG_ADDR), u32RegVal);    /*open i2c0 clk, rmove i2c0 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C_CRG_ADDR));
    u32RegVal |= (0x1 << 8);
    u32RegVal &=~(0x1 << 9);
    I2C_WRITE_REG(IO_ADDRESS(I2C_CRG_ADDR), u32RegVal);    /*open i2c1 clk, rmove i2c1 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C_CRG_ADDR));
    u32RegVal |= (0x1 << 12);
    u32RegVal &= ~(0x1 << 13);
    I2C_WRITE_REG(IO_ADDRESS(I2C_CRG_ADDR), u32RegVal);    /*open i2c_adc clk, rmove i2c_adc soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C_CRG_ADDR));
    u32RegVal |= (0x1 << 0);
    u32RegVal &= ~(0x1 << 1);
    I2C_WRITE_REG(IO_ADDRESS(I2C_CRG_ADDR), u32RegVal);    /*open qam_i2c clk, rmove qam_i2c soft reset*/
    ...
    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C0_CRG_ADDR));
    u32RegVal |= (0x1 << 8);
    u32RegVal &= ~0x1;
    I2C_WRITE_REG(IO_ADDRESS(I2C0_CRG_ADDR), u32RegVal);    /*open i2c0 clk, rmove i2c0 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C1_CRG_ADDR));
    u32RegVal |= (0x1 << 8);
    u32RegVal &= ~0x1;
    I2C_WRITE_REG(IO_ADDRESS(I2C1_CRG_ADDR), u32RegVal);    /*open i2c1 clk, rmove i2c1 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C2_CRG_ADDR));
    u32RegVal |= (0x1 << 8);
    u32RegVal &= ~0x1;
    I2C_WRITE_REG(IO_ADDRESS(I2C2_CRG_ADDR), u32RegVal);    /*open i2c2 clk, rmove i2c2 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C3_CRG_ADDR));
    u32RegVal |= (0x1 << 8);
    u32RegVal &= ~0x1;
    I2C_WRITE_REG(IO_ADDRESS(I2C3_CRG_ADDR), u32RegVal);    /*open i2c3 clk, rmove i2c3 soft reset*/

    /* add by q46153 at 20110512, fix QAM's clock  */
    {
        HI_REG_READ(IO_ADDRESS(QAMI2C_CRG_ADDR), u32RegVal);

        HI_DRV_SYS_GetChipVersion( &g_enChipType, &g_u32ChipVersion );
        if (HI_CHIP_TYPE_HI3716M == g_enChipType)
        {
            /* QAM clock: 62.5M */
            u32RegVal = (((((u32RegVal & ~(0x3)) | (0x7 << 8)) | (0x1 << 16)) & ~(0x3 << 17)) | (0x1 << 19));
            HI_REG_WRITE(IO_ADDRESS(QAMI2C_CRG_ADDR), u32RegVal);
        }
...
HI_VOID HI_DRV_I2C_DeInit(HI_VOID)
{
    ...
    I2C_WRITE_REG(IO_ADDRESS(I2C_CRG_ADDR), u32RegVal);    /*open i2c0 clk,  i2c0 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C_CRG_ADDR));
    u32RegVal |= ((0x1 << 8) | (0x1 << 9));
    I2C_WRITE_REG(IO_ADDRESS(I2C_CRG_ADDR), u32RegVal);    /*open i2c1 clk,  i2c1 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C_CRG_ADDR));
    u32RegVal |= ((0x1 << 12) | (0x1 << 13));
    I2C_WRITE_REG(IO_ADDRESS(I2C_CRG_ADDR), u32RegVal);    /*open i2c_adc clk,  i2c_adc soft reset*/


    u32RegVal = I2C_READ_REG(IO_ADDRESS(I2C_CRG_ADDR));
	u32RegVal |= ((0x1 << 0) | (0x1 << 1));                /*open i2c_qam clk,  i2c_qam soft reset*/
    I2C_WRITE_REG(IO_ADDRESS(I2C_CRG_ADDR), u32RegVal);

	u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C_CRG_ADDR));
    u32RegVal &= (~(0x1 << 4));
    I2C_WRITE_REG(IO_ADDRESS(I2C_CRG_ADDR), u32RegVal);    /*close i2c0 clk,  i2c0 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C_CRG_ADDR));
    u32RegVal &= (~(0x1 << 8));
    I2C_WRITE_REG(IO_ADDRESS(I2C_CRG_ADDR), u32RegVal);    /*close i2c1 clk,  i2c1 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C_CRG_ADDR));
    u32RegVal &= (~(0x1 << 12));
    I2C_WRITE_REG(IO_ADDRESS(I2C_CRG_ADDR), u32RegVal);    /*close i2c_adc clk,  i2c_adc soft reset*/
    ...
    I2C_WRITE_REG(IO_ADDRESS(I2C0_CRG_ADDR), u32RegVal);    /*open i2c0 clk,  i2c0 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C1_CRG_ADDR));
    u32RegVal |= ((0x1 << 8) | (0x1 << 0));
    I2C_WRITE_REG(IO_ADDRESS(I2C1_CRG_ADDR), u32RegVal);    /*open i2c1 clk,  i2c1 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C2_CRG_ADDR));
    u32RegVal |= ((0x1 << 8) | (0x1 << 0));
    I2C_WRITE_REG(IO_ADDRESS(I2C2_CRG_ADDR), u32RegVal);    /*open i2c2 clk,  i2c2 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C3_CRG_ADDR));
    u32RegVal |= ((0x1 << 8) | (0x1 << 0));
    I2C_WRITE_REG(IO_ADDRESS(I2C3_CRG_ADDR), u32RegVal);    /*open i2c3 clk,  i2c3 soft reset*/

    u32RegVal = I2C_READ_REG(IO_ADDRESS(QAMI2C_CRG_ADDR));
    ...
    I2C_WRITE_REG(IO_ADDRESS(QAMI2C_CRG_ADDR), u32RegVal);   /* open QAM clk, i2c4 QAM soft reset*/


    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C0_CRG_ADDR));
    u32RegVal &= (~(0x1 << 8));
    I2C_WRITE_REG(IO_ADDRESS(I2C0_CRG_ADDR), u32RegVal);    /*close i2c0 clk,  i2c0 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C1_CRG_ADDR));
    u32RegVal &= (~(0x1 << 8));
    I2C_WRITE_REG(IO_ADDRESS(I2C1_CRG_ADDR), u32RegVal);    /*close i2c1 clk,  i2c1 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C2_CRG_ADDR));
    u32RegVal &= (~(0x1 << 8));
    I2C_WRITE_REG(IO_ADDRESS(I2C2_CRG_ADDR), u32RegVal);    /*close i2c2 clk,  i2c2 soft reset*/

    u32RegVal  = I2C_READ_REG(IO_ADDRESS(I2C3_CRG_ADDR));
    u32RegVal &= (~(0x1 << 8));
    I2C_WRITE_REG(IO_ADDRESS(I2C3_CRG_ADDR), u32RegVal);    /*close i2c3 clk,  i2c3 soft reset*/
    ...
    I2C_WRITE_REG(IO_ADDRESS(QAMI2C_CRG_ADDR), u32RegVal);   /* close QAM clk, i2c4 QAM soft reset*/
#endif
    i2cState = 0;
...
#else
    Ret = I2C_DRV_FifoWrite(I2cNum, I2cDevAddr, I2cRegAddr, I2cRegAddrByteNum, pData, DataLen, HI_TRUE);
#endif
...
#ifdef HI_I2C_FIFO_SUPPORT	
	Ret = I2C_DRV_FifoWrite(I2cNum, I2cDevAddr, I2cRegAddr, I2cRegAddrByteNum, pData, DataLen, HI_FALSE);
#else
...
#ifdef HI_I2C_FIFO_SUPPORT	
	Ret = I2C_DRV_FifoRead(I2cNum, I2cDevAddr, HI_TRUE, I2cRegAddr, I2cRegAddrByteNum, pData, DataLen);
#else
