...
int hi_ssp_set_frameform(unsigned char framemode,unsigned char spo,unsigned char sph,unsigned char datawidth)
{
    ...
    if(framemode > 3)
    {
        //HI_PRINT("set frame parameter err.\n");
        return -1;
    }
    ...
    if((ret & 0x30) == 0)
    {
        if(spo > 1)
        {
            //HI_PRINT("set spo parameter err.\n");
            return -1;
        }
        if(sph > 1)
        {
            //HI_PRINT("set sph parameter err.\n");
            return -1;
        }
    ...
    {
        //HI_PRINT("set datawidth parameter err.\n");
        return -1;
    }
...
int hi_ssp_set_serialclock(unsigned char scr,unsigned char cpsdvsr)
{
    ...
    if((cpsdvsr & 0x1))
    {
        //HI_PRINT("set cpsdvsr parameter err.\n");
        return -1;
    }
...
int hi_ssp_dmac_init(void * prx_dmac_hook,void * ptx_dmac_hook)
{
	...
	if(ssp_dmac_rx_ch < 0)
	{
	    //HI_PRINT("SSP no available rx channel can allocate.\n");
	    return -1;
	}
	ssp_dmac_tx_ch = dmac_channel_allocate(ptx_dmac_hook);
	if(ssp_dmac_tx_ch < 0)
	{
	    //HI_PRINT("SSP no available tx channel can allocate.\n");
