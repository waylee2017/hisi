...
typedef enum 
{
    ...
    HIFB_LAYER_HD_0,
    HIFB_LAYER_HD_1,    
    HIFB_LAYER_AD_0,        /*G2 zone1*/
    HIFB_LAYER_CURSOR,
    HIFB_LAYER_AD_1,        /*G2 zone2*/
    HIFB_LAYER_ID_BUTT
} HIFB_LAYER_ID_E;
...
typedef enum 
{
    ...
    HIFB_ZORDER_MOVEUP,             /**<Move up*/ /**<CNcomment:  向上移动 */
    HIFB_ZORDER_MOVEBOTTOM,         /**<Move to the bottom*/ /**<CNcomment:  移到最底部 */
    HIFB_ZORDER_MOVEDOWN,           /**<Move down*/ /**<CNcomment:  向下移动 */
    HIFB_ZORDER_BUTT
} HIFB_ZORDER_E;
...
/***************************************************************************************
* func			: hifb_rgb
* description	: Obtains the extended 32-bit color value based on color components. 
                  The upper bits are padded with lower bits during color extension.
                  CNcomment: 根据颜色分量，获取其扩展到32位后值。扩展规则:低位补其高位 CNend\n
* param[in] 	: HI_VOID
* retval		: NA
* others:		: NA
***************************************************************************************/
...
/***************************************************************************************
* func			: hifb_color2key
* description	: Converts a colorkey to a 32-bit colorkey based on the pixel format. 
                  A lower-bit color (such as 16 bits) is extended to an upper-bit color 
                  (such as 32 bits) as follows: For each chip, upper bits can be padded
                  with lower bits in multiple mode. The padding mode of the chip must be
                  consistent with the preceding extension mode. Otherwise, the colorkey
                  of the graphics layer is invalid.
                  CNcomment: 根据像素格式信息，将一个颜色值，转换成一个32位的颜色key值,
                             将低色深如16bit扩展为高色深如32bit的策略是:低位补高几位，芯
                             片有多种方式。芯片的方式必须如这个方式一致，否则图层colorkey无效 CNend\n
* param[in] 	: HI_VOID
* retval		: NA
* others:		: NA
***************************************************************************************/
