...
static struct hifmc_host *hifmc100_get_resource(struct platform_device *pdev)
{
	...
	if (!res) {
		pr_err("Can't get resource for FMC regbase address.\n");
		return NULL;
	}
	...
	if (!res) {
		pr_err("Can't get resource for FMC buffer address.\n");
		return NULL;
	}
...
#else
#define hisilicon_hifmc100_match NULL
#endif
...
static struct platform_device hifmc100_pltm_dev =
{
	.name           = DEVNAME,
	.id             = -1,
...
static int __init hifmc100_module_init(void)
{
	...
	result = find_cpu_resource("hifmc100", &hifmc100_pltm_dev.resource,
		&hifmc100_pltm_dev.num_resources);
	if (result)
		return result;

	result = platform_driver_register(&hifmc100_pltm_driver);
	if (result < 0)
		return result;

	result = platform_device_register(&hifmc100_pltm_dev);
	if (result < 0) {
