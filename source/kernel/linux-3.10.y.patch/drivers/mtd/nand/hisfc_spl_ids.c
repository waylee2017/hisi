...
static int check_valid_id(unsigned char *id, unsigned int len)
{
	int ix;
	for (ix = 0; ix < len; ix++)
		if ((id[ix] != 0x00) && (id[ix] != 0xff))
			return 1; /* may be valid id data. */	
	return 0;
}
/*****************************************************************************/
					...
					struct nand_dev_t *nand_dev)
{
	...
	for (spl_dev = nand_spl_dev; spl_dev->length; spl_dev++) {
		...
		if (memcmp(byte, byte + spl_dev->length, spl_dev->length)
		    && check_valid_id(byte + spl_dev->length, 
				      spl_dev->length))
			continue;
