					...
					int *max_bitsflags)
{
	...
	hinfc->ecc.bbm_bitsflap = __bitsflap[31];
}
/*****************************************************************************/
				       ...
				       int *max_bitsflags)
{
	...
	hinfc->ecc.bbm_bitsflap = __bitsflap[15];
}
/*****************************************************************************/
				      ...
				      int *max_bitsflags)
{
	...
	hinfc->ecc.bbm_bitsflap = __bitsflap[7];
}
/*****************************************************************************/
				      ...
				      int *max_bitsflags)
{
	...
	hinfc610_read_bitsflaps_reg(__bitsflap, 0, 4, hinfc_read(hinfc, 0xA0));
	...
	hinfc->ecc.bbm_bitsflap = __bitsflap[3];
}
/*****************************************************************************/
				      ...
				      int *max_bitsflags)
{
	...
	hinfc610_read_bitsflaps_reg(__bitsflap, 0, 2, hinfc_read(hinfc, 0xA0));
	...
	hinfc->ecc.bbm_bitsflap = __bitsflap[1];
}
/*****************************************************************************/
...
void hinfc610_set_oob_mark(struct hinfc_host *host,
			   struct nand_ctrl_info_t *best)
{
	host->bbm = (unsigned char *)(host->buffer + host->pagesize + HINFC610_OOB_OFFSET);

	if (best->layout == &nand_ecc_default)
		host->epm = (unsigned short *)(host->buffer + host->pagesize + 30);
	else if (best->layout == &nand_ecc_2k_13bit)
		host->epm = (unsigned short *)(host->buffer + host->pagesize + 14);
	else {
		pr_err("init empty page mark fail.\n");
		dump_stack();
	}
}
