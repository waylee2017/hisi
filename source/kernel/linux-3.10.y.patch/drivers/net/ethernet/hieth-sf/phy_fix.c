...
static const u32 phy_fix_param_mv330[] = {
0x33f9,0xbd,
0x33fa,0x34,
0x33fb,0x00,
0x33fc,0x39,
...
void hieth_use_default_ate(struct mii_bus *bus, int phyaddr)
{
	unsigned short v;
	int timeout = 3;
...
void hieth_config_festa_phy_ate(struct mii_bus *bus, int phyaddr)
{
	...
	otp_base = (void __iomem *)IO_ADDRESS(REG_BASE_OTP);
	ate_params = readl(otp_base + OTP_FEPHY_ATE);

	printk("Fephy otp value %08x\n", ate_params);

	if (!ate_params) {
		hieth_use_default_ate(bus, phyaddr);
		return;
	}
...
int hisilicon_fephy_fix_mv330(struct phy_device *phy_dev)
{
	int count;

	count = sizeof(phy_fix_param_mv330) / sizeof(phy_fix_param_mv330[0]);
	if (count % 2)
		pr_warn("internal FEPHY fix register count is not right.\n");
	count /= 2;

	phy_expanded_write_bulk(phy_dev, phy_fix_param_mv330, count);
...
//			HISILICON_PHY_MASK, hisilicon_fephy_fix);
	...
	phy_register_fixup_for_uid(HISILICON_PHY_ID_FESTAV271,
			HISILICON_PHY_MASK, hisilicon_fephy_fix_mv330);

	phy_register_fixup_for_uid(HISILICON_PHY_ID_FE_HI3716MV310,
			HISILICON_PHY_MASK, hisilicon_fephy_fix);
