...
extern struct mii_bus *hieth_mdiobus;
...
struct hisf_gpio  hisf_gpio_up={0,0};   /* internal fephy */
struct hisf_gpio  hisf_gpio_down={0,0}; /* external fephy */
...
static void phy_quirk(struct hieth_mdio_local *mdio, int phyaddr)
{
	...
	if (phy_id == HISILICON_PHY_ID_FE_HI3716MV310) {
		unsigned int val;
		/* only for MV310 internal FEPHY. */
		/* after phy cancel reset, need at least 3ms */
		msleep(3);
		val = 0x1E57201E;
		val |= (hisf_phy_addr_up & 0x1F) << 8;
		writel(val, (void __iomem *)IO_ADDRESS(REG_BASE_SF + MDIO_RWCTRL));
		/* need at least 50us */
		msleep(1);
...
static int hieth_plat_driver_probe(struct platform_device *pdev)
{
	...
	/* MV320 support only i internal fe phy. */
	if (_HI3716M_V310 == chipid)
...
static void hieth_phy_scan_fixups(int port)
{
	struct hieth_netdev_local *ld;

	if (hieth_devs_save[port]) {
		ld = netdev_priv(hieth_devs_save[port]);
		phy_scan_fixups(ld->phy);
	}
...
int hieth_plat_driver_resume(struct platform_device *pdev)
{
	...
	hieth_fix_festa_phy_ate(hieth_mdiobus, hisf_phy_addr_up);
