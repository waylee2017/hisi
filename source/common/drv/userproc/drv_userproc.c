...
static int ump_seq_show(struct seq_file *m, void *unused)
{
    ...
    if ((HI_NULL != pstEntry) && (HI_NULL != pstEntry->stInfo.pfnShowFunc))
    {
        ...
        if (pstEntry->stInfo.pReadBuf)
        {
            PROC_PRINT(m, "%s", (HI_CHAR*)pstEntry->stInfo.pReadBuf);

            HI_VFREE(HI_ID_PROC, pstEntry->stInfo.pReadBuf);
            pstEntry->stInfo.pReadBuf = HI_NULL;
        }
    }
    UPROC_K_UNLOCK(g_stUProcParam.stSem);
...
static ssize_t ump_seq_write (struct file *file, const char __user *buf, size_t size, loff_t *pos)
{
    ...
    {   
        ...
        if (pstEntry->stInfo.pWriteBuf)
        {
            if (strlen((HI_CHAR*)pstEntry->stInfo.pWriteBuf))
            {
                HI_DRV_PROC_EchoHelper((HI_CHAR*)pstEntry->stInfo.pWriteBuf);
            }

            HI_VFREE(HI_ID_PROC, pstEntry->stInfo.pWriteBuf);
            pstEntry->stInfo.pWriteBuf = HI_NULL;
        }
    }
...
static HI_VOID PROC_RemoveEntry(UMP_ENTRY_S * pstEntry)
{
    ...
    if (HI_NULL != pstEntry->stInfo.pReadBuf)
    {
        HI_VFREE(HI_ID_PROC, pstEntry->stInfo.pReadBuf);
        pstEntry->stInfo.pReadBuf = HI_NULL;
    }

    if (HI_NULL != pstEntry->stInfo.pWriteBuf)
    {
        HI_VFREE(HI_ID_PROC, pstEntry->stInfo.pWriteBuf);
        pstEntry->stInfo.pWriteBuf = HI_NULL;
    }
				...
				unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        {
            HI_PROC_SHOW_BUFFER_S       stShowBuf;
            HI_DRV_USRMODEPROC_ENTRY_S *pstProcEntry = (HI_DRV_USRMODEPROC_ENTRY_S*)proc->current_cmd.pEntry;

            if (0 == copy_from_user(&stShowBuf, (void __user *)arg, sizeof(stShowBuf)))
            {
                if (stShowBuf.u32Size <= HI_PROC_BUFFER_SIZE)
                {
                    UPROC_K_LOCK(g_stUProcParam.stSem);

                    pstProcEntry->pReadBuf = HI_VMALLOC(HI_ID_PROC, stShowBuf.u32Size);
                    if (pstProcEntry->pReadBuf)
                    {
                        if (copy_from_user(pstProcEntry->pReadBuf, (void __user *)stShowBuf.pu8Buf, stShowBuf.u32Size))
                        {
                            HI_VFREE(HI_ID_PROC, pstProcEntry->pReadBuf);
                            pstProcEntry->pReadBuf = HI_NULL;
                        }
                    }
        ...
        case UMPIOC_WAKE_WRITE_TASK:
        {
            ...
            if (0 == copy_from_user(&stShowBuf, (void __user *)arg, sizeof(stShowBuf)))
            {
                if (stShowBuf.u32Size <= HI_PROC_BUFFER_SIZE)
                {
                    ...
                    pstProcEntry->pWriteBuf = HI_VMALLOC(HI_ID_PROC, stShowBuf.u32Size);
                    if (pstProcEntry->pWriteBuf)
                    {
                        if (copy_from_user(pstProcEntry->pWriteBuf, (void __user *)stShowBuf.pu8Buf, stShowBuf.u32Size))
                        {
                            HI_VFREE(HI_ID_PROC, pstProcEntry->pWriteBuf);
                            pstProcEntry->pWriteBuf = HI_NULL;
                        }
                    }
