...
#define MIN_USER_MODULE_NUMBER  (HI_ID_BUTT+1)
...
{
    int ret;
    ret=HI_DRV_UserCopy(file->f_path.dentry->d_inode, file, cmd, arg, CMPI_Module_Ioctl);
...
HI_S32 MMNGR_DRV_ModInit(HI_U32 u32ModuleCount, HI_U32 u32ModuleMemCount)
{
    ...
    HI_DRV_MODULE_Register(HI_ID_SYS,    "HI_SYS", NULL);
    HI_DRV_MODULE_Register(HI_ID_MODULE, "HI_MODULE", NULL);
    HI_DRV_MODULE_Register(HI_ID_LOG,    "HI_LOG", NULL);
    HI_DRV_MODULE_Register(HI_ID_PROC,   "HI_PROC", NULL);
    HI_DRV_MODULE_Register(HI_ID_EVENT,  "HI_EVENT", NULL);
    HI_DRV_MODULE_Register(HI_ID_MEM,    "HI_MEM", NULL);
    HI_DRV_MODULE_Register(HI_ID_STAT,   "HI_STAT", NULL);
#endif
...
#ifdef HI_MOD_POSTPID
    HI_OSAL_Snprintf(aszModuleName, sizeof(aszModuleName), "%s_%d", pu8ModuleName, current->pid);
#else
    HI_OSAL_Snprintf(aszModuleName, sizeof(aszModuleName), "%s", pu8ModuleName);
#endif
    aszModuleName[MAX_MODULE_NAME+12-1] = 0;
