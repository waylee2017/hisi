...
extern HI_S32 get_partname_by_addr(const char *media_name,  /* hi_sfc, hinand */
			           char *ptn_name, unsigned long long addr);
...
#define CHECK_ADDR_LEN_VALID(Address, Len, LimitLeng) \
    ...
    { \
        ...
        { \
            ...
            (void)pthread_mutex_unlock(&gFlashMutex); \
            return HI_FAILURE; \
        } \
    } while (0)
...
static HI_FLASH_TYPE_E get_flashtype_by_bootargs(HI_CHAR *pszPartitionName)
{
    ...
    if((HI_NULL == pszPartitionName) || (*pszPartitionName == '\0'))
    {
    ...
    pszTmpStr[sizeof(pszTmpStr) - 1] = '\0';
            ...
            || (HI_FLASH_STAT_INSTALL == g_eDevStat[HI_FLASH_TYPE_NAND_0]))
    {
        ...
        if (fp)
        {
            ...
            for(i=0; fgets(line, sizeof(line), fp) != 0; i++)
            {
                ...
                gPartInfo[i].BlockSize = (HI_U32)strtol((const char*)argv[2], (char**)NULL,16); //erase size
                                     ...
                                     HI_U64 u64Len)
{
    ...
    (void)pthread_mutex_lock(&gFlashMutex);

#ifdef HI_EMMC_SUPPORT
    if( HI_FLASH_TYPE_EMMC_0 == enFlashType )
    {
        EMMC_CB_S *pstEmmcCB = emmc_raw_open(u64Address, u64Len);
        if( NULL == pstEmmcCB )
        {
            (void)pthread_mutex_unlock(&gFlashMutex);
            return (HI_HANDLE)INVALID_FD;
                                     ...
                                     HI_CHAR *pPartitionName)
{
    ...
    if (!pPartitionName || *(pPartitionName) == '\0')
    {
    ...
    if( HI_FLASH_TYPE_EMMC_0 == enFlashType)
    {
        ...
        {
#if defined (ANDROID)
	    ptr = pPartitionName + strlen("/dev/block/mmcblk0p");
#else
	    ptr = pPartitionName + strlen("/dev/mmcblk0p");
#endif
	    partnum = (HI_U8)strtol(ptr, NULL, 10);
	    if (!partnum || (partnum > MAX_PARTS)) {
		(void)pthread_mutex_unlock(&gFlashMutex);
		return (HI_HANDLE)INVALID_FD;
	    }
		...
		(HI_VOID)emmc_raw_close(pstEmmcCB);
            ...
            gFlashInfo[hFlash].fd = (HI_S32)pstEmmcCB;
            gFlashInfo[hFlash].FlashType = HI_FLASH_TYPE_EMMC_0;
            gFlashInfo[hFlash].pPartInfo = &gPartInfo[partnum - 1];
            (void)pthread_mutex_unlock(&gFlashMutex);
            return  (HI_HANDLE)hFlash;
        }
        ...
        if(NULL != ptr)
        {
            ...
            strncpy(media_name, "mmcblk0", sizeof(media_name) - 1);
            media_name[sizeof(media_name) - 1] = '\0';
            if ((partnum = find_flash_part(ptr, media_name, pPartitionName, &u64Address, &u64Len)) == 0)
            {
       ...
       || strstr((char*)g_au8Bootargs, "hi_sfc:"))
    {
        for(i=0; i<MAX_PARTS; i++)
        {
            if(!strncmp(gPartInfo[i].DevName, pPartitionName, sizeof(gPartInfo[i].DevName)) && (gPartInfo[i].FlashType == enFlashType)) //eg: "/dev/mtd* "
            {
                break;
            }
            if(!strncmp(gPartInfo[i].PartName, pPartitionName, sizeof(gPartInfo[i].PartName)) && (gPartInfo[i].FlashType == enFlashType))
            {
                break;
            }
        }
        ...
        {
            if(!strncmp(gPartInfo[j].DevName, pPartitionName, sizeof(gPartInfo[j].DevName)) && (gPartInfo[j].FlashType == enFlashType)) //eg: "/dev/mtd* "
            {
                break;
            }
            if(!strncmp(gPartInfo[j].PartName, pPartitionName, sizeof(gPartInfo[j].PartName)) && (gPartInfo[j].FlashType == enFlashType))
            {
                break;
            }
        }
...
HI_HANDLE HI_Flash_OpenByName(HI_CHAR *pPartitionName)
{
       ...
       || strstr((char*)g_au8Bootargs, "hi_sfc:"))
    {
        for(i=0; i<MAX_PARTS; i++)
        {
            if(!strncmp(gPartInfo[i].DevName, pPartitionName, 
			(strlen(pPartitionName) + 1) > sizeof(gPartInfo[i].DevName) 
			? sizeof(gPartInfo[i].DevName) : (strlen(pPartitionName) + 1))) //eg: "/dev/mtd* "
            {
                break;
            }
            if(!strncmp(gPartInfo[i].PartName, pPartitionName, 
			(strlen(pPartitionName) + 1) > sizeof(gPartInfo[i].PartName)
			? sizeof(gPartInfo[i].PartName) : (strlen(pPartitionName) + 1)))
            {
                break;
            }
        }
        ...
        for(j=MAX_PARTS - 1; j>= 0; j--)
        {
            if(!strncmp(gPartInfo[j].DevName, pPartitionName, 
			(strlen(pPartitionName) + 1) > sizeof(gPartInfo[j].DevName) 
			? sizeof(gPartInfo[j].DevName) : (strlen(pPartitionName) + 1))) //eg: "/dev/mtd* "
            {
                break;
            }
            if(!strncmp(gPartInfo[j].PartName, pPartitionName, 
			(strlen(pPartitionName) + 1) > sizeof(gPartInfo[j].PartName)
			? sizeof(gPartInfo[j].PartName) : (strlen(pPartitionName) + 1)))
            {
                break;
            }
        }
...
HI_S32 HI_Flash_Erase(HI_HANDLE hFlash, HI_U64 u64Address, HI_U64 u64Len)
{
    ...
    if( HI_FLASH_TYPE_EMMC_0 == gFlashInfo[hFlash].FlashType)
    {
        ...
        //printf("HI_Flash_Erase -> emc_raw_erase\nu64Address=0x%08llx, u64Len=0x%08llx\n", u64Address, u64Len);
    ...
    ret = (HI_S32)gFlashInfo[hFlash].pFlashopt->raw_erase((unsigned long long)StartAddr, u64Len, gFlashInfo[hFlash].OpenAddr, LimitLeng);
    HI_INFO_FLASH("end.\n");
    (void)pthread_mutex_unlock(&gFlashMutex);
...
HI_S64 HI_Flash_Erase64(HI_HANDLE hFlash, HI_U64 u64Address, HI_U64 u64Len)
{
    ...
    ret = (HI_S64)gFlashInfo[hFlash].pFlashopt->raw_erase((unsigned long long)StartAddr, u64Len, gFlashInfo[hFlash].OpenAddr, LimitLeng);
    HI_INFO_FLASH("end.\n");
    (void)pthread_mutex_unlock(&gFlashMutex);
...
HI_S32 HI_Flash_Read(HI_HANDLE hFlash, HI_U64 u64Address, HI_U8 *pBuf, HI_U32 u32Len, HI_U32 u32Flags)
{
    ...
    ret = gFlashInfo[hFlash].pFlashopt->raw_read((unsigned long long *)&StartAddr, pBuf, u32Len, gFlashInfo[hFlash].OpenAddr, LimitLeng, wtihoob, 1);
...
HI_S32 HI_Flash_Write(HI_HANDLE hFlash, HI_U64 u64Address, HI_U8 *pBuf, HI_U32 u32Len, HI_U32 u32Flags)
{
    ...
    if (erase)
    {
        ...
        ret = gFlashInfo[hFlash].pFlashopt->raw_erase((unsigned long long)StartAddr, (unsigned long long)u32EraseLen, gFlashInfo[hFlash].OpenAddr, LimitLeng);
        if (0 >= ret)
        {
    ...
    ret = gFlashInfo[hFlash].pFlashopt->raw_write((unsigned long long *)&StartAddr, pBuf, u32Len, gFlashInfo[hFlash].OpenAddr, LimitLeng, wtihoob);
...
HI_S32 HI_Flash_GetInfo(HI_HANDLE hFlash, HI_Flash_InterInfo_S *pFlashInfo)
{
	...
	if (pFlashInfo->pPartInfo) {
		memset(pFlashInfo->pPartInfo->PartName, 0, sizeof(pFlashInfo->pPartInfo->PartName));
		ret = get_partname_by_addr("mmcblk0", pFlashInfo->pPartInfo->PartName, pFlashInfo->OpenAddr);
		if(ret != HI_SUCCESS)
		{
			HI_ERR_FLASH("get partname by addr(0x%x) failed\n", pFlashInfo->OpenAddr);
			return HI_FAILURE;
		}
	}
