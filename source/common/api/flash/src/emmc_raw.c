...
#if defined (ANDROID)
    if(strncmp((const char *)pu8Node, "/dev/block/mmcblk0p", strlen("/dev/block/mmcblk0p")))
#else
    if(strncmp((const char *)pu8Node, "/dev/mmcblk0p", strlen("/dev/mmcblk0p")))
#endif
    {
        return NULL;
    }
...
#if defined (ANDROID)
    ptr = (char *)pu8Node + strlen("/dev/block/mmcblk0p");
#else
    ptr = (char *)pu8Node + strlen("/dev/mmcblk0p");
#endif
