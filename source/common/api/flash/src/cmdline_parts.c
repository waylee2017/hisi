...
/* 0 - find, -1 - no find */
HI_S32 get_partname_by_addr(const char *media_name,  /* hi_sfc, hinand */
			    char *ptn_name, unsigned long long addr)
{
	...
	if (!got || !tmp_cmdline_parts) {
		HI_PRINT("%s not found\n", media_name);
		return -1;
	}

	tmp_subpart = cmdline_parts_head->subpart;
	for(;tmp_subpart;)
	{
		if((addr >= tmp_subpart->from) && (addr <= (tmp_subpart->from + tmp_subpart->size)))
		{
			memcpy(ptn_name, tmp_subpart->name, sizeof(tmp_subpart->name));
			return 0;
		}
