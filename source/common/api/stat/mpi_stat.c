...
HI_S32 HI_MPI_STAT_DeInit(HI_VOID)
{
    ...
    if(s_s32StatFd != -1)
    {
        ...
        {
            HI_ERR_STAT("close s_s32StatFd failed\n");
            HI_STAT_UNLOCK();
            return HI_FAILURE;
        }
        ...
        if (g_Timer7_addr)
        {
            s32Ret = HI_MUNMAP((void *)g_Timer7_addr);
            if (HI_FAILURE == s32Ret)
            {
                HI_ERR_STAT("HI_MUNMAP failed\n");
                HI_STAT_UNLOCK();
                return HI_FAILURE;
            }
            g_Timer7_addr = NULL;
...
HI_S32 HI_MPI_STAT_ThreadUnregister(HI_STAT_HANDLE * pHandle)
{
    ...
    {
        s32Ret = HI_MUNMAP((void *)(*pHandle)->stat_thread_uvirtaddr);
        if ( HI_FAILURE == s32Ret)
        {
            HI_ERR_STAT("HI_MUNMAP failed\n");
            return HI_FAILURE;
