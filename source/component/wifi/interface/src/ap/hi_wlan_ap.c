                           ...
                           HI_WLAN_AP_CONFIG_S *pstApCfg)
{
    ...
    if (device_id == WIFI_RALINK_RT5572)
        sprintf(mBuf, "WirelessMode=5");
    else
        sprintf(mBuf, "WirelessMode=9");
    wrq.u.data.length = strlen(mBuf) + 1;
    if(ioctl(s, cmd, &wrq) < 0)
        return HI_FAILURE;

    /* configure Channel */
    sprintf(mBuf, "Channel=%d", pstApCfg->channel);
    wrq.u.data.length = strlen(mBuf) + 1;
    if(ioctl(s, cmd, &wrq) < 0)
        return HI_FAILURE;
    ...
    if (pstApCfg->security == HI_WLAN_SECURITY_WEP) {
        sprintf(mBuf, "DefaultKeyID=1");
        wrq.u.data.length = strlen(mBuf) + 1;
        if(ioctl(s, cmd, &wrq) < 0)
            return HI_FAILURE;

        sprintf(mBuf, "Key1=%s", pstApCfg->psswd);
        wrq.u.data.length = strlen(mBuf) + 1;
        if(ioctl(s, cmd, &wrq) < 0)
            return HI_FAILURE;
    }
    ...
    /* configure hide SSID */
    sprintf(mBuf, "HideSSID=%d", pstApCfg->hidden_ssid ? 1 : 0);
    wrq.u.data.length = strlen(mBuf) + 1;
    if(ioctl(s, cmd, &wrq) < 0)
        return HI_FAILURE;

    /* configure SSID */
    sprintf(mBuf, "SSID=%s", pstApCfg->ssid);
    wrq.u.data.length = strlen(mBuf) + 1;
    if(ioctl(s, cmd, &wrq) < 0)
        return HI_FAILURE;
    ...
    if (pstApCfg->security == HI_WLAN_SECURITY_WPA_WPA2_PSK) {
        sprintf(mBuf, "WPAPSK=%s", pstApCfg->psswd);
        wrq.u.data.length = strlen(mBuf) + 1;
        if(ioctl(s, cmd, &wrq) < 0)
            return HI_FAILURE;
...
static HI_S32 ap_start_hostapd(const HI_CHAR *config_file, const HI_CHAR *entropy_file)
{
    ...
    sprintf(param[0], "%s", config_file);
    sprintf(param[1], "-e%s", entropy_file);
    spawn_args[1] = "-B";
            ...
            HI_WLAN_AP_CONFIG_S *pstApCfg, const HI_CHAR *config_file)
{
     ...
     */
    if (device_id == WIFI_BROADCOM_BCM43236 || device_id == WIFI_RALINK_MT7601U)
        memset(ht40, 0, sizeof(ht40));
...
HI_S32 HI_WLAN_AP_Open(HI_CHAR *ifname)
{
    ...
    for (count = 0; count < 50; count++) {
        if (device_id == WIFI_RALINK_RT3070 || device_id == WIFI_RALINK_RT5370
          ...
          || device_id == WIFI_RALINK_MT7601U || device_id == WIFI_ATHEROS_AR9271
          || device_id == WIFI_BROADCOM_BCM43236)
...
{    
    ...
    if(ioctl(s, SIOCGIFHWADDR, &ifr) >= 0) {
        sprintf(pstMac, MACSTR, MAC2STR(ifr.ifr_hwaddr.sa_data));
        ret = HI_SUCCESS;
    }
