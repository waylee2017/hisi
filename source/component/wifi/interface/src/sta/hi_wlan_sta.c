                     ...
                     HI_WLAN_STA_ACCESSPOINT_S *pstApList, HI_U32 *pstApNum)
{
    ...
    while (size = wlan_util_read_line(pos, line)) {
        pos += size;
...
HI_S32 HI_WLAN_STA_Connect(HI_CHAR *ifname, HI_WLAN_STA_CONFIG_S *pstStaCfg)
{
    ...
    sprintf(cmd, "SET_NETWORK %d ssid P\"%s\"", netId, pstStaCfg->ssid);
#else
    sprintf(cmd, "SET_NETWORK %d ssid \"%s\"", netId, pstStaCfg->ssid);
#endif
    if (sta_wpa_request(cmd, NULL, 0) != HI_SUCCESS)
        return HI_WLAN_SEND_COMMAND_FAIL;

    if (pstStaCfg->bssid[0] != '\0') {
        sprintf(cmd, "SET_NETWORK %d bssid %s", netId, pstStaCfg->bssid);
        if (sta_wpa_request(cmd, NULL, 0) != HI_SUCCESS)
            return HI_WLAN_SEND_COMMAND_FAIL;
    }
    ...
    if (pstStaCfg->security == HI_WLAN_SECURITY_WEP) {
        ...
        if(pwd_len == 5 || pwd_len == 13)
            sprintf(cmd, "SET_NETWORK %d wep_key0 \"%s\"", netId, pstStaCfg->psswd);
    ...
    if (pstStaCfg->security == HI_WLAN_SECURITY_WPA_WPA2_PSK) {
        sprintf(cmd, "SET_NETWORK %d proto %s", netId, "WPA RSN");
        if (sta_wpa_request(cmd, NULL, 0) != HI_SUCCESS)
            return HI_WLAN_SEND_COMMAND_FAIL;
        sprintf(cmd, "SET_NETWORK %d key_mgmt %s", netId, "WPA-PSK");
        if (sta_wpa_request(cmd, NULL, 0) != HI_SUCCESS)
            return HI_WLAN_SEND_COMMAND_FAIL;
        sprintf(cmd, "SET_NETWORK %d pairwise %s", netId, "CCMP TKIP");
        if (sta_wpa_request(cmd, NULL, 0) != HI_SUCCESS)
            return HI_WLAN_SEND_COMMAND_FAIL;
        sprintf(cmd, "SET_NETWORK %d group %s", netId, "CCMP TKIP");
        if (sta_wpa_request(cmd, NULL, 0) != HI_SUCCESS)
            return HI_WLAN_SEND_COMMAND_FAIL;
        if (strlen(pstStaCfg->psswd) == 64)
            /* password is hexadecimal */
            sprintf(cmd, "SET_NETWORK %d psk %s", netId, pstStaCfg->psswd);
        else
            sprintf(cmd, "SET_NETWORK %d psk \"%s\"", netId, pstStaCfg->psswd);
        if (sta_wpa_request(cmd, NULL, 0) != HI_SUCCESS)
            return HI_WLAN_SEND_COMMAND_FAIL;
                              ...
                              HI_WLAN_STA_CONN_STATUS_S *pstConnStatus)
{
    ...
    if (strncmp(pos, "COMPLETED", 9) == 0) {
        ...
        while (len = wlan_util_read_line(pos, line)) {
