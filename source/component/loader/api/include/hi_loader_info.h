...
/**Controls whether to enable the embedded serial number*/
/**CNcomment:内置序列号是否开启的开关*/
...
#define HI_SYSCFG_STBID_LEN 26 /**< Length of STB external serial number *//**<CNcomment: 机顶盒外部序列号长度 */
#define HI_SYSCFG_STBSN_LEN 15 /**< Length of STB internal serial number *//**<CNcomment: 机顶盒内部序列号长度 */
...
typedef struct hiLOADER_STBINFO_S
{
    ...
    HI_U32 u32HWBatch; /**< Hardware batch *//**<CNcomment: 硬件批次 */
    HI_U8  au8OutSn[HI_SYSCFG_STBID_LEN]; /**< STB external serial number */  /**<CNcomment: 机顶盒外部序列号 */
#ifdef LOADER_INNER_STB_SN
    HI_U8 au8Sn[HI_SYSCFG_STBSN_LEN]; /**< STB internal serial number *//**<CNcomment: 机顶盒内部序列号 */
#endif
...
A serial number contains the following information:\n
          External serial number, such as  15010300101010001c1d000021      \n
          Internal serial number (optional). \n
          External serial number is in the following format:\n
          AAAA BB CCC DD EE X FFFF FFFF FFFF\n
          AAAA 	Four-digit region code \n 	\n
          BB 	Terminal product vendor authentication number\n
          CCC 	Terminal product STB manufacture batch\n
          DD 	Terminal type\n
          EE 	Hardware version\n
          X 	Identification code\n
          FFFFFFFFFFFF 	Number defined based on the previous digit. 	MAC address .\n
...
When this API is called, the last 12 digits of a serial number are set to the MAC address automatically. \n
You can change the serial number rules as required. \n
This API responds to the au8OutSn(STB  external serial number), \n
and au8Sn parameters. If the STB internal serial number is defined by the LOADER_INNER_STB_SN macro.)
CNcomment:序列号含有升级时需要比较的硬件版本号,厂商编号。\n CNend
CNcomment:不同的运营商和厂家有不同的规格机顶盒的序列号，\n CNend
CNcomment:用户可根据实际需要在修改序列号规则。\n CNend
CNcomment:该接口只理会au8OutSn(机顶盒外部序列号)参数和\n CNend
CNcomment:au8Sn参数(如果机顶盒内部序列号LOADER_INNER_STB_SN宏定义的话)。\n CNend
A serial number contains the following information:\n
          External serial number, such as  15010300101010001c1d000021      \n
          Internal serial number (optional). \n
          External serial number is in the following format:\n
