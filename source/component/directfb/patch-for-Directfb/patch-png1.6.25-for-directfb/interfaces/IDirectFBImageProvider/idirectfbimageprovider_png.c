...
{
#ifdef LIBPNG14
     if ((int)!png_sig_cmp(ctx->header, (png_size_t)0, 8))
#else
...
void /* PRIVATE */
IDirectFb_png_read_data(png_structp png_ptr, png_bytep data, png_size_t length)
{
    DFBResult            ret;
    IDirectFBImageProvider_PNG_data *pngdata;
    IDirectFBDataBuffer *buffer;
    unsigned int  len;
    unsigned int remainLen = length;

    pngdata = (IDirectFBImageProvider_PNG_data *)png_get_io_ptr(png_ptr);
    ...
    while (remainLen)
    {
        if (buffer->HasData( buffer ) == DFB_OK) 
        {
            ...
            ret = buffer->GetData( buffer, remainLen, data, &len );
            if (ret)
                png_error(png_ptr, "GetData Failed!\n");
        ...
        if (remainLen)
        {
            if (buffer->WaitForData( buffer, remainLen ) == DFB_EOF)
                png_error(png_ptr, "WaitForData Failed!\n");
        }
    }
           ...
           ... )
{
     ...
     png_set_progressive_read_fn( data->png_ptr, data,
                                  ...
                                  png_end_callback );
#else
    png_set_read_fn(data->png_ptr, data, IDirectFb_png_read_data);
#endif
...
#else
    data = png_get_io_ptr(png_read_ptr);
#endif
	  ...
	  png_get_tRNS(data->png_ptr, data->info_ptr, &trans, &num_trans, &trans_color);
          ...
          if (data->color_type == PNG_COLOR_TYPE_PALETTE) {
	       ...
	       num_colors = MIN( MAXCOLORMAPSIZE, num_palette );
               ...
               png_color_16p trans = trans_color;
     ...
     switch (data->color_type) {
          ...
          default:
               ...
               if (!(data->color_type & PNG_COLOR_MASK_ALPHA))
#ifdef LIBPNG14
                    png_set_add_alpha(data->png_ptr, 0xFF, PNG_FILLER_AFTER);
#else
                  ...
                  int         pass_num )
{
     ...
     data = png_get_progressive_ptr( png_read_ptr );
#else
    data = (IDirectFBImageProvider_PNG_data *)png_get_io_ptr(png_read_ptr);
#endif
...
#else
    png_bytep image[data->height];
    unsigned int i;
    for (i = 0; i < data->height; i++)
    {
        image[i] = (png_bytep) (data->image + i * data->pitch);
    }
    png_read_image(data->png_ptr, image);
    data->rows = data->height;

    if (data->render_callback) 
    {
          DIRenderCallbackResult r;
          DFBRectangle rect = { 0, 0, data->width, data->height };
                       ...
                       int                              buffer_size)
{
    ...
    if (stage == STAGE_INFO)
    {
        ...
        png_info_callback(data->png_ptr, data->info_ptr);
    }
    else if (stage == STAGE_END)
    {
        png_row_callback(data->png_ptr, NULL, 0, 0);
        png_end_callback(data->png_ptr, data->info_ptr);
    }
