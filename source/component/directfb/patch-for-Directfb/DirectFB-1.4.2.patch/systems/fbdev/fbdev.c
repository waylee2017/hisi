...
static DFBResult dfb_fbdev_test_mode_simple( const VideoMode  *mode, FBDevShared   *shared);
...
static DFBResult dfb_fbdev_open( void )
{
     ...
     if (dfb_config->fb_device) {
          ...
          dfb_fbdev->fd = direct_try_open( "/dev/fb2", "/dev/fb/2", O_RDWR, true );//modiry by w66592 : hd layer is fb2
          if (dfb_fbdev->fd < 0) {
...
system_get_accelerator( void )
{
     ...
     if (!strncmp( dfb_fbdev->shared->fix.id, "MATROX DH",9 ))
          return FB_ACCEL_MATROX_MGAG400;
#endif
#ifdef FB_ACCEL_EP9X
     if (!strncmp( dfb_fbdev->shared->fix.id, "ep9xfb",6))
...
init_modes( void )
{
     ...
     if (!dfb_fbdev->shared->modes) {
          ...
          *dfb_fbdev->shared->modes = dfb_fbdev->shared->current_mode;
          ...
          if (dfb_fbdev_test_mode_simple(dfb_fbdev->shared->modes,dfb_fbdev->shared)) {
                  ...
                  void       *screen_data )
{
     ...
     //add by w66592, wait for VBLANK, attention : this interface is support by hisilicon fb       
                      ...
                      int        *ret_height )
{
     ...
     if (FBDEV_IOCTL( FBIOGET_SCREENSIZE, &screensize ) < 0)
	 {
         D_PERROR( "DirectFB/FBDev: Could not get screen size!\n" );
         return errno2result(errno);
     }
     *ret_width  = screensize.u32Width;
     *ret_height = screensize.u32Height;
...
static DFBResult calpixelformat(int bpp,struct fb_var_screeninfo *var)
{
	switch (bpp) 
	{
		case 12:
                  ...
                  DFBColorAdjustment         *adjustment )
{
     ...
     description->caps = DLCAPS_SURFACE    | DLCAPS_CONTRAST |
                         ...
                         DLCAPS_OPACITY|DLCAPS_SCREEN_SIZE;
     ...
     if (dfb_config->mode.format)
          ...
          config->pixelformat = calpixelformat(default_mode->bpp ,&(dfb_fbdev->shared->current_var));//dfb_pixelformat_for_depth( default_mode->bpp );
...
#if 0
     if (config->options) //tobe modiry for add options
          fail |= CLRCF_OPTIONS;
#endif
...
static DFBResult dfb_fbdev_set_opacity(u8 opacity)
{
    HIFB_ALPHA_S Alpha;
    ...
    Alpha.u8Alpha1 = 255;
     ...
     if (FBDEV_IOCTL( FBIOPUT_ALPHA_HIFB, &Alpha ) < 0) {
...
static DFBResult dfb_fbdev_setscreensize(DFBRectangle *pDstRectangle)
{
    ...
    FbLayerInfo.u32ScreenWidth = pDstRectangle->w; //attentio use HD 
    FbLayerInfo.u32ScreenHeight = pDstRectangle->h;

    FbLayerInfo.u32Mask = HIFB_LAYERMASK_SCREENSIZE;

     if (FBDEV_IOCTL( FBIOPUT_LAYER_INFO, &FbLayerInfo ) < 0) {
         D_PERROR( "DirectFB/FBDev: Could not set layer info!\n" );
         return errno2result(errno);
     }
#endif 
...
static DFBResult dfb_fbdev_setdeflicker()
{
    //attention anflicker is not support by DFB
#if 0
    HIFB_LAYER_INFO_S FbLayerInfo;

    
    FbLayerInfo.eAntiflickerLevel = HIFB_LAYER_ANTIFLICKER_AUTO;
    FbLayerInfo.u32Mask = HIFB_LAYERMASK_ANTIFLICKER_MODE;
    if (FBDEV_IOCTL( FBIOPUT_LAYER_INFO, &FbLayerInfo ) < 0) {
         D_PERROR( "DirectFB/FBDev: Could not set the alpha!\n" );
         return errno2result(errno);
    }
...
static DFBResult dfb_fbdev_set_srckey(DFBColorKey *pKey, BOOL bEnable)
{
    ...
    struColorKey.u32Key = (pKey->r << 16) | (pKey->g << 8) | (pKey->b);
     ...
     if (FBDEV_IOCTL( FBIOPUT_COLORKEY_HIFB, &struColorKey ) < 0) {
                  ...
                  CoreSurfaceBufferLock      *lock )
{
     ...
     if (updated & CLRCF_SOURCE||updated & CLRCF_FORMAT ) {
#if 0     
    ...
    if (updated & CLRCF_OPACITY)
        dfb_fbdev_set_opacity(config->opacity);

    if (updated & CLRCF_SRCKEY)//attention there is no colorkey
        dfb_fbdev_set_srckey(&(config->src_key), HI_TRUE);

    if (updated & CLRCF_DEST)
        dfb_fbdev_setscreensize(&(config->dest));
...
#if  0//support refresh
    HIFB_BUFFER_S UpdateCavs;
    
    memset(&UpdateCavs, 0x0, sizeof(UpdateCavs));
    UpdateCavs.UpdateRect.x = update->x1;
    UpdateCavs.UpdateRect.y = update->y1;
    UpdateCavs.UpdateRect.w = update->x2 - update->x1 + 1;
    UpdateCavs.UpdateRect.h = update->y2 - update->y1 + 1;

    //printf("===>>%s, %d, urect:(%d,%d,%d,%d)\n", __FUNCTION__, __LINE__, 
        //UpdateCavs.UpdateRect.x, UpdateCavs.UpdateRect.y, UpdateCavs.UpdateRect.w, UpdateCavs.UpdateRect.h);

    
    ret = ioctl(dfb_fbdev->fd, FBIO_REFRESH, &UpdateCavs);
    /*if (ret < 0)
    {
          D_BUG( "primaryUpdateRegion fail" );
    }*/
    
#else// not supprot refresh

     struct fb_var_screeninfo *var;
     FBDevShared  *shared = dfb_fbdev->shared;
     var = &shared->current_var;
     
    dfb_fbdev_pan( var->xoffset, var->yoffset, 0);
...
static DFBResult
dfb_fbdev_test_mode_simple( const VideoMode *mode, FBDevShared   *shared )
{
	DFBResult                ret;
	struct fb_var_screeninfo var;
	...
	struct fb_var_screeninfo strvar;
	if(shared)
	{
		strvar= shared->current_var;
	}
	...
	ret = dfb_fbdev_mode_to_var( mode, calpixelformat(mode->bpp, &strvar), mode->xres, mode->yres,
...
dfb_fbdev_read_modes( void )
{
     ...
     while (fgets(line,79,fp)) {
          if (sscanf(line, "mode \"%31[^\"]\"",label) == 1) {
               ...
               if (geometry && timings && !dfb_fbdev_test_mode_simple(&temp_mode, NULL)) {
