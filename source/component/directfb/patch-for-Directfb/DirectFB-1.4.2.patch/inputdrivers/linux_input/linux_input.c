...
udev_hotplug_EventThread(DirectThread *thread, void * hotplug_data)
{
     ...
     const int buffersize = MAX_LENGTH_OF_EVENT_STRING;  
     ...
     setsockopt(socket_fd, SOL_SOCKET, SO_RCVBUFFORCE, &buffersize, sizeof(buffersize));  
     rt = bind(socket_fd, (struct sockaddr *) &snl, sizeof(struct sockaddr_nl));  
     if (rt < 0)
     {  
         close(socket_fd);  
         goto errorExit;
