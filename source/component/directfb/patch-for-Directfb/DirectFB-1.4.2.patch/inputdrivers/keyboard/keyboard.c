...
static void* keyboardEventThread( DirectThread *thread, void *driver_data )
{
	...
	{
		DFBInputEvent stEvt;
		s32Ret =  HI_UNF_KEY_GetValue(&penPressStatus, &pu32KeyId);
		if(s32Ret)
		{
			 usleep(50000);
			 continue;
		...
		for(i = 0; i < HI_KEY_NUM; i++)
		{
			if(g_KeyBoardMap[i].s32Code == pu32KeyId )
			{
                    ...
                    InputDeviceInfo  *info, void  **driver_data )
{
    ...
    HI_UNF_KEYLED_TYPE_E enKeyLedType = HI_UNF_KEYLED_TYPE_BUTT;

    #ifdef CFG_HI_KEYLED_74HC164_SUPPORT
    enKeyLedType = HI_UNF_KEYLED_TYPE_STD;
	#endif

	#ifdef CFG_HI_KEYLED_CT1642_SUPPORT
    enKeyLedType = HI_UNF_KEYLED_TYPE_CT1642;
	#endif

	#ifdef CFG_HI_KEYLED_PT6961_SUPPORT
    enKeyLedType = HI_UNF_KEYLED_TYPE_PT6961;
	#endif

	#ifdef CFG_HI_KEYLED_PT6964_SUPPORT
    enKeyLedType = HI_UNF_KEYLED_TYPE_PT6964;
	#endif
	...
	nRet = HI_UNF_KEYLED_SelectType(enKeyLedType);
    if (nRet)
    {
