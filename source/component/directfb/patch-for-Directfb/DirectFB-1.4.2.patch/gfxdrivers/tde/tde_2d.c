...
inline void CONVERT_RECT_TO_REGION (TDE2_RECT_S *pRect, DFBRegion *pRegion)
{
...
inline void  CONVERT_REGION_TO_RECT (DFBRegion *pRegion, TDE2_RECT_S *pRect) \
{
...
unsigned long convert2ARGBcolor(DFBSurfacePixelFormat format, unsigned long src_color)
{
    switch(format) 
    { 
        case DSPF_ARGB1555: 
...
HI_S32 ADP_ColorExpand(TDE2_COLOR_FMT_E PFmt, unsigned long Color, HI_U32 *pFillData)
{
    switch (PFmt)
    {
...
bool      TDEBlit         ( void *drv, void *dev, DFBRectangle *srect, int dx, int dy )
{
	...
	TDE2_RECT_S DstIntersectRect = {dx, dy, (HI_U32)srect->w, (HI_U32)srect->h};
	TDE2_OPT_S stOpt;
	TDE2_OPT_S tmpstOpt;
	...
	if(tde_check_rect(&DstIntersectRect, &pDev->dst_surface) < 0)
	{
		return HI_FALSE;
	}
	
	SrcRect.s32Xpos += (DstIntersectRect.s32Xpos - DstRect.s32Xpos);
	SrcRect.s32Ypos += (DstIntersectRect.s32Ypos - DstRect.s32Ypos);
	SrcRect.u32Width = DstIntersectRect.u32Width;
	SrcRect.u32Height = DstIntersectRect.u32Height;
	
	DstRect = DstIntersectRect;
...
bool      TDEStretchBlit  ( void *drv, void *dev, DFBRectangle *srect, DFBRectangle  *drect )
{
	...
	TDE2_RECT_S DstIntersectRect = {drect->x, drect->y, (HI_U32)drect->w, (HI_U32)drect->h};
	...
	SrcRect.s32Xpos += (DstIntersectRect.s32Xpos - DstRect.s32Xpos) * SrcRect.u32Width / DstRect.u32Width;
	SrcRect.s32Ypos += (DstIntersectRect.s32Ypos - DstRect.s32Ypos) * SrcRect.u32Height / DstRect.u32Height;
	SrcRect.u32Width = DstIntersectRect.u32Width * SrcRect.u32Width / DstRect.u32Width;
	SrcRect.u32Height = DstIntersectRect.u32Height * SrcRect.u32Height / DstRect.u32Height;
