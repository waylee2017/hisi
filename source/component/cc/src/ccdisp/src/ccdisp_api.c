...
HI_S32 CCDISP_Screen_Clear(void)
{
    ...
    if (pstScreen->stOpt.pfnDisplay)
    {
        ...
        (HI_VOID)pstScreen->stOpt.pfnDisplay(pstScreen->stOpt.u32UserData, &stDispParam);
                               ...
                               HI_S32 s32Width, HI_S32 s32Height, HI_S32 s32SrcX, HI_S32 s32SrcY)
{
    ...
    {
        (HI_VOID)pstScreen->stOpt.pfnBlit(pstScreen->stOpt.u32UserData, &stSrcRect, &stDstRect);
    }
                            ...
                            HI_U16 u16StartY, HI_U16 u16EndY, HI_U16 u16Offset, CCDISP_ROLLSPEED_E enSpeed)
{
    ...
    while(s32RollupOffset > 0)
    {
        ...
        if(pstScreen->stOpt.pfnBlit)
        {
            s32Ret = pstScreen->stOpt.pfnBlit(pstScreen->stOpt.u32UserData, &stSrcRect, &stDstRect);
        }
        ...
        if(pstScreen->stOpt.pfnDisplay)
        {
            ...
            CCDISP_MUTEX_LOCK(&drawMutex);
            s32Ret |= pstScreen->stOpt.pfnDisplay(pstScreen->stOpt.u32UserData, &stDispParam);
            CCDISP_MUTEX_UNLOCK(&drawMutex);
        }
        //u16SrcY = u16DstY;
    }
                           ...
                           HI_S32 *ps32Width, HI_S32 *ps32Height)
{
    ...
    {
        s32Ret = pstScreen->stOpt.pfnGetTextSize(pstScreen->stOpt.u32UserData, pu16Str, s32Length, &s32Width, &s32Height);
    }
    if (ps32Width)
    {
        *ps32Width = s32Width;
                        ...
                        HI_U16* pu16Str, HI_S32 s32StrLen, HI_S32* ps32DrawWidth, HI_U32 flag)
{
    ...
    if(pstScreen->stOpt.pfnGetTextSize && pstScreen->stOpt.pfnDisplay)
    {
        ...
        do
        {
            s32Ret = pstScreen->stOpt.pfnGetTextSize(pstScreen->stOpt.u32UserData, pu16Str, s32StrLen, &s32Width, &s32Height);
...
HI_S32 _DtvCC_CharFlash(CCDISP_FlashText_S * text,HI_U32 flag)
{
    ...
    {   
         ...
         stDispParam.enOpt = HI_UNF_CC_OPT_FILLRECT;
         CCDISP_MUTEX_LOCK(&drawMutex);
         pstScreen->stOpt.pfnDisplay(pstScreen->stOpt.u32UserData, &stDispParam);
         CCDISP_MUTEX_UNLOCK(&drawMutex);
         return HI_SUCCESS;
    }
    else
...
HI_S32 CCDISP_Bitmap_Draw(CCDISP_WINDOW_ID_E enWinID, HI_UNF_CC_RECT_S stRect, HI_UNF_CC_BITMAP_S *pstBitmap)
{
    ...
    if (pstScreen->stOpt.pfnDisplay)
    {
        ...
        (HI_VOID)pstScreen->stOpt.pfnDisplay(pstScreen->stOpt.u32UserData, &stCCdispalyParm);
    }
...
HI_S32 CCDISP_SendVbiData(HI_UNF_CC_VBI_DADA_S *pstVbiDataField1,HI_UNF_CC_VBI_DADA_S *pstVbiDataField2)
{
    ...
    {
        (HI_VOID)pstScreen->stOpt.pfnVBIOutput(pstScreen->stOpt.u32UserData, pstVbiDataField1,pstVbiDataField2);
    }
...
HI_S32 CCDISP_GetCurPts(HI_S64 *ps64CurPts)
{
    ...
    {
       (HI_VOID)pstScreen->stOpt.pfnGetPts(pstScreen->stOpt.u32UserData, ps64CurPts);
    }
    return HI_SUCCESS;
}
