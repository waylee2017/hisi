    ...
    if( pstObject->enCurMode == CC608_MODE_CAPTION )
    {
        ...
        else
        {
            ...
            if( ret_val == 2 && pstObject->enDispStyle == CC608_DISP_PAINTON)
            {
                ...
                {
                    CC608_DEC_MidRow(module_id,0x11,ccdata&0xff);
                }
            }
...
static void CC608_TimeoutErase_Callback(void *pParam)
{
    HI_U32 ucModuleID = (HI_U32)pParam;
...
HI_S32 CC608_TimeoutErase_TimerInit(HI_U8 module_id)
{
    ...
    (HI_VOID)CCTimer_Open(CCTIMER_ID_608ERASE, CC608_TimeoutErase_Callback, (HI_VOID *)u32ModuleID);
...
static inline HI_S32 _CC608_DATA_StoreCaptionToBuffer(HI_U8 module_id,HI_U16 *caption,HI_U16 len,CC608_BUFFER_TYPE_E buf_type)
{
    ...
    CC608_BUFFER_S *pstBuffer;
    HI_U8 i,row,column;
    HI_U8 u8FirstRow_HaveChar = 0,u8Rows_HaveChar = 0;
    HI_U8 u8Offset = 0;
    HI_S32 s32Ret = 1;
...
//            (HI_VOID)_CC608_DATA_ClearRows(module_id,u8FirstRow_HaveChar,u8FirstRow_HaveChar);
        }
    }
