                                    ...
                                    HI_U8 *pu8ManagementData, HI_U8 **ppu8DataUnitLoop, HI_U32 *pu32DataUnitLoopLength)
{
    ...
    DEC_TRACE("DecodeManagement "
                   "(enCaptionType = %s, pstDataGroup = 0x%8.8X, pu8ManagementData = 0x%8.8X)\n",
                   (enCaptionType==ARIB_CAP_CAPTION?"CAPTION":"SUPERIMPOSE"),
                   (HI_S32)pstDataGroup, (HI_S32)pu8ManagementData);
    ...
    *pu32DataUnitLoopLength = 0;
    ...
    for (i = 0; i < u8NumLanguages; i++)
    {
        ...
        /* DC (Display condition designation) */
        u8DC = 0x00;
        if (GET_DMF_RECEPTION(u8DMF) == DMF_SP_CONDITION)
        {
            u8DC = (HI_U8)GET_BIT(pu8Ptr, 0, 8);
            if (DC_RAIN_ATTENUATION != u8DC)
            {
                HI_ERR_CC("DC (Display condition designation) is error\n");
                return HI_FAILURE;
            }
            pu8Ptr ++;
            DEC_TRACE("\t DC = %#x\n",u8DC);
                                    ...
                                    HI_U8 *pu8ManagementData, HI_U8 **ppu8DataUnitLoop, HI_U32 *pu32DataUnitLoopLength)
{
    ...
    DEC_TRACE("DecodeStatement "
                   "(enCaptionType = %s, pstDataGroup = 0x%8.8X, pu8ManagementData = 0x%8.8X) is Called!\n",
                   (enCaptionType==ARIB_CAP_CAPTION?"CAPTION":"SUPERIMPOSE"),
                   (HI_S32)pstDataGroup, (HI_S32)pu8ManagementData);
    ...
    *pu32DataUnitLoopLength = 0;
    ...
    if ((u8DMF != DMF_AUTO_AND_AUTO) && (u8DMF != DMF_AUTO_AND_SELECT) && (u8DMF != DMF_SELECT_AND_SELECT))
    {
        HI_ERR_CC("DMF = 0x%2.2X is error\n", u8DMF);
        return HI_FAILURE;
    }
                                ...
                                ARIBCC_UnitGroup_S **ppstUnitGroup, HI_U32 *pu32UnitGroupLength)
{
    ...
    DEC_TRACE("DecodeDataUnitLoop (enCaptionType = %s, "
                   "pu8DataUnitLoop = 0x%8.8X, u32DataUnitLoopLength = %u) is Called!\n",
                   (enCaptionType==ARIB_CAP_CAPTION?"CAPTION":"SUPERIMPOSE"),
                   (HI_S32) pu8DataUnitLoop, s32DataUnitLoopLength);
    ...
    *pu32UnitGroupLength = 0;
         ...
         pu8DataUnit += (u32DataUnitSize + DU_HEADER_LENGTH))
    {
        ...
        {
            HI_ERR_CC("Data Unit Parameter : 0x%2.2X\n", u8DataUnitParameter);
            s32Ret = HI_FAILURE;
            break;
        }
                                 ...
                                 ARIBCC_StatementBody_S *pstStatementBody, HI_U32 *pu32StatementBodySize)
{
    ...
    DEC_TRACE("DecodeStatementBody (enCaptionType = %s, "
                   "pu8StatementBody = 0x%8.8X, u32StatementBodySize = %u) is Called!\n",
                   (enCaptionType==ARIB_CAP_CAPTION?"CAPTION":"SUPERIMPOSE"),
                   (HI_S32) pu8StatementBody, u32StatementBodySize);
                                   ...
                                   HI_U32 u32InDrcsDataSize, HI_U8 *pu8OutDrcsData, HI_U32 *pu32OutDrcsDataSize)
{
    ...
    HI_ERR_CC("DecodeCsiControl (enCaptionType = %s, "
                   "enDrcsType = %d, pu8InDrcsData = 0x%8.8X, u32InDrcsDataSize = %u) is Called!\n",
                   (enCaptionType==ARIB_CAP_CAPTION?"CAPTION":"SUPERIMPOSE"),
                   enDrcsType, (HI_S32) pu8InDrcsData, u32InDrcsDataSize);
                              ...
                              HI_U8 *pu8OutBitmapData, HI_U32 *pu32OutBitmapDataSize)
{
    ...
    HI_U32  *pu32ChunkCRC = HI_NULL;
    HI_BOOL bIHDR;
    //HI_BOOL bIEND;
    HI_S32  s32Ret = HI_SUCCESS;

    HI_ERR_CC("DecodeCsiControl (enCaptionType = %s, "
                   "pu8InBitmapData = 0x%8.8X, u32InBitmapDataSize = %u) is Called!\n",
                   (enCaptionType==ARIB_CAP_CAPTION?"CAPTION":"SUPERIMPOSE"),
                   (HI_S32) pu8InBitmapData, u32InBitmapDataSize);
...
//    memset (pu8OutBitmapData, 0, sizeof(AribCC_BitmapData_t));
    *pu32OutBitmapDataSize = 0;
    ...
    if (u8NumOfFlcColors > BMP_FLC_COLOR_MAX)
    {
        ...
        HI_ERR_CC("  0x%8.8X :     u8NumOfFlcColors = %u\n",
...
HI_S32 _ARIBCC_DEC_CheckCCPESHeader (HI_U8 *pu8CcPesPtr, HI_U64 *pu64Pts, HI_U8 **ppu8PesDataByte)
{
    ...
    *ppu8PesDataByte = HI_NULL;
    ...
    {
        HI_ERR_CC("  Data Identifier   : 0x%2.2X\n", u8DataIdentifier);
        return HI_FAILURE;
    }
    ...
    {
        HI_ERR_CC("  Private Stream ID : 0x%2.2X\n", u8PrivateStreamID);
        return HI_FAILURE;
    }
