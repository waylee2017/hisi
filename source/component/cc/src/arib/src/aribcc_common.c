...
static HI_S32 CCDISP_MS_QID  = -1;
static HI_S32 CCDISP_MS_QID2 = -1;
...
HI_S32 ARIBCC_Com_AllocPESPtr (ARIBCC_CaptionType_E enCaptionType, HI_U8 **ppu8CcPesPtr, HI_U32 u32CcPesSize)
{
    ...
    *ppu8CcPesPtr = pu8WPtr;
    ...
    HI_INFO_CC("AllocPESPtr (%s, 0x%8.8X, 0x%8.8X)\n",
                   (enCaptionType==ARIB_CAP_CAPTION?"CAPTION":"SUPERIMPOSE"),
                   (HI_S32) *ppu8CcPesPtr, (HI_S32) u32CcPesSize);
...
HI_S32 ARIBCC_Com_FreePESPtr (ARIBCC_CaptionType_E enCaptionType, HI_U8 *pu8CcPesPtr)
{
    ...
    HI_INFO_CC("FreePESPtr (%s, 0x%8.8X)\n",
                   (enCaptionType==ARIB_CAP_CAPTION?"CAPTION":"SUPERIMPOSE"), (HI_S32) pu8CcPesPtr);
...
HI_S32 _ARIBCC_Com_NotifyTimer (ARIBCC_CaptionType_E enCaptionType)
{
    ...
    if (pstTimerData[u16Rp].s32MsgType == MSGTYPE_CC_DISP_PARSE)
    {
        ...
        HI_INFO_CC("ARIBCC_Com_MessageSend (CC_QID, MSGTYPE_CC_DISP_PARSE, 0x%8.8X, %d)\n",
                       (HI_S32) &stMsgData, sizeof(stMsgData));
                               ...
                               HI_VOID *pvUnitGroup, struct timeval *pstTimeValue)
{
    ...
    HI_INFO_CC("RegistTimer (CaptionType=%s, MsgType=%s, "
                   "Management=0x%8.8X, UnitGroup=0x%8.8X, TimeValue=%ld.%6.6ld)\n",
                   (enCaptionType==ARIB_CAP_CAPTION?"CAPTION":"SUPERIMPOSE"),
                   (s32MsgType==MSGTYPE_CC_DISP_PARSE?"DISP":"CLEAR"),
                   (HI_U32) pvManagement, (HI_U32) pvUnitGroup,
...
{
    CCDISP_MS_QID  = msgget( CCDISP_MSG_KEY, 0666|IPC_CREAT );
    CCDISP_MS_QID2 = msgget( CCDISP_MSG_KEY2, 0666|IPC_CREAT );
...
HI_S32 ARIBCC_Com_MessageDeInit(HI_VOID)
{
    if (CCDISP_MS_QID > 0)
    {
        msgctl(CCDISP_MS_QID, IPC_RMID, NULL );
        CCDISP_MS_QID = -1;
    }
    if (CCDISP_MS_QID2 > 0)
    {
        msgctl(CCDISP_MS_QID2, IPC_RMID, NULL );
        CCDISP_MS_QID2 = -1;
    }
    return HI_SUCCESS;
}
...
HI_S32 ARIBCC_Com_MessageSend(ARIBCC_CaptionType_E enCaptionType, HI_S32 s32Type, HI_U8 *pu8Data, HI_S32 s32Size )
{
    ...
    if (ARIB_CAP_CAPTION == enCaptionType)
    {
        s32MsgqID = CCDISP_MS_QID;
    }
    else
    {
        s32MsgqID = CCDISP_MS_QID2;
    }
    ...
    s32Ret = msgsnd(s32MsgqID, &stMsg, (size_t)s32Size, IPC_NOWAIT);

    /* If status is success, return 0. or if status is not success, return -1. */
    if( s32Ret != 0 )
    {
...
HI_S32 ARIBCC_Com_MessageReceive(ARIBCC_CaptionType_E enCaptionType, HI_S32 *ps32Type, HI_U8 *pu8Data, HI_S32 *ps32Size )
{
    ...
    rev_size = msgrcv(s32MsgqID, &stMsg, (size_t)*ps32Size, 0, 0);
    if( rev_size < 0 )
    {
        *ps32Type = 0;
        *pu8Data = 0;
