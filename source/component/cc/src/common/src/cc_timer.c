...
static HI_VOID _cctimer_trigger(CCTIMER_HOOK_S *pstHook, HI_U32 escape)
{
    if (HI_TRUE == pstHook->bStart)
    {
        ...
        if(pstHook->u32Escape >= pstHook->u32Interval)
        {
            if (pstHook->pfnHook)
            {
                pstHook->pfnHook((HI_VOID *)pstHook->u32Args);
            }
...
HI_S32 CCTimer_Open(CCTIMER_ID_E enTimerID, HI_VOID (*pfnHook)(HI_VOID *), HI_VOID *args)
{
