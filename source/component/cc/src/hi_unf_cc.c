...
/*
static const HI_U8 s_szCCVersion[] __attribute__((used)) = "SDK_VERSION:["\
...
static HI_HANDLE s_ahCC[CC_MAX_MODULE_NUM];
...
{
    HI_HANDLE h708cc = (HI_HANDLE)arg;
...
{
    HI_HANDLE h608cc = (HI_HANDLE)arg;
...
static void StartCCDataThread(CC_LOCAL_PARAM_S *pstParam)
{
    ...
    {
        if(pthread_create(&pstParam->u32CC708Thread, HI_NULL, (void* (*)(void *))CC708DataThread, (void*)pstParam->h708CC)!=0)
        {
    ...
    {
        if(pthread_create(&pstParam->u32CC608Thread, HI_NULL, (void* (*)(void *))CC608Thread, (void*)pstParam->h608CC)!=0)
        {
...
HI_S32 HI_UNF_CC_DeInit(HI_VOID)
{
    ...
    if( HI_TRUE == s_bCCInit )
    {
        ...
        for(u8Index = 0;u8Index < CC_MAX_MODULE_NUM; u8Index ++)
        {
            if(s_ahCC[u8Index])
            {
                (HI_VOID)HI_UNF_CC_Destroy(s_ahCC[u8Index]);
                s_ahCC[u8Index] = 0;
            }
        }
        s_bCCInit = HI_FALSE;
...
HI_S32 HI_UNF_CC_Create( HI_UNF_CC_PARAM_S *pstCCParam, HI_HANDLE *phCC)
{
    ...
    stDispInitParm.stOpt.u32UserData = pstCCParam->u32UserData;
    ...
    for (u8Index = 0; u8Index < CC_MAX_MODULE_NUM; u8Index++)
    {
        if (0 == s_ahCC[u8Index])
        {
            s_ahCC[u8Index] = (HI_HANDLE)pstParam;
            break;
        }
    }

    if(u8Index >= CC_MAX_MODULE_NUM)
    {
        HI_ERR_CC("HI_UNF_CC_Create fail,module num is over!\n");
        free(pstParam);
        pstParam = HI_NULL;
        return HI_FAILURE;
    }
...
HI_S32 HI_UNF_CC_Destroy(HI_HANDLE hCC)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U8 u8Index = 0;
    CC_LOCAL_PARAM_S *pstParam = (CC_LOCAL_PARAM_S *)hCC;
