...
\retval ::HI_SUCCESS    Parsing is successful. CNcomment:解析成功 CNend
\retval ::HI_FAILURE    Parsing fails. CNcomment:解析失败 CNend
\see \n
None.
*/
...
/**
\brief Parses a dialog message. CNcomment:解析出一句Dialog信息,包括对白的动态显示样式,其它属性会被放在句柄中 CNend
\attention \n
None.
\param[in]      pstSubInfo Subtitle message to be parsed. CNcomment:需要解析的字幕信息 CNend
\param[in]      pstParseHand Parser handle. It must be successfully initialized. CNcomment:解析器句柄, 必须是被初始化成功的句柄 CNend
...
CNcomment:初始化一个解析器句柄,内部的属性需要调用::HI_SO_SUBTITLE_ASS_OutputParse接口成功后才能使用 CNend
\attention \n
None.
\param[in]      pstParseHand Parser handle to be initialized. CNcomment:需要初始化的解析器句柄 CNend

\retval ::HI_SUCCESS    Initialization is successful. CNcomment:初始化成功 CNend
\retval ::HI_FAILURE    Initialization fails. CNcomment:初始化失败 CNend
\see \n
None.
*/
...
CNcomment:释放一个解析器句柄,必须是被初始化成功后的句柄才能释放 CNend
\attention \n
None.
\param[in]      pstParseHand Parser handle to be released. CNcomment:需要释放的解析器句柄 CNend

\retval ::HI_SUCCESS   A handle is successfully released. CNcomment:释放成功 CNend
\retval ::HI_FAILURE   A handle fails to be released. CNcomment:释放失败 CNend
\see \n
None.
*/
...
/**
\brief Releases a Dialog linked list. CNcomment:释放一个Dialog链表 CNend
\attention \n
None.
\param[in]      pstDialog Node of the linked list to be released. CNcomment:需要释放的链表头结点 CNend

\retval ::HI_SUCCESS A linked list is successfully released.  CNcomment: 释放成功 CNend
\retval ::HI_FAILURE   A linked list fails to be released. CNcomment:释放失败 CNend
\see \n
None.
*/
...
/**
\brief Prints the extra data corresponding to the ASS subtitles. CNcomment:打印出ASS字幕的扩展数据 CNend
\attention \n
None.
\param[in]      pstAssParse ASS parser handle. CNcomment:打印的ASS句柄 CNend
