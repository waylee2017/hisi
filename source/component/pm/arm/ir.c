...
IRREGSETTING irRegSetting[IR_STD_NUM] = { {0x00001f17, 0x033c03cc, 0x019e01e6, 0x00b4010e, 0x002d0043, 0x008700cb}, //nec simple
                                       ...
                                       {0x00021017, 0x0014002a, 0x0014002a, 0x00000000, 0x0014002a, 0x0014002a}}; //rstep
...
HI_U32 irPmocHval[IR_PMOC_NUM];
HI_U32 irPmocLval[IR_PMOC_NUM];
...
RC6IRFMT irfmtrc6[IR_RC6_NUM] = { { 320, 213, 107, 71, 61, 33, 47, 115, 62, 89, 37, 15} };
RC5IRFMT irfmtrc5[IR_RC5_NUM] = { { 116, 62, 89, 14, 11}};

void ir_enable(void) 
{ 
	HI_U32 u32RegVal;
	HI_REG_READ32(SYS_BASE_ADDR + 0x48,u32RegVal);
	u32RegVal |= 0x10; //open clk
	u32RegVal &= 0xffffffdf;
	//      HI_REG_WRITE32(IR_BASE_ADDR + IR_EN, 0x1);      
	HI_REG_WRITE32(SYS_BASE_ADDR + 0x48,u32RegVal);
...
void ir_disable(void) 
{
	...
	//      HI_REG_WRITE32(IR_BASE_ADDR + IR_EN, 0x0);
	HI_REG_WRITE32(SYS_BASE_ADDR + 0x48,u32RegVal);
	return;
}
...
void ir_start(void) 
{
	...
	HI_REG_WRITE32(IR_BASE_ADDR + IR_START, 0x1);
...
{
    if (key_match(irfmtnec[irnum].bit0_s_up, irfmtnec[irnum].bit0_s_low, g_IrVal & 0xff)
        && key_match(irfmtnec[irnum].bit0_e_up, irfmtnec[irnum].bit0_e_low, (g_IrVal >> 16 )& 0xff )) //regData.val8[1]))
    {
        /* symbol matches to 0 */
        rec_data_bit(irrcvnum, 0);
        irrcvnum++;
    }
    else if (key_match(irfmtnec[irnum].bit1_s_up, irfmtnec[irnum].bit1_s_low, g_IrVal & 0xff)
             && key_match(irfmtnec[irnum].bit1_e_up, irfmtnec[irnum].bit1_e_low, (g_IrVal >> 16 )& 0xff))
    {
        /* symbol matches to 1 */
        rec_data_bit(irrcvnum, 1);
        irrcvnum++;
    }
    else if (key_match(irfmtnec[irnum].lead2_s_up, irfmtnec[irnum].lead2_s_low, (g_IrVal & 0xffff)) //regData.val16[1])
             && key_match(irfmtnec[irnum].lead2_e_up, irfmtnec[irnum].lead2_e_low, (g_IrVal >> 16 ) & 0xffff))
    {
...
void ir_raw_getfmtval(void)
{
    ...
    {
        if (key_match(irfmtnec[i].lead_s_up, irfmtnec[i].lead_s_low, g_IrVal & 0xffff)//regData.val16[1])
            && key_match(irfmtnec[i].lead_e_up, irfmtnec[i].lead_e_low,  (g_IrVal>>16)& 0xffff))//regData.val16[0]))
        {
            irrcvnum = 0;
            irfmt = IR_TYPE_NEC;
            irnum = i;
    ...
    {
        if (key_match(irfmtrc6[i].lead_s_up, irfmtrc6[i].lead_s_low,  g_IrVal & 0xffff)
            && key_match(irfmtrc6[i].lead_e_up, irfmtrc6[i].lead_e_low,(g_IrVal>>16) & 0xffff))
        {
            irrcvnum = 0;
            irfmt = IR_TYPE_RC6;
            irnum = i;
    ...
    {
  		if ((key_match(irfmtrc5[i].bit_up, irfmtrc5[i].bit_low, g_IrVal & 0xff)
			||(key_match(irfmtrc5[i].bit_up+irfmtrc5[i].bit_wth, irfmtrc5[i].bit_low+irfmtrc5[i].bit_wth, g_IrVal & 0xff)))
			&&(key_match(irfmtrc5[i].bit_up, irfmtrc5[i].bit_low, (g_IrVal>>16)&0xff)
			|| key_match(irfmtrc5[i].bit_up+irfmtrc5[i].bit_wth, irfmtrc5[i].bit_low+irfmtrc5[i].bit_wth, (g_IrVal>>16)&0xff)))
        {
            irrcvnum = 0;
            irfmt = IR_TYPE_RC5;
            irnum = i;
...
void ir_rcx_posthandle(HI_U8 pos)
{
    ...
    if (pos < 32)
	{
		if (irRawLKey & (u32Tmp << pos))
		{
			irRsvLKey &= ~(u32Tmp << pos);
		}
		else
		{
			irRsvLKey |= (u32Tmp << pos);
	...
	{
		pos = pos - 32;
		if (irRawHKey & (u32Tmp << pos))
		{
			irRsvHKey &= ~(u32Tmp << pos);
		}
		else
		{
			irRsvHKey |= (u32Tmp << pos);
...
void ir_raw_cmpval(void)
{
    ...
    if (irfmt == IR_TYPE_RC5)
    {
		...
		//	irRawLKey.val8[2] &= 0x07;
			irRawLKey = irRawLKey & 0xFFFF07FF;
        }
    }
    
    for (i = 0; i < ir_pmocnum; i++)
    {
        if (((irPmocLval[i] == irRawLKey) && (irPmocHval[i] == irRawHKey))
            || ((irPmocLval[i] == irRsvLKey) && (irPmocHval[i] == irRsvHKey)))
        {
...
void ir_raw_isr()
{
	...
	HI_REG_READ32(IR_BASE_ADDR + IR_INT_STATUS, u32RegVal);
	isrflg = (u32RegVal >>24) & 0xff;
    if ((isrflg & 0x07) == 0)
    {
        return;
    }
    ...
    /* get symbol count */
	HI_REG_READ32(IR_BASE_ADDR + IR_DATAH, u32RegVal);
    cnt = (u32RegVal & 0xff) & 0x3f;
    ...
    if (isrflg & 0x04)
    {
        ...
        while (cnt > 0)
        {
			HI_REG_READ32(IR_BASE_ADDR + IR_DATAL, u32RegVal);
            cnt--;
        }
    }
    else //symbol or timeout interrupt
    {
        ...
        while (cnt-- > 0)
        {
            ...
            regValLow = u32RegVal & 0xff;//regData.val8[3];           
            regValHigh = (u32RegVal >> 16) & 0xff;//regData.val8[1]; 
			g_IrVal = u32RegVal;
		//	printf_val(u32RegVal);
		//	printf_val((g_IrVal>>16)&0xffff);
		//	printf_val((g_IrVal)&0xffff);
            if (irfmt != 0xff)
            {
                if (irfmt == IR_TYPE_NEC)
                {
    ...
    /* clear interrupt */
	HI_REG_WRITE32(IR_BASE_ADDR + IR_INT_CLR, 0x00070000);
...
void ir_std_isr()
{
    ...
    if (((u32RegVal>>16) & 0x0f) == 0)
    {    	
        return;
    }

    /* if receive data */
    if ((u32RegVal>>16) & 0x01)
    {
        /* read low 32 bit of ir key */
		HI_REG_READ32(IR_BASE_ADDR + IR_DATAL, u32RegVal);
        if (u32RegVal == irPmocLval[0])
        {
            pmocType = HI_UNF_PMOC_WKUP_IR;
            pmocflag = HI_TRUE;
        }
	...
	HI_REG_WRITE32(IR_BASE_ADDR + IR_INT_CLR, 0x0f);
    return;
}
...
void IR_Init(void)
{
	...
	if(ir_type == 4)  //raw HI_UNF_IR_CODE_RAW
	{
		ir_type = IR_STD_NUM;
	}

	if(ir_type == 9)  //rstep HI_UNF_IR_CODE_RSTEP
	{
		ir_type = 4;
	}
    ...
    // 0  busy
	do {
		HI_REG_READ32(IR_BASE_ADDR + IR_BUSY, u32RegVal);
		wait_minute_1(10);
	} while (u32RegVal & 0xff);

    // 1  cfg
    if (ir_type < IR_STD_NUM)
    {
        u32RegVal = irRegSetting[ir_type].u32IrCfg;
    }
    else 
    {
        u32RegVal = 0x3e800197;
    }
	HI_REG_WRITE32(IR_BASE_ADDR + IR_CFG, u32RegVal);
    ...
    // 2 leads
	HI_REG_WRITE32(IR_BASE_ADDR + IR_LEADS, irRegSetting[ir_type].u32Leads);

    // 3 leade
	HI_REG_WRITE32(IR_BASE_ADDR + IR_LEADE, irRegSetting[ir_type].u32Leade);
	
    // 4 sleade
	HI_REG_WRITE32(IR_BASE_ADDR + IR_SLEADE, irRegSetting[ir_type].u32SLeade);

    // 5 b0
	HI_REG_WRITE32(IR_BASE_ADDR + IR_B0, irRegSetting[ir_type].u32B0);

    // 6 b1
	HI_REG_WRITE32(IR_BASE_ADDR + IR_B1, irRegSetting[ir_type].u32B1);

over:
    // 7 int mask
    if (ir_type < IR_STD_NUM)
    {
        u32RegVal = 0x0007000e;
    }
    else
    {
        u32RegVal = 0x0f;
	...
	HI_REG_WRITE32(IR_BASE_ADDR + IR_INT_MASK, u32RegVal);
