...
HI_S32 KEYLED_SetMode(HI_KEYLED_MODE_S keyled_mode)
{
    ...
    {
        //printf("set keyled mode parameter err \n");
        return HI_FAILURE;
    }
    ...
    if (HI_KEYLED_1MUL8 == keyled_mode.KeyScanMode)
    {
        ...
        //printf("read1 GpioFunctionConfig = %d \n",GpioFunctionConfig);
    ...
    else
    {
        regvalue &= ~KEY_SCAN_MODE;
        HI_REG_READ(MultConfigVir + 0x88,GpioFunctionConfig);
        //printf("read2 GpioFunctionConfig = %d \n",GpioFunctionConfig);
...
//	printf("KeyScanMode =%d LedNum = %d \n ",KeyLedMode.KeyScanMode ,KeyLedMode.LedNum);
	
	Ret = KEYLED_SetMode(KeyLedMode);
	if (HI_SUCCESS != Ret)
	{
		//printf("KEYLED_SetMode pram err\n");
	}
...
static HI_VOID keyled_config_clktim(HI_U32 clk)
{
	...
	if(0 == clk)
	{
		//printf("prompt: clk can't be 0, set to default %d\n",CLKTIM_DEFAULT);
		clk = CLKTIM_DEFAULT;
	}
...
static HI_VOID keyled_config_fretim(HI_U32 fre)
{
	...
	if(0 == fre)
	{
		//printf("prompt: fre can't be 0, set to default %d\n",FRETIM_DEFAULT);
		fre = FRETIM_DEFAULT;
	}
...
static HI_VOID keyled_config_keytim(HI_U32 freq)
{
	...
	if(0 == freq)
	{
		//printf("prompt: freq can't be 0, set to default %d\n",KEY_DEFAULT);
		freq = KEY_DEFAULT;
	}
