...
#if 0
HI_VOID hi_gpio_dirset_bit(HI_U32 idx, HI_U32 val)
{
    HI_U32 regData;
...
HI_VOID hi_gpio_write_bit(HI_U32 idx, HI_U32 val)
{
    HI_U32 regData;
	idx = idx - 40;	
    HI_REG_READ(GPIO5_REG_DIR, regData);
...
HI_VOID hi_gpio_read_bit(HI_U32 idx, HI_U32 * val)
{
    HI_U32 regData;
	idx = idx - 40;
...
#define  GPIO_CLOCK_SET(val) HI_DRV_GPIO_WriteBit(CLK_PT6964, val)
#define  GPIO_STB_SET(val) HI_DRV_GPIO_WriteBit(STB_PT6964, val)
//#define hi_gpio_dirset_bit(number, val) HI_DRV_GPIO_SetDirBit(number, val)

HI_VOID keyled_gpio_dirset(HI_VOID)
{
    HI_DRV_GPIO_SetDirBit(STB_PT6964, 0);
    HI_DRV_GPIO_SetDirBit(CLK_PT6964, 0);
}
...
HI_S32 keyled_gpio_switch(HI_U32 mode)
{
    if (mode)
    {
        HI_REG_READ((0x10203000 + 0x78), regDeInit[0]);
        HI_REG_READ((0x10203000 + 0x7c), regDeInit[1]);
        HI_REG_READ((0x10203000 + 0x80), regDeInit[2]);

        HI_REG_WRITE((0x10203000 + 0x78), 0x00);
        HI_REG_WRITE((0x10203000 + 0x7c), 0x00);
        HI_REG_WRITE((0x10203000 + 0x80), 0x00);
    }
    else
    {
        HI_REG_WRITE((0x10203000 + 0x78), regDeInit[0]);
        HI_REG_WRITE((0x10203000 + 0x7c), regDeInit[1]);
        HI_REG_WRITE((0x10203000 + 0x80), regDeInit[2]);
    }
...
HI_S32 keyled_tx_byte(HI_U8 u8data)
{
    ...
    HI_DRV_GPIO_SetDirBit(DIN_PT6964, 0);
    ...
    for (j = 0; j < 8; j++)
    {
        ...
        bitval = (u8data >> j) & 0x1;
        HI_DRV_GPIO_WriteBit(DIN_PT6964, bitval);
...
HI_S32 keyled_rx_byte(HI_U8 *u8data)
{
    ...
    HI_DRV_GPIO_SetDirBit(DOUT_PT6964, 1);
    ...
    for (j = 0; j < 8; j++)
    {
		...
		HI_DRV_GPIO_ReadBit(DOUT_PT6964, &bitval);
...
HI_S32 keyled_display_addr_inc(HI_U8 *u8buf)
{
  ...
  //  printf("> %s: %d\n", __FUNCTION__, __LINE__);
    ...
    keyled_write(display_mode_array[1]);
...
HI_S32 HI_KEYLED_GetValue(HI_U32 *pu32PressStatus, HI_U32 *pu32KeyId)
{
        ...
        if (KEY_MACRO_NO != g_OldKey.KeyVal)
        {
            if (KEY_PRESS == g_OldKey.KeyPressState)
            {
                ...
                *pu32PressStatus = g_OldKey.KeyPressState;
				//printf("\n 1g_OldKey.KeyVal = %d g_OldKey.KeyPressState %d \n", 
				//	g_OldKey.KeyVal, g_OldKey.KeyPressState);
                return HI_SUCCESS;
            }
            else
            {
				//printf("\n 2g_OldKey.KeyVal = %d g_OldKey.KeyPressState %d \n", 
				//	g_OldKey.KeyVal, g_OldKey.KeyPressState);
                g_OldKey.KeyVal = KEY_MACRO_NO;
                g_OldKey.KeyPressState = KEY_RELEASE;
                return HI_FAILURE;
    ...
    else
    {
        ...
        {
			//printf("\n 3g_OldKey.KeyVal = %d g_OldKey.KeyPressState %d \n", 
				//g_OldKey.KeyVal, g_OldKey.KeyPressState);
            return HI_FAILURE;
        }
		...
		//printf("\n 4g_OldKey.KeyVal = %d g_OldKey.KeyPressState %d \n", 
		//	g_OldKey.KeyVal, g_OldKey.KeyPressState);
...
{
    HI_S32 i = 0;
    HI_U32 u32ValueDisp = data;

    CheckKeyledDeviceFd();

    disp_buf_arr[0] = ((HI_U32)u32ValueDisp >> (8 * i)) & 0xff;
    i++;
    disp_buf_arr[2] = ((HI_U32)u32ValueDisp >> (8 * i)) & 0xff;
    i++;
    disp_buf_arr[6] = ((HI_U32)u32ValueDisp >> (8 * i)) & 0xff;
    i++;
    disp_buf_arr[8] = ((HI_U32)u32ValueDisp >> (8 * i)) & 0xff;
...
HI_S32 HI_KEYLED_DisplayString(HI_CHAR *fpString)
{
    ...
    //printf("led code 0x%x , 0x%x \n" , *(HI_U32*)DispCode , tmpData);
