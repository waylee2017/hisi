...
HI_S32 avl6211_init(HI_U32 u32TunerPort, HI_U8 enI2cChannel,
                    HI_UNF_TUNER_DEV_TYPE_E enTunerDevType, HI_VOID* pstTunerAttr)
{
    ...
    HI_ASSERT(HI_NULL != pstTunerAttr);
    ...
    pstAttr = (HI_UNF_TUNER_SAT_ATTR_S*)pstTunerAttr;

    pstParam->stConfig.u8ChannelId = u32TunerPort;
    //pstParam->stConfig.u8DemodAddress = (HI_U8)pstAttr->u16DemodAddress;
    pstParam->stConfig.u8DemodAddress = (HI_U8)g_stTunerOps[u32TunerPort].u32DemodAddress;
    pstParam->stConfig.u32ResetGpioNo = pstAttr->u32ResetGpioNo;
    ...
    //pstParam->stConfig.u16TunerAddress = pstAttr->u16TunerAddress;
    ...
    if (HI_SUCCESS != tuner_chip_reset(pstParam->stConfig.u32ResetGpioNo))
    {
        return HI_FAILURE;
    }
#if 0
    /* All demod reset once */
    if (!s_bReseted)
    {
        r  = HI_DRV_GPIO_SetDirBit(pstParam->stConfig.u32ResetGpioNo, HI_FALSE);
        r |= HI_DRV_GPIO_WriteBit(pstParam->stConfig.u32ResetGpioNo, 0);
        msleep(10);
        r |= HI_DRV_GPIO_WriteBit(pstParam->stConfig.u32ResetGpioNo, 1);
        if (AVL_DVBSx_EC_OK != r)
        {
