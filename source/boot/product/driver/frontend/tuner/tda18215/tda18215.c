...
static Bool tda_write (UInt8 uAddress, UInt8 uSubAddress, UInt32 uNbData, UInt8 * pDataBuff)
{
	...
	{
		TUNER_DEBUG("[tda18215 error]%s %d IIC write err!status:0x%x\n", __FUNCTION__, __LINE__, s32Ret);
		return HI_FAILURE;
	}
...
static Bool tda_read (UInt8 uAddress, UInt8 uSubAddress, UInt32 uNbData, UInt8 * pDataBuff)
{
	...
	if (HI_SUCCESS != s32Ret)
	{
		TUNER_DEBUG("[tda18215 error]%s %d IIC read err! status:0x%x\n", __FUNCTION__, __LINE__, s32Ret);
		return HI_FAILURE;
	}
...
HI_S32 tda18215_init_tuner(HI_U32 u32TunerPort)
{
    ...
    if(err != TM_OK)
    {
        TUNER_DEBUG("Call tmbslTDA182I5a_Open failure!\n");
        return HI_FAILURE;
    }
    ...
    if(err != TM_OK)
    {
        TUNER_DEBUG("Call tmbslTDA182I5a_HwInit failure!\n");
        goto TUNER_CLOSE;
    }
    ...
    if(err != TM_OK)
    {
        TUNER_DEBUG("Call tmbslTDA182I5a_SetPowerState failure!\n");
        goto TUNER_CLOSE;
    }
...
TUNER_CLOSE:
    ...
    if(err != TM_OK)
    {
        TUNER_DEBUG("Call tmbslTDA182I5a_Close failure!\n");
    }    
...
HI_S32 tda18215_get_signal_strength(HI_U32 u32TunerPort, HI_U32 *pu32SignalStrength)
{
    ...
    pu32SignalStrength[1] = (HI_U32)( (s32SignalStrength)/2 + 3 );
