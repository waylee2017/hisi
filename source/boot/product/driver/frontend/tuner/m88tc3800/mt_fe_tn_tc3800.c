...
S32 _mt_fe_tn_get_reg_tc3800(MT_FE_Tuner_Handle_TC3800 handle, U8 reg_addr, U8 *reg_data)
{
	...
	{
		TUNER_DEBUG("[m88tc3800 error]%s %d IIC read err! status:0x%x\n", __FUNCTION__, __LINE__, s32Ret);
		return HI_FAILURE;
	}
...
S32 _mt_fe_tn_set_reg_tc3800(MT_FE_Tuner_Handle_TC3800 handle, U8 reg_addr, U8 reg_data)
{
	...
	if (HI_SUCCESS != s32Ret)
	{
		TUNER_DEBUG("[m88tc3800 error]%s %d IIC write err!status:0x%x\n", __FUNCTION__, __LINE__, s32Ret);
		return HI_FAILURE;
	}
...
HI_S32 m88tc3800_set_tuner(HI_U32 u32TunerPort, HI_UNF_TUNER_I2cChannel_E enI2cChannel, HI_U32 puRF)
{
	U32 freq_KHz;
