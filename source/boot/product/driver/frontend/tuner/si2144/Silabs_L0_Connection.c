...
HI_U32  si2144_tda_write(HI_U8 uAddress, HI_U32 uNbData, HI_U8 * pDataBuff)
{
	HI_U8  i;
	HI_S32 s32Ret;
	...
	qam_config_i2c_out (s_u32Si2144TunerPort, 1);
	...
	s32Ret = tuner_i2c_send_data (s_u32Si2144I2cChannel, uAddress, &stI2cDataStr);
	if (HI_SUCCESS != s32Ret)
	{
		HI_ERR_TUNER("[si2144 error]%s %d IIC write err!status:0x%x\n", __FUNCTION__, __LINE__, s32Ret);
		return HI_FAILURE;
	}
 ...
 HI_U32  si2144_tda_read(HI_U8 uAddress,HI_U32 uNbData, HI_U8 * pDataBuff)
{
   ...
   //HI_ERR_TUNER("=========si2144_tda_read==================si2147_tda_read==================\n");
	qam_config_i2c_out (s_u32Si2144TunerPort, 1);
	...
	s32Ret = tuner_i2c_receive_data_si(g_stTunerOps[s_u32Si2144TunerPort].enI2cChannel, uAddress /*|0x01 */ ,&stI2cDataStr);
	//s32Ret = tuner_i2c_receive_data(s_u32Si2144I2cChannel, uAddress /*|0x01 */ ,&stI2cDataStr);
	if (HI_SUCCESS != s32Ret)
	{
  ...
  switch (i2c->connectionType) {
    case CUSTOMER:
		...
		nbReadBytes = iNbBytes;
				si2144_tda_read ((HI_U8)g_stTunerOps[s_u32Si2144TunerPort].u32TunerAddress, iNbBytes, pucDataBuffer);
  ...
  switch (i2c->connectionType) {
    case CUSTOMER:
		...
		nbWrittenBytes = iNbBytes;
		   ...
		   si2144_tda_write ((HI_U8)g_stTunerOps[s_u32Si2144TunerPort].u32TunerAddress,  iNbBytes, pucDataBuffer);
        /* <porting> Insert here whatever is needed to
        (option 1)
