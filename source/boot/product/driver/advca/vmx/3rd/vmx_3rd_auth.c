					...
					unsigned char *errorCode )
{
	...
	*errorCode = 0;
	...
	VMX_PrintBuffer("before decrypt by swpk, src(start):", src, 32);
	VMX_PrintBuffer("before decrypt by swpk, src(end):", src+len-256, 32);
	...
	/* verify signature */
	VMX_PrintBuffer("before verify rsa, signature:", signature, 32);
	VMX_PrintBuffer("before verify rsa, src(start):", src, 32);
	VMX_PrintBuffer("before verify rsa, src(end):", src+len-256, 32);
	...
	VMX_PrintBuffer("after verify rsa, src(start):", src, 32);
	VMX_PrintBuffer("after verify rsa, src(end):", src+len-256, 32);
