...
{
    OTP_CUSTOMER_KEY_S stOtpCustomerKey;
    ...
    memcpy((HI_U8 *)stOtpCustomerKey.u32CustomerKey, pKey, u32KeyLen);

    return OTP_V200_setCustomerKey(&stOtpCustomerKey);
}
...
HI_S32 HI_UNF_OTP_GetIDWordLockFlag(HI_BOOL *pbLockFlag)
{
	...
	value = HI_OTP_Read(OTP_ID_WORD_LOCK_ADDR); //aligned by 4 bytes
...
HI_S32 HI_UNF_OTP_LockIDWord(HI_VOID)
{
	...
	/* ID_WORD_lock */
	u32Value = HI_OTP_Read(OTP_ID_WORD_LOCK_ADDR); //aligned by 4 bytes
	...
	if(0 == u8Value)
	{
		...
		ret = HI_OTP_WriteByte(OTP_ID_WORD_LOCK_ADDR + 1, u8Value);
		if(HI_SUCCESS != ret)
		{
	...
	u32Value = HI_OTP_Read(OTP_SECURE_CHIP_FLAG_LOCK_ADDR); //aligned by 4 bytes
	...
	if(0 == u8Value)
	{
		...
		ret = HI_OTP_WriteByte(OTP_SECURE_CHIP_FLAG_LOCK_ADDR, u8Value);
		if(HI_SUCCESS != ret)
		{
...
HI_S32 HI_UNF_OTP_BurnToSecureChipset(HI_VOID)
{
	...
	/* echo write 0x04 0x01 > /proc/msp/otp //secure_chip_flag */
	ret = HI_OTP_WriteByte(OTP_SECURE_CHIP_FLAG_ADDR, 0x01);
	if(HI_SUCCESS != ret)
	{
...
HI_S32 HI_OTP_WriteByte(HI_U32 addr, HI_U8 tdata)
{
	(HI_VOID)HI_UNF_OTP_Init();

    return OTP_SetByte(addr, tdata);
}
