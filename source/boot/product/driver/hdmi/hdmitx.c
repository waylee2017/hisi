...
void SET_CEC_IO(void)
{
    ...
    u32Reg1 = HDMI_REG_READ(CEC_OTP);
    
    u32Reg2 = HDMI_REG_READ(CEC_IO);
    ...
    HDMI_REG_WRITE(CEC_IO, u32Reg2);

    COM_INFO("add:0x%x,w:0x%x,r:0x%x \n",CEC_IO,u32Reg2,HDMI_REG_READ(CEC_IO));
}
...
{
    //U_PERI_CRG67 crg67;
    HDMI_PERI_CRG67 crg67;

    crg67.u32 = g_pHDMIRegCrg->PERI_CRG67.u32;
    ...
    else
    {
     ...
     g_pHDMIRegCrg->PERI_CRG67.u32 = crg67.u32;
}

HI_VOID HW_ResetPhy(int iEnable)
{
    //U_PERI_CRG68 crg68;
    HDMI_PERI_CRG68 crg68;

    crg68.u32 =  g_pHDMIRegCrg->PERI_CRG68.u32;
...
#endif
     COM_INFO("--> HW_ResetHDMITX %x.\n",crg68.u32);
     g_pHDMIRegCrg->PERI_CRG68.u32 = crg68.u32;
}

void HW_ResetHDMITX(void)
{
    COM_INFO("--> HW_ResetHDMITX.\n");
#if 0 /*--NO MODIFY : COMMENT BY CODINGPARTNER--*/
    HdmiHardwareReset(1);
    DelayMS(18);
    HdmiHardwareReset(0);
...
void SW_ResetHDMITX(void)
{
    ...
    if (TimeOut == 0)
    {
        ...
        DelayMS(1);          // allow TCLK (sent to Rx across the HDMS link) to stabilize
        COM_INFO("TClock UnStable before sw reset <--\n");
    }
...
HI_S32 TX_CTRL_DVIEncoder(HI_BOOL bEnable)
{
    ...
    if(bEnable)
    {
        u32Value &= ~BIT_DVI_ENC_BYPASS;    //0 - normal operation (default).   
    }
    else
    {
        u32Value |= BIT_DVI_ENC_BYPASS;     //- bypass DVI encoder logic.   
    }
...
HI_U32 TX_Status( void )
{
    //return (GetSysStat() & (BIT_HPD_PIN))? HI_TRUE : HI_FALSE;
    return (HI_U32)((GetSysStat() & (BIT_HPD_PIN))? HI_TRUE : HI_FALSE);
}
