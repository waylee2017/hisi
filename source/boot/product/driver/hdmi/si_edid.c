...
static HI_S32 BlockCheckSum(HI_U8 *pEDID)
{
    ...
    {
         COM_INFO("addr(0x7F):0x%02x,checksum:0x%02x,sum:0x%x\n",
...
HI_VOID SI_DDCStatus (HI_U32 u32Status)
{
     ...
     {
        COM_INFO("DDC FIFO Write In Use\n");
     }
     if (u32Status & FRD_USE)
     {
        COM_INFO("DDC FIFO Read In Use\n");
     }
     if (u32Status & FIFO_EMP)
     {
        COM_INFO("DDC FIFO Empty\n");
     }
     if (u32Status & FIFO_FULL)
     {
        COM_INFO("DDC FIFO Full\n");
     }
     if (u32Status & IN_PROG)
     {
        COM_INFO(" DDC operation in progress\n");
     }
     if (u32Status & NO_ACK)
     {
        COM_INFO(" DDC NO_ACK\n");
     ...
     {
        COM_INFO(" DDC BUS_LOW\n");
     }
     COM_INFO("DDC u32Status:0x%x\n",u32Status);
}
...
HI_S32 SI_ReadSinkEDID(HI_VOID)
{
   ...
   // DDC_Adjust();
    ...
    for(u32Index = 0; u32Index < EDID_BLOCK_SIZE; u32Index += u16Byte)
    {
        ...
        {
            COM_INFO("EDID Debug Begin, Block_Num:%d(*128bytes):\n", (u32Index/EDID_BLOCK_SIZE + 1));
        }
        COM_INFO("%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X\n",
    ...
    if(u32ExtBlockNum > 3)
    {
        COM_INFO("Extern block too big, correct to 3 \n");
        u32ExtBlockNum = 3;
    }
    ...
    {
        COM_INFO("First Block Crc Error! \n");
        g_bErrList[_1ST_BLOCK_CRC_ERROR] = HI_TRUE;
    }

    //read extern block
    COM_INFO("read extern block edid\n");
...
static HI_S32 ParseVendorInfo(EDID_FIRST_BLOCK_INFO *pData)
{
	...
	for(u16Index = 0; u16Index < 3; u16Index++)
	{
		...
		{
		    COM_INFO("can't parse manufacture name\n");
...
HI_S32 ParseStandardTiming(HI_U8 *pData)
{
    ...
    for(i = 0; i < STANDARDTIMING_SIZE; i += 2)
    {
        ...
        {
            COM_INFO("Mode %d wasn't defined! \n", (int)pData[i]);
        }
        else
        {
            Hor = (pData[i]+31)*8;
            COM_INFO(" Hor Act pixels %d \n", Hor);
            TmpVal = pData[i + 1] & 0xC0;
            if(TmpVal==0x00)
            {
                COM_INFO("Aspect ratio:16:10\n");
                aspect_ratio = _16_10;
                Ver = Hor *10/16;
            }
            else  if(TmpVal==0x40)
            {
                COM_INFO("Aspect ratio:4:3\n");
                aspect_ratio = _4;
                Ver = Hor *3/4;
            }
            else  if(TmpVal==0x80)
            {
                COM_INFO("Aspect ratio:5:4\n");
                aspect_ratio = _5_4;
                Ver = Hor *4/5;
            }
            else //0xc0
            {
                COM_INFO("Aspect ratio:16:9\n");
                aspect_ratio = _16;
                Ver = Hor *9/16;
            }
            freq = ((pData[i + 1])& 0x3F) + 60;
            COM_INFO(" Refresh rate %d Hz \n", freq);
        }
    }
    return HI_SUCCESS;
}
...
static HI_S32 ParseEstablishTiming(HI_U8 * pData)
{
    ...
    if(pData[0]& 0x80)
        COM_INFO("720 x 400 @ 70Hz\n");
    if(pData[0]& 0x40)
        COM_INFO("720 x 400 @ 88Hz\n");
    if(pData[0]& 0x20)
    {
        COM_INFO("640 x 480 @ 60Hz\n");
        g_stEdidInfo.bSupportFormat[HI_UNF_ENC_FMT_861D_640X480_60] = HI_TRUE;
    }
    if(pData[0]& 0x10)
        COM_INFO("640 x 480 @ 67Hz\n");
    if(pData[0]& 0x08)
        COM_INFO("640 x 480 @ 72Hz\n");
    if(pData[0]& 0x04)
        COM_INFO("640 x 480 @ 75Hz\n");
    if(pData[0]& 0x02)
        COM_INFO("800 x 600 @ 56Hz\n");

    if(pData[0]& 0x01)
    {
        g_stEdidInfo.bSupportFormat[HI_UNF_ENC_FMT_VESA_800X600_60] = HI_TRUE;
        COM_INFO("800 x 400 @ 60Hz\n");
    }

    if(pData[1]& 0x80)
        COM_INFO("800 x 600 @ 72Hz\n");
    if(pData[1]& 0x40)
        COM_INFO("800 x 600 @ 75Hz\n");
    if(pData[1]& 0x20)
        COM_INFO("832 x 624 @ 75Hz\n");
    if(pData[1]& 0x10)
        COM_INFO("1024 x 768 @ 87Hz\n");
    
    if(pData[1]& 0x08)
    {
        g_stEdidInfo.bSupportFormat[HI_UNF_ENC_FMT_VESA_1024X768_60] = HI_TRUE;
        COM_INFO("1024 x 768 @ 60Hz\n");
    }
    if(pData[1]& 0x04)
        COM_INFO("1024 x 768 @ 70Hz\n");
    if(pData[1]& 0x02)
        COM_INFO("1024 x 768 @ 75Hz\n");
    if(pData[1]& 0x01)
        COM_INFO("1280 x 1024 @ 75Hz\n");

    if(pData[2]& 0x80)
        COM_INFO("1152 x 870 @ 75Hz\n");

    if((!pData[0])&&(!pData[1])&&(!pData[2]))
        COM_INFO("No established video modes\n");
...
HI_S32 ParsePreferredTiming(HI_U8 * pData)
{
	...
	{
		case 0x02:
            COM_INFO("stereo sequential R\n");
			break;

		case 0x04:                 
            COM_INFO("stereo sequential L\n");
			break;

		case 0x03:                 
            COM_INFO("stereo interleaved 2R\n");
			break;

		case 0x05: 
            COM_INFO("stereo interleaved 2L\n");
			break;
            
		case 0x06: 
             COM_INFO("stereo interleaved 4\n");
			break;

		case 0x07: 
            COM_INFO("stereo interleaved SBS\n");
			break;
...
{  
    ...
    COM_INFO("IT Underscan:0x%02x\n", ((pEDID[0] & BIT7)>>7));
    //pColorSpace->unScanInfo.u32ScanInfo = (pEDID[0] & BIT7) >> 4;
    COM_INFO("basic audio:0x%02x\n",  (pEDID[0] & BIT6));
    COM_INFO("bYCbCr444:%d\n", pColorSpace->bYCbCr444);
    COM_INFO("bYCbCr422:%d\n", pColorSpace->bYCbCr422);
    COM_INFO("DTDS num:0x%02x\n",  (pEDID[0] & 0x0F));
}
...
static HI_S32 ParseVendorSpecificDataBlock(HI_U8 *pData, HI_U8 u8Len )
{
    ...
    COM_INFO("IEERegId:0x%02x, 0x%02x, 0x%02x\n", pData[0], pData[1], pData[2]);
    if ((pData[0] == 0x03) && (pData[1] == 0x0c)
      && (pData[2] == 0x00))
    {
        COM_INFO("This is HDMI Device\n");
        g_stEdidInfo.bSupportHdmi = HI_TRUE;
    }
    else
    {
        g_stEdidInfo.bSupportHdmi = HI_FALSE;
        COM_INFO("This is DVI Device, we don't parse it\n");
        return HI_FAILURE;
    }
    ...
    COM_INFO("DVI_Dual:%d,Y444:%d,30bit:%d,48bit:%d,AI:%d\n",g_stEdidInfo.bSupportDVIDual,
   ...
   // g_stEdidInfo.u32MaxTMDSClk   = (pData[6] & 0xff) * 5;
    ...
    COM_INFO("support 3d:%d\n",g_stEdidInfo.st3DInfo.bSupport3D);
    COM_INFO("bLatency_Fields_Present:%d\n",(pData[7] & 0x80) >> 7);
    COM_INFO("bI_Latency_Fields_Present:%d\n",(pData[7] & 0x40) >> 6);
    COM_INFO("CNC:%d\n",(pData[7] & 0x0F));
    if(u8Len < 9)
    {
        COM_INFO("len:%d\n", u8Len);
    ...
    COM_INFO("u8Video_Latency:%d\n",(pData[8] & 0xff));
    COM_INFO("u8Audio_Latency:%d\n",(pData[9] & 0xff));
    COM_INFO("u8Interlaced_Video_Latency:%d\n",(pData[10] & 0xff));
    COM_INFO("u8Interlaced_Audio_Latency:%d\n",(pData[11] & 0xff));
    ...
    if((HDMI_3D_LEN > 0) && (u8Len >= (HDMI_VIC_LEN + HDMI_3D_LEN + 14))) 
    {
        ...
        if(u83D_present)
        {
            if ((u83D_Multi_present == 0x1)||(u83D_Multi_present == 0x2))
            {
                ...
                COM_INFO("3D structure_All_15...8 resever\n");
                u8Temp = pData[14 + HDMI_VIC_LEN + 1] & 0xFF;
                COM_INFO("u8Temp:0x%x\n",u8Temp);
                if(0x01 == u8Temp)
                {
    ...
    for(index +=15 ; index < u8Len; index ++)
    {
        ...
        COM_INFO("2D_VIC_order:0x%02x\n", u83D_Structure);
        u83D_Structure = (pData[index]&0x0F);
        if(u83D_Structure >= 0x08)
        {
            COM_INFO("3D_Detailed > 0x08 :0x%02x\n", u83D_Structure);
            u83D_Structure = (pData[index]&0xF0)>>4;
        }
        COM_INFO("3D_Detailed :0x%02x\n", u83D_Structure);
    }
     ...
     switch(pData[0])
     {
        ...
        case COLORIMETRY_DATA_BLOCK:
           // g_stEdidInfo.unColorimetry.u32Colorimetry = (pData[1]|(pData[2]<<8));
            ...
            COM_INFO("Colorimetry:0x%02x\n", pData[1]);
            break;
        case CEA_MISCELLANENOUS_AUDIO_FIELDS:
            COM_INFO("CEA miscellanenous audio data fileds\n");
            break;
        case VENDOR_SPECIFIC_AUDIO_DATA_BLOCK:
            COM_INFO("vendor specific audio data block\n");
            break;
        case RESERVED_HDMI_AUDIO_DATA_BLOCK:
            COM_INFO("reserved hdmi audio data block\n");
            break;
         default:
            break;
     }
...
HI_S32 ParseDTVBlock(HI_U8 *pData)
{
    ...
    if(s32Ret != HI_SUCCESS)
    {
        COM_INFO("Extend Block Crc Error! \n");
    ...
    length = pData[2];
    COM_INFO("data block length:0x%x\n",length);
    if(length == 0)
    {
    ...
    while(offset < length)
    {
        ...
        switch((pData[offset] & DATA_BLOCK_TAG_CODE) >> 5)
        {
            ...
            case VESA_DTC_DATA_BLOCK:
                COM_INFO("VESA_DTC parase\n");
                break;
            case USE_EXT_DATA_BLOCK:
                ParseExtDataBlock(&pData[offset + 1], len);
                break;
             default:
                COM_INFO("resvered block tag code define");
                break;
        }
        offset += len + 1;
    }
...
{   
    ...
    {
        COM_INFO("Extern block too big: 0x%02x \n",g_stEdidInfo.u8ExtBlockNum);
        COM_INFO("Setting to 3 Block \n");
    ...
    for(Index = 1; Index <= g_stEdidInfo.u8ExtBlockNum; Index++)
    {
        ...
        if(HI_SUCCESS == s32Ret)
        {
            COM_INFO("Successfully resolved ext block NO:0x%x\n",Index);
            break;
        }
    }
    ...
    if(g_stEdidInfo.bSupportHdmi == HI_FALSE)
    {
        ...
        {
            COM_INFO("Unknown mode, Go to Force Mode");
            return HI_FAILURE;
        }
        COM_INFO("DVI DEVICE\n");
    }
