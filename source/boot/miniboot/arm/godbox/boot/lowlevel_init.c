		...
		if ((get_chipid_reg() == _HI3716M_V300) &&
			...
			if (pm) {
				...
				if (ddrphy_train_route())
					reset_cpu(0);
				/* save ddr phy reg */
				regval = readl(REG_BASE_SCTL + DDRC_PHY_REG4B);
				writel(regval, REG_BASE_PHY + REG_SC_GEN16);
				regval = readl(REG_BASE_SCTL + DDRC_PHY_REG29);
				writel(regval, REG_BASE_PHY + REG_SC_GEN17);
				regval = readl(REG_BASE_SCTL + DDRC_PHY_REG8);
				writel(regval, REG_BASE_PHY + REG_SC_GEN18);
				regval = readl(REG_BASE_SCTL + DDRC_PHY_REG11);
				writel(regval, REG_BASE_PHY + REG_SC_GEN19);
			}
		}
	}
