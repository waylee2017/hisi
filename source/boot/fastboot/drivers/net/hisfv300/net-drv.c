...
static void hieth_adjust_link(struct eth_device* dev)
{
	u16 reg = 0;
	int stat = 0;
	int timeout_us = DEFAULT_PHY_LINK_TIME_US;
...
static int hieth_dev_probe_init(struct eth_device* dev, int port)
{
	...
	if ((_HI3110E_V500 != get_chipid()) && (_HI3716M_V330 != get_chipid()) && (ld->port == UP_PORT)) {
		/* only for MV310/MV320 internal FEPHY. */
	...
	} else if (_HI3716M_V330 == get_chipid()) {
		extern hisilicon_fephy_fix_mv330(struct eth_device *dev);
		hisilicon_fephy_fix_mv330(dev);
	}
