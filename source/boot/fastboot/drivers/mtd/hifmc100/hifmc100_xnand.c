...
{
#ifndef CONFIG_ARCH_HI3716MV310 
	u32 regval;
			     ...
			     struct nand_dev_t *nand_dev)
{
	...
	for (spl_dev = flash_ids_hisi; spl_dev->id_len; spl_dev++) {
		if (memcmp(byte, spl_dev->ids.id, spl_dev->id_len))
			continue;
...
{
	int isdump = 0;
	struct hifmc_xnand *xnand = (host->spinand ? host->spinand : host->nand);
...
static int hifmc100_xnand_probe_device(struct hifmc_xnand *xnand)
{
	...
	for (spl_dev = flash_ids_hisi; spl_dev->name; spl_dev++) {
		if (spl_dev->id_len) {
			if (memcmp(xnand->id, spl_dev->ids.id, spl_dev->id_len))
				continue;
		} else {
			if (spl_dev->ids.ids.dev_id != devid)
				continue;
		}
