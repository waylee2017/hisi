...
/******************************************************************************/
static void micron_chip_prepare_default(struct flash_regop *regop)
{
	u8 stat, new_stat;

	/* disable block protect */
	new_stat = stat = (u8)spinand_get_feature(regop, 0xa0);
	if (new_stat & SPINAND_BLOCK_PROT_MICRON)
		new_stat &= ~SPINAND_BLOCK_PROT_MICRON;
...
static struct spinand_driver spinand_driver_table[] = {
	{
		.id        = {0x2c, 0x14},
		.id_len    = 2,
		.chip_prepare = micron_chip_prepare_default,
	},{
		/* Winbond devices */
		.id        = {0xef},
		.id_len    = 1,
