...
static void dqs_bdl_enable(void)
{
	...
	tmp = tmp & 0xFFEFFFFF;
	io_write(REG_DDRC_BASE + DDRC_MISC, tmp);
	tmp = io_read(REG_DDRC_BASE + DDRC_PHYINITCTRL);
	tmp = tmp | (1 << 15);
	io_write(REG_DDRC_BASE + DDRC_PHYINITCTRL, tmp);
	tmp = tmp & 0xffff7fff;
	io_write(REG_DDRC_BASE + DDRC_PHYINITCTRL, tmp);
}
...
static void ddr_rdbdl_adj(struct training_data *training)
{
	...
	for(i=0;i<training->ddr_byte_num;i++){
		...
		rdqs_bdl = ( io_read(REG_DDRC_BASE + DDRC_DXNRDQSDLY(i)) &0x7f) + bdl_offest;
