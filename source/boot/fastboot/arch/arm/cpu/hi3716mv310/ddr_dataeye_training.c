 ...
 */
#  define DEFINE_RESULT(X) static struct ddrtr_result_t *(X) = 0X600B0000;
#endif
...
#define DDRT_WAIT_TIMEOUT	(400)
	...
	if(!(regval & 0x02)){
		ddrt_dq_err_ovfl = io_read(REG_DDRT_BASE_CRG + DDRT_DQ_ERR_OVFL) & (1<<dq_num);
		if(ddrt_dq_err_ovfl)
			return -1;
		ddrt_dq_err_cnt = io_read(REG_DDRT_BASE_CRG + DDRT_DQ_ERR_CNT((bytenum<<1) +(dqindex>>2 )));
...
static void ddr_rdbdl_adj(struct training_data *training)
{
	...
	for(i=0;i<training->ddr_byte_num;i++){
		...
		rdqs_bdl = ( io_read(REG_BASE_DDRPHY + DDRC_DXNRDQSDLY(i)) &0x7f) + bdl_offest;
...
/*****************************************************************************/
static void hw_training(unsigned int byte_num)
{
	//gating init
	unsigned int result;
	int i;
	...
	for (i = 0; i< byte_num; i++) {
		io_write(REG_BASE_DDRPHY + DDRC_DXNRDQBDL0(i), 0);
		io_write(REG_BASE_DDRPHY + DDRC_DXNRDQBDL1(i), 0);
		io_write(REG_BASE_DDRPHY + DDRC_DXNRDQSDLY(i), 0);
	}

	//hw read data eye training enable
	io_write(REG_BASE_DDRPHY + DDRC_PHYINITCTRL,0x105);

	while((io_read(REG_BASE_DDRPHY + DDRC_PHYINITCTRL)&0xffff)){
	}
	if(io_read(REG_BASE_DDRPHY + PHYINITSTATUS) & 0x100){
		result = 0x4;
...
static void ddr_training_entry(void)
{
	...
	training->ddrt_reversed_data = io_read(REG_BASE_SYSCTRL + REG_SC_GEN8);
	training->hw_training_flag = 0;

	training->ddr_byte_num = (io_read(REG_BASE_MDDRC + 0x50) >> 4) & 0x3;
	...
	// DDRT  init
	//io_write(REG_DDRT_BASE_CRG + DDRT_MEM_CONFIG, 0x1152);
	io_write(REG_DDRT_BASE_CRG + 0x1c, 0);
	io_write(REG_DDRT_BASE_CRG + 0x8, 0x4f);
	io_write(REG_DDRT_BASE_CRG + 0x14, 0xffffffff);
...
/***********************************************************/
	...
	if(training->hw_training_flag){
		...
		training->hw_training_flag = 0;
		io_write(DDR_TRAINING_STAT_REG, 0);
		hw_training(training->ddr_byte_num);
		read_config_ddr(training);
		...
		for (i = 0; i < training->ddr_byte_num; i++) {
			...
			if(((!first_deskew_flag)&&(byte_deskew_flag))||
				...
				first_deskew_flag = 1;
				rd_dqsbdl = io_read(REG_BASE_DDRPHY + DDRC_DXNRDQSDLY(i));
			...
			continue;
next:
			io_write(REG_BASE_DDRPHY + DDRC_DXNRDQSDLY(i), rd_dqsbdl);
			dqs_bdl_enable();
			i--;
			io_write(DDR_TRAINING_STAT_REG,
				io_read(DDR_TRAINING_STAT_REG)
