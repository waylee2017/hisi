...
int ddrphy_train_route(void)
{
	...
	if (!(REG_READ(REG_BASE_SYSCTRL + 0xd0) & 0x10)) {  /* Write Leveling Train Bypass */
		...
		tmp = ((mr1_raw | 0x80) << 16) + 0x12;          /* Set A7 = 1, Rank = 0, Cmd = MRS */
        ...
        tmp = ((mr1_raw & 0xff7f) << 16) + 0x12;          /*  Set A7 = 0, Rank = 0, Cmd = MRS */
		REG_WRITE(tmp, REG_BASE_MDDRC + 0x210);
		REG_WRITE(0x1, REG_BASE_MDDRC + 0x218);
		REG_WRITE(0x1, REG_BASE_MDDRC + 0x00c);
	...
	if (!(REG_READ(REG_BASE_SYSCTRL + 0xd0) & 0x100)) {  /* Gate Train Bypass */
		...
		if (DDRPHY_GATE_TRAIN_ROUTE() != 0) {
			return 2;                                  /* Find Gate BDL Error Return */
		}
		step(20);
		for (i = 0; i < ddr_byte_num; i++) {
			dqsg_phase[i] -= 8;
		...
		step(21);
		update_ddrphy_timing();
	}  /* Software Gate Train End */
	else if(REG_READ(REG_BASE_DDRPHY + 0x08) & 0x20){
			return 3;                                  /* Hardware Gate Train Error Return */
	}
	step(22);
	REG_WRITE(auto_ref_timing, REG_BASE_MDDRC + 0x88);
//	REG_WRITE(0xffff0700, REG_BASE_DDRPHY + 0x180); /*Enable Address 2T Timing */
