/*
 *  this file only for hi3716mv310
 */
#include <config.h>
...
/*****************************************************************************/
void ddrt_write(unsigned int addr,unsigned int data)
{
//	unsigned int tmp;
//	tmp = io_read(addr);
	io_write(addr, data);
...
#define DDRT_READ_TIMEOUT	(20)
	...
	do {
		...
		tmp2 = io_read(addr);
		times++;
	} while (((tmp0!=tmp1)||(tmp1!=tmp2)) && (times < DDRT_READ_TIMEOUT));
	return tmp0;
}
			     ...
			     unsigned int bytenum, unsigned int dqindex)
{
	...
	dqindex &= 0x07;
		...
		if (dqindex < 4) {
			val = io_read(REG_BASE_DDRPHY + DDRC_DXNRDQBDL0(bytenum));
			val &= ~(0xFF << (dqindex << 3));
			val |= ((0x1F & deskew_value) << (dqindex << 3));
			io_write(REG_BASE_DDRPHY + DDRC_DXNRDQBDL0(bytenum), val);
		} else {
			dqindex -= 4;
			val = io_read(REG_BASE_DDRPHY + DDRC_DXNRDQBDL1(bytenum));
			val &= ~(0xFF << (dqindex << 3));
			val |= ((0x1F & deskew_value) << (dqindex << 3));
			io_write(REG_BASE_DDRPHY + DDRC_DXNRDQBDL1(bytenum), val);
		}
			     ...
			     unsigned int bytenum, unsigned int dqindex)
{
	...
	if(!check_multi_ssn(training, dqindex,bytenum)){
		...
		if (check_multi_ssn(training, dqindex,bytenum)) 	/* ddrt tset is ERROR */
	...
	while (ix< max_level) {
		...
		if (!check_multi_ssn(training, dqindex,bytenum))  	/* ddrt tset is OK */
			left_bound = ix;
	...
	if(interrupt_fl){
		training->wr_bit_result[dq_num] &= (0xffffffff >>(WRDQ_LEVEL-left_bound-1));
			     ...
			     unsigned int bytenum, unsigned int dqindex)
{
	...
	if(interrupt_fl)
		training->rd_bit_result[dq_num] &= (0xffffffff >>(RDDQ_LEVEL-left_bound-1));
...
static void ddr_rdbdl_adj(struct training_data *training)
{
	...
	for(i=0;i<training->ddr_byte_num;i++){
		...
		rdqs_bdl = io_read(REG_BASE_DDRPHY + DDRC_DXNRDQSDLY(i)) + bdl_offest;
		...
		for(j=0; j<8; j++){
			...
			config_ddr_dq_rd(training, training->rddq_val[dq_index], i, j);
		}
...
/*****************************************************************************/
unsigned int  byte_deskew_judge(struct training_data *training, unsigned int byte_index,unsigned int mode)
{
	...
	if((left_bound_overnum>=4) && (right_bound_overnum <1))
		return DQPHASE_DECREASE; //need move the window to right¡ê?decrease DQ phae
	if((left_bound_overnum<4) && (right_bound_overnum >=1))
		return DQPHASE_INCREASE; //need move the window to left¡ê?increase DQ phae
...
static void ddr_training_entry(void)
{
	...
	training->ddrt_reversed_data = io_read(0x101e00a0);
	training->ddr_byte_num = (io_read(0x10101050) >> 4) & 0x3;
	...
	ddrt_write(0x1001f01c, 0);
	ddrt_write(0x1001f008, 0x4f);
	ddrt_write(0x1001f014, 0xffffffff);
