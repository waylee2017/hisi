...
# endif	/* CONFIG_BOOT_RETRY_TIME */
	...
	if (bootdelay >= 0 && !tstc())
		hibernate_boot(-1);
#ifndef CONFIG_HIBERNATE_DRV_FLOATING
	hibernate_drvload();
#endif
#endif
...
/* 
 *  For TaiYan's HDMI PHY. To avoid the surge current to damage the HDMI PHY. 
 *  If you are remainning in u-boot, it will keep to detecting HDMI Phy'status all the
 *  time. To Changing DMI Phy's Output Register & Power Register if the status is changed.
 */
#ifdef CONFIG_PRODUCT_WITH_BOOT
		{
			extern void update_hdmi_status();
			update_hdmi_status();
		}
#endif
