...
int download_process_usbhost(void)
{
	...
	/* setp4: android update need to replace bootargs from recoverybootargs */
	bootargs_change();

	/* setp5: read recovery.img and go */
	ret = usb_file_check(CONFIG_FILE_RECOVERY_NAME);
	if (ret)
		goto error;

	filesize = usb_file_read(CONFIG_FILE_RECOVERY_NAME,
				(void *)CONFIG_FILE_READ_ADDR,  0, CONFIG_FILE_MAX_SIZE);
	if(filesize <= 0)
		goto error;
...
{
	if (CHIPSET_CATYPE_NORMAL == get_ca_type())
