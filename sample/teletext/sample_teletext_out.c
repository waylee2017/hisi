...
#define   TTX_OSD_PAGEWIDTH (640)    /** The width in display buffer, not in srceen. The max value of buffer width */
#define   TTX_OSD_PAGEHEIGHT (536)     /** The height  in display buffer, not in srceen. The max value of buffer height*/
#else
...
HI_S32  Higo_Teletext_Init()
{
...
#ifdef CHIP_TYPE_hi3110ev500
    s32Ret = HI_GO_GetLayerDefaultParam(HIGO_LAYER_SD_0, &stLayerInfo);
#else
    ...
    stLayerInfo.ScreenWidth   = 720;   /**  the width of screen */
    stLayerInfo.ScreenHeight  = 576;   /**  the height of  srceen */
    stLayerInfo.DisplayWidth  = 720;  /**  the width of display buffer */
    stLayerInfo.DisplayHeight = 576;    /** the height of  display buffer */
#else
    stLayerInfo.PixelFormat   = HIGO_PF_8888;
    ...
    else
    {
        ...
        s32File = open("./res/font0.bin", O_RDONLY, 0);
        if (-1 == s32File)
        {
            goto ERR3;
        }
...
static HI_VOID TTX_DrawChar(HI_UNF_TTX_DRAWCAHR_S* pstParam)
{
    ...
    HI_U32 i = 0, j = 0, k = 0, h = 0, m = 0, n = 0, u32Index;
             ...
             j < (u32DesColumn + u32DesColCount) * TTX_CHARWIDTH; j += u32DesColCount, m++)
        {
            for (k = 0; k < u32DesRowCount; k++)
            {
                for (h = 0; h < u32DesColCount; h++)
                {
                    ...
                    {
                        *((HI_U32 *)(pu8MemAddr + (i + k) * u32BytesPerRowInMem + (j + h) * TTX_PIXBYTE)) =
                            pstParam->u32Foreground;
                    }
                    else
                    {
                        if (!bRedrawChar)
...
static HI_VOID TTX_FillRect(HI_UNF_TTX_FILLRECT_S* pstParam)
{
    ...
    for (i = u32Row * TTX_CHARHEIGHT; i < (u32Row + u32RowCount) * TTX_CHARHEIGHT; i++)
    {
        ...
        {
            *((HI_U32 *)(pu8MemAddr + i * u32BytesPerRowInMem + j * TTX_PIXBYTE)) = pstParam->u32Color;
        }
    }
    return;
}
...
static HI_VOID   TTX_ShowOsd(HI_UNF_TTX_REFRESHLAYER_S* pstParam)
{
    ...
    //printf("srcRect x=%d,y=%d,w=%d,h=%d,DesRect x=%d,y=%d,w=%d,h=%d\n",stSrcRect.x,stSrcRect.y,stSrcRect.w,stSrcRect.h,stDesRect.x,stDesRect.y,stDesRect.w,stDesRect.h);
