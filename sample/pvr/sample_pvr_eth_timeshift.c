...
HI_S32 SwitchToLivePlay(HI_U32 u32PlayChn, HI_HANDLE hAvplay, const PMT_COMPACT_PROG *pProgInfo)
{
    ...
    PVR_SwitchDmxSource(PVR_DMX_ID_LIVE, HI_UNF_DMX_PORT_0_TUNER);
...
HI_VOID * StatuThread(HI_VOID *args)
{
    ...
    while (HI_FALSE == g_bIsPlayStop)
    {
        ...
        if ( g_PlayChn != 0xffffffff)
        {
            ...
            if (HI_SUCCESS == Ret)
            {
                sprintf(timestr, "%d", stStatus.u32CurPlayTimeInMs/1000);
...
HI_S32 main(HI_S32 argc,HI_CHAR *argv[])
{
    ...
    PVR_SAMPLE_Printf("enter and argc is %d\n", argc);
...
//    HIADP_MCE_Exit();
    ...
    Ret = HI_UNF_DMX_CreateTSBuffer(PVR_DMX_PORT_ID_IP, 0x200000, &g_hIpTsBuf);
    if (Ret != HI_SUCCESS)
    {
    ...
    while (0 == u32ProgNum)
    {
        ...
        ret = SAMPLE_GET_INPUTCMD(InputCmd);
        if (!ret)
        {
            sleep(1);
            continue;
