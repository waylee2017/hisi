...
static HI_S32 file_dec_rle(HI_CHAR *pszFileName, HI_HANDLE *pSurface, HI_HANDLE hLayer, HI_HANDLE hDstSurface)
{
    ...
    if (HI_SUCCESS != ret)
    {
        PVR_SAMPLE_Printf("HI_GO_CreateDecoder failed %s..........!\n", pszFileName);
        return ret;
    }
    ...
    if (HI_SUCCESS != ret)
    {
        ...
        PVR_SAMPLE_Printf("======line:%d failed!\n", __LINE__);
        return HI_FAILURE;
    }
    ...
    while (Index < PrimaryInfo.Count)
    {
        ...
        if (HI_SUCCESS != ret)
        {
            PVR_SAMPLE_Printf("======line:%d failed %x!\n", __LINE__, ret);
            break;
        }
        /** 刷新图层显示 */
...
HI_S32 OsdInit(HI_VOID)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        PVR_SAMPLE_Printf("failed to create the font:./res/DroidSansFallbackLegacy.ttf ! ret=%#x\n",s32Ret);
        return HI_FAILURE;
    }
...
//    pthread_t              pthreadShowPic = 0;
    ...
    {
        PVR_SAMPLE_Printf("Usage: %s file_path freq [srate] [qamtype or polarization] [vo_format] [JPEG]\n"
                ...
                "       JPEG: this parameter could be ignored\n", argv[0]);
...
//        PVR_StartNetStream("239.0.0.1", 1234);
    ...
    PVR_SAMPLE_Printf("please input the number of program to timeshift:\n");
    ...
    while(1)
    {
        ...
        else if ('s' == InputCmd[0])
        {
            ...
            HI_UNF_PVR_PlayGetStatus(g_PlayChn, &stStatus);
            if (stStatus.enState != HI_UNF_PVR_PLAY_STATE_SF)
            {
                stStatus.enSpeed = HI_UNF_PVR_PLAY_SPEED_NORMAL;
            }
            stStatus.enSpeed = stStatus.enSpeed/2;
            if (stStatus.enSpeed < HI_UNF_PVR_PLAY_SPEED_64X_SLOW_FORWARD)
            {
                stStatus.enSpeed = HI_UNF_PVR_PLAY_SPEED_2X_SLOW_FORWARD;
            }
            stTrickMode.enSpeed = stStatus.enSpeed;
        ...
        else if ('k' == InputCmd[0])
        {
            ...
            {
                PVR_SAMPLE_Printf("Now in live mode, can't seek to start!\n");
            }
        }
        else if ('e' == InputCmd[0])
        {
            ...
            {
                PVR_SAMPLE_Printf("Now in live mode, can't seek to end!\n");
            }
        }
        else if ('a' == InputCmd[0])
        {
            ...
            {
                PVR_SAMPLE_Printf("Now in live mode, can't seek backward!\n");
            }
        }
        else if ('d' == InputCmd[0])
        {
            ...
            {
                PVR_SAMPLE_Printf("Now in live mode, can't seek forward!\n");
            }
        }
