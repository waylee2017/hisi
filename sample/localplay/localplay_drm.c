...
#define LOGI(fmt, arg...) sample_printf(fmt"\n", ##arg)
#define EXPECT_EQ(exp1, exp2) \
    if (exp1 != exp2) {\
        sample_printf("expect eq (%s) failed at [%s,%d]\n", #exp1 "==" #exp2, __FUNCTION__, __LINE__);\
    }
#define EXPECT_TRUE(exp) \
    if (!exp) {\
        sample_printf("expect true (%s) failed at [%s,%d]\n", #exp, __FUNCTION__, __LINE__);\
    }
#define CHECK(cond, action, msg, ...) \
    if (cond) {\
        sample_printf("Check " #cond " true at [%s,%d], we will:"#action" msg:"#msg"\n", __FUNCTION__, __LINE__, ##__VA_ARGS__);\
        action;\
    }
...
static  HI_S32 DEVCERT_BUFFER_PREPARE_ONE(HI_CHAR* file, HI_U8** buf, HI_S32* len)
{
    ...
    *len = (HI_S32)ftell(fp_DevCert);
    ...
    if(NULL == pu8Tmp)
    {
        sample_printf("malloc size 0x%x error!\n", *len);
        s32Ret = HI_FAILURE;
        goto __Quit;
    }
    ...
    if(u32Len != *len)
    {
        sample_printf("Read '%s' failed. real size(0x%x), should be size(0x%08x).\n", file, u32Len, *len);
        s32Ret = HI_FAILURE;
        goto __Quit;
    }
...
static  HI_S32 DevCert_Buffer_Prepare(HI_PLAYREADY_DEVCERT_BUF_S *pOpenParam, const HI_CHAR* path)
{
    ...
    if ( NULL == pOpenParam )
    {
        sample_printf("Invalid param, NULL Pointer!\n");
        return HI_FAILURE;
    }
    ...
    if ( HI_FAILURE == s32Ret)
    {
        sample_printf("buffer prepare for %s failed!\n", pf_priv);
        goto __Quit;
    }
...
static HI_DRM_INFO_PPTR doAcquireDrmInfo(HI_S32 type, const HI_CHAR* request, const HI_CHAR* mimeType)
{
    ...
    if (request)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            sample_printf("put %s:%s into request\n", dict[i].key, dict[i].val);
        }
        free(tmp_str);
    }
...
void doProcessDrmInfo(HI_DRM_INFO_PPTR info, const HI_CHAR* config)
{
    ...
    if (config)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            sample_printf("put %s:%s into info\n", dict[i].key, dict[i].val);
        }
    }
...
void drm_test_handle()
{
    HI_U32 i = 0;
    HI_S32 ret;
    HI_DRM_CLIENT_PPTR client;
    for (i = 0; i < 100; i++)
    {
        ret = HI_DRM_CreateClient(&client);
        HI_DRM_DestroyClient(&client);
    }
}
...
//    CHECK(!type || !acquire || !config, return, "invalid parameter");
    sample_printf("do transaction, trans:%d; %s; %s; %s\n", type, mimeType, acquire, config);
...
void drm_cmd(const HI_CHAR* arg)
{
    ...
    sample_printf("doing drm commands:%s\n", arg);
    ...
    {
        sample_printf("Execute commands:%s\n", arg);
        HI_S32 len = 0;
    ...
    {
        sample_printf("Not supported commands:%s\n", arg);
    }
...
HI_S32 drm_check_right_status(const HI_CHAR* path, const HI_CHAR* mime_type)
{
    ...
    case HI_DRM_RIGHTS_EXPIRED: {
        sample_printf("DRM right status error:%d", ret);
        return HI_FAILURE;
    } break;
    default: {
        sample_printf("Unrecognized right status:%d", ret);
        return HI_FAILURE;
    } break;
    }
    return HI_FAILURE;
