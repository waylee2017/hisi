...
static struct filename_list* createListPoint(char *filename, char * filepath)
{
    ...
    if(NULL == list_point)
    {
        sample_printf("struct filename_list malloc failed!\n");
        return NULL;
    }
    ...
    {
        sample_printf("one filename to list  malloc failed\n");
        free(list_point);
        return NULL;
    }
    ...
    {
        sample_printf("one filepath to list  malloc failed\n");
        free(list_point);
...
static int insertFilenameToList(struct filename_list** header, char *filename, char* filepath,int flags)
{
    if(!filename)
    {
        sample_printf("now you read input file is wrong!");
        return HI_FAILURE;
    }
    if(!(*header))
    {
        *header = createListPoint(filename, filepath);
        if(*header == NULL)
        {
            sample_printf("createListPoint failed!\n");
            return HI_FAILURE;
        }
    }
    else
    {
        ...
        if( NULL == tmp1)
        {
            ...
            if(!new_tmp)
            {
                sample_printf(" [%d] create file list point failed\n",__LINE__);
                return HI_FAILURE;
            }
            tmp2->next = new_tmp;
        }
...
static int freeList(struct filename_list** header)
{
    if(!*header)
    {
        sample_printf("now your list is already free!\n");
        return HI_SUCCESS;
    }
...
static int putFilenameToM3U9File(struct filename_list **header)
{
    ...
    if(-1 == fd)
    {
        sample_printf("creat m3u9 file failed!");
        return HI_FAILURE;
    }
    ...
    while(tmp)
    {
        //sample_printf("tmp->pfile_path = %s\n",tmp->pfile_name);
...
static int DvdBdGaplessPlay(int formattype ,char * path_name,struct filename_list **header, int flags)
{
    ...
    if(ret != 0)
    {
        sample_printf("register regular expression failed\n");
        return HI_FAILURE;
    }

    if(NULL == (dp = opendir(path_name)))
    {
        sample_printf("open dir failed!\n");
        regfree( &reg);
        return HI_FAILURE;
    ...
    while((dirp = readdir(dp)) != NULL)
    {
        ...
        {
            sample_printf("[%d]get filename stat failed\n",__LINE__);
            regfree(&reg);
            chdir("..");
            closedir(dp);
            return HI_FAILURE;
        }
        //sample_printf("dirp->d_name = %s\n",dirp->d_name);
        if(S_ISREG(stat_file.st_mode))
        {
            ...
            if(0 == ret)
            {
                ...
                num ++;
                //sample_printf("there is one match!\n");
            }
        }
    }
    chdir("..");
    ...
    {
        //no match
        sample_printf("there is no one match!\n");
        regfree( &reg);
        closedir(dp);
        return HI_FAILURE;
    }
...
static int openDirectory(char * path, struct filename_list **header)
{
    ...
    {
        sample_printf("[%d]open dir failed!\n",__LINE__);
        return HI_FAILURE;
    }
    chdir(path);
    while((dirp = readdir(dp)) != NULL)
    {
        ...
        if(stat(dirp->d_name, &stat_file) < 0)
        {
            sample_printf("[%d] get filename or derectory stat failed\n",__LINE__);
            chdir("..");
            closedir(dp);
            return HI_FAILURE;
        }
...
static int throughWholeDerectory(char * path)
{
    ...
    if(0 == strncasecmp(p1 + 1, "VIDEO_TS", 8))
    {
        ...
        if(HI_FAILURE == ret)
        {
            sample_printf("[%d] DvdBdGaplessPlay failed\n",__LINE__);
            return HI_FAILURE;
        }
    }
...
static int hasBrothersFile(char *path)
{
    ...
    if(NULL == p1)
    {
       sample_printf("Find characters failed!\n");
       free(path_name);
       return HI_FAILURE;
    }
    ...
    {
        //sample_printf("Now this format is  not surport!\n");
        free(path_name);
        freeList(&header);
        return HI_FAILURE;
    }
    ...
    if(HI_FAILURE == ret)
    {
        sample_printf("Create m3u9 file failed\n");
        free(path_name);
        freeList(&header);
        return HI_FAILURE;
    }
...
int playM3U9Main(char * path)
{
    ...
    {
        //sample_printf("playM3U9Main stat wrong!\n");
        return HI_FAILURE;
    }
    ...
    {
        //go to directory
        //sample_printf("directory\n");
        if(HI_FAILURE == throughWholeDerectory(path))
        {
            return HI_FAILURE;
            sample_printf("failed\n");
        }
