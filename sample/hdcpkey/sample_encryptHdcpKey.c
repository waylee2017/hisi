...
HI_S32 sample_encryptHdcpKey_usingOtpRootkEY(HI_CHAR *pString)
{
    ...
    memset(u8InEncryptKey, 0x00, 384);
    memset(u8OutEncryptKey, 0x00, 332);
...
//    HI_HDCPKEY_EQUAL(CopyFromBufferToFile(u8InEncryptKey + 48, 320, "input_320_of_384.bin"), HI_SUCCESS);
    memcpy(stHdcpKey.key.EncryptData.u8EncryptKey, u8InEncryptKey, 384);
    HI_HDCPKEY_EQUAL(HI_UNF_HDCP_EncryptHDCPKey(stHdcpKey, HI_TRUE, u8OutEncryptKey), HI_SUCCESS);
//    HI_PrintBuf("Encrypted hdcp key [length 332 bytes]", u8OutEncryptKey, 332);
    
    HI_HDCPKEY_EQUAL(CopyFromBufferToFile(u8OutEncryptKey, 332, "EncryptedKey_332bytes.bin"), HI_SUCCESS);
