...
static HI_UNF_KEYLED_TYPE_E g_KeyledType = HI_UNF_KEYLED_TYPE_GPIOKEY;
#else
...
#define IR_CHN_UP    0x35caff00//0xf40bec40
#define IR_CHN_DOWN  0x2dd2ff00//0xf10eec40
...
static HI_UNF_DMX_PORT_E  g_DmxPort[4] = {0, 0, 0, 0};
...
HI_S32 DVB_StartProg(HI_HANDLE hAvplay,PMT_COMPACT_PROG *pProgInfo,HI_BOOL bPreStart)
{
    ...
    if (VidPid != INVALID_TSPID)
    {
        ...
        if ( HI_FALSE == bPreStart )
        {
			...
			if (Ret != HI_SUCCESS)
	        {
	            printf("call HI_UNF_AVPLAY_Start VID failed.\n");
	            return Ret;
	        }
        }
        else
        {
			...
			if (Ret != HI_SUCCESS)
	        {
	            printf("call HI_UNF_AVPLAY_PreStart VID failed.\n");
	            return Ret;
	        }
		}
    ...
    if (AudPid != INVALID_TSPID)
    {
        ...
        else
        {
			...
			if (Ret != HI_SUCCESS)
	        {
	            printf("call HI_UNF_AVPLAY_PreStart to start audio failed.\n");
	            //return Ret;
	        }
		}
...
static  HI_S32 DVB_PlayProg(HI_U32 u32ProgIdx)
{
	...
	HI_UNF_SND_Attach(HI_UNF_SND_0, hAvplay,HI_UNF_SND_MIX_TYPE_MASTER, 100);
...
//   HI_UNF_SYNC_ATTR_S SyncAttr;
  ...
  //  HIADP_MCE_Exit();
    ...
    for ( i = 0 ; i < HI_TUNER_NUMBER ; i++ )
    {
        ...
        stDmxPortAttr.enPortType = HI_UNF_DMX_PORT_TYPE_PARALLEL_NOSYNC_188;
        stDmxPortAttr.u32TunerInClk = 0;
    ...
    for ( i = 0 ; i < MAX_AVPLAY_NUM; i++ )
    {
	    ...
	    SyncAttr.stSyncStartRegion.s32VidNegativeTime = -60;
...
//		HI_UNF_AVPLAY_RegisterEvent(g_hAvplay[i],HI_UNF_AVPLAY_EVENT_NEW_VID_FRAME,AVPLAY_NEW_Frame_Event_Callback);
    }
    DEBUG
...
//SND_DETACH:
	HI_UNF_SND_Detach(HI_UNF_SND_0, g_hAvplay[g_AvplayIdx[0]]);
