...
HI_S32 main(HI_S32 argc,HI_CHAR *argv[])
{
    ...
    if (argc < 6 || argc > 9)
    {
        printf("Usage: sample_dvbplay_pid vpid vtype apid atype freq [srate] [qamtype] [vo_format]\n"
        ...
        printf(" examples: \n");
        printf("   sample_dvbplay_pid vpid h264 \"null\" \"null\" 610 6875 64 1080i_50\n");
        printf("   sample_dvbplay_pid \"null\" \"null\" apid mp3 610 6875 64 1080i_50\n");
        printf("   sample_dvbplay_pid vpid h264 apid mp3 610 6875 64 1080i_50\n");
        return HI_FAILURE;
    }
    ...
    if (9 == argc)
    {
        ...
        u32TunerQam   = strtol(argv[7],NULL,0);
    ...
    Ret = HIADP_Tuner_Connect(TUNER_USE, u32TunerFreq, u32TunerSrate, u32TunerQam);
    if (HI_SUCCESS != Ret)
    {
    ...
    while(1)
    {
        printf("please input the q to quit!, s to toggle spdif pass-through,,v20(set volume 20),vxx(set volume xx)\n");
		...
		if ('v' == InputCmd[0])
		{
			...
			u32Volume = atoi(InputCmd+1);
			if(u32Volume>100)
				u32Volume = 100;
				
			printf("Volume=%d\n",u32Volume);
			HI_UNF_SND_SetVolume(HI_UNF_SND_0, u32Volume);
		}
