...
{
#if 1
	HI_U32     u32HdmiConfig;
	HI_U32     u32SpdifConfig;
	HI_CHAR    hdmi[3][10];
	HI_CHAR    spdif[2][10];
#else
...
/**
 * DVB play event
 * @param  u32WindHandle: window handle
 * @param  u32KeyId: key ID
  * @param phAvplay: pointer to avplay
 * @param  pstChannelbar: pointer to channel bar
 * @return: success--HI_SUCCESS; failure--HI_FAILURE
*/
HI_S32 HI_DOLBY_DvbKeyEvent(HI_U32 u32WindHandle, HI_U32 u32KeyId, HI_HANDLE* phAvplay, CHNBAR_S*  pstChannelbar, HI_BOOL* pbUpdteChanBar);
