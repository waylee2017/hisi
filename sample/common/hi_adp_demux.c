...
static HI_BOOL bIPInjectFlag[DMX_TOTALPORT_CNT];
static HI_BOOL bFileInjectFlag[DMX_TOTALPORT_CNT];

HI_HANDLE g_PortTsBuf[DMX_TOTALPORT_CNT];
...
#else
#define SAMPLE_PRINTF(fmt,args...)  printf(fmt,##args)
#endif
...
HI_S32 DMX_Prepare(HI_U32 u32DmxID, HI_U32 u32PortID, HI_U32 u32BufSize)
{
    ...
    if ((u32DmxID >= DMX_CNT) || (u32PortID >= DMX_TOTALPORT_CNT))
    {
        return HI_FAILURE;
    }
...
HI_S32 DMX_DataRead(HI_HANDLE hChannel, HI_U32 u32TimeOutms, HI_U8 *pBuf, HI_U32* pAcquiredNum , HI_U32 * pBuffSize)
{
    ...
    while (--u32Times)
    {
        ...
        {
            SAMPLE_PRINTF("HI_UNF_DMX_AcquireBuf time out\n");
            return HI_FAILURE;
        }
    }
...
HI_VOID FileTsTthread(HI_VOID *args)
{
    ...
    while (!FILE_Play_Handles[u32Handle].bStopFileThread)
    {
        ...
        if (Readlen <= 0)
        {
            SAMPLE_PRINTF("read ts file error!\n");
...
HI_S32  DMX_IPStopInject(HI_U32 u32PortID)
{
    ...
    if ((u32PortID < DMX_DVBPORT_CNT) || (u32PortID >= DMX_TOTALPORT_CNT))
    {
        return HI_FAILURE;
    }
