...
/*add for scte subtitle dispose,scte subtitle stream type is same as DTS audio*/
HI_BOOL bScteSubtFlag = HI_FALSE;
    ...
    while (len >= 2)
    {
        ...
        switch (tag)
        {
            case REGISTRATION_DESCRIPTOR:
                if (STREAM_TYPE_DTS_AUDIO_IDENTIFY == htonl(*((HI_U32*)buf)))
                {
                    return HI_TRUE;
                }
                break;
...
HI_S32 SRH_ParsePMT ( const HI_U8 *pu8SectionData, HI_S32 s32Length, HI_U8 *pSectionStruct)
{
   ...
   // printf("pcr pid = %#x, deslength:%d, s32Length = %d\n", u16Pid, u16DesLen, s32Length);
    ...
    while (s32Length > 4)
    {
        ...
        case STREAM_TYPE_SCTE:/*stream type is 0x82,STREAM_TYPE_DTS_AUDIO*/
        {
            if(bScteSubtFlag == HI_TRUE)
            {
                if (u16DesLen > 0)
                {
                    ParsePMTDesc(pu8SectionData, u16DesLen, pPmtTb);
        ...
        if (HI_FAILURE != DB_GetDVBProgInfo(i, &stProgInfo))
        {
            ...
            for (j = 0; j < stProgInfo.u8AudioChannel; j++)
            {
                ...
                case HA_AUDIO_ID_DOLBY_TRUEHD:
                    printf("\tStream Type DOLBY TRUEHD\n");
                    break;
                default:
                    printf("\tStream Type error\n");
                }
    ...
    for (i = 0; i < pat_tb.u16ProgNum; i++)
    {
        ...
        for (j = 0; j < pmt_tb.u16AudoNum; j++)
        {
            ...
            case HA_AUDIO_ID_DTSHD:
                ...
                break;
            case HA_AUDIO_ID_DRA:
                printf("\tAudio Stream Type DRA\n");
                break;
            case HA_AUDIO_ID_DOLBY_TRUEHD:
                printf("\tAudio Stream Type DOLBY TRUEHD\n");
                break;
...
HI_VOID HIADP_Search_SetScteSubtParseFlag(HI_BOOL bFlag)
{
    bScteSubtFlag = bFlag;
    return;
}
