...
static HI_VOID ParsePMTDescSe( const HI_U8* pu8DesData, HI_S32 s32DescLength, PMT_TABLE_S* pstruProg)
{
    HI_U8 u8Tag = 0;
    HI_U8 u8Length = 0;
    HI_U8 u8CntIndex = 0;
...
static HI_BOOL IsTtxStream(const HI_U8 *pu8Data, HI_S32 s32Length)
{
    HI_BOOL bRet = HI_FALSE;
...
HI_S32 SRH_ParsePMTSe ( const HI_U8 *pu8SectionData, HI_S32 s32Length, HI_U8 *pSectionStruct)
{
    ...
    HI_U8 AudioTag = 0;
    PMT_TABLE_S *pPmtTb = (PMT_TABLE_S *)pSectionStruct;
   ...
   // printf("pcr pid = %#x, deslength:%d, s32Length = %d\n", u16Pid, u16DesLen, s32Length);
    ...
    if (u16DesLen > 0)
    {
        ParsePMTDescSe(pu8SectionData, u16DesLen, pPmtTb);
        ...
        case STREAM_TYPE_AC3_AUDIO:
        {
           // printf("audio stream type is %#x, pid = %#x\n", u8StreamType, u16Pid);
            if(pPmtTb->u16AudoNum < PROG_MAX_AUDIO)        
            {
                ...
                else if (u8StreamType == STREAM_TYPE_14496_3_AUDIO)
                {
                    pPmtTb->Audioinfo[pPmtTb->u16AudoNum].u32AudioEncType = HA_AUDIO_ID_AAC;
                }
                else
...
HI_S32 SRH_PMTRequestSe(HI_U32 u32DmxID, PMT_TABLE_S *pmt_tb, HI_U16 u16PmtPid, HI_U16 u16ServiceId)
{
    ...
    /* set call back func*/
    stDataFilter.funSectionFunCallback = &SRH_ParsePMTSe;
...
HI_S32 DVB_SearchStartSe(HI_U32 u32DmxID)
{
    ...
    for (i = 0; i < pat_tb.u16ProgNum; i++)
    {
        ...
        memset(&pmt_tb, 0, sizeof(PMT_TABLE_S));
        if (SRH_PMTRequestSe(u32DmxID, &pmt_tb, stProgInfo.u16PmtPid, stProgInfo.u16ServiceID) == HI_SUCCESS)
        {
...
HI_S32  HIADP_Search_GetAllPmtSe(HI_U32 u32DmxId, PMT_COMPACT_TABLE_S **ppProgTable)
{
    ...
    PMT_TABLE_S pmt_tb;
    SDT_TB sdt_tb;
    PMT_COMPACT_TABLE_S *pProgTable = HI_NULL;
    HI_U32 u32Promnum = 0;
    ...
    pProgTable = (PMT_COMPACT_TABLE_S*)malloc(sizeof(PMT_COMPACT_TABLE_S));
    ...
    pProgTable->proginfo = (PMT_COMPACT_PROGRAM_S*)malloc(pat_tb.u16ProgNum * sizeof(PMT_COMPACT_PROGRAM_S));
    ...
    memset(pProgTable->proginfo, 0, pat_tb.u16ProgNum * sizeof(PMT_COMPACT_PROGRAM_S));
    ...
    for (i = 0; i < pat_tb.u16ProgNum; i++)
    {
        ...
        memset(&pmt_tb, 0, sizeof(PMT_TABLE_S));
        s32Ret = SRH_PMTRequestSe(u32DmxId, &pmt_tb, pat_tb.PatInfo[i].u16PmtPid, pat_tb.PatInfo[i].u16ServiceID);
        if (HI_SUCCESS != s32Ret)
        {
