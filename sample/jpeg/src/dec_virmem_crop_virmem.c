...
*
******************************************************************************
File Name	    : sample_dec_virmem_crop_virmem.c
Version		    : Initial Draft
Author		    : 
Created		    : 2014/06/20
Description	    : 码流来源于虚拟内存，裁剪，输出为用户的虚拟内存
Function List 	: 
...
/*****************************************************************************
* func		: sample_dec_virmemin_crop_virmemout
* description: CNcomment: 虚拟内存输入，裁剪，虚拟内存输出  CNend\n
* param[in]	: *pFileName		CNcomment:要解码的文件	  CNend\n
* retval 	: NA
* others:	: NA
*****************************************************************************/
HI_VOID sample_dec_virmemin_crop_virmemout(HI_CHAR *pFileName)
{
		 ...
		 **/
		 cinfo.scale_num   = 1;
	     cinfo.scale_denom = 4;
         cinfo.out_color_space = JCS_ABGR_1555;

		
		stSurfaceDesc.bCrop = HI_TRUE;
	    stSurfaceDesc.stCropRect.x = 0;
	    stSurfaceDesc.stCropRect.y = 0;
	    stSurfaceDesc.stCropRect.w = 10;
	    stSurfaceDesc.stCropRect.h = 10;
...
int main(int argc,char** argv)
{
		...
		while((ptr = readdir(dir))!=NULL)
        {
			 ...
			 sample_dec_virmemin_crop_virmemout(pFileName);
		}
