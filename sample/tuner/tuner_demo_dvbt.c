...
CMD_HELP_S g_cmdhelp[] =
{
        ...
        {"monitorlayer", "monitorlayer 1: receive layer A.[FOR ISDB-T]\n"
                                "\t Param1: 0 receive layer A/B/C, 1 only receive layer A, 2 only receive layer B, 3 only receive layer C.\n"},
        {"------------cab cmd", "*****************************************"},
...
//#define HI3716M310TST   /*SKAμ?°?×ó*/
//#define NO_DEMOD_INIT /*è?òa3?ê??ˉdemod￡??òDè×￠êí??oê*/
                ...
                switch (s_stConnectPara[s_s32TunerPort].enSigType)
                {
    				...
    				case (HI_UNF_TUNER_SIG_TYPE_DVB_T|HI_UNF_TUNER_SIG_TYPE_DVB_T2):
                        ...
                        if (HI_UNF_TUNER_SIG_TYPE_DVB_T2 & s_stConnectPara[s_s32TunerPort].enSigType)
                        {
                            ...
                            printf("plp index       common plp id       combination\n");
                            for(i=0;i<k;i++)
                            {
...
HI_VOID hi_tuner_selectCurrentPort(char * pPort)
{
    ...
    //s32DemuxPort = (s_s32TunerPort == 0) ? (HI_DEMUX_PORT) : (HI_DEMUX1_PORT);
...
#else
/*tuner0oítuner1?údemuxμ?TS???úê?D?ò???μ??é????￡??é?±?ó????*/
...
HI_VOID hi_tuner_getsignalinfo(char * para)
{
        ...
        (HI_UNF_TUNER_SIG_TYPE_J83B >= s_stConnectPara[s_s32TunerPort].enSigType))
    {
        ...
        else if (HI_UNF_TUNER_SIG_TYPE_ISDB_T == stInfo.enSigType)
        {
            ...
            HI_UNF_TUNER_FE_FFT_E           enLayerFECRate = 0;
            HI_UNF_MODULATION_TYPE_E        enLayerModType = 0;

            printf("EmergencyFlag:\t%d\nPhaseShiftCorr:\t%d\nPartialFlag:\t\t%d\n",
                    stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.u8EmergencyFlag,
                    stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.u8PhaseShiftCorr,
                    stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.u8PartialFlag);
                
            for (i = 0; i < 3; i++)
            {
                if (0 == i)
                {
                    if (0 == stInfo.unSignalInfo.stIsdbT.unISDBTLayers.bits.u8LayerAExist)
                    {
                    ...
                    {
                        printf("LayerA info:\n");
                        printf("\tLayerSegNum:\t%d\n",stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersAInfoBits.u8LayerSegNum);
                        enLayerModType = stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersAInfoBits.enLayerModType;
                        enLayerFECRate = stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersAInfoBits.enLayerFECRate;
                        enLayerHierMod = stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersAInfoBits.enLayerHierMod;
                    }
                }

                if (1 == i)
                {
                    if (0 == stInfo.unSignalInfo.stIsdbT.unISDBTLayers.bits.u8LayerBExist)
                    {
                    ...
                    {
                        printf("LayerB info:\n");
                        printf("\tLayerSegNum:\t%d\n",stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersBInfoBits.u8LayerSegNum);
                        enLayerModType = stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersBInfoBits.enLayerModType;
                        enLayerFECRate = stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersBInfoBits.enLayerFECRate;
                        enLayerHierMod = stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersBInfoBits.enLayerHierMod;
                    }
                }

                if (2 == i)
                {
                    if (0 == stInfo.unSignalInfo.stIsdbT.unISDBTLayers.bits.u8LayerCExist)
                    {
                    ...
                    {
                        printf("LayerC info:\n");
                        printf("\tLayerSegNum:\t%d\n",stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersCInfoBits.u8LayerSegNum);
                        enLayerModType = stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersCInfoBits.enLayerModType;
                        enLayerFECRate = stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersCInfoBits.enLayerFECRate;
                        enLayerHierMod = stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersCInfoBits.enLayerHierMod;
                    }
                }
...
HI_VOID hi_tuner_setAntenaPower(char *pPower)
{
    ...
    if (HI_SUCCESS != HI_UNF_TUNER_SetAntennaPower(s_s32TunerPort, atoi(pPower)))
    {
        HI_TUNER_ERR("Set Antenna Power fail:%d\n", atoi(pPower));
    }
}
    ...
    else
    {
        ...
        for(i=0;i < stTerScanPara.u32ChanNum;i++)
        {
            printf("%d			%d			%d			%d\n",
                    ...
                    stTerScanPara.enChanArray[i].u8PlpId,
                    stTerScanPara.enChanArray[i].u8CommId,
...
{
    /*12D??üá?*/
    {"help", hi_showhelp},                                  /*??è??μ??*/
    {"setport", hi_tuner_selectCurrentPort},              /*可用于配置端口号*/
    {"setchnl", hi_tuner_setchannel},                 /*锁频命令*/
    {"getsignalinfo", hi_tuner_getsignalinfo},       /*获取信号信息*/
    {"getber", hi_tuner_getmsc_ber},                /*获取误码率*/
    {"getoffset", hi_tuner_getoffset},                 /*获取频偏*/
    {"standby", hi_tuner_standby},                    /*待机*/
    {"play", hi_tuner_play },                             /*播放码流*/
    ...
    {"ant", hi_tuner_setAntenaPower},   /*配置天线电源*/
