...
/*------------------------------------------------------------------------------------------------*
Function        :HI_S32 HI_ETH_GetLinkStatus_Special(Eth_Port_E ePort, PTR_Eth_LinkStatus_E ptrLinkStatus)
Description     :get link status for ePort
Calls           :
Data Accessed   :
Data Updated    :
...
HI_S32 HI_ETH_GetLinkStatus_Special(Eth_Port_E ePort, PTR_Eth_LinkStatus_E ptrLinkStatus)
{
    ...
    Vaddr = (HI_VOID *)mmap(NULL, size, PROT_READ, MAP_SHARED, Mfdd, Paddr);
    if (MAP_FAILED == Vaddr)
    {
        printf("mmap %x failed \n", Paddr);
        close(Mfdd);
        return HI_FAILURE;
    }

    u32status = *(unsigned int *)(Vaddr + 0x2000 * ePort + 0x204);
    *ptrLinkStatus = (u32status >> 1) & 0x1;
...
/*----------------------------------------------------------------------------------------*
Function        :HI_VOID HI_ETH_WorkModeSet(Eth_WorkMode_E eWorkMode)
Description     :set eth work mode,
                 ETH_WORKMODE_UMDS means up port for arm,down port for pc
                 ETH_WORKMODE_DMUS means down port for arm,up port for pc
                 ETH_WORKMODE_UMDM means up port for arm,down port for arm
Calls           :
Data Accessed   :
Data Updated    :
Input           :eWorkMode
...
HI_S32 HI_ETH_WorkModeSet(Eth_WorkMode_E eWorkMode)
{
#if 0
    HI_U8 *buf = NULL;
    if (ETH_WORKMODE_MAX <= eWorkMode)
    {
        API_DEBUG("invalid parameter:%d\n", eWorkMode);
        return HI_ERR_ETH_INVALID_PARA;
    }

    buf = (HI_U8 *)malloc(WRITE_CMD_BUF_LEN);

    memset(buf, 0, WRITE_CMD_BUF_LEN);

    sprintf(buf, "echo 0x%x>/proc/x5net/client_iface/work_mode\n", eWorkMode);
    ...
    Vaddr = (HI_VOID *)mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, Mfdd, Paddr);
    if (MAP_FAILED == Vaddr)
    {
        printf("mmap %x failed \n", Paddr);
        close(Mfdd);
    ...
    val = *(HI_U32*)(Vaddr + reg);
    switch (eWorkMode)
    {
        /*up port forward data to arm & down port, down port forward data to up port & pc but not forward to arm*/
    case ETH_WORKMODE_UMDS:
        val &= 0xf;
        val |= (0x5f << 4);
        break;

        /*up port forward data to down port & pc but not to arm, down port forward data to arm & up port*/
    case ETH_WORKMODE_DMUS:
        val &= 0xf;
        val |= (0xf5 << 4);
        break;

        /*up port forward data to arm only, down port forward data to arm only*/
    case ETH_WORKMODE_UMDM:
        val &= 0xf;
        val |= (0xaa << 4);
        break;
    default:
...
HI_S32 HI_ETH_MacTableEnable(Eth_Port_E ePort)
{
    ...
    u8BitShift = (ETH_PORT_UP == ePort)?(BIT_MACT_ENA_UP_SHIFT):(BIT_MACT_ENA_DOWN_SHIFT);
