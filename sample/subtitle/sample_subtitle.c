...
static HI_S32 GetCurPts(HI_U32 u32UserData, HI_S64 *ps64CurrentPts)
{
...
static HI_S32 UnfSubtCallback(HI_U32 u32UserData, HI_UNF_SUBT_DATA_S *pstData)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
static HI_U8 SelectProgram(PMT_COMPACT_TBL *pSubTitleProgTbl)
{
    ...
    if (pSubTitleProgTbl->prog_num > 1)
    {
        ...
        TRACE_INFO("Please input a number from 0 to %d for program:", pSubTitleProgTbl->prog_num-1);
        memset(au8ProgNo, 0, sizeof(au8ProgNo));
        fgets((char*)au8ProgNo, 30, stdin);
...
static HI_S32 StartAVPlay(PMT_COMPACT_TBL *pSubTitleProgTbl, HI_U8 u8ProgNo)
{
    ...
    if (0 == hAvplay)
    {
        ...
        (HI_VOID)HI_UNF_DISP_SetLayerZorder(HI_UNF_DISP_SD0, HI_UNF_DISP_LAYER_VIDEO_0, HI_LAYER_ZORDER_MOVEBOTTOM);
#else
       s32Ret = HIADP_Disp_Init(HI_UNF_ENC_FMT_720P_50);
        ...
        (HI_VOID)HI_UNF_DISP_SetLayerZorder(HI_UNF_DISP_HD0, HI_UNF_DISP_LAYER_VIDEO_0, HI_LAYER_ZORDER_MOVEBOTTOM);
#endif
...
static HI_S32 CreateDVBSubt(PMT_COMPACT_PROG *pstProginfo)
{
    ...
    stSubtParam.u32UserData = (HI_U32)hUnfSO;
...
static HI_S32 CreateSCTESubt()
{
    ...
    s32Ret = HI_UNF_SUBT_RegGetPtsCb(hUnfSubt,GetCurPts,(HI_U32)hAvplay);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_UNF_SUBT_ITEM_S g_stSubtItem;
static HI_S32 StartSubtitle(PMT_COMPACT_TBL *pSubTitleProgTbl, HI_U8 u8ProgNo)
{
    ...
    if (0 == hUnfSO)
    {
        ...
        s32Ret = HI_UNF_SO_RegGetPtsCb(hUnfSO, GetCurPts, (HI_U32)hAvplay);
        s32Ret |= HI_UNF_SO_RegOnDrawCb(hUnfSO, Subt_Output_OnDraw, Subt_Output_OnClear, (HI_U32)hSubtOut);
        if (HI_SUCCESS != s32Ret)
        {
    ...
    if (hUnfSubt)
    {
        ...
        if(HI_UNF_SUBT_DVB == g_enSubtType)
        {
            ...
            memset(&g_stSubtItem, 0, sizeof(g_stSubtItem));
            g_stSubtItem.u32SubtPID = pstProginfo->SubtitingInfo[u8SubtNo].u16SubtitlingPID;
            g_stSubtItem.u16PageID = pstProginfo->SubtitingInfo[u8SubtNo].DesInfo[0].u16PageID;
            g_stSubtItem.u16AncillaryID = pstProginfo->SubtitingInfo[u8SubtNo].DesInfo[0].u16AncillaryPageID;
            TRACE_INFO("select subtitle[%d,%d,%d]\n",g_stSubtItem.u32SubtPID,g_stSubtItem.u16PageID,g_stSubtItem.u16AncillaryID);
            s32Ret = HI_UNF_SUBT_SwitchContent(hUnfSubt, &g_stSubtItem);
            if (HI_SUCCESS != s32Ret)
            {
...
static HI_S32 StartDataFilter(PMT_COMPACT_TBL *pSubTitleProgTbl, HI_U8 u8ProgNo)
{
    ...
    if(HI_UNF_SUBT_DVB == g_enSubtType)/*DVB*/
    {
        ...
        #else
        stInstallParam.u16SubtPID = g_stSubtItem.u32SubtPID;
        stInstallParam.u32UserData = (HI_U32)g_stSubtItem.u32SubtPID;
        stInstallParam.enDataType = HI_UNF_SUBT_DVB;
        s32Ret |= SUBT_Data_Install(&stInstallParam, &hSubtData);
        #endif
    }
...
static HI_S32 StartDmx(Subt_Data_Source_Param_S *pstParam)
{
    ...
    if (DATA_SOURCE_TUNER == pstParam->type)
    {
        ...
        s32Ret |= HI_UNF_DMX_AttachTSPort(DMXID,HI_UNF_DMX_PORT_0_TUNER);
        if (HI_SUCCESS != s32Ret)
        {
    ...
    {
        s32Ret = HI_UNF_DMX_Init();
        s32Ret |= HI_UNF_DMX_AttachTSPort(DMXID, HI_UNF_DMX_PORT_3_RAM);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_SUBT("failed to HI_UNF_DMX_AttachTSPort\n");
            (HI_VOID)HI_UNF_DMX_DeInit();
        ...
        s32Ret = HI_UNF_DMX_CreateTSBuffer(HI_UNF_DMX_PORT_3_RAM, 0x200000, &hTsBuffer);
        if (s32Ret != HI_SUCCESS)
        {
...
static HI_S32 SwitchAudio(PMT_COMPACT_TBL *pSubTitleProgTbl, HI_U8 u8ProgNo)
{
    ...
    fgets((char * __restrict__)inputCmd, 30, stdin);
...

{
    ...
    HIADP_Search_SetScteSubtParseFlag(HI_TRUE);
#endif
