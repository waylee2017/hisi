...
HI_VOID draw_spectrum_win(HI_HANDLE pLayerInfo, HI_HANDLE hLayer,HI_U16 *pSpectrum, HI_U32 u32BandNum)
{
    HI_U32 i;
    HI_S32 ret;
    HI_U32 height, width;
    ...
    ret = HI_GO_FillRect(pLayerInfo,NULL,0x00000000,HIGO_COMPOPT_NONE);
    if(ret!=HI_SUCCESS)
    {
    ...
    for (i = 0; i < u32BandNum; i++)
    {
        ...
        pRect.h = height;
        pRect.w = width;
        ret = HI_GO_FillRect(pLayerInfo,&pRect,0xff6db6f9,HIGO_COMPOPT_NONE);
...
HI_S32 main(int argc, char *argv[])
{
    ...
    else
    {
        printf("Usage:       ./sample_mp3_spectrum inputfile [band_number] [vo_format]\n"
               ...
               "vo_format:   1080P_60|1080P_50|1080i_60|1080i_50|720P_60|720P_50i|PAL|NTSC\n");
...
//	HI_SYS_PreAV(NULL);
    ...
    ret = HI_UNF_SND_Attach(enSound, red_handle, HI_UNF_SND_MIX_TYPE_MASTER, 100);
    if (ret != HI_SUCCESS)
    {
...
AVPLAY_DETACH_SND:
    HI_UNF_SND_Detach(enSound, red_handle);    
AVPLAY_CLOSE:
