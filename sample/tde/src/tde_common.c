...
HI_S32 HI_TDE_CreateSurfaceByFile_D(char *file, TDE2_SURFACE_S *pstSurface, HI_U32 lineNum)
{
    ...
    {
        pstSurface->pu8ClutPhyAddr = (HI_U8*)HI_MMB_New(256 * 4, 1, NULL, NULL);
        ...
        pVirt = (HI_U8 *)HI_MMB_Map((HI_U32)pstSurface->pu8ClutPhyAddr, 0);
        fread(pVirt, 1, 256 * 4, fp);
        HI_MMB_Unmap((HI_U32)pstSurface->pu8ClutPhyAddr);
    }
    ...
    pstSurface->u32PhyAddr = (HI_U32)HI_MMB_New((stride) * h, 1, NULL, tmpStr);
    ...
    if(0 == pstSurface->u32PhyAddr)
    {
		...
		{
			HI_MMB_SafeDelete((HI_U32)(pstSurface->pu8ClutPhyAddr));
		}
        return -1;
    }
...
HI_S32 HI_TDE_CreateMBSurfaceByFile_D(char *file, TDE2_MB_S *pstMBSur, HI_U32 lineNum)
{
	...
	snprintf(tmpStr, 11, "%d", lineNum);
    pstMBSur->u32YPhyAddr = (HI_U32)HI_MMB_New(pstMBSur->u32YStride * pstMBSur->u32YHeight, 4, NULL, tmpStr);
    ...
    if(pstMBSur->u32CbCrStride)
    {
        ...
        pstMBSur->u32CbCrPhyAddr = (HI_U32)HI_MMB_New(pstMBSur->u32CbCrStride * u32CHeight, 4, NULL, NULL);
        if(0 == pstMBSur->u32YPhyAddr)
        {
 ...
 HI_S32 HI_TDE_DuplicateSurface(TDE2_SURFACE_S *pstInSurface, TDE2_SURFACE_S *pstOutSurface)
{
    ...
    pstOutSurface->u32PhyAddr = (HI_U32)HI_MMB_New(pstOutSurface->u32Stride * pstOutSurface->u32Height, 4, NULL, NULL);
...
HI_S32 HI_TDE_DestroyMBSurface(TDE2_MB_S *pstMBSur)
{
    ...
    {
        HI_MMB_SafeDelete((HI_U32)(pstMBSur->u32CbCrPhyAddr));
