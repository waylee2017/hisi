...
** Notes : If you want to test Panaccess CSA2 keyladder(2 level), please set DVB_ROOT_KEY first,
...
static HI_U32 loadR2RLadderSessionKey(HI_HANDLE hCipher)
{
    ...
    HI_U8 u8SessionKey1[16]          = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f};
    HI_U8 u8ContentkKey[16]          = {0x0f,0x0e,0x0d,0x0c,0x0b,0x0a,0x09,0x08,0x07,0x06,0x05,0x04,0x03,0x02,0x01,0x00};
    HI_U8 u8SessionKey2[16]          = {0x2f,0x0e,0x0d,0x0c,0x0b,0x0a,0x09,0x08,0x07,0x06,0x05,0x04,0x03,0x02,0x01,0x00};
    ...
    {
        Ret |= HI_UNF_ADVCA_SetR2RSessionKey(HI_UNF_ADVCA_KEYLADDER_LEV1, u8SessionKey1);
        if (HI_SUCCESS != Ret)
        {
            return HI_FAILURE;
        }
        ...
        for ( i = 0 ; i < 16; i++ )
        {
            HI_DEBUG_ADVCA("%02x",u8SessionKey1[i]);
        }        
        HI_DEBUG_ADVCA("\n");
    }
    ...
    {
        Ret |= HI_UNF_ADVCA_SetR2RSessionKey(HI_UNF_ADVCA_KEYLADDER_LEV2, u8SessionKey2);
        if (HI_SUCCESS != Ret)
        {
            return HI_FAILURE;
        }
        ...
        for ( i = 0 ; i < 16; i++ )
        {
            HI_DEBUG_ADVCA("%02x",u8SessionKey2[i]);
        }        
        HI_DEBUG_ADVCA("\n");
    }
    ...
    memcpy(stCtrl.u32Key, u8ContentkKey, 16);
    ...
    for ( i = 0 ; i < 16; i++ )
    {
        HI_DEBUG_ADVCA("%02x",u8ContentkKey[i]);
    }
    HI_DEBUG_ADVCA("\n");
...
HI_S32 r2r_test(HI_VOID)
{
    HI_S32 Ret,i, index;
    HI_HANDLE hCipher;
    HI_MMZ_BUF_S PlainBuf,EncrytedBuf;
    HI_UNF_CIPHER_ATTS_S stCipherAttr;
    HI_U8 u8PlainText[16]      = {0x12,0x34,0x56,0x78,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
    ...
    memcpy(PlainBuf.user_viraddr, u8PlainText, 16);
    memset(EncrytedBuf.user_viraddr, 0, EncrytedBuf.bufsize);   
    Ret = HI_UNF_CIPHER_Encrypt(hCipher,PlainBuf.phyaddr, EncrytedBuf.phyaddr, 16);
    if (HI_SUCCESS != Ret)
    {
    ...
    Ret = HI_UNF_CIPHER_Decrypt(hCipher, EncrytedBuf.phyaddr, PlainBuf.phyaddr,16);
    if (HI_SUCCESS != Ret)
    {
        HI_DEBUG_ADVCA("HI_UNF_CIPHER_Encrypt failed:%x\n",Ret);
        goto DESTROY;
    ...
    if (!memcmp(PlainBuf.user_viraddr, u8PlainText, 16))
    {
        printf("R2R test succeed!\n");
    }
    else
    {
        printf("R2R test fail!\n");
    }
...
HI_S32 run_cmdline(HI_S32 argc,HI_CHAR argv[MAX_ARGS_COUNT][MAX_ARGS_LEN])
{
    ...
    else if (!strcmp(argv[0],"getchipid"))
    {
        ...
        if (HI_SUCCESS != Ret)
        {
            HI_DEBUG_ADVCA("HI_UNF_ADVCA_GetOtpFuse failed:%x\n",Ret);
            return HI_FAILURE;
        }
...
HI_S32 main(HI_S32 argc,HI_CHAR *argv[])
{
    ...
    if(argc < 2)
    {
        printf("Usage: sample_tsplay file\n");
        return 0;
    }
    ...
    Ret = HI_UNF_AVPLAY_ChnOpen(g_hAvplay, HI_UNF_AVPLAY_MEDIA_CHAN_VID, &maxCapbility);
    if (Ret != HI_SUCCESS)
    {
