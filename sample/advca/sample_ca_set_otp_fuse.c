...
HI_S32 run_cmdline(HI_S32 argc, HI_CHAR** argv)
{
    ...
    else
    {
        ...
        case HI_UNF_ADVCA_OTP_SECURE_BOOT_ACTIVATION:
            ...
            HI_DEBUG_ADVCA("Please input the startup flash: [0] - SPI, [1] - NAND, [2] - SPI_NAND, [3] - reserved \n");
			...
			switch(enFlash)
			{
				...
				default:
				{
					HI_DEBUG_ADVCA("Error, invalid input flash type:%d\n", enFlash);
					return HI_FAILURE;
				}
			}
        ...
        case HI_UNF_ADVCA_OTP_ESCK_ROOTKEY: 
			...
			printBuffer("root key:", stOtpAttr.unOtpFuseAttr.stKeyladderRootKey.u8RootKey, 16);
            Ret = HI_UNF_ADVCA_SetOtpFuse((HI_UNF_ADVCA_OTP_FUSE_E)u32otpfuse, &stOtpAttr);
            break;
        ...
        case HI_UNF_ADVCA_OTP_CHIP_ID:
            ...
            HI_DEBUG_ADVCA("Please input the chipid, the length of the chipid is 8bytes(e.g. verimatrix advanced CA) or 4bytes.\n"
			"For example 1: input the chipid as the following format(if the chipid length is 8 bytes):\n"
			"1122334455667788\n"
			"For example 2: input the chipid as the following format(if the chipid length is 4 bytes)\n"
			"1122334400000000\n");
            ...
            for(i = 0; i < 8; i++)
            {
                memset(cTmpBuf, 0, sizeof(cTmpBuf));
                memcpy(cTmpBuf, cUserInput + i * 2, 2);
                stOtpAttr.unOtpFuseAttr.stChipId.au8ChipId[i] = strtol(cTmpBuf, NULL, 16);
            }
            Ret = HI_UNF_ADVCA_SetOtpFuse((HI_UNF_ADVCA_OTP_FUSE_E)u32otpfuse, &stOtpAttr);
            break;
