...
HI_S32 CC_Data_DeInit(HI_VOID)
{
    ...
    if (HI_TRUE == s_bInit)
    {
        for (i = 0; i < DATA_RECV_MAX_NUM; i++)
        {
            ...
            {
                (HI_VOID)CC_Data_Uninstall((HI_HANDLE)pstCCData);
            }
        }
        memset(s_astCCDataRecv, 0, sizeof(s_astCCDataRecv));
...
HI_S32 CC_Data_Install(CC_DATA_INSTALL_PARAM_S *pstInstallParam, HI_HANDLE *hData)
{
    ...
    {
        HI_ERR_CC("install too much, max is %d !\n", DATA_RECV_MAX_NUM);
    ...
    *hData = (HI_HANDLE)pstCCData;

    return HI_SUCCESS;
}

HI_S32 CC_Data_Uninstall(HI_HANDLE hData)
{
    CC_DATA_S *pstCCData = (CC_DATA_S *)hData;
