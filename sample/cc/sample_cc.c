...
static HI_U8 SelectProgram(PMT_COMPACT_TBL *pProgTbl)
{
    ...
    if (pProgTbl->prog_num > 1)
    {
        ...
        TRACE_INFO("Please input a number from 0 to %d for program:", pProgTbl->prog_num-1);
...
static HI_S32 StartCC(PMT_COMPACT_TBL *pProgTbl, HI_U8 u8ProgNo)
{
...
#ifdef CHIP_TYPE_hi3110ev500
        stCCAttr.unCCConfig.stCC708ConfigParam.enCC708DispFormat = HI_UNF_CC_DF_720X576;
#else
    ...
    else
    {
        ...
        HI_UNF_CC_GetDefaultAttr(&stCCAttr);
#ifdef CHIP_TYPE_hi3110ev500        
        stCCAttr.unCCConfig.stCC608ConfigParam.enCC608DispFormat = HI_UNF_CC_DF_720X576;
#else
...
static HI_S32 StartDataFilter(PMT_COMPACT_TBL *pSubTitleProgTbl, HI_U8 u8ProgNo)
{
    ...
    s32Ret |= CC_Data_Install(&stInstallParam, &hCCData);
    if (HI_SUCCESS != s32Ret)
    {
...
EXIT:
    ...
    if (hAvplay)
    {
       ...
       // (HI_VOID)HI_UNF_SND_Detach(HI_UNF_SND_0, hAvplay);hTrack
